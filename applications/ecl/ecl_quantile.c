...
typedef struct {
    ...
    time_t end_time;
    const ecl_sum_type *
        refcase; /* Pointer to an arbitrary ecl_sum instance in the ensemble - to have access to indexing functions. */
...
void ensemble_add_case(ensemble_type *ensemble, const char *data_file) {
    sum_case_type *sum_case =
        sum_case_fread_alloc(data_file, ensemble->interp_time);
    ...
    pthread_rwlock_wrlock(&ensemble->rwlock);
    {
        ...
        if (ensemble->start_time > 0)
            ensemble->start_time =
                util_time_t_min(ensemble->start_time, sum_case->start_time);
        else
            ensemble->start_time = ecl_sum_get_start_time(sum_case->ecl_sum);

        ensemble->end_time =
            util_time_t_max(ensemble->end_time, sum_case->end_time);
    }
...
void ensemble_init_time_interp(ensemble_type *ensemble) {
    ...
    for (i = 0; i < ensemble->num_interp; i++)
        time_t_vector_append(
            ensemble->interp_time,
            ensemble->start_time +
                i * (ensemble->end_time - ensemble->start_time) /
                    (ensemble->num_interp - 1));
}
...
void ensemble_load_from_glob(ensemble_type *ensemble, const char *pattern,
    ...
    for (i = 0; i < pglob.gl_pathc; i++) {
        ...
        arg_pack_append_owned_ptr(
            arg_pack, util_alloc_string_copy(pglob.gl_pathv[i]), free);
...
void ensemble_init(ensemble_type *ensemble, config_content_type *config) {
    ...
    /*1a: Loading the eclipse summary cases. */
    {
        thread_pool_type *tp = thread_pool_alloc(LOAD_THREADS, true);
        {
            ...
            if (config_content_has_item(config, "CASE_LIST")) {
                const config_content_item_type *case_item =
                    config_content_get_item(config, "CASE_LIST");
                for (j = 0; j < config_content_item_get_size(case_item); j++) {
                    const config_content_node_type *case_node =
                        config_content_item_iget_node(case_item, j);
                    for (i = 0; i < config_content_node_get_size(case_node);
                         i++) {
                        const char *case_glob =
                            config_content_node_iget(case_node, i);
    ...
    if (config_content_has_item(config, "NUM_INTERP"))
        ensemble->num_interp =
            config_content_iget_as_int(config, "NUM_INTERP", 0, 0);

    /*2: Remaining initialization */
    ensemble_init_time_interp(ensemble);
    if (vector_get_size(ensemble->data) < MIN_SIZE)
        util_exit("Sorry - quantiles make no sense with with < %d "
                  "realizations; should have ~> 100.\n",
                  MIN_SIZE);
}
...
static void output_add_key(const ecl_sum_type *refcase, output_type *output,
                           const char *qkey) {
    int tokens;
    double quantile;
    char **tmp;
    ...
    if (tokens == 1)
        util_exit("Hmmm - the key:%s is malformed - must be of the form "
                  "SUMMARY_KEY:QUANTILE.\n",
                  qkey);

    if (!util_sscanf_double(tmp[tokens - 1], &quantile))
        util_exit("Hmmmm - failed to interpret:%s as a quantile - must be a "
                  "number (0,1).\n",
                  tmp[tokens - 1]);
    ...
    sum_key =
        util_alloc_joined_string((const char **)tmp, tokens - 1, SUMMARY_JOIN);
    {
        stringlist_type *matching_keys = stringlist_alloc_new();
        int i;
        ecl_sum_select_matching_general_var_list(refcase, sum_key,
                                                 matching_keys);
        for (i = 0; i < stringlist_get_size(matching_keys); i++)
            vector_append_owned_ref(
                output->keys,
                quant_key_alloc(stringlist_iget(matching_keys, i), quantile),
                quant_key_free__);

        if (stringlist_get_size(matching_keys) == 0)
            fprintf(stderr,
                    "** Warning: No summary vectors matching:\'%s\' found?? \n",
                    sum_key);
...
void output_table_init(const ecl_sum_type *refcase, hash_type *output_table,
                       const config_content_type *config) {
    int i, j;
    if (config_content_has_item(config, "OUTPUT")) {
        const config_content_item_type *output_item =
            config_content_get_item(config, "OUTPUT");
        for (i = 0; i < config_content_item_get_size(output_item); i++) {
            const config_content_node_type *output_node =
                config_content_item_iget_node(output_item, i);

            const char *file = config_content_node_iget(output_node, 0);
            const char *format_string =
                config_content_node_iget(output_node, 1);
            ...
            for (j = 2; j < config_content_node_get_size(output_node); j++)
                output_add_key(refcase, output,
                               config_content_node_iget(output_node, j));

            hash_insert_hash_owned_ref(output_table, file, output,
                                       output_free__);
        }
    }
}
...
static void print_var(FILE *stream, const char *var, double q,
                      const char *kw_fmt) {
...
void output_save_S3Graph(const output_type *output, ensemble_type *ensemble,
                         const double **data) {
    ...
    for (column_nr = 0; column_nr < data_columns; column_nr++) {
        ...
        print_var(stream, ecl_sum_get_keyword(ensemble->refcase, qkey->sum_key),
                  qkey->quantile, kw_fmt);
    }
    fprintf(stream, "\n");
    ...
    for (column_nr = 0; column_nr < data_columns; column_nr++) {
        ...
        fprintf(stream, unit_fmt,
                ecl_sum_get_unit(ensemble->refcase, qkey->sum_key));
    }
    fprintf(stream, "\n");
    ...
    fprintf(stream, time_blank);
    {
        for (column_nr = 0; column_nr < data_columns; column_nr++) {
            ...
            ecl_smspec_var_type var_type =
                ecl_sum_get_var_type(ensemble->refcase, ecl_key);
    ...
    for (row_nr = 0; row_nr < data_rows; row_nr++) {
        ...
        fprintf(stream, days_fmt,
                1.0 * (interp_time - ensemble->start_time) / 86400);
...
void output_save_plain__(const output_type *output, ensemble_type *ensemble,
                         const double **data, bool add_header) {
    FILE *stream = util_mkdir_fopen(output->file, "w");
...
void output_save(const output_type *output, ensemble_type *ensemble,
                 const double **data) {
...
void output_run_line(const output_type *output, ensemble_type *ensemble) {
  ...
  */
    for (column_nr = 0; column_nr < vector_get_size(output->keys);
         column_nr++) {
        const quant_key_type *qkey = vector_iget(output->keys, column_nr);
        {
            bool OK = true;
            ...
            for (int iens = 0; iens < vector_get_size(ensemble->data); iens++) {
                const sum_case_type *sum_case =
                    vector_iget_const(ensemble->data, iens);

                if (!ecl_sum_has_general_var(sum_case->ecl_sum,
                                             qkey->sum_key)) {
                    OK = false;
                    fprintf(stderr,
                            "** Sorry: the case:%s does not have the summary "
                            "key:%s \n",
    ...
    */
        ...
        for (row_nr = 0; row_nr < data_rows; row_nr++) {
            time_t interp_time =
                time_t_vector_iget(ensemble->interp_time, row_nr);
            for (column_nr = 0; column_nr < vector_get_size(output->keys);
                 column_nr++) {
                const quant_key_type *qkey =
                    vector_iget(output->keys, column_nr);
                double_vector_type *interp_data;
                ...
                if (!hash_has_key(interp_data_cache, qkey->sum_key)) {
                    ...
                    hash_insert_hash_owned_ref(interp_data_cache, qkey->sum_key,
                                               interp_data,
                                               double_vector_free__);
                }
                ...
                if (double_vector_size(interp_data) == 0) {
                    for (int iens = 0; iens < vector_get_size(ensemble->data);
                         iens++) {
                        const sum_case_type *sum_case =
                            vector_iget_const(ensemble->data, iens);

                        if ((interp_time >= sum_case->start_time) &&
                            (interp_time <=
                             sum_case
                                 ->end_time)) /* We allow the different simulations to have differing length */
                            double_vector_append(
                                interp_data,
                                ecl_sum_get_general_var_from_sim_time(
                                    sum_case->ecl_sum, interp_time,
                                    qkey->sum_key));

                        double_vector_sort(interp_data);
                    }
                }
                data[row_nr][column_nr] = statistics_empirical_quantile__(
...
void usage() {
    ...
    printf("\n");
    printf(
        "The ecl_quantile program will load an ensemble of ECLIPSE summary\n");
    printf("files, it can then output quantiles of summary vectors over the "
           "time\n");
    printf("span of the simulation. The program is based on a simple "
           "configuration\n");
    printf("file which must be given as a commandline argument. The "
           "configuration\n");
    printf("file only has three keywords:\n");
    printf("\n");
    printf("\n");
    printf("   CASE_LIST   simulation*X/run*X/CASE*.DATA\n");
    printf("   CASE_LIST   extra_simulation.DATA    "
           "even/more/simulations*GG/run*.DATA\n");
    printf("   OUTPUT      FILE1   S3GRAPH WWCT:OP_1:0.10  WWCT:OP_1:0.50   "
           "WOPR:OP_3\n");
    printf("   OUTPUT      FILE2   PLAIN   FOPT:0.10  FOPT:0.90  FGPT:0.10  "
           "FGPT:0.90   FWPT:0.10  FWPT:0.90\n");
    printf("   NUM_INTERP  100\n");
    printf("\n");
    printf("\n");
    printf("CASE_LIST: This keyword is used to give the path to ECLIPSE data "
           "files\n");
    printf("  corresponding to summaries which you want to load, observe that "
           "the\n");
    printf(
        "  argument given to the CASE_LIST keyword can contain unix-style\n");
    printf("  wildcards like '*'. One CASE_LIST keyword can point to several "
           "cases, \n");
    printf("  and in addition you can have several CASE_LIST keywords.\n");
    printf("\n");
    printf("\n");
    printf("OUTPUT: This keyword is used to denote what output you want from "
           "the\n");
    printf("  program. The first argument to the OUTPUT keyword is the name "
           "output\n");
    printf("  file you want to produce, in the example above we will create "
           "two\n");
    printf("  output files (FILE1 and FILE2 respectively). The second argument "
           "is \n");
    printf("  the wanted type of the output file, the three types currently "
           "supported\n");
    printf("  are: \n\n");
    ...
    printf(
        "  The remaining arguments on the output line corresponds to the \n");
    printf("  summary vector & quantile you are interested in. Each of these "
           "values\n");
    printf("  is a \":\" separated string consting of:\n");
    printf("  \n");
    printf("     VAR: The ECLIPSE summary variable we are interested in, "
           "(nearly)\n");
    printf(
        "          all variables found in the summary file are available,\n");
    printf("          e.g. RPR, WWCT or GOPT. \n");
    printf("\n");
    printf("     WG?: This is extra information added to the variable to make "
           "it\n");
    printf("          unique, e.g. the name of a well or group for rate "
           "variables\n");
    printf("          and the region number for a region. Not all variables, "
           "in\n");
    printf("          particalar the field rates, Fxxx, have this string.\n");
    printf("\n");
    printf("     Q:   The quantile we are interested in, e.g 0.10 to get the "
           "P10\n");
    printf("          quantile and 0.90 to get the P90 quantile.\n");
    printf("\n");
    printf("  For the 'VAR' and 'WG?' parts of the keys you can use "
           "shell-style\n");
    printf(
        "  wildcards to get all summary vectors matching a criteria, i.e. \n");
    ...
    printf("  Examples are:\n");
    printf("\n");
    printf("     WWCT:OPX:0.75     The P75 quantile of the watercut in well "
           "OPX.\n");
    printf("     BPR:10,10,5:0.50  The P50 quantile of the Block Pressure in "
           "block 10,10,5\n");
    printf("     FOPT:0.90         The P90 quantile of the field oil "
           "production total.\n");
    printf("     RPR:*:0.50        The P50 quantile of all regions.\n");
    printf("\n");
    printf("\n");
    printf("NUM_INTERP: Before the program can calculate quantiles it must\n");
    printf("  interpolate all the simulated data down on the same time axis. "
           "This\n");
    printf("  keyword regulates how many points should be used when "
           "interpolating\n");
    printf("  the time axis; the default is 50 which is probably quite OK. "
           "Observe\n");
    printf("  that for rate variable the program will not do linear "
           "interpolation\n");
    printf("  between ECLIPSE report steps, the might therefore look a bit "
           "jagged\n");
    printf("  if NUM_INTERP is set too high. This keyword is optional.\n");
    printf("\n");
    printf("All filenames in the configuration file will be interpreted "
           "relative to\n");
    printf("the location of the configuration file, i.e. irrespective of the "
           "current\n");
...
int main(int argc, char **argv) {
    if (argc != 2)
        ...
        ensemble_type *ensemble = ensemble_alloc();
        {
            ...
            config_init(config);
            content = config_parse(config, config_arg, "--", NULL, NULL, NULL,
                                   CONFIG_UNRECOGNIZED_WARN, true);

            if (config_content_is_valid(content)) {
                char *config_path;
                util_alloc_file_components(config_arg, &config_path, NULL,
                                           NULL);
                if (config_path != NULL) {
            ...
            output_table_init(ensemble_get_refcase(ensemble), output_table,
                              content);
