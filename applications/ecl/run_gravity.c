...
static const float *safe_get_float_ptr(const ecl_kw_type *ecl_kw,
                                       const float *alternative) {
    if (ecl_kw != NULL)
...
void print_usage(int line) {
    printf("LINE: %d \n", line);
    fprintf(stderr, "This program is used to calculate the change in "
                    "graviational response\n");
    fprintf(stderr, "between two timesteps in an eclipse simulation. To do the "
                    "calculations\n");
    fprintf(stderr, "the program needs the following information:\n");
    fprintf(stderr, "\n");
    fprintf(stderr,
            "  1. Restart file(s) with solution data for the two timesteps.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "  2. An EGRID or GRID file.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "  3. An INIT file.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "  4. A configuration file which lists at which "
                    "geographical locations\n");
    fprintf(stderr, "     you want to measure the gravitational response. This "
                    "file should\n");
    fprintf(stderr,
            "     contain one position on each line, formatted as this:\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "\n");
    fprintf(stderr,
            "             name1   utm_x  utm_y   depth   g_obs   g_std \n");
    fprintf(stderr,
            "             name2   utm_x  utm_y   depth   g_obs   g_std \n");
    fprintf(stderr, "             .....\n");
    fprintf(stderr, "\n");
    fprintf(
        stderr,
        "     The name string is completely arbitrary - but can NOT contain\n");
    fprintf(stderr, "     spaces. The two last columns - g_obs and g_std ar "
                    "optional, but\n");
    fprintf(stderr, "     must be present on all lines - or on no lines. \n");
    fprintf(stderr, "\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "The required information should be passed from the user "
                    "with the help\n");
    fprintf(
        stderr,
        "of commandline arguments. This can be done in roughly speaking two\n");
    ...
    fprintf(
        stderr,
        "In the case where all the files are found in one directory you can\n");
    fprintf(
        stderr,
        "just give an ECLIPSE basename, and the run_gravity program will by\n");
    fprintf(stderr, "itself find the required restart/init/grid files. Observe "
                    "that both\n");
    fprintf(stderr, "unified and non-unified restart files will be checked. In "
                    "addition to\n");
    fprintf(stderr, "the ECLIPSE basename you must give two numbers indicating "
                    "which report\n");
    fprintf(stderr, "steps you are interested in comparing, and finally the "
                    "configuration\n");
    fprintf(stderr, "file with all the measurement positions.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Example:\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "    bash%%  run_gravity.x  BASE_CASE  10 178  "
                    "../config/grav_stations\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "This will look up restart/grid/init files in the current "
                    "dirtectory,\n");
    fprintf(
        stderr,
        "for a simulation with baseame 'BASE_CASE'. It will compare report\n");
    fprintf(stderr,
            "steps 10 and 178, and load station locations from the file\n");
    ...
    fprintf(stderr, "-----------------------------------\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "If the different ECLIPSE files are not in the same "
                    "directory you can\n");
    fprintf(stderr, "not let the run_gravity program find the required files "
                    "automatically,\n");
    fprintf(stderr, "and you must give all the required files as arguments on "
                    "the command\n");
    fprintf(stderr, "line. This is the most flexible approach, in addition to "
                    "files stored\n");
    fprintf(
        stderr,
        "different places this also allows to combine files with different\n");
    fprintf(
        stderr,
        "ECLISPE basenames. There are two different ways to enter restart\n");
    fprintf(
        stderr,
        "information, depending on whether you use unified or non-unified\n");
    fprintf(stderr, "restart files.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Example 1 (unified restart):\n");
    fprintf(stderr, "\n");
    fprintf(stderr,
            "     bash%% run_gravity.x /path/to/restart_files/CASE_3.UNRST 10 "
            "178  /path/init/BASE_CASE.INIT   /path/to/grid/BASE_CASE.EGRID  "
            "../config/stations.txt\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "\n");
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "     bash%% run_gravity.x CASE_3.X0010  "
                    "../path/CASE_2.X0178  /path/init/BASE_CASE.INIT   "
                    "/path/to/grid/BASE_CASE.EGRID  ../config/stations.txt\n");
    fprintf(stderr, "     \n");
    fprintf(stderr, "\n");
    fprintf(stderr, "  When the program has completed succesfully it will "
                    "write the changes\n");
    fprintf(stderr, "  in local gravity to a file 'RUN_GRAVITY.out', in "
                    "addition the same\n");
    fprintf(stderr,
            "  information (with something more) will be sent to stdout.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "\n");
    exit(1);
}
...
static grav_station_type *grav_station_alloc_new(const char *name, double x,
                                                 double y, double d) {
...
static void load_stations(vector_type *grav_stations, const char *filename) {
    ...
    */
        ...
        while (!(at_eof)) {
            ...
            if (target_width == 4)
                fscanf_return =
                    fscanf(stream, "%s %lg %lg %lg", station_name, &x, &y, &d);
            else
                fscanf_return =
                    fscanf(stream, "%s %lg %lg %lg %lg %lg", station_name, &x,
                           &y, &d, &obs_gdiff, &std_gdiff);

            if (fscanf_return == target_width) {
                grav_station_type *g =
                    grav_station_alloc_new(station_name, x, y, d);
                if (target_width == 6)
...
ecl_file_type **load_restart_info(
    const char **input, /* Input taken directly from argv */
    int input_length,   /* The length of input. */
    int *
        arg_offset, /* Integer - value corresponding to the *NEXT* element in input which should be used by the calling scope. */
    bool
        *use_eclbase, /* Should input[0] be interpreted as an ECLBASE string? */
    bool *
        fmt_file) { /* Only relevant if (*use_eclbase == true): was formatted file used? */
    ...
    *use_eclbase = false;
    ...
    if (file_type == ECL_RESTART_FILE) {
        ...
        if (input_length >= 3) {
            int report1, report2;
            if ((util_sscanf_int(input[1], &report1) &&
        ...
        */
                ...
                unified_file = ecl_util_alloc_filename(
                    NULL, eclbase, ECL_UNIFIED_RESTART_FILE, false, -1);
                if (util_file_exists(unified_file))
                    /* Binary unified */
                    storage_mode = ECL_BINARY_UNIFIED;
                else {
                    /* Binary non-unified */
                    file1 = ecl_util_alloc_filename(
                        NULL, eclbase, ECL_RESTART_FILE, false, report1);
                    file2 = ecl_util_alloc_filename(
                        NULL, eclbase, ECL_RESTART_FILE, false, report2);
                    if ((util_file_exists(file1) && util_file_exists(file2)))
                        ...
                        /* ASCII unified */
                        unified_file = ecl_util_alloc_filename(
                            NULL, eclbase, ECL_UNIFIED_RESTART_FILE, true, -1);
                        if (util_file_exists(unified_file))
                            ...
                            free(file2);
                            file1 = ecl_util_alloc_filename(
                                NULL, eclbase, ECL_RESTART_FILE, true, report1);
                            file2 = ecl_util_alloc_filename(
                                NULL, eclbase, ECL_RESTART_FILE, true, report2);
                            if ((util_file_exists(file1) &&
                                 util_file_exists(file2)))
                                storage_mode = ECL_FORMATTED_UNIFIED;
                        }
                    }
                }
                ...
                if (storage_mode == ECL_INVALID_STORAGE) {
                    ...
                    util_exit("Could not find any restart information for "
                              "ECLBASE:%s in %s \n",
                              eclbase, cwd);
                    free(cwd);
                }

                if ((storage_mode == ECL_BINARY_UNIFIED) ||
                    (storage_mode == ECL_FORMATTED_UNIFIED)) {
                    restart_files[0] = ecl_file_open(input[0]);
                    restart_files[1] = ecl_file_open(input[0]);

                    if (!ecl_file_select_rstblock_report_step(restart_files[0],
                                                              report1))
                        util_exit("Failed to load report:%d from %s \n",
                                  report1, unified_file);

                    if (!ecl_file_select_rstblock_report_step(restart_files[1],
                                                              report2))
                        util_exit("Failed to load report:%d from %s \n",
...
static double gravity_response(const ecl_grid_type *ecl_grid,
                               ...
                               int model_phases, int file_phases) {
    ...
    /** Extracting the densities */
    {
        ...
        if (has_phase(model_phases, OIL)) {
            if (simulator == ECLIPSE100) {
                oil_den1_kw =
                    ecl_file_iget_named_kw(restart_file1, "OIL_DEN", 0);
                oil_den2_kw =
                    ecl_file_iget_named_kw(restart_file2, "OIL_DEN", 0);
            } else { // ECLIPSE300
        ...
        if (has_phase(model_phases, GAS)) {
            if (simulator == ECLIPSE100) {
                gas_den1_kw =
                    ecl_file_iget_named_kw(restart_file1, "GAS_DEN", 0);
                gas_den2_kw =
                    ecl_file_iget_named_kw(restart_file2, "GAS_DEN", 0);
            } else { // ECLIPSE300
        ...
        if (has_phase(model_phases, WATER)) {
            if (simulator == ECLIPSE100) {
                wat_den1_kw =
                    ecl_file_iget_named_kw(restart_file1, "WAT_DEN", 0);
                wat_den2_kw =
                    ecl_file_iget_named_kw(restart_file2, "WAT_DEN", 0);
            } else { // ECLIPSE300
        ...
        aquifern_kw = ecl_file_iget_named_kw(init_file, "AQUIFERN", 0);
    {
        ...
        float *zero = util_calloc(
            nactive,
            sizeof *zero); /* Fake vector of zeros used for densities / sturations when you do not have data. */
    ...
    */
        ...
        }
        {
            ...
            for (global_index = 0;
                 global_index < ecl_grid_get_global_size(ecl_grid);
                 global_index++) {
                const int act_index =
                    ecl_grid_get_active_index1(ecl_grid, global_index);
                if (act_index >= 0) {
            ...
            */

                        {
                            ...
                            mas1 = rporv1[act_index] *
                                   (soil1 * oil_den1[act_index] +
                                    sgas1 * gas_den1[act_index] +
                                    swat1 * wat_den1[act_index]);
                            mas2 = rporv2[act_index] *
                                   (soil2 * oil_den2[act_index] +
                                    sgas2 * gas_den2[act_index] +
                                    swat2 * wat_den2[act_index]);

                            ecl_grid_get_xyz1(ecl_grid, global_index, &xpos,
                                              &ypos, &zpos);
                            {
                                double dist_x = xpos - utm_x;
                                double dist_y = ypos - utm_y;
                                double dist_d = zpos - tvd;
                                double dist_sq = dist_x * dist_x +
                                                 dist_y * dist_y +
                                                 dist_d * dist_d;
                                ...
                                local_deltag +=
                                    6.67428E-3 * (mas2 - mas1) * dist_d /
                                    pow(dist_sq,
                                        1.5); // Gravity in units of \mu Gal = 10^{-8} m/s^2
                            }
                        }
                    }
                }
...
static void *gravity_response_mt(void *arg) {
    ...
    for (station_nr = station1; station_nr < station2; station_nr++) {
        ...
        gs->grav_diff =
            gravity_response(ecl_grid, init_file, restart_files[0],
                             restart_files[1], gs, model_phases, file_phases);
    }
    return NULL;
}
...
static int gravity_check_input(const ecl_grid_type *ecl_grid,
                               ...
                               const ecl_file_type *restart_file1,
                               const ecl_file_type *restart_file2,
                               int *__model_phases, int *__file_phases) {
    {
        int model_phases = 0;
        int file_phases = 0;
      ...
      */
            ...
            if (!has_phase(model_phases, WATER))
                util_exit(
                    "Could not locate WAT_DEN keyword in restart files\n");

            if (has_phase(file_phases, GAS)) {
                /** Restart file has both water and gas - means we need all three densities. */
                if (!(has_phase(model_phases, GAS) &&
                      has_phase(model_phases, OIL)))
                    util_exit("Could not find GAS_DEN and OIL_DEN keywords in "
                              "restart files\n");
            } else {
                /* This is (water + oil) or (water + gas) system. We enforce one of the densities.*/
                if (!has_phase(model_phases, GAS + OIL))
                    util_exit("Could not find either GAS_DEN or OIL_DEN "
                              "kewyords in restart files\n");
            }
        }
    ...
    if (!(ecl_file_has_kw(restart_file1, "RPORV") &&
          ecl_file_has_kw(restart_file2, "RPORV")))
  ...
  */
    {
        const ecl_kw_type *rporv1_kw =
            ecl_file_iget_named_kw(restart_file1, "RPORV", 0);
        const ecl_kw_type *rporv2_kw =
            ecl_file_iget_named_kw(restart_file2, "RPORV", 0);
        const ecl_kw_type *init_porv_kw =
            ecl_file_iget_named_kw(init_file, "PORV", 0);
        ...
        for (active_index = active_delta; active_index < active_size;
             active_index += active_delta) {
            int global_index =
                ecl_grid_get_global_index1A(ecl_grid, active_index);
            double init_porv = ecl_kw_iget_as_double(
                init_porv_kw,
                global_index); /* NB - this uses global indexing. */
            ...
            double fraction = util_double_min(init_porv, rporv12) /
            ...
            if (fraction < 0.50) {
                ...
                fprintf(stderr, "veeery different from the initial rporv "
                                "value. This might indicated\n");
                fprintf(stderr, "an ECLIPSE bug. Version 2007.2 is known to be "
                                "ok in this respect, \n");
                fprintf(stderr,
                        "whereas version 2008.2 is known to have a bug. \n");
                fprintf(stderr, "----------------------------------------------"
                                "-------------------\n");
                exit(1);
            }
...
int main(int argc, char **argv) {
    ...
    if (argc < 2)
        ...
        /* Restart info */
        restart_files =
            load_restart_info((const char **)input, input_length, &input_offset,
                              &use_eclbase, &fmt_file);
        ...
        */
                init_filename = ecl_util_alloc_exfilename_anyfmt(
                    NULL, input[0], ECL_INIT_FILE, fmt_file, -1);
                grid_filename = ecl_util_alloc_exfilename_anyfmt(
                    NULL, input[0], ECL_EGRID_FILE, fmt_file, -1);
                if (grid_filename == NULL)
                    grid_filename = ecl_util_alloc_exfilename_anyfmt(
                        NULL, input[0], ECL_GRID_FILE, fmt_file, -1);
                ...
                if ((input_length - input_offset) > 1) {
                    ...
                    grid_filename =
                        util_alloc_string_copy(input[input_offset + 1]);
                    input_offset += 2;
                } else
                    print_usage(__LINE__);
            }
    ...
    */
        {
            ...
            arg_pack_type **arg_list =
                util_calloc(num_threads, sizeof *arg_list);
            {
                int station_delta =
                    vector_get_size(grav_stations) / num_threads;
...

        {
            ...
            for (station_nr = 0; station_nr < vector_get_size(grav_stations);
                 station_nr++) {
                const grav_station_type *g_s =
                    vector_iget_const(grav_stations, station_nr);
                fprintf(stream, "%f", g_s->grav_diff);
                printf("DELTA_G %4s[%02d]: %12.6f %12.6f %12.6f %12.6f",
                       g_s->name, station_nr, g_s->grav_diff, g_s->utm_x,
                       g_s->utm_y, g_s->depth);

                if (g_s->has_obs) {
                    double y =
                        (g_s->grav_diff - g_s->obs_gdiff) / g_s->std_gdiff;
                    double chi_sq = y * y;
                    total_chisq += chi_sq;
