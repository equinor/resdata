...
static void fprintf_line(const ecl_sum_type *ecl_sum,
                         const ecl_sum_fmt_type *fmt, const char *well,
                         int time_index, const stringlist_type *var_list,
                         FILE *stream) {
    /* WELL */
    ...
    /* DATE */
    {
        ...
        char *date_string =
            util_malloc(DATE_STRING_LENGTH * sizeof *date_string);
...

    {
        ...
        for (ivar = 0; ivar < stringlist_get_size(var_list); ivar++) {
            ...
            if (ecl_sum_has_well_var(ecl_sum, well, var))
                ...
                fprintf(stderr,
                        "Missing variable:%s for well:%s - substituting 0.0 \n",
                        var, well);
...
int main(int argc, char **argv) {
    {
        ...
        if (argc != 2) {
            printf("You must supply the name of a case as:\n\n   "
                   "summary2csv.exe  ECLIPSE_CASE\n\nThe case can optionally "
                   "contain a leading path component.\n");
            exit(1);
        }

        {
            ...
            ecl_sum =
                ecl_sum_fread_alloc_case__(data_file, ":", include_restart);
            if (ecl_sum != NULL) {
                char *csv_file = util_alloc_filename(
                    NULL, ecl_sum_get_base(ecl_sum),
                    "txt"); // Will save to current path; can use ecl_sum_get_path() to save to target path instead.
                FILE *stream = util_fopen(csv_file, "w");

                stringlist_type *well_list =
                    ecl_sum_alloc_well_list(ecl_sum, NULL);
                ...
                {
                    int ivar;
                    for (ivar = 0; ivar < stringlist_get_size(var_list);
                         ivar++) {
                        const char *var = stringlist_iget(var_list, ivar);
                        fprintf(stream, fmt.sep);
...

                {
                    ...
                    for (iw = 0; iw < stringlist_get_size(well_list); iw++) {
                        ...
                        if (ecl_sum_is_oil_producer(ecl_sum, well)) {
                            int time_index;
                            for (time_index = 0;
                                 time_index < ecl_sum_get_data_length(ecl_sum);
                                 time_index++)
                                fprintf_line(ecl_sum, &fmt, well, time_index,
                                             var_list, stream);
                        }
                    }
                }
                ...
                fprintf(stderr,
                        "summary2csv2: No summary data found for case:%s\n",
                        data_file);
