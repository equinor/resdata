...
int main(int argc, char **argv) {
        ...
        util_exit("Usage: rft.x BASENAME \n");
    {
        ...
        if (input_type == ECL_RFT_FILE)
            ...
            rft_file_formatted =
                ecl_util_alloc_filename(path, base, ECL_RFT_FILE, true, -1);
            rft_file_unformatted =
                ecl_util_alloc_filename(path, base, ECL_RFT_FILE, false, -1);

            if (util_file_exists(rft_file_formatted) &&
                util_file_exists(rft_file_unformatted))
                file_name = util_alloc_string_copy(
                    util_newest_file(rft_file_formatted, rft_file_unformatted));
