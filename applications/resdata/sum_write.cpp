...
/*
  The summary data is stored in two different files:
   ...
   - The actual data is stored as blocks of data where each block
     contains all the variables for one timestep (MINISTEP). The data
     can be stored in a unified summary file
  ...
  is (mainly) based on three different rd_kw vectors which together
  ...
  Observe the following points:
   ...
   1. The variable name supplied in the KEYWORD array is significant;
      ...
      rd_smspec_identify_var_type() function.
   ...
   4. Each block of data is exactly as large as the header. This
      ...
      rd_sum implementation is "the index of a variable" - i.e. the
   ...
   6. In the rd_sum library the concept of a 'gen_key' is used quite
  ...
  use. Very briefly the usage pattern will be like:
    ...
    1. Create a rd_sum instance and add variables to it with the
       rd_sum_add_var() function.

    2. Use your simulator to step forward in time, and add timesteps
       with rd_sum_add_tstep().
...
int main(int argc, char **argv) {
    ...
    arguments are:
      ...
      1: The case - this the basename, with an optional leading
         path component. Can later be modified with rd_sum_set_case().

      2: Should formatted files be used? Can be modified with
         rd_sum_set_fmt_output().
  ...
  */
    ...
    The arguments to this function are:
      ...
      2. The KEYWORD value for the variable we are considering; the
         function rd_smspec_identify_var_type() will be called with
  ...
  */
    ...
    rd::smspec_node instance (implemented in file smspec_node.c);
    ...
    return value from the rd_sum_add_var() function; which affect how
...

    {
        ...
        for (report_step = 0; report_step < num_dates; report_step++) {
            for (step = 0; step < num_step; step++) {
             ...
             added explicitly; the rd_sum layer will just see if this
             ...
             are holding on to from the rd_sum_add_var() calls.
          ...
          */
            ...
            rd_sum_tstep_iset() function. The index value should come
            from the smspec_get_params_index() function.
          */
                    rd_sum_tstep_iset(tstep, 3, 0.98);
          ...
          */
                    rd_sum_tstep_set_from_key(tstep, "WWCT:OP-1",
                                              sim_days / 10);
                    if (report_step >= 5) {
                        /*
              We can use the smspec_node value from the
              rd_sum_add_var() function directly:
            */
                        rd_sum_tstep_set_from_node(tstep, *wwct_wellx,
                                                   sim_days);
                        rd_sum_tstep_set_from_node(tstep, *wopr_wellx,
                                                   sim_days * 100);
                    }
                }
            }
