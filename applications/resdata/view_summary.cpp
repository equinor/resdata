...
void print_help_and_exit() {
    ...
    printf("from summary files. The program is invoked as:\n");
    printf("\n");
    printf("computer> summary.x /Path/to/CASE key1 key2 key3 ....\n");
    printf("\n");
    printf("Here CASE is the name of an existing case, you can give it with\n");
    printf("extension, or without; the case need not be in the current "
           "directory. \n");
    printf("\n");
    printf("The keys are formed by combining variable names and\n");
    printf(
    ...
    printf("   SPR:F-12:18        - The segment pressure in well F-12, segment "
           "18.\n");
    printf("   BPR:10,10,10       - The block pressure in cell 10,10,10.\n");
    printf("   LBPR:LGR3:10,10,10 - The block pressure in cell 10,10,10 - in "
           "LGR3\n");
    printf("\n");
    ...
    printf("\n");
    printf("  --no-restart: If the simulation in question is a restart, i.e a "
           "prediction \n");
    printf("      which starts at the end of the historical period, the "
           "summary.x \n");
    printf("      program will by default also load historical data. If the "
           "--no-restart \n");
    printf("      option is used the program will not look for old results.\n");
    printf("\n");
    printf("  --no-header: By default summary.x will print a header line at "
           "the top, with the\n");
    printf("               option --no-header this will be suppressed.   \n");
    printf("\n");
    printf("  --report-only: Will only report results at report times (i.e. "
           "DATES).\n");
    printf("\n");
    printf("  --help: Print this message and exit.\n");
    printf("\n");
    printf("The options should come before the basename.\n");
    printf("\n");
    printf("Example1:\n");
    printf("\n");
    printf("  computer> summary.x  CASE1_XXX WWCT:F-36   FOPT   FWPT\n");
    printf("\n");
    printf("  This example will load results from case 'CASE1_XXX' and print "
           "the\n");
    printf(
...
static void build_key_list(const rd_sum_type *rd_sum, stringlist_type *key_list,
    ...
    for (iarg = 0; iarg < argc; iarg++) {
       ...
       rd_sum_fprintf() function print a message on stderr about
       missing keys.
    */

        if (util_string_has_wildcard(argv[iarg]))
            rd_sum_select_matching_general_var_list(rd_sum, argv[iarg],
                                                    key_list);
        else
    ...
    install_SIGNALS();
    {
        ...
        if (argc == 1)
            ...
            static struct option long_options[] = {
                {"no-restart", 0, 0, 'n'},  {"list", 0, 0, 'l'},
                {"report-only", 0, 0, 'r'}, {"no-header", 0, 0, 'x'},
                {"help", 0, 0, 'h'},        {0, 0, 0, 0}};
            ...
            while (1) {
                ...
                c = getopt_long(argc, argv, "nlRrx", long_options,
                                &option_index);
                if (c == -1)
                    break;
            ...
            arg_offset =
                optind; /* External static variable in the getopt scope*/
        }
#endif
...

        {
            ...
            rd_sum = rd_sum_fread_alloc_case2__(data_file, ":", include_restart,
                                                true, 0);
          ...
          */
                    ...
                    if (num_keys == 0) {
                        rd_sum_select_matching_general_var_list(rd_sum, "*",
                                                                keys);
                        stringlist_sort(keys, NULL);
                    } else
                        build_key_list(rd_sum, keys, num_keys, arg_list);
                    ...
                    build_key_list(rd_sum, key_list, num_keys, arg_list);

                    if (print_header)
                        rd_sum_fmt_init_summary_x(rd_sum, &fmt);
                    else
                        fmt.print_header = false;

                    rd_sum_fprintf(rd_sum, stdout, key_list, report_only, &fmt);
                ...
                fprintf(stderr,
                        "summary.x: No summary data found for case:%s\n",
                        data_file);
        }
    }
}
