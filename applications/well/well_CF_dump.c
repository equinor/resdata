    ...
    well_info_load_rstfile(well_info, rst_file, true);
    {
        ...
        for (iw = 0; iw < well_info_get_num_wells(well_info); iw++) {
            ...
            well_state_type *well_state =
                well_info_get_state_from_report(well_info, well_name, 100);
            const well_conn_collection_type *connections =
                well_state_get_global_connections(well_state);
            printf("Well: %s \n", well_state_get_name(well_state));

            for (ic = 0; ic < well_conn_collection_get_size(connections);
                 ic++) {
                well_conn_type *conn =
                    well_conn_collection_iget(connections, ic);
                printf("    %2d  %2d  %2d   CF: %g \n", well_conn_get_i(conn),
                       well_conn_get_j(conn), well_conn_get_k(conn),
