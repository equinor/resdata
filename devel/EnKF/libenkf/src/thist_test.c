...
int main(int argc , char ** argv)  {
  ...
  thist_type * thist = thist_alloc(ens_size , thist_both);
  ...
  for (time_step = 0; time_step < length ; time_step++) {
    ...
    std *= rand_normal(0.80 , 0.15);

    thist_update_forecast_vector(thist , time_step , forecast_data);

    for (iens = 0; iens < ens_size; iens++) 
      analyzed_data[iens] = rand_normal(mu , std);
    
    thist_update_analyzed_vector(thist , time_step , forecast_data);
    
    mu  += rand_normal(0    , 0.25 * std);
