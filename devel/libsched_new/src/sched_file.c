...
/* This sched_file.c contains code for internalizing an ECLIPSE
   ...
   block contains one or more keywords. The last keyword in a block
   will always be a timing keyword like DATES or TSTEP. Thus, the number
   of blocks in the sched_file_struct will always cooincide with the 
   of report steps in the ECLIPSE simulation. In order to do so,
   TSTEP and DATES keyword containing multiple data, are split into
   a sequence of keywords.

   # Note that this implies that scheduling data after the last timing
     keyword is irrelevant. This is similar to how ECLIPSE works.

   # Note also that scheduling data after keyword END is ignored, since
     this is interpreted as the end of the SCHEDULE section.
...
static void sched_block_fwrite(sched_block_type * block, FILE * stream)
{
  ...
  util_fwrite(&block->block_start_date, sizeof block->block_start_date, 1, stream, __func__);
}
...
static sched_block_type * sched_block_fread_alloc(FILE * stream)
{
  ...
  util_fread(&block->block_start_date, sizeof block->block_start_date, 1, stream, __func__);
  return block;
}
...
static sched_block_type * sched_file_iget_block(const sched_file_type * sched_file, int i)
{
...
static sched_kw_type * sched_block_get_last_kw(const sched_block_type * block)
{
...
static void sched_file_build_block_start_times(sched_file_type * sched_file, time_t start_date)
{
  int num_report_steps = sched_file_get_nr_report_steps(sched_file);
  time_t curr_time, new_time;

  curr_time = start_date;
  for(int i=0; i<num_report_steps; i++)
  {
      sched_block_type * sched_block = sched_file_iget_block(sched_file, i);
      sched_block->block_start_date   = curr_time;

      sched_kw_type * timing_kw = sched_block_get_last_kw(sched_block);
...
void sched_file_parse(sched_file_type * sched_file, time_t start_date, const char * filename)
{
  ...
  current_block->block_start_date = start_date;
  ...
  { 
    ...
    {
      int num_steps;
      sched_kw_type ** sched_kw_dates = sched_kw_report_step_split_alloc(current_kw, &num_steps);
  ...
  sched_file_build_block_start_times(sched_file, start_date);
}



int sched_file_get_nr_report_steps(const sched_file_type * sched_file)
{
...
void sched_file_fprintf(const sched_file_type * sched_file, int last_report_step, const char * file)
{
  FILE * stream = util_fopen(file, "w");
  for(int i=0; i<last_report_step; i++)
  {
    list_node_type * sched_block_node = list_iget_node(sched_file->blocks, i);
...
{
  int len = sched_file_get_nr_report_steps(sched_file);
