...
class ConfigurationLineTest(ExtendedTestCase):
    ...
    def test_num_realizations(self):
        ...
        self.assertEqual(config_line.keyword().keywordDefinition(), keywords["NUM_REALIZATIONS"])
    ...
    def test_unknown_keyword_with_comment(self):
        ...
        config_line = clb.configurationLine()
        self.assertFalse(config_line.keyword().validationStatus())
        self.assertEqual(config_line.keyword().validationStatus().message(), ConfigurationLine.UNKNOWN_KEYWORD)

        self.assertEqual(config_line.keyword().value(), "KEYWORD")
        self.assertEqual(config_line.arguments()[0].value(), "nothing")
        self.assertIsNone(config_line.keyword().keywordDefinition())

        self.assertTrue(clb.hasComment())
        self.assertEqual(clb.commentIndex(), 16)

        self.assertFalse(config_line.arguments()[0].validationStatus())
        self.assertEqual(config_line.arguments()[0].validationStatus().message(), ConfigurationLine.ARGUMENT_NOT_EXPECTED)
    ...
    def test_queue_option_keyword(self):
        ...
        self.assertTrue(arguments[0].validationStatus())
        self.assertTrue(arguments[1].validationStatus())
        self.assertTrue(arguments[2].validationStatus())
    ...
    def test_queue_option_keyword_too_few_arguments(self):
        ...
        self.assertTrue(arguments[0].validationStatus())
        self.assertTrue(arguments[1].validationStatus())
        self.assertFalse(arguments[2].validationStatus())

        self.assertEqual(arguments[2].validationStatus().message(), ArgumentDefinition.MISSING_ARGUMENT)
