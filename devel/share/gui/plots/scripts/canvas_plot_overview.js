...
Plot.prototype.setData = function(data) {
    ...
    for(var ensemble_index in data["ensemble_names"]) {
        ...
        for (var index = 0; index < data["ensemble_statistics"][ensemble_index].length; index++) {
            var ensemble_sample = data["ensemble_statistics"][ensemble_index][index];
            x_samples[index] = ensemble_sample["max_x"];
            case_samples[index] = ensemble_sample["max_y"];

        }

        for (var index = data["ensemble_statistics"][ensemble_index].length - 1; index >= 0; index--) {
            var ensemble_sample = data["ensemble_statistics"][ensemble_index][index];
            x_samples.push(ensemble_sample["max_x"]);
            case_samples.push(ensemble_sample["min_y"]);
        }
    }
...
//    var now = Date.now();
    ...
    if(data["observations"] != null) {
        if(data["observations"]["continuous_line"]) {
            ...
            obs_x_area_samples.push(obs_x_area_samples[0]);
            obs_y_area_samples.push(obs_y_area_samples[0]);
            this.line_renderer.style(STYLES["observation_area"]);
            this.line_renderer(context, obs_x_area_samples, obs_y_area_samples);
    ...
    for(var ensemble_index in data["ensemble_names"]) {
        ...
        this.area_renderer.style(style);
        this.area_renderer(context, x_samples, y_samples[ensemble_index]);

        legends.push({"style": style, "name": data["ensemble_names"][ensemble_index], "render_function": CanvasPlotLegend.filledCircle});
    }
