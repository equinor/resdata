...
void ecl_file_replace_kw(ecl_file_type *ecl_file, ecl_kw_type *old_kw,
    ...
    ecl_file_view_replace_kw(ecl_file->active_view, old_kw, new_kw,
                             insert_copy);
}
...
void ecl_file_indexed_read(const ecl_file_type *file, const char *kw, int index,
    ...
    ecl_file_view_index_fload_kw(file->active_view, kw, index, index_map,
                                 buffer);
}
...
time_t ecl_file_iget_restart_sim_date(const ecl_file_type *restart_file,
                                      int index) {
    return ecl_file_view_iget_restart_sim_date(restart_file->active_view,
                                               index);
}
...
int ecl_file_get_phases(const ecl_file_type *init_file) {
    ecl_kw_type *intehead_kw =
        ecl_file_iget_named_kw(init_file, INTEHEAD_KW, 0);
...
bool ecl_file_save_kw(const ecl_file_type *ecl_file,
    ...
    if (file_kw !=
        ...
        if (fortio_assert_stream_open(
            ...
            if (ecl_file_view_check_flags(ecl_file->flags,
...
bool ecl_file_select_rstblock_sim_time(ecl_file_type *ecl_file,
                                       time_t sim_time) {
    int seqnum_index = ecl_file_view_seqnum_index_from_sim_time(
        ecl_file->global_view, sim_time);
...
ecl_file_type *ecl_file_fast_open(const char *file_name,
    ...
    if (ecl_file_index_valid1(file_name, istream)) {
        ...
        if (fortio) {
            ...
            ecl_file->fortio = fortio;
            ecl_file->global_view =
                ecl_file_view_fread_alloc(ecl_file->fortio, &ecl_file->flags,
                                          ecl_file->inv_view, istream);
            if (ecl_file->global_view) {
                ecl_file_select_global(ecl_file);
                if (ecl_file_view_check_flags(ecl_file->flags,
                                              ECL_FILE_CLOSE_STREAM))
