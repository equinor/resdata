...
static void inv_map_add_kw(inv_map_type *map, const ecl_file_kw_type *file_kw,
...
ecl_file_kw_type *inv_map_get_file_kw(inv_map_type *inv_map,
  ...
  inv_map_assert_sort(inv_map);
  {
    ...
    if (index == -1)
      ...
      return (ecl_file_kw_type *)size_t_vector_iget(inv_map->file_kw_ptr,
                                                    index);
  }
}
...
static UTIL_SAFE_CAST_FUNCTION(ecl_file_kw, ECL_FILE_KW_TYPE_ID)
    UTIL_IS_INSTANCE_FUNCTION(ecl_file_kw, ECL_FILE_KW_TYPE_ID)
        ...
        ecl_file_kw_type *ecl_file_kw_alloc0(const char *header,
                                             ecl_data_type data_type, int size,
                                             offset_type offset) {
...
ecl_file_kw_type *ecl_file_kw_alloc(const ecl_kw_type *ecl_kw,
                                    offset_type offset) {
  return ecl_file_kw_alloc0(ecl_kw_get_header(ecl_kw),
                            ecl_kw_get_data_type(ecl_kw),
                            ecl_kw_get_size(ecl_kw), offset);
}
...
ecl_file_kw_type *ecl_file_kw_alloc_copy(const ecl_file_kw_type *src) {
  return ecl_file_kw_alloc0(src->header, ecl_file_kw_get_data_type(src),
...
static void ecl_file_kw_assert_kw(const ecl_file_kw_type *file_kw) {
  if (!ecl_type_is_equal(ecl_file_kw_get_data_type(file_kw),
...
static void ecl_file_kw_drop_kw(ecl_file_kw_type *file_kw,
...
static void ecl_file_kw_load_kw(ecl_file_kw_type *file_kw, fortio_type *fortio,
                                inv_map_type *inv_map) {
  if (fortio == NULL)
    util_abort("%s: trying to load a keyword after the backing file has been "
               "detached.\n",
               __func__);
...
void ecl_file_kw_replace_kw(ecl_file_kw_type *file_kw, fortio_type *target,
                            ecl_kw_type *new_kw) {
  if (!ecl_type_is_equal(ecl_file_kw_get_data_type(file_kw),
                         ecl_kw_get_data_type(new_kw)))
    util_abort(
        "%s: sorry type mismatch between in-file keyword and new keyword \n",
        __func__);
  if (file_kw->kw_size != ecl_kw_get_size(new_kw))
    util_abort(
        "%s: sorry size mismatch between in-file keyword and new keyword \n",
        __func__);
...
bool ecl_file_kw_fskip_data(const ecl_file_kw_type *file_kw,
                            fortio_type *fortio) {
  return ecl_kw_fskip_data__(ecl_file_kw_get_data_type(file_kw),
...
ecl_file_kw_type **ecl_file_kw_fread_alloc_multiple(FILE *stream, int num) {
  ...
  size_t file_kw_size = ECL_STRING8_LENGTH + 2 * sizeof(int) +
  ...
  {
    ecl_file_kw_type **kw_list =
        (ecl_file_kw_type **)util_malloc(num * sizeof *kw_list);
    for (int ikw = 0; ikw < num; ikw++) {
      ...
      kw_list[ikw] = ecl_file_kw_alloc0(
          header, ecl_type_create(ecl_type, type_size), kw_size, file_offset);
    }
