...
struct ecl_grav_struct {
  const ecl_file_type *init_file; /* The init file - a shared reference owned by
                                     calling scope. */
  ecl::ecl_grid_cache *grid_cache; /* An internal specialized structure to
                                      facilitate fast grid lookup. */
...
struct ecl_grav_survey_struct {
  ...
  double *porv; /* Reference shared by the ecl_grav_phase structures - i.e. it
                   must not be updated. */
  std::vector<ecl_grav_phase_type *>
      phase_list; /* ecl_grav_phase_type objects - one for each phase present in
                     the model. */
  std::unordered_map<std::string, ecl_grav_phase_type *>
      phase_map; /* The same objects as in the phase_list vector - accessible by
                    the "SWAT", "SGAS" and "SOIL" keys. */
};
...
struct ecl_grav_phase_struct {
  ...
  const ecl::ecl_grid_cache *grid_cache;
  const bool *aquifer_cell;
  double *fluid_mass; /* The total fluid in place (mass) of this phase - for
                         each active cell.*/
  ...
  if (ecl_version == ECLIPSE100) {
    ...
    default:
      ...
      return NULL;
             ...
             (ecl_version == ECLIPSE300_THERMAL)) {
    switch (phase) {
    case (ECL_OIL_PHASE):
    ...
    */
    {
      ...
      if (monitor_phase == NULL) {
        ...
        for (index = 0; index < grid_cache.size(); index++)
          mass_diff[index] =
              monitor_phase->fluid_mass[index] - base_phase->fluid_mass[index];
      }
    }
    ...
    /**
       ...
       10^6 => 6.67E-3.
    */
    deltag = 6.67428E-3 * ecl_grav_common_eval_biot_savart(grid_cache, region,
                                                           aquifer, mass_diff,
                                                           utm_x, utm_y, depth);
...
static ecl_grav_phase_type *ecl_grav_phase_alloc(
    ecl_grav_type *ecl_grav, ecl_grav_survey_type *survey, ecl_phase_enum phase,
    const ecl_file_view_type *restart_file, grav_calc_type calc_type) {
  ...
  const char *sat_kw_name = ecl_util_get_phase_name(phase);
  {
    ...
    if (calc_type == GRAV_CALC_FIP) {
      ...
      const std::vector<double> std_density =
          ecl_grav->std_density[std::string(ecl_util_get_phase_name(phase))];
      ecl_kw_type *fip_kw;
      ...
      const ecl_kw_type *den_kw =
          ecl_file_view_iget_named_kw(restart_file, den_kw_name, 0);
      ...
      if (calc_type == GRAV_CALC_RFIP) {
        ...
        if (ecl_file_view_has_kw(restart_file, sat_kw_name))
          ...
          /* We are targeting the residual phase, e.g. the OIL phase in a three
           * phase system. */
          const ecl_kw_type *swat_kw =
              ecl_file_view_iget_named_kw(restart_file, "SWAT", 0);
          ...
          if (ecl_file_view_has_kw(restart_file, "SGAS")) {
            const ecl_kw_type *sgas_kw =
                ecl_file_view_iget_named_kw(restart_file, "SGAS", 0);
...
static void ecl_grav_survey_add_phases(ecl_grav_type *ecl_grav,
  ...
  if (phases & ECL_OIL_PHASE) {
    ecl_grav_phase_type *oil_phase = ecl_grav_phase_alloc(
        ecl_grav, survey, ECL_OIL_PHASE, restart_file, calc_type);
    ecl_grav_survey_add_phase(survey, ECL_OIL_PHASE, oil_phase);
  }

  if (phases & ECL_GAS_PHASE) {
    ecl_grav_phase_type *gas_phase = ecl_grav_phase_alloc(
        ecl_grav, survey, ECL_GAS_PHASE, restart_file, calc_type);
    ecl_grav_survey_add_phase(survey, ECL_GAS_PHASE, gas_phase);
  }

  if (phases & ECL_WATER_PHASE) {
    ecl_grav_phase_type *water_phase = ecl_grav_phase_alloc(
        ecl_grav, survey, ECL_WATER_PHASE, restart_file, calc_type);
    ecl_grav_survey_add_phase(survey, ECL_WATER_PHASE, water_phase);
  }
}

static ecl_grav_survey_type *
ecl_grav_survey_alloc_empty(const ecl_grav_type *ecl_grav, const char *name,
                            grav_calc_type calc_type) {
  ...
  if (calc_type & GRAV_CALC_USE_PORV)
    survey->porv = (double *)util_calloc(ecl_grav->grid_cache->size(),
                                         sizeof *survey->porv);
  else
    survey->porv = NULL;
...
static void ecl_grav_survey_assert_RPORV(const ecl_grav_survey_type *survey,
  ...
  const ecl_kw_type *init_porv_kw =
      ecl_file_iget_named_kw(init_file, PORV_KW, 0);
  ...
  while (check_nr < check_points) {
    ...
    if (init_porv > 0) {
      ...
      if (fabs(log_pormod) > 1) {
        ...
        fprintf(stderr, "Hmmm - the RPORV values extracted from the restart "
                        "file seem to be \n");
        fprintf(stderr, "veeery different from the initial porv value. This "
                        "might indicate \n");
        fprintf(stderr, "an ECLIPSE bug in the RPORV handling. Try using "
...
static ecl_grav_survey_type *
ecl_grav_survey_alloc_RPORV(ecl_grav_type *ecl_grav,
                            const ecl_file_view_type *restart_file,
                            const char *name) {
  ecl_grav_survey_type *survey =
      ecl_grav_survey_alloc_empty(ecl_grav, name, GRAV_CALC_RPORV);
  if (ecl_file_view_has_kw(restart_file, RPORV_KW)) {
    ecl_kw_type *rporv_kw =
        ecl_file_view_iget_named_kw(restart_file, RPORV_KW, 0);
    int iactive;
...
ecl_grav_survey_alloc_PORMOD(ecl_grav_type *ecl_grav,
  ...
  ecl_grav_survey_type *survey =
      ecl_grav_survey_alloc_empty(ecl_grav, name, GRAV_CALC_PORMOD);
  ecl_kw_type *init_porv_kw = ecl_file_iget_named_kw(
      ecl_grav->init_file, PORV_KW, 0); /* Global indexing */
  ecl_kw_type *pormod_kw = ecl_file_view_iget_named_kw(restart_file, PORMOD_KW,
                                                       0); /* Active indexing */
  const int size = grid_cache.size();
  ...
  for (active_index = 0; active_index < size; active_index++)
    survey->porv[active_index] =
        ...
        ecl_kw_iget_float(init_porv_kw, global_index[active_index]);
...
ecl_grav_survey_alloc_FIP(ecl_grav_type *ecl_grav,
  ...
  ecl_grav_survey_type *survey =
      ecl_grav_survey_alloc_empty(ecl_grav, name, GRAV_CALC_FIP);
...
ecl_grav_survey_alloc_RFIP(ecl_grav_type *ecl_grav,
  ...
  ecl_grav_survey_type *survey =
      ecl_grav_survey_alloc_empty(ecl_grav, name, GRAV_CALC_RFIP);
  ...
  for (std::size_t phase_nr = 0; phase_nr < base_survey->phase_list.size();
    ...
    if (base_phase->phase & phase_mask) {
      if (monitor_survey != NULL) {
        ...
        deltag += ecl_grav_phase_eval(base_phase, monitor_phase, region, utm_x,
                                      utm_y, depth);
      } else
        deltag +=
            ecl_grav_phase_eval(base_phase, NULL, region, utm_x, utm_y, depth);
    }
  }
  return deltag;
}
...
ecl_grav_type *ecl_grav_alloc(const ecl_grid_type *ecl_grid,
  ...
  ecl_grav->aquifer_cell = ecl_grav_common_alloc_aquifer_cell(
      *(ecl_grav->grid_cache), ecl_grav->init_file);

  return ecl_grav;
}

static void ecl_grav_add_survey__(ecl_grav_type *grav, const char *name,
                                  ecl_grav_survey_type *survey) {
...
ecl_grav_add_survey_RPORV(ecl_grav_type *grav, const char *name,
                          const ecl_file_view_type *restart_file) {
  ecl_grav_survey_type *survey =
      ecl_grav_survey_alloc_RPORV(grav, restart_file, name);
...
ecl_grav_add_survey_FIP(ecl_grav_type *grav, const char *name,
                        const ecl_file_view_type *restart_file) {
  ecl_grav_survey_type *survey =
      ecl_grav_survey_alloc_FIP(grav, restart_file, name);
  ecl_grav_add_survey__(grav, name, survey);
  return survey;
}
...
ecl_grav_add_survey_RFIP(ecl_grav_type *grav, const char *name,
                         const ecl_file_view_type *restart_file) {
  ecl_grav_survey_type *survey =
      ecl_grav_survey_alloc_RFIP(grav, restart_file, name);
  ecl_grav_add_survey__(grav, name, survey);
  return survey;
}
...
ecl_grav_add_survey_PORMOD(ecl_grav_type *grav, const char *name,
                           const ecl_file_view_type *restart_file) {
  ecl_grav_survey_type *survey =
      ecl_grav_survey_alloc_PORMOD(grav, restart_file, name);
  ecl_grav_add_survey__(grav, name, survey);
  return survey;
}

static ecl_grav_survey_type *ecl_grav_get_survey(const ecl_grav_type *grav,
                                                 const char *name) {
  if (name == NULL)
...
double ecl_grav_eval(const ecl_grav_type *grav, const char *base,
  ...
  return ecl_grav_survey_eval(base_survey, monitor_survey, region, utm_x, utm_y,
                              depth, phase_mask);
}
...
void ecl_grav_add_std_density(ecl_grav_type *grav, ecl_phase_enum phase,
  ...
  std::vector<double> &std_density =
      grav->std_density[std::string(ecl_util_get_phase_name(phase))];
  if (std_density.size() <= static_cast<std::size_t>(pvtnum))
    std_density.resize(
        pvtnum + 1,
        grav->default_density[std::string(ecl_util_get_phase_name(phase))]);
