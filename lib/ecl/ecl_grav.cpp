...
struct ecl_grav_survey_struct {
    ...
    double *
        porv; /* Reference shared by the ecl_grav_phase structures - i.e. it must not be updated. */
    std::vector<ecl_grav_phase_type *>
        phase_list; /* ecl_grav_phase_type objects - one for each phase present in the model. */
...
static double ecl_grav_phase_eval(ecl_grav_phase_type *base_phase,
    ...
    */
       ...
       10^6 => 6.67E-3.
    */
        deltag = 6.67428E-3 * ecl_grav_common_eval_biot_savart(
...
static ecl_grav_phase_type *ecl_grav_phase_alloc(
    ...
    const char *sat_kw_name = ecl_util_get_phase_name(phase);
    {
        ...
        if (calc_type == GRAV_CALC_FIP) {
            ecl_kw_type *pvtnum_kw =
                ecl_file_iget_named_kw(init_file, PVTNUM_KW, 0);
            const std::vector<double> std_density =
                ecl_grav
                    ->std_density[std::string(ecl_util_get_phase_name(phase))];
            ecl_kw_type *fip_kw;

            if (phase == ECL_OIL_PHASE)
                fip_kw =
                    ecl_file_view_iget_named_kw(restart_file, FIPOIL_KW, 0);
            else if (phase == ECL_GAS_PHASE)
                fip_kw =
                    ecl_file_view_iget_named_kw(restart_file, FIPGAS_KW, 0);
            else
                fip_kw =
                    ecl_file_view_iget_named_kw(restart_file, FIPWAT_KW, 0);
            ...
            if (calc_type == GRAV_CALC_RFIP) {
                ecl_kw_type *rfip_kw;
                if (phase == ECL_OIL_PHASE)
                    rfip_kw = ecl_file_view_iget_named_kw(restart_file,
                                                          RFIPOIL_KW, 0);
                else if (phase == ECL_GAS_PHASE)
                    rfip_kw = ecl_file_view_iget_named_kw(restart_file,
                                                          RFIPGAS_KW, 0);
                ...
                if (ecl_file_view_has_kw(restart_file, sat_kw_name))
                    sat_kw = ecl_file_view_iget_named_kw(restart_file,
                                                         sat_kw_name, 0);
                else {
                    ...
                    if (ecl_file_view_has_kw(restart_file, "SGAS")) {
                        const ecl_kw_type *sgas_kw =
                            ecl_file_view_iget_named_kw(restart_file, "SGAS",
                                                        0);
...
ecl_grav_survey_alloc_RPORV(ecl_grav_type *ecl_grav,
...

    {
        ...
        ecl_grav_survey_add_phases(ecl_grav, survey, restart_file,
                                   GRAV_CALC_RPORV);
    }
    return survey;
}
...
ecl_grav_survey_alloc_PORMOD(ecl_grav_type *ecl_grav,
    ...
    ecl_kw_type *pormod_kw = ecl_file_view_iget_named_kw(
        restart_file, PORMOD_KW, 0); /* Active indexing */
    const int size = grid_cache.size();
    ...
    for (std::size_t phase_nr = 0; phase_nr < base_survey->phase_list.size();
        ...
        if (base_phase->phase & phase_mask) {
            if (monitor_survey != NULL) {
                ...
                deltag += ecl_grav_phase_eval(base_phase, monitor_phase, region,
                                              utm_x, utm_y, depth);
            } else
                deltag += ecl_grav_phase_eval(base_phase, NULL, region, utm_x,
                                              utm_y, depth);
        }
    }
    return deltag;
...
double ecl_grav_eval(const ecl_grav_type *grav, const char *base,
    ...
    return ecl_grav_survey_eval(base_survey, monitor_survey, region, utm_x,
