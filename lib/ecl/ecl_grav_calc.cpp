...
static void ecl_grav_check_rporv(const ecl_grid_type *ecl_grid,
    ...
    for (active_index = active_delta; active_index < active_size;
        ...
        double init_porv = ecl_kw_iget_as_double(
            init_porv_kw, global_index); /* NB - this uses global indexing. */
        ...
        if (fraction < 0.50) {
            ...
            fprintf(stderr, "veeery different from the initial rporv value. "
                            "This might indicate\n");
            fprintf(stderr, "an ECLIPSE bug. Version 2007.2 is known to be ok "
                            "in this respect, \n");
            fprintf(stderr,
...
ecl_grav_phase_deltag(double utm_x, double utm_y, double tvd,
                      ...
                      const ecl_kw_type *sat1_kw, const ecl_kw_type *rho1_kw,
                      const ecl_kw_type *porv1_kw, const ecl_kw_type *sat2_kw,
                      const ecl_kw_type *rho2_kw, const ecl_kw_type *porv2_kw) {
    ...
    if (ecl_file_has_kw(init_file, "AQUIFERN")) {
        const ecl_kw_type *aquifern_kw =
            ecl_file_iget_named_kw(init_file, "AQUIFERN", 0);
...

    {
        ...
        for (active_index = 0; active_index < ecl_grid_get_active_size(grid);
            ...
            if (aquifern != NULL && aquifern[active_index] != 0)
                ...
                mas1 = rho1[active_index] * porv1[active_index] *
                       sat1[active_index];
                mas2 = rho2[active_index] * porv2[active_index] *
                       sat2[active_index];
          ...
          */
                    deltag +=
                        6.67E-3 * (mas2 - mas1) * dist_z / pow(dist_sq, 1.5);
                }
            }
        }
    }
