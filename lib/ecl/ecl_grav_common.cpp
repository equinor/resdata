...
bool *ecl_grav_common_alloc_aquifer_cell(const ecl::ecl_grid_cache &grid_cache,
    ...
    if (ecl_file_has_kw(init_file, AQUIFER_KW)) {
        ecl_kw_type *aquifer_kw =
            ecl_file_iget_named_kw(init_file, AQUIFER_KW, 0);
...
double ecl_grav_common_eval_biot_savart(const ecl::ecl_grid_cache &grid_cache,
    ...
    if (region == NULL) {
        ...
        */
        ...
        const int_vector_type *index_vector =
            ecl_region_get_active_list(region);
...
ecl_grav_common_eval_geertsma_kernel(int index, const double *xpos,
    ...
    if (region == NULL) {
        ...
        for (index = 0; index < size; index++) {
            if (!aquifer[index]) {
                ...
                double displacement = ecl_grav_common_eval_geertsma_kernel(
