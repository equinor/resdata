...
/**
  ...
  the lgr implementation in is based on the following main principles:
   ...
   4. when it comes to indexing and so on there is no difference
      ...
      {
         ecl_file_type * restart_data = ecl_file_fread_alloc(restart_filename ,
  true);                      // load some restart info to inspect ecl_grid_type
  * grid         = ecl_grid_alloc(grid_filename , true); // bootstrap ecl_grid
  instance stringlist_type * lgr_names  = ecl_grid_alloc_name_list( grid ); //
  get a list of all the lgr names.

         printf("grid:%s has %d a total of %d lgr's \n", grid_filename ,
  stringlist_get_size( lgr_names )); for (int lgr_nr = 0; lgr_nr <
  stringlist_get_size( lgr_names); lgr_nr++) { ecl_grid_type * lgr_grid  =
  ecl_grid_get_lgr( grid , stringlist_iget( lgr_names , lgr_nr ));    // get the
  ecl_grid instance of the lgr - by name. ecl_kw_type   * pressure_kw; int
  nx,ny,nz,active_size; ecl_grid_get_dims( lgr_grid , &nx , &ny , &nz ,
  &active_size);                             // get some size info from this
  lgr. printf("lgr:%s has %d x %d x %d elements \n",stringlist_iget(lgr_names ,
  lgr_nr ) , nx , ny , nz);

            // ok - now we want to extract the solution vector (pressure)
  corresponding to this lgr: pressure_kw = ecl_file_iget_named_kw( ecl_file ,
  "pressure" , ecl_grid_get_grid_nr( lgr_grid ));
                                                                                      /|\
                                                                                       |
                                                                                       |
                                                                        we query
  the lgr_grid instance to find which occurence of the solution data we should
  look up in the ecl_file instance with restart data. puuhh!!

            {
               int center_index = ecl_grid_get_global_index3( lgr_grid , nx/2 ,
  ny/2 , nz/2 );          // ask the lgr_grid to get the index at the center of
  the lgr grid. printf("the pressure in the middle of %s is %g \n",
  stinglist_iget( lgr_names , lgr_nr ) , ecl_kw_iget_as_double( pressure_kw ,
  center_index ));
            }
         }
...
/*
 ...
 The api for coarse related tasks is briefly:
   ...
   - ecl_coarse_cell_type * ecl_grid_iget_coarse_group( const ecl_grid_type *
 ecl_grid , int coarse_nr );
   - ecl_coarse_cell_type * ecl_grid_get_cell_coarse_group1( const ecl_grid_type
 * ecl_grid , int global_index);
  ...
  from the grid files:
    ...
    - Properties and solution data in restart/init/grdecl files are
...

      {
         ...
         ecl_kw_type * M = ecl_kw_alloc_sub_copy( swat , matrix_kw   , 0  ,
  matrix_size ); ecl_kw_type * F = ecl_kw_alloc_sub_copy( swat , fracture_kw ,
  matrix_size  , fracture_size );
      }
*/
...
/*
  ...
  --------------
     ...
     for (int i=0; i < int_vector_size( cells_with_nnc ); i++) {
         ...
         const nnc_info_type * nnc_info = ecl_grid_get_nnc_info1( grid ,
  cell_index);

         // Get all the nnc connections from @cell_index to other cells in the
  same grid
         {
            const int_vector_type * nnc_list = nnc_info_get_self_index_list(
  nnc_info ); for (int j=0; j < int_vector_size( nnc_list ); j++)
               printf("Cell[%d] -> %d  in the same grid \n",cell_index ,
  int_vector_iget(nnc_list , j));
         }


         {
             for (int lgr_index=0; lgr_index < nnc_info_get_size( nnc_info );
  lgr_index++) { nnc_vector_type * nnc_vector = nnc_info_iget_vector( nnc_info ,
  lgr_index ); int lgr_nr = nnc_vector_get_lgr_nr( nnc_vector ); if (lgr_nr !=
  nnc_info_get_lgr_nr( nnc_info )) { const int_vector_type * nnc_list =
  nnc_vector_get_index_list( nnc_vector ); for (int j=0; j < int_vector_size(
  nnc_list ); j++) printf("Cell[%d] -> %d  in lgr:%d/%s \n",cell_index ,
  int_vector_iget(nnc_list , j) , lgr_nr  , ecl_grid_get_lgr_name(ecl_grid ,
  lgr_nr));
                }
             }
         }
...
/*
  ...
  are as follows:
   ...
   the number scheme of the corners in a cell used by eclipse:
       ...
       lower layer:   upper layer  (higher value of z - i.e. lower down in
  resrvoir).
...
#define CELL_FLAG_VALID                                                        \
  1 /* In the case of GRID files not necessarily all cells geometry values set \
       - in that case this will be left as false. */
#define CELL_FLAG_CENTER                                                       \
  2 /* Has the center value been calculated - this is by default not done to   \
       speed up loading a tiny bit. */
#define CELL_FLAG_TAINTED                                                      \
  4 /* Keep invalid cells out of real-world calculations with some             \
       heuristics.*/
...
struct ecl_cell_struct {
  ...
  double volume; /* Cache volume - whether it is initialized or not is handled
                    by a cell_flags. */
  int active;
  int active_index[2]; /* [0]: The active matrix index; [1]: the active fracture
                          index */
  const ecl_grid_type
      *lgr;      /* if this cell is part of an lgr; this will point to a grid
                    instance for that lgr; NULL if not part of lgr. */
  int host_cell; /* the global index of the host cell for an lgr cell, set to -1
                    for normal cells. */
  int coarse_group; /* The index of the coarse group holding this cell -1 for
                       non-coarsened cells. */
  int cell_flags;
  nnc_info_type *nnc_info; /* Non-neighbour connection info*/
};

static ert_ecl_unit_enum
ecl_grid_check_unit_system(const ecl_kw_type *gridunit_kw);
static void ecl_grid_init_mapaxes_data_float(const ecl_grid_type *grid,
                                             float *mapaxes);
...
struct ecl_grid_struct {
  ...
  int lgr_nr; /* EGRID files: corresponds to item 4 in gridhead - 0 for the main
                 grid.
                 GRID files: 0 for the main grid, then 1 -> number of LGRs in
                 order read from file*/
  char *name; /* the name of the file for the main grid - name of the lgr for
                 lgrs. */
  int ny, nz, nx;
  int size; /* == nx*ny*nz */
  int total_active;
  int total_active_fracture;
  bool *visited;  /* internal helper struct used when searching for index - can
                     be NULL. */
  int *index_map; /* this a list of nx*ny*nz elements, where value -1 means
                     inactive cell .*/
  int *inv_index_map; /* this is list of total_active elements - which point
                         back to the index_map. */

  int *fracture_index_map; /* For fractures: this a list of nx*ny*nz elements,
                              where value -1 means inactive cell .*/
  int *inv_fracture_index_map; /* For fractures: this is list of total_active
                                  elements - which point back to the index_map.
                                */

  ecl_cell_type *cells;

  char *parent_name; /* the name of the parent for a nested lgr - for the main
                        grid, and also a lgr descending directly from the main
                        grid this will be NULL. */
  std::unordered_map<std::string, ecl_grid_type *> children;
  const ecl_grid_type *parent_grid; /* the parent grid for this (lgr) - NULL for
                                       the main grid. */
  const ecl_grid_type
  ...
  /*
    ...
    lgr_nr > 0.
  */
  vector_type *LGR_list; /* a vector of ecl_grid instances for LGRs - the index
                            corresponds to the order LGRs are read from file*/
  int_vector_type *lgr_index_map; /* a vector that maps LGR-nr for EGRID files
                                     to index into the LGR_list.*/
  std::unordered_map<std::string, ecl_grid_type *>
      LGR_hash; /* a hash of pointers to ecl_grid instances - for name based
                   lookup of lgr. */
  int parent_box[6]; /* integers i1,i2, j1,j2, k1,k2 of the parent grid region
                        containing this lgr. the indices are inclusive - zero
                        offset */
                     /* not used yet .. */
  ...
  double origo[2];
  float *mapaxes;
  /*------------------------------:       the fields below this line are used
   * for blocking algorithms - and not allocated by default.*/
...
static void ecl_cell_compare(const ecl_cell_type *c1, const ecl_cell_type *c2,
...
static void ecl_cell_dump_ascii(ecl_cell_type *cell, int i, int j, int k,
...
static void ecl_cell_fwrite_GRID(const ecl_grid_type *grid,
                                 const ecl_cell_type *cell, bool fracture_cell,
                                 int coords_size, int i, int j, int k,
                                 int global_index, ecl_kw_type *coords_kw,
                                 ecl_kw_type *corners_kw, fortio_type *fortio) {
  ...
  int ri_offset = ecl_offset * 3;
  {
    ...
    for (point_nr = 0; point_nr < 2; point_nr++) {
      ...
      ri_points[ri_offset + point_nr * 3] =
          cell->corner_list[ecl_offset + point_nr].x;
      ri_points[ri_offset + point_nr * 3 + 1] =
          cell->corner_list[ecl_offset + point_nr].y;
      ri_points[ri_offset + point_nr * 3 + 2] =
          -cell->corner_list[ecl_offset + point_nr].z;
    }
  }
    ...
    */
    for (ecl_point = 2; ecl_point < 4; ecl_point++) {
      ...
      ri_points[ri_offset + ri_point * 3] =
          cell->corner_list[ecl_offset + ecl_point].x;
      ri_points[ri_offset + ri_point * 3 + 1] =
          cell->corner_list[ecl_offset + ecl_point].y;
      ri_points[ri_offset + ri_point * 3 + 2] =
          -cell->corner_list[ecl_offset + ecl_point].z;
    }
  }
}
...
static void ecl_cell_memcpy(ecl_cell_type *target_cell,
                            const ecl_cell_type *src_cell) {
  memcpy(target_cell, src_cell, sizeof *target_cell);
}

static void ecl_cell_install_lgr(ecl_cell_type *cell,
                                 const ecl_grid_type *lgr_grid) {
  cell->lgr = lgr_grid;
}
...
/**
       lower layer:   upper layer
         ...
         0---1           4---5
*/
static void ecl_cell_init_regular(ecl_cell_type *cell, const double *offset,
...
/**
   ...
   ecl_cell_taint_cell() which actually does it.
*/
...
static int ecl_grid_get_global_index__(const ecl_grid_type *ecl_grid, int i,
                                       int j, int k) {
  return i + j * ecl_grid->nx + k * ecl_grid->nx * ecl_grid->ny;
}

static void ecl_grid_set_cell_EGRID(ecl_grid_type *ecl_grid, int i, int j,
                                    int k, double x[4][2], double y[4][2],
  ...
  for (iz = 0; iz < 2; iz++) {
    for (ip = 0; ip < 4; ip++) {
      ...
      if (ecl_grid->use_mapaxes)
        point_mapaxes_transform(&cell->corner_list[c], ecl_grid->origo,
                                ecl_grid->unit_x, ecl_grid->unit_y);
    }
  }
...
static void ecl_grid_set_cell_GRID(ecl_grid_type *ecl_grid, int coords_size,
  ...
  */
  ...
  /* the coords keyword can optionally contain 4,5 or 7 elements:
        ...
        ----
        coords[4]    = 1,0 for active/inactive cells. Does NOT differentiate
     between matrix and fracture cells. coords[5]    = 0 for normal cells, icell
     of host cell for lgr cell. coords[6]    = 0 for normal cells, coarsening
     group for coarsened cell [not treated yet].
        ...
        4/5 element COORDS keywords and LGRs might come any day.
  */

  {
    ...
    */
    if ((ECL_GRID_MAINGRID_LGR_NR != ecl_grid->lgr_nr) && (coords_size != 7))
      util_abort("%s: Need 7 element coords keywords for LGR - or reimplement "
                 "to use LGRILG keyword.\n",
                 __func__);
    ...
    if (matrix_cell) {
      for (c = 0; c < 8; c++) {
        point_set(&cell->corner_list[c], corners[3 * c], corners[3 * c + 1],
                  corners[3 * c + 2]);
...
static void ecl_grid_init_index_map__(ecl_grid_type *ecl_grid, int *index_map,
...
static void ecl_grid_realloc_index_map(ecl_grid_type *ecl_grid) {
  ...
  ecl_grid->index_map = (int *)util_realloc(
      ecl_grid->index_map, ecl_grid->size * sizeof *ecl_grid->index_map);
  ecl_grid->inv_index_map = (int *)util_realloc(
      ecl_grid->inv_index_map,
      ecl_grid->total_active * sizeof *ecl_grid->inv_index_map);
  ecl_grid_init_index_map__(ecl_grid, ecl_grid->index_map,
                            ecl_grid->inv_index_map, CELL_ACTIVE_MATRIX,
                            MATRIX_INDEX);

  /* Create the inverse mapping for the fractures. */
  if (ecl_grid->dualp_flag != FILEHEAD_SINGLE_POROSITY) {
    ecl_grid->fracture_index_map = (int *)util_realloc(
        ecl_grid->fracture_index_map,
        ecl_grid->size * sizeof *ecl_grid->fracture_index_map);
    ecl_grid->inv_fracture_index_map =
        (int *)util_realloc(ecl_grid->inv_fracture_index_map,
                            ecl_grid->total_active_fracture *
                                sizeof *ecl_grid->inv_fracture_index_map);
    ecl_grid_init_index_map__(ecl_grid, ecl_grid->fracture_index_map,
                              ecl_grid->inv_fracture_index_map,
  ...
  */
  ...
  for (int coarse_group = 0; coarse_group < size; coarse_group++) {
    ecl_coarse_cell_type *coarse_cell =
        ecl_grid_iget_coarse_group(ecl_grid, coarse_group);
    ...
    if (ecl_coarse_cell_get_num_active(coarse_cell) > 0) {
      ...
      int active_fracture_index =
          ecl_coarse_cell_get_active_fracture_index(coarse_cell);

      if (active_value &
          CELL_ACTIVE_MATRIX) // active->global mapping point to one "random"
                              // cell in the coarse group
      ...
      const int_vector_type *global_index_list =
          ecl_coarse_cell_get_index_vector(coarse_cell);
      for (int ic = 0; ic < coarse_size; ic++) {
        int gi = int_vector_iget(global_index_list, ic);

        if (active_value &
            CELL_ACTIVE_MATRIX) // All cells in the coarse group point to the
                                // same active index.
...
static void ecl_grid_set_active_index(ecl_grid_type *ecl_grid) {
  ...
  if (!ecl_grid_have_coarse_cells(ecl_grid)) {
    ...
    for (int coarse_index = 0;
         coarse_index < vector_get_size(ecl_grid->coarse_cells);
         coarse_index++) {
      ecl_coarse_cell_type *coarse_cell =
          (ecl_coarse_cell_type *)vector_iget_const(ecl_grid->coarse_cells,
                                                    coarse_index);
    ...
    for (global_index = 0; global_index < ecl_grid->size; global_index++) {
      ...
      if (cell->active != CELL_NOT_ACTIVE) {
        if (cell->coarse_group == COARSE_GROUP_NONE) {
          ...
          ecl_coarse_cell_type *coarse_cell =
              ecl_grid_iget_coarse_group(ecl_grid, cell->coarse_group);
          ecl_coarse_cell_update_index(coarse_cell, global_index, &active_index,
    ...
    for (int coarse_group = 0; coarse_group < size; coarse_group++) {
      ...
      if (ecl_coarse_cell_get_num_active(coarse_cell) > 0) {
        ...
        int cell_active_value =
            ecl_coarse_cell_iget_active_value(coarse_cell, 0);
        int group_size = ecl_coarse_cell_get_size(coarse_cell);
        const int *coarse_cell_list =
            ecl_coarse_cell_get_index_ptr(coarse_cell);
        ...
        for (int i = 0; i < group_size; i++) {
          ...
          /* Coarse cell and dual porosity - that is probably close to zero
           * measure. */
...
ecl_grid_get_or_create_coarse_cell(ecl_grid_type *ecl_grid, int coarse_nr) {
  if (vector_safe_iget(ecl_grid->coarse_cells, coarse_nr) == NULL)
    vector_iset_owned_ref(ecl_grid->coarse_cells, coarse_nr,
                          ecl_coarse_cell_alloc(), ecl_coarse_cell_free__);
...
static void ecl_grid_init_coarse_cells(ecl_grid_type *ecl_grid) {
  if (ecl_grid->coarsening_active) {
    ...
    for (global_index = 0; global_index < ecl_grid->size; global_index++) {
      ...
      if (cell->coarse_group != COARSE_GROUP_NONE) {
        ecl_coarse_cell_type *coarse_cell =
            ecl_grid_get_or_create_coarse_cell(ecl_grid, cell->coarse_group);
        int i, j, k;
...
ecl_coarse_cell_type *ecl_grid_iget_coarse_group(const ecl_grid_type *ecl_grid,
                                                 int coarse_nr) {
  return (ecl_coarse_cell_type *)vector_iget(ecl_grid->coarse_cells, coarse_nr);
}

ecl_coarse_cell_type *
ecl_grid_get_cell_coarse_group1(const ecl_grid_type *ecl_grid,
                                int global_index) {
  ecl_cell_type *cell = ecl_grid_get_cell(ecl_grid, global_index);
  if (cell->coarse_group == COARSE_GROUP_NONE)
    return NULL;
...
ecl_coarse_cell_type *
ecl_grid_get_cell_coarse_group3(const ecl_grid_type *ecl_grid, int i, int j,
                                int k) {
  int global_index = ecl_grid_get_global_index3(ecl_grid, i, j, k);
  return ecl_grid_get_cell_coarse_group1(ecl_grid, global_index);
}

bool ecl_grid_cell_in_coarse_group1(const ecl_grid_type *main_grid,
                                    int global_index) {
...
bool ecl_grid_cell_in_coarse_group3(const ecl_grid_type *main_grid, int i,
                                    int j, int k) {
...
static void ecl_grid_init_mapaxes(ecl_grid_type *ecl_grid, bool apply_mapaxes,
                                  const float *mapaxes) {
  if (ecl_grid->global_grid != NULL)
    util_abort("%s: GridError: Trying to grid transformation data from mapaxes "
               "for a subgrid(lgr)\n",
               __func__);
  {
...
static void ecl_grid_add_lgr(ecl_grid_type *main_grid,
...
static void ecl_grid_install_lgr_common(ecl_grid_type *host_grid,
                                        ecl_grid_type *lgr_grid) {
...
static void ecl_grid_install_lgr_EGRID(ecl_grid_type *host_grid,
                                       ecl_grid_type *lgr_grid,
                                       const int *hostnum) {
...
*/
static void ecl_grid_install_lgr_GRID(ecl_grid_type *host_grid,
                                      ecl_grid_type *lgr_grid) {
  int global_lgr_index;
  ...
  for (global_lgr_index = 0; global_lgr_index < lgr_grid->size;
       global_lgr_index++) {
    ecl_cell_type *lgr_cell = ecl_grid_get_cell(lgr_grid, global_lgr_index);
    ecl_cell_type *host_cell =
        ecl_grid_get_cell(host_grid, lgr_cell->host_cell);
    ecl_cell_install_lgr(host_cell, lgr_grid);
  }
  ecl_grid_install_lgr_common(host_grid, lgr_grid);
}
...
static void ecl_grid_set_lgr_name_EGRID(ecl_grid_type *lgr_grid,
                                        const ecl_file_type *ecl_file,
                                        int grid_nr) {
  ecl_kw_type *lgrname_kw =
      ecl_file_iget_named_kw(ecl_file, LGR_KW, grid_nr - 1);
  lgr_grid->name = (char *)util_alloc_strip_copy((const char *)ecl_kw_iget_ptr(
      lgrname_kw, 0)); /* trailing zeros are stripped away. */
  if (ecl_file_has_kw(ecl_file, LGR_PARENT_KW)) {
    ecl_kw_type *parent_kw =
        ecl_file_iget_named_kw(ecl_file, LGR_PARENT_KW, grid_nr - 1);
    char *parent = (char *)util_alloc_strip_copy(
        (const char *)ecl_kw_iget_ptr(parent_kw, 0));
...
static void ecl_grid_set_lgr_name_GRID(ecl_grid_type *lgr_grid,
                                       const ecl_file_type *ecl_file,
                                       int grid_nr) {
      ...
      lgr_kw, 0)); /* trailing zeros are stripped away. */
  {
    ...
    */
    if (ecl_kw_get_size(lgr_kw) == 2) {
      char *parent = (char *)util_alloc_strip_copy(
          (const char *)ecl_kw_iget_ptr(lgr_kw, 1));
...
int ecl_grid_zcorn_index(const ecl_grid_type *grid, int i, int j, int k,
                         int c) {
...

    {
      ...
      for (k = 0; k < nz; k++) {
...

        {
          ...
          for (c = 0; c < 2; c++) {
            z[0][c] =
                zcorn[k * 8 * nx * ny + j * 4 * nx + 2 * i + c * 4 * nx * ny];
            z[1][c] = zcorn[k * 8 * nx * ny + j * 4 * nx + 2 * i + 1 +
                            c * 4 * nx * ny];
            z[2][c] = zcorn[k * 8 * nx * ny + j * 4 * nx + 2 * nx + 2 * i +
                            c * 4 * nx * ny];
            z[3][c] = zcorn[k * 8 * nx * ny + j * 4 * nx + 2 * nx + 2 * i + 1 +
                            c * 4 * nx * ny];
          }
        }

        {
          int ip;
          for (ip = 0; ip < 4; ip++)
            ecl_grid_pillar_cross_planes(&pillars[ip][0], ex[ip], ey[ip],
                                         ez[ip], z[ip], x[ip], y[ip]);
        }
...
void ecl_grid_init_GRDECL_data(ecl_grid_type *ecl_grid, const double *zcorn,
...
#pragma omp parallel for
  for (j = 0; j < ny; j++)
    ecl_grid_init_GRDECL_data_jslice(ecl_grid, zcorn, coord, actnum, corsnum,
                                     j);
}

void ecl_grid_init_GRDECL_data(ecl_grid_type *ecl_grid, const float *zcorn,
...
static ecl_grid_type *ecl_grid_alloc_GRDECL_data__(
    ecl_grid_type *global_grid, ert_ecl_unit_enum unit_system, int dualp_flag,
  ...
  ecl_grid_type *ecl_grid = ecl_grid_alloc_empty(
  ...
  if (ecl_grid) {
    ...
    ecl_grid->coord_kw =
        ecl_kw_alloc("COORD", 6 * (nx + 1) * (ny + 1), ECL_FLOAT);
    {
...
static ecl_grid_type *ecl_grid_alloc_GRDECL_data__(
  ...
  if (ecl_grid) {
    ...
    ecl_grid->coord_kw =
        ecl_kw_alloc_new("COORD", 6 * (nx + 1) * (ny + 1), ECL_FLOAT, coord);
    ecl_grid_init_GRDECL_data(ecl_grid, zcorn, coord, actnum, corsnum);
...
static void ecl_grid_copy_mapaxes(ecl_grid_type *target_grid,
                                  const ecl_grid_type *src_grid) {
  target_grid->use_mapaxes = src_grid->use_mapaxes;
  if (src_grid->mapaxes)
    target_grid->mapaxes = (float *)util_realloc_copy(
        target_grid->mapaxes, src_grid->mapaxes, 6 * sizeof *src_grid->mapaxes);
  else
    target_grid->mapaxes =
...
static ecl_grid_type *ecl_grid_alloc_copy__(const ecl_grid_type *src_grid,
                                            ecl_grid_type *main_grid) {
  ecl_grid_type *copy_grid = ecl_grid_alloc_empty(
      main_grid, src_grid->unit_system, src_grid->dualp_flag,
      ecl_grid_get_nx(src_grid), ecl_grid_get_ny(src_grid),
      ecl_grid_get_nz(src_grid), 0, false);
  if (copy_grid) {
    ecl_grid_copy_content(
        copy_grid,
        src_grid); // This will handle everything except LGR relationships which
                   // is established in the calling routine
...
ecl_grid_type *ecl_grid_alloc_copy(const ecl_grid_type *src_grid) {
...

  {
    ...
    for (grid_nr = 0; grid_nr < vector_get_size(src_grid->LGR_list);
         grid_nr++) {
      const ecl_grid_type *src_lgr =
          (const ecl_grid_type *)vector_iget_const(src_grid->LGR_list, grid_nr);
...

      {
        ...
        for (global_lgr_index = 0; global_lgr_index < copy_lgr->size;
             global_lgr_index++) {
          ecl_cell_type *lgr_cell =
              ecl_grid_get_cell(copy_lgr, global_lgr_index);
          ecl_cell_type *host_cell =
              ecl_grid_get_cell(host_grid, lgr_cell->host_cell);
...
/*
  Does not handle LGR
*/
ecl_grid_type *ecl_grid_alloc_processed_copy(const ecl_grid_type *src_grid,
                                             const double *zcorn,
                                             const int *actnum) {
  ecl_grid_type *grid;
  ...
  if (zcorn == NULL) {
    ...
    grid = ecl_grid_alloc_GRDECL_data(nx, ny, nz, zcorn_float, coord, actnum,
...
ecl_grid_type *ecl_grid_alloc_GRDECL_data(int nx, int ny, int nz,
                                          const float *zcorn,
                                          const float *coord, const int *actnum,
                                          bool apply_mapaxes,
                                          const float *mapaxes) {
  ...
  return ecl_grid_alloc_GRDECL_data__(
...
static ecl_grid_type *
ecl_grid_alloc_GRDECL_kw__(ecl_grid_type *global_grid, int dualp_flag,
                           bool apply_mapaxes, const ecl_kw_type *gridhead_kw,
  ...
  /*
    The code used to have this test:
       ...
       if (grid_nr != ecl_kw_iget_int( gridhead_kw , GRIDHEAD_LGR_INDEX))
          util_abort("%s: internal error in grid loader - lgr index
    mismatch\n",__func__);
  ...
  */
  ...
  if (gtype != GRIDHEAD_GRIDTYPE_CORNERPOINT)
    util_abort("%s: gtype:%d fatal error when loading grid - must have corner "
               "point grid - aborting\n",
               __func__, gtype);

  {
    ...
    return ecl_grid_alloc_GRDECL_data__(
        ...
        ecl_kw_get_float_ptr(zcorn_kw), ecl_kw_get_float_ptr(coord_kw),
...
ecl_grid_type *
ecl_grid_alloc_GRDECL_kw(int nx, int ny, int nz, const ecl_kw_type *zcorn_kw,
                         const ecl_kw_type *coord_kw,
  ...
  ecl_grid_type *ecl_grid = ecl_grid_alloc_GRDECL_kw__(
...
static void ecl_grid_init_cell_nnc_info(ecl_grid_type *ecl_grid,
                                        int global_index) {
  ...
  peculiarities with this implementation:
   ...
   2. To get valid NNC information to load in e.g. Resinsight the
      ...
      to the grid:
         ...
         ecl_grid_type * grid ...
         ecl_kw_type * trannnc_kw = ecl_kw_alloc( "TRANNNC" , num_nnc ,
  ECL_FLOAT_TYPE );
...
void ecl_grid_add_self_nnc(ecl_grid_type *grid, int cell_index1,
                           int cell_index2, int nnc_index) {
...
void ecl_grid_add_self_nnc_list(ecl_grid_type *grid, const int *g1_list,
...
static void ecl_grid_init_nnc_cells(ecl_grid_type *grid1, ecl_grid_type *grid2,
                                    const ecl_kw_type *keyword1,
                                    const ecl_kw_type *keyword2) {
...
/*
  This function reads the non-neighbour connection data from file and
  initializes the grid structure with the the nnc data
*/
static void ecl_grid_init_nnc(ecl_grid_type *main_grid,
  ...
  */
  ...
  for (i = 0; i < num_nnchead_kw; i++) {
    ecl_file_view_type *lgr_view =
        ecl_file_alloc_global_blockview(ecl_file, NNCHEAD_KW, i);
    ecl_kw_type *nnchead_kw =
        ecl_file_view_iget_named_kw(lgr_view, NNCHEAD_KW, 0);
    int lgr_nr = ecl_kw_iget_int(nnchead_kw, NNCHEAD_LGR_INDEX);

    if (ecl_file_view_has_kw(lgr_view, NNC1_KW)) {
      const ecl_kw_type *nnc1 =
          ecl_file_view_iget_named_kw(lgr_view, NNC1_KW, 0);
      const ecl_kw_type *nnc2 =
          ecl_file_view_iget_named_kw(lgr_view, NNC2_KW, 0);

      {
        ecl_grid_type *grid =
            (lgr_nr > 0) ? ecl_grid_get_lgr_from_lgr_nr(main_grid, lgr_nr)
                         : main_grid;
    ...
    if (ecl_file_view_has_kw(lgr_view, NNCL_KW)) {
      const ecl_kw_type *nncl =
          ecl_file_view_iget_named_kw(lgr_view, NNCL_KW, 0);
      const ecl_kw_type *nncg =
          ecl_file_view_iget_named_kw(lgr_view, NNCG_KW, 0);
      {
        ecl_grid_type *grid =
            (lgr_nr > 0) ? ecl_grid_get_lgr_from_lgr_nr(main_grid, lgr_nr)
                         : main_grid;
...
*/
static void ecl_grid_init_nnc_amalgamated(ecl_grid_type *main_grid,
                                          ecl_file_type *ecl_file) {
...
static ecl_grid_type *ecl_grid_alloc_EGRID__(ecl_grid_type *main_grid,
                                             const ecl_file_type *ecl_file,
                                             int grid_nr, bool apply_mapaxes,
                                             const int *ext_actnum) {
  ecl_kw_type *gridhead_kw =
      ecl_file_iget_named_kw(ecl_file, GRIDHEAD_KW, grid_nr);
  ...
  int eclipse_version;
  if (grid_nr == 0) {
    ecl_kw_type *filehead_kw =
        ecl_file_iget_named_kw(ecl_file, FILEHEAD_KW, grid_nr);
  ...
  */
  ...
  {
    ecl_grid_type *ecl_grid = ecl_grid_alloc_GRDECL_kw__(
        main_grid, dualp_flag, apply_mapaxes, gridhead_kw, zcorn_kw, coord_kw,
        gridunit_kw, mapaxes_kw, corsnum_kw, actnum_data);

    if (ECL_GRID_MAINGRID_LGR_NR != grid_nr)
      ecl_grid_set_lgr_name_EGRID(ecl_grid, ecl_file, grid_nr);
...
static ecl_grid_type *ecl_grid_alloc_EGRID_all_grids(const char *grid_file,
                                                     bool apply_mapaxes,
                                                     const int *ext_actnum) {
               ...
               __func__, grid_file);
  {
    ...
    if (ecl_file) {
      ...
      ecl_grid_type *main_grid =
          ecl_grid_alloc_EGRID__(NULL, ecl_file, 0, apply_mapaxes, ext_actnum);
      int grid_nr;
      ...
      for (grid_nr = 1; grid_nr < num_grid; grid_nr++) {
        ...
        ecl_grid_type *lgr_grid =
            ecl_grid_alloc_EGRID__(main_grid, ecl_file, grid_nr, false, NULL);
        ecl_grid_add_lgr(main_grid, lgr_grid);
        {
          ecl_grid_type *host_grid;
          ecl_kw_type *hostnum_kw =
              ecl_file_iget_named_kw(ecl_file, HOSTNUM_KW, grid_nr - 1);
          if (lgr_grid->parent_name == NULL)
            host_grid = main_grid;
          else
            host_grid = ecl_grid_get_lgr(main_grid, lgr_grid->parent_name);

          ecl_grid_install_lgr_EGRID(host_grid, lgr_grid,
                                     ecl_kw_get_int_ptr(hostnum_kw));
        }
      }
...
static ecl_grid_type *ecl_grid_alloc_GRID_data__(
    ecl_grid_type *global_grid, int num_coords, ert_ecl_unit_enum unit_system,
  ...
  if (dualp_flag != FILEHEAD_SINGLE_POROSITY)
    nz = nz / 2;
  {
    ecl_grid_type *grid = ecl_grid_alloc_empty(
    ...
    if (grid) {
...

      {
        ...
        for (index = 0; index < num_coords; index++)
          ecl_grid_set_cell_GRID(grid, coords_size, coords[index],
                                 corners[index]);
      }
...
ecl_grid_type *ecl_grid_alloc_GRID_data(int num_coords, int nx, int ny, int nz,
                                        int coords_size, int **coords,
                                        float **corners, bool apply_mapaxes,
                                        const float *mapaxes) {
  ert_ecl_unit_enum unit_system = ECL_METRIC_UNITS;
  return ecl_grid_alloc_GRID_data__(
      NULL, num_coords, unit_system,
      FILEHEAD_SINGLE_POROSITY, /* Does currently not support to determine
                                   dualp_flag from inspection. */
...
static int ecl_grid_dual_porosity_GRID_check(ecl_file_type *ecl_file) {
  ...
  if ((nz % 2) == 1)
    ...
    while (true) {
      ...
      fracture_kw =
          ecl_file_iget_named_kw(ecl_file, CORNERS_KW, fracture_index);
...
static ecl_grid_type *ecl_grid_alloc_GRID__(ecl_grid_type *global_grid,
                                            const ecl_file_type *ecl_file,
  ...
  {
    ecl_kw_type *dimens_kw =
        ecl_file_iget_named_kw(ecl_file, DIMENS_KW, grid_nr);
  ...
  //    keyword is optional, and is only applicable to the global grid.
  {
    if ((grid_nr == 0) && (ecl_file_has_kw(ecl_file, MAPAXES_KW))) {
      const ecl_kw_type *mapaxes_kw =
          ecl_file_iget_named_kw(ecl_file, MAPAXES_KW, 0);
      mapaxes_data = ecl_grid_get_mapaxes_from_kw__(mapaxes_kw);
    }
  }

  if ((grid_nr == 0) && (ecl_file_has_kw(ecl_file, GRIDUNIT_KW)))
    unit_system = ecl_grid_check_unit_system(
        ecl_file_iget_named_kw(ecl_file, GRIDUNIT_KW, 0));
  ...
  */

  {
    ...
    //    CORNERS keywords.
    {
      ...
      for (index = 0; index < num_coords; index++) {
        const ecl_kw_type *coords_kw =
            ecl_file_iget_named_kw(ecl_file, COORDS_KW, index + cell_offset);
        const ecl_kw_type *corners_kw =
            ecl_file_iget_named_kw(ecl_file, CORNERS_KW, index + cell_offset);
      ...
      // Create the grid:
      grid = ecl_grid_alloc_GRID_data__(
          global_grid, num_coords, unit_system, dualp_flag, apply_mapaxes, nx,
          ny, nz, grid_nr, coords_size, coords, corners, mapaxes_data);
  ...
  if (grid_nr > 0)
    ecl_grid_set_lgr_name_GRID(grid, ecl_file, grid_nr);
  return grid;
}
...
ecl_grid_type *ecl_grid_alloc_GRID(const char *grid_file, bool apply_mapaxes) {
...

  {
    ...
    main_grid = ecl_grid_alloc_GRID__(NULL, ecl_file, cell_offset, 0,
                                      dualp_flag, apply_mapaxes);
    cell_offset += ecl_grid_get_global_size(main_grid);

    for (grid_nr = 1; grid_nr < num_grid; grid_nr++) {
      ecl_grid_type *lgr_grid = ecl_grid_alloc_GRID__(
          main_grid, ecl_file, cell_offset, grid_nr, dualp_flag, false);
...
*/
ecl_grid_type *ecl_grid_alloc_regular(int nx, int ny, int nz,
                                      const double *ivec, const double *jvec,
                                      const double *kvec, const int *actnum) {
  ert_ecl_unit_enum unit_system = ECL_METRIC_UNITS;
  ecl_grid_type *grid = ecl_grid_alloc_empty(
    ...
    for (k = 0; k < nz; k++) {
      for (j = 0; j < ny; j++) {
        for (i = 0; i < nx; i++) {
          ...
          ecl_cell_init_regular(cell, offset, i, j, k, global_index, ivec, jvec,
                                kvec, actnum);
        }
      }
    }
...
ecl_grid_type *ecl_grid_alloc_rectangular(int nx, int ny, int nz, double dx,
                                          double dy, double dz,
                                          const int *actnum) {
...
ecl_grid_type *ecl_grid_alloc_dxv_dyv_dzv(int nx, int ny, int nz,
...
ecl_grid_type *
ecl_grid_alloc_dxv_dyv_dzv_depthz(int nx, int ny, int nz, const double *dxv,
...
ecl_grid_type *ecl_grid_alloc_dx_dy_dz_tops(int nx, int ny, int nz,
...
// This function is used to override use of the keyword ACTNUM from the EGRID
// file. ext_actnum must have size equal to the number of cells in the main grid
// if ext_actnum = NULL, actnum is taken from file, otherwise ext_actnums
// determines which cells are active.
ecl_grid_type *ecl_grid_alloc_ext_actnum(const char *grid_file,
                                         const int *ext_actnum) {
...
static bool ecl_grid_file_EGRID_dims(fortio_type *grid_fortio,
...
static bool ecl_grid_file_GRID_dims(fortio_type *grid_fortio,
                                    fortio_type *data_fortio, int *dims) {
  ...
  if ((grid_file_type == ECL_GRID_FILE) || (grid_file_type == ECL_EGRID_FILE)) {
    ...
    if (grid_fortio) {
      ...
      if (init_restart_filename) {
        ...
        data_fortio = fortio_open_reader(init_restart_filename, data_fmt_file,
                                         ECL_ENDIAN_FLIP);
      }
      ...
      if (grid_file_type == ECL_GRID_FILE)
        ...
        input_file_OK =
            ecl_grid_file_EGRID_dims(grid_fortio, data_fortio, dims);
...
char *ecl_grid_alloc_case_filename(const char *case_input) {
  ...
  if (file_type == ECL_GRID_FILE)
    ...
    if ((file_type == ECL_OTHER_FILE) ||
        (file_type == ECL_DATA_FILE)) { /* Case 3 - only basename recognized */
      char *EGRID =
          ecl_util_alloc_filename(path, basename, ECL_EGRID_FILE, false, -1);
      char *GRID =
          ecl_util_alloc_filename(path, basename, ECL_GRID_FILE, false, -1);
      char *FEGRID =
          ecl_util_alloc_filename(path, basename, ECL_EGRID_FILE, true, -1);
      char *FGRID =
          ecl_util_alloc_filename(path, basename, ECL_GRID_FILE, true, -1);
      ...
      */
      ...
      char *EGRID =
          ecl_util_alloc_filename(path, basename, ECL_EGRID_FILE, fmt_file, -1);
      char *GRID =
          ecl_util_alloc_filename(path, basename, ECL_GRID_FILE, fmt_file, -1);
...
ecl_grid_type *ecl_grid_load_case__(const char *case_input,
                                    bool apply_mapaxes) {
  ecl_grid_type *ecl_grid = NULL;
...
static bool ecl_grid_compare_coarse_cells(const ecl_grid_type *g1,
                                          const ecl_grid_type *g2,
                                          bool verbose) {
  if (vector_get_size(g1->coarse_cells) == vector_get_size(g2->coarse_cells)) {
    ...
    for (c = 0; c < vector_get_size(g1->coarse_cells); c++) {
      const ecl_coarse_cell_type *coarse_cell1 =
          (const ecl_coarse_cell_type *)vector_iget_const(g1->coarse_cells, c);
      const ecl_coarse_cell_type *coarse_cell2 =
          (const ecl_coarse_cell_type *)vector_iget_const(g2->coarse_cells, c);
...
static bool ecl_grid_compare_cells(const ecl_grid_type *g1,
                                   const ecl_grid_type *g2, bool include_nnc,
                                   bool verbose) {
  int g;
  bool equal = true;
  for (g = 0; g < g1->size; g++) {
...
static bool ecl_grid_compare_index(const ecl_grid_type *g1,
                                   const ecl_grid_type *g2, bool verbose) {
  bool equal = true;
...
static bool ecl_grid_compare_mapaxes(const ecl_grid_type *g1,
                                     const ecl_grid_type *g2, bool verbose) {
  bool equal = true;
...
static bool ecl_grid_compare__(const ecl_grid_type *g1, const ecl_grid_type *g2,
  ...
  // The name of the parent grid corresponds to a filename; they can be
  // different.
  ...
  if (equal && include_lgr) {
    if (vector_get_size(g1->LGR_list) == vector_get_size(g2->LGR_list)) {
      ...
      for (grid_nr = 0; grid_nr < vector_get_size(g1->LGR_list); grid_nr++) {
        const ecl_grid_type *lgr1 =
            (const ecl_grid_type *)vector_iget_const(g1->LGR_list, grid_nr);
        const ecl_grid_type *lgr2 =
            (const ecl_grid_type *)vector_iget_const(g2->LGR_list, grid_nr);
...
*/
static bool ecl_grid_cube_contains(const ecl_cell_type *cell,
                                   const point_type *p) {
...
/*
   Returns true if and only if p is on plane "plane" of cell when decomposed by
   "method".
*/
...
/*
   ...
   "belongs" to this cell. This is done such that every point is contained in at
   most one point.
   ...
   Note: The correctness of this function relies *HEAVILY* on the permutation of
   the tetrahedrons in the decompositions.
*/
static face_status_enum
ecl_grid_on_cell_face(const ecl_cell_type *cell, const int method,
                      const point_type *p, const bool max_i, const bool max_j,
                      const bool max_k) {
  ...
  bool on[6];
  for (int i = 0; i < 6; ++i) {
    on[i] = (ecl_grid_on_plane(cell, method, 2 * i, p) ||
...
/*
 Returns true if and only if the cell "cell" decomposed by "method" contains the
 point "p". This is done by decomposing the cell into 5 tetrahedrons according
 to the decomposition method for the faces.

 Assumes the cell to not be self-intersecting!

 Note: This function relies *HEAVILY* on the permutation of
 tetrahedron_permutations.
*/
static bool concave_cell_contains(const ecl_cell_type *cell, int method,
                                  const point_type *p) {
  ...
  if (tetrahedron_by_points_contains(dia[0][0], dia[1][0], dia[0][1], dia[1][1],
                                     p))
    return true;
...
*/
bool ecl_grid_cell_contains_xyz3(const ecl_grid_type *ecl_grid, int i, int j,
                                 int k, double x, double y, double z) {
  point_type p;
  ecl_cell_type *cell = ecl_grid_get_cell(
      ecl_grid, ecl_grid_get_global_index3(ecl_grid, i, j, k));
  point_set(&p, x, y, z);
  int method = (i + j + k) %
  ...
  face_status_enum face_status =
      ecl_grid_on_cell_face(cell, method, &p, max_i, max_j, max_k);
...
bool ecl_grid_cell_contains_xyz1(const ecl_grid_type *ecl_grid,
                                 int global_index, double x, double y,
                                 double z) {
  int i, j, k;
  ecl_grid_get_ijk1(ecl_grid, global_index, &i, &j, &k);
...
int ecl_grid_get_global_index_from_xy(const ecl_grid_type *ecl_grid, int k,
                                      bool lower_layer, double x, double y) {
  ...
  for (j = 0; j < ecl_grid->ny; j++)
    for (i = 0; i < ecl_grid->nx; i++) {
      int global_index = ecl_grid_get_global_index3(ecl_grid, i, j, k);
      if (ecl_cell_layer_contains_xy(ecl_grid_get_cell(ecl_grid, global_index),
                                     lower_layer, x, y))
        return global_index;
    }
...
int ecl_grid_get_global_index_from_xy_top(const ecl_grid_type *ecl_grid,
                                          double x, double y) {
  return ecl_grid_get_global_index_from_xy(ecl_grid, ecl_grid->nz - 1, false, x,
                                           y);
}

int ecl_grid_get_global_index_from_xy_bottom(const ecl_grid_type *ecl_grid,
                                             double x, double y) {
...
*/
static int ecl_grid_box_contains_xyz(const ecl_grid_type *grid, int i1, int i2,
    ...
    for (j = j1; j < j2; j++)
      for (i = i1; i < i2; i++) {
        ...
        if (!grid->visited[global_index]) {
          ...
          if (ecl_grid_cell_contains_xyz1(grid, global_index, p->x, p->y,
                                          p->z)) {
            return global_index;
          }
        }
 ...
 */
static int ecl_grid_get_global_index_from_xyz_around_box(
    ecl_grid_type *grid, double x, double y, double z, int start_index, int bx,
    point_type *p) {
  /* Try neighbours */
  int i, j, k;
...
*/
int ecl_grid_get_global_index_from_xyz(ecl_grid_type *grid, double x, double y,
  ...
  if (start_index >= 0) {
    ...
    if (ecl_grid_cell_contains_xyz1(grid, start_index, x, y, z))
      ...
      for (int bx = 1; bx <= 6; bx++) {
        global_index = ecl_grid_get_global_index_from_xyz_around_box(
...
bool ecl_grid_get_ijk_from_xyz(ecl_grid_type *grid, double x, double y,
                               double z, int start_index, int *i, int *j,
                               int *k) {
...
static bool ecl_grid_sublayer_contanins_xy__(const ecl_grid_type *grid,
                                             double x, double y, int k, int i1,
  ...
  for (i = i1; i < i2; i++) {
    double corner_pos[3];
    ecl_grid_get_corner_xyz(grid, i, j1, k, &corner_pos[0], &corner_pos[1],
                            &corner_pos[2]);
  ...
  for (j = j1; j < j2; j++) {
    double corner_pos[3];
    ecl_grid_get_corner_xyz(grid, i2, j, k, &corner_pos[0], &corner_pos[1],
                            &corner_pos[2]);
    geo_polygon_add_point(polygon, corner_pos[0], corner_pos[1]);
  }
  ...
  for (i = i2; i > i1; i--) {
    double corner_pos[3];
    ecl_grid_get_corner_xyz(grid, i, j2, k, &corner_pos[0], &corner_pos[1],
                            &corner_pos[2]);
    geo_polygon_add_point(polygon, corner_pos[0], corner_pos[1]);
  }
  ...
  for (j = j2; j > j1; j--) {
    double corner_pos[3];
    ecl_grid_get_corner_xyz(grid, i1, j, k, &corner_pos[0], &corner_pos[1],
                            &corner_pos[2]);
    geo_polygon_add_point(polygon, corner_pos[0], corner_pos[1]);
  }
...
bool ecl_grid_get_ij_from_xy(const ecl_grid_type *grid, double x, double y,
                             int k, int *i, int *j) {
  geo_polygon_type *polygon = geo_polygon_alloc(NULL);
  int nx = ecl_grid_get_nx(grid);
  int ny = ecl_grid_get_ny(grid);
  bool inside =
      ecl_grid_sublayer_contanins_xy__(grid, x, y, k, 0, nx, 0, ny, polygon);
  if (inside) {
    int i1 = 0;
    int i2 = nx;
    int j1 = 0;
    ...
    while (true) {
      if ((i2 - i1) > 1) {
        int ic = (i1 + i2) / 2;
        if (ecl_grid_sublayer_contanins_xy__(grid, x, y, k, i1, ic, j1, j2,
                                             polygon))
          i2 = ic;
        else {
          if (!ecl_grid_sublayer_contanins_xy__(grid, x, y, k, ic, i2, j1, j2,
                                                polygon))
      ...
      if ((j2 - j1) > 1) {
        int jc = (j1 + j2) / 2;
        if (ecl_grid_sublayer_contanins_xy__(grid, x, y, k, i1, i2, j1, jc,
                                             polygon))
          j2 = jc;
        else {
          if (!ecl_grid_sublayer_contanins_xy__(grid, x, y, k, i1, i2, jc, j2,
                                                polygon))
            util_abort("%s: point nowhere to be found ... \n", __func__);
          j1 = jc;
        }
...
bool ecl_grid_block_value_3d(ecl_grid_type *grid, double x, double y, double z,
                             double value) {
  if (grid->block_dim != 3)
...
double ecl_grid_block_eval3d(ecl_grid_type *grid, int i, int j, int k,
...
void ecl_grid_get_distance(const ecl_grid_type *grid, int global_index1,
...
void ecl_grid_get_dims(const ecl_grid_type *grid, int *nx, int *ny, int *nz,
                       int *active_size) {
  if (nx != NULL)
    *nx = grid->nx;
  if (ny != NULL)
...
int ecl_grid_get_nz(const ecl_grid_type *grid) { return grid->nz; }

int ecl_grid_get_nx(const ecl_grid_type *grid) { return grid->nx; }

int ecl_grid_get_ny(const ecl_grid_type *grid) { return grid->ny; }
...
int ecl_grid_get_global_index3(const ecl_grid_type *ecl_grid, int i, int j,
                               int k) {
...
int ecl_grid_get_global_index1A(const ecl_grid_type *ecl_grid,
                                int active_index) {
  return ecl_grid->inv_index_map[active_index];
}

int ecl_grid_get_global_index1F(const ecl_grid_type *ecl_grid,
                                int active_fracture_index) {
...
int ecl_grid_get_active_index3(const ecl_grid_type *ecl_grid, int i, int j,
                               int k) {
  int global_index = ecl_grid_get_global_index3(ecl_grid, i, j,
                                                k); /* In range: [0,nx*ny*nz) */
...
int ecl_grid_get_active_index1(const ecl_grid_type *ecl_grid,
                               int global_index) {
  return ecl_grid->index_map[global_index];
}

int ecl_grid_get_active_fracture_index3(const ecl_grid_type *ecl_grid, int i,
                                        int j, int k) {
  int global_index = ecl_grid_get_global_index3(ecl_grid, i, j,
                                                k); /* In range: [0,nx*ny*nz) */
...
int ecl_grid_get_active_fracture_index1(const ecl_grid_type *ecl_grid,
                                        int global_index) {
...
void ecl_grid_get_ijk1(const ecl_grid_type *grid, int global_index, int *i,
                       int *j, int *k) {
...
void ecl_grid_get_ijk1A(const ecl_grid_type *ecl_grid, int active_index, int *i,
                        int *j, int *k) {
  if (active_index >= 0 && active_index < ecl_grid->total_active) {
    ...
    util_abort(
        "%s: error active_index:%d invalid - grid has only:%d active cells. \n",
...
void ecl_grid_get_xyz1(const ecl_grid_type *grid, int global_index,
...
void ecl_grid_get_xyz3(const ecl_grid_type *grid, int i, int j, int k,
                       double *xpos, double *ypos, double *zpos) {
...
void ecl_grid_get_cell_corner_xyz1(const ecl_grid_type *grid, int global_index,
...
void ecl_grid_export_cell_corners1(const ecl_grid_type *grid, int global_index,
...
void ecl_grid_get_cell_corner_xyz3(const ecl_grid_type *grid, int i, int j,
                                   int k, int corner_nr, double *xpos,
                                   double *ypos, double *zpos) {
  const int global_index = ecl_grid_get_global_index__(grid, i, j, k);
  ecl_grid_get_cell_corner_xyz1(grid, global_index, corner_nr, xpos, ypos,
                                zpos);
}

void ecl_grid_get_corner_xyz(const ecl_grid_type *grid, int i, int j, int k,
                             double *xpos, double *ypos, double *zpos) {
  if (i < 0 || i > grid->nx)
...
void ecl_grid_get_xyz1A(const ecl_grid_type *grid, int active_index,
                        double *xpos, double *ypos, double *zpos) {
...
int ecl_grid_locate_depth(const ecl_grid_type *grid, double depth, int i,
                          int j) {
...
double ecl_grid_get_bottom2(const ecl_grid_type *grid, int i, int j) {
  const int global_index =
      ecl_grid_get_global_index__(grid, i, j, grid->nz - 1);
...
double ecl_grid_get_cell_thickness1(const ecl_grid_type *grid,
                                    int global_index) {
  return ecl_grid_get_cell_dz1(grid, global_index);
}

double ecl_grid_get_cell_thickness3(const ecl_grid_type *grid, int i, int j,
                                    int k) {
  return ecl_grid_get_cell_dz3(grid, i, j, k);
}

double ecl_grid_get_cell_thickness1A(const ecl_grid_type *grid,
                                     int active_index) {
...
const nnc_info_type *ecl_grid_get_cell_nnc_info1(const ecl_grid_type *grid,
                                                 int global_index) {
  const ecl_cell_type *cell = ecl_grid_get_cell(grid, global_index);
  return cell->nnc_info;
}

const nnc_info_type *ecl_grid_get_cell_nnc_info3(const ecl_grid_type *grid,
                                                 int i, int j, int k) {
  const int global_index = ecl_grid_get_global_index3(grid, i, j, k);
...
bool ecl_grid_cell_invalid3(const ecl_grid_type *ecl_grid, int i, int j,
                            int k) {
  int global_index = ecl_grid_get_global_index3(ecl_grid, i, j, k);
...
static void __assert_main_grid(const ecl_grid_type *ecl_grid) {
  if (ecl_grid->lgr_nr != ECL_GRID_MAINGRID_LGR_NR)
    util_abort("%s: tried to get LGR grid from another LGR_grid - only main "
               "grid can be used as first input \n",
               __func__);
}
...
ecl_grid_type *ecl_grid_get_lgr(const ecl_grid_type *main_grid,
...
ecl_grid_type *ecl_grid_iget_lgr(const ecl_grid_type *main_grid,
                                 int lgr_index) {
  __assert_main_grid(main_grid);
...
ecl_grid_type *ecl_grid_get_lgr_from_lgr_nr(const ecl_grid_type *main_grid,
                                            int lgr_nr) {
  __assert_main_grid(main_grid);
  {
...
const ecl_grid_type *ecl_grid_get_cell_lgr1(const ecl_grid_type *grid,
                                            int global_index) {
  const ecl_cell_type *cell = ecl_grid_get_cell(grid, global_index);
  return cell->lgr;
}

const ecl_grid_type *ecl_grid_get_cell_lgr3(const ecl_grid_type *grid, int i,
                                            int j, int k) {
  const int global_index = ecl_grid_get_global_index__(grid, i, j, k);
  return ecl_grid_get_cell_lgr1(grid, global_index);
}

const ecl_grid_type *ecl_grid_get_cell_lgr1A(const ecl_grid_type *grid,
                                             int active_index) {
  const int global_index = ecl_grid_get_global_index1A(grid, active_index);
  return ecl_grid_get_cell_lgr1(grid, global_index);
}
...
/**
    Allocates a stringlist instance with the lookup names of the lgr names in
   this grid.
*/
...
const char *ecl_grid_iget_lgr_name(const ecl_grid_type *ecl_grid,
                                   int lgr_index) {
  __assert_main_grid(ecl_grid);
  if (lgr_index < (vector_get_size(ecl_grid->LGR_list))) {
    const ecl_grid_type *lgr =
        (const ecl_grid_type *)vector_iget(ecl_grid->LGR_list, lgr_index);
    return lgr->name;
  } else
    return NULL;
}
...
bool ecl_grid_cell_regular3(const ecl_grid_type *ecl_grid, int i, int j,
                            int k) {
  int global_index = ecl_grid_get_global_index3(ecl_grid, i, j, k);
...
int ecl_grid_get_cell_twist3(const ecl_grid_type *ecl_grid, int i, int j,
                             int k) {
  int global_index = ecl_grid_get_global_index3(ecl_grid, i, j, k);
  return ecl_grid_get_cell_twist1(ecl_grid, global_index);
}

double ecl_grid_get_cell_volume1(const ecl_grid_type *ecl_grid,
                                 int global_index) {
  ecl_cell_type *cell = ecl_grid_get_cell(ecl_grid, global_index);
  int i, j, k;
  ecl_grid_get_ijk1(ecl_grid, global_index, &i, &j, &k);
...
double ecl_grid_get_cell_volume1A(const ecl_grid_type *ecl_grid,
                                  int active_index) {
  int global_index = ecl_grid_get_global_index1A(ecl_grid, active_index);
  return ecl_grid_get_cell_volume1(ecl_grid, global_index);
}

double ecl_grid_get_cell_volume3(const ecl_grid_type *ecl_grid, int i, int j,
                                 int k) {
  int global_index = ecl_grid_get_global_index3(ecl_grid, i, j, k);
  return ecl_grid_get_cell_volume1(ecl_grid, global_index);
}
...
void ecl_grid_summarize(const ecl_grid_type *ecl_grid) {
  ...
  if (ECL_GRID_MAINGRID_LGR_NR == ecl_grid->lgr_nr) {
    int grid_nr;
    for (grid_nr = 1; grid_nr < vector_get_size(ecl_grid->LGR_list);
         grid_nr++) {
      printf("\n");
      ecl_grid_summarize((const ecl_grid_type *)vector_iget_const(
          ecl_grid->LGR_list, grid_nr));
    }
  }
...
static int ecl_grid_get_property_index__(const ecl_grid_type *ecl_grid,
                                         const ecl_kw_type *ecl_kw, int i,
                                         int j, int k) {
...
static bool ecl_grid_get_property__(const ecl_grid_type *ecl_grid,
                                    const ecl_kw_type *ecl_kw, int i, int j,
                                    int k, void *value) {
...
double ecl_grid_get_double_property(const ecl_grid_type *ecl_grid,
                                    const ecl_kw_type *ecl_kw, int i, int j,
                                    int k) {
  ecl_data_type data_type = ecl_kw_get_data_type(ecl_kw);
...
int ecl_grid_get_int_property(const ecl_grid_type *ecl_grid,
                              const ecl_kw_type *ecl_kw, int i, int j, int k) {
  ecl_data_type data_type = ecl_kw_get_data_type(ecl_kw);
...
float ecl_grid_get_float_property(const ecl_grid_type *ecl_grid,
                                  const ecl_kw_type *ecl_kw, int i, int j,
                                  int k) {
  ecl_data_type data_type = ecl_kw_get_data_type(ecl_kw);
...
double ecl_grid_get_property(const ecl_grid_type *ecl_grid,
                             const ecl_kw_type *ecl_kw, int i, int j, int k) {
  ecl_data_type data_type = ecl_kw_get_data_type(ecl_kw);
  if (ecl_type_is_numeric(data_type)) {
    int lookup_index = ecl_grid_get_property_index__(ecl_grid, ecl_kw, i, j, k);
...
void ecl_grid_get_column_property(const ecl_grid_type *ecl_grid,
                                  const ecl_kw_type *ecl_kw, int i, int j,
  ...
  if (ecl_type_is_numeric(data_type)) {
    ...
    if (kw_size == ecl_grid->nx * ecl_grid->ny * ecl_grid->nz)
      ...
      util_abort(
          "%s: incommensurable sizes: nx*ny*nz = %d  nactive=%d  kw_size:%d \n",
          __func__, ecl_grid->size, ecl_grid->total_active,
      ...
      for (k = 0; k < ecl_grid->nz; k++) {
        if (use_global_index) {
          ...
          double_vector_iset(column, k,
                             ecl_kw_iget_as_double(ecl_kw, global_index));
        } else {
          int active_index = ecl_grid_get_active_index3(ecl_grid, i, j, k);
          if (active_index >= 0)
            double_vector_iset(column, k,
                               ecl_kw_iget_as_double(ecl_kw, active_index));
        }
      }
    }
  } else
...
int ecl_grid_get_region_cells(const ecl_grid_type *ecl_grid,
                              const ecl_kw_type *region_kw, int region_value,
            ...
            if (!active_only || (ecl_grid->index_map[global_index] >= 0)) {
              ...
              if (export_active_index)
                int_vector_iset(index_list, cells_found,
                                ecl_grid->index_map[global_index]);
              else
...
void ecl_grid_grdecl_fprintf_kw(const ecl_grid_type *ecl_grid,
  ...
  if (src_size == ecl_grid->size)
    ...
    {
      ecl_kw_type *tmp_kw = ecl_kw_alloc_scatter_copy(
          ecl_kw, ecl_grid->size, ecl_grid->inv_index_map, default_ptr);
      ecl_kw_fprintf_grdecl__(tmp_kw, special_header, stream);
      ecl_kw_free(tmp_kw);
    }
  } else
    util_abort("%s: size mismatch. ecl_kw must have either nx*ny*ny elements "
               "or nactive elements\n",
               __func__);
}
...
static bool ecl_grid_test_lgr_consistency2(const ecl_grid_type *parent,
  ...
  for (child_index = 0; child_index < child_size; child_index++) {
    ...
    if (parent_cell >= 0) {
      const ecl_grid_type *child_test =
          ecl_grid_get_cell_lgr1(parent, parent_cell);
      if (child != child_test) {
...
static void ecl_grid_dump_ascii__(ecl_grid_type *grid, bool active_only,
                                  FILE *stream) {
...
void ecl_grid_dump(const ecl_grid_type *grid, FILE *stream) {
  ecl_grid_dump__(grid, stream);
  {
    ...
    for (i = 0; i < vector_get_size(grid->LGR_list); i++)
      ecl_grid_dump__(
          (const ecl_grid_type *)vector_iget_const(grid->LGR_list, i), stream);
  }
}
...
void ecl_grid_dump_ascii(ecl_grid_type *grid, bool active_only, FILE *stream) {
  ecl_grid_dump_ascii__(grid, active_only, stream);
  {
    int i;
    for (i = 0; i < vector_get_size(grid->LGR_list); i++)
      ecl_grid_dump_ascii__((ecl_grid_type *)vector_iget(grid->LGR_list, i),
                            active_only, stream);
  }
}

void ecl_grid_dump_ascii_cell1(ecl_grid_type *grid, int global_index,
                               FILE *stream, const double *offset) {
  ecl_cell_type *cell = ecl_grid_get_cell(grid, global_index);
  int i, j, k;
...
void ecl_grid_dump_ascii_cell3(ecl_grid_type *grid, int i, int j, int k,
                               FILE *stream, const double *offset) {
  int global_index = ecl_grid_get_global_index3(grid, i, j, k);
  ecl_cell_type *cell = ecl_grid_get_cell(grid, global_index);
  ecl_cell_dump_ascii(cell, i, j, k, stream, offset);
...
void ecl_grid_init_mapaxes_data_double(const ecl_grid_type *grid,
                                       double *mapaxes) {
  int i;
  for (i = 0; i < 6; i++)
...
static void ecl_grid_fwrite_mapaxes(const ecl_grid_type *grid,
                                    fortio_type *fortio) {
...
static void ecl_grid_fwrite_main_GRID_headers(const ecl_grid_type *ecl_grid,
                                              fortio_type *fortio,
                                              ert_ecl_unit_enum output_unit) {
...
static void ecl_grid_fwrite_GRID__(const ecl_grid_type *grid, int coords_size,
                                   fortio_type *fortio,
                                   ert_ecl_unit_enum output_unit) {
    ...
    for (k = 0; k < grid->nz; k++) {
      for (j = 0; j < grid->ny; j++) {
        for (i = 0; i < grid->nx; i++) {
          ...
          ecl_cell_fwrite_GRID(grid, cell, false, coords_size, i, j, k,
                               global_index, coords_kw, corners_kw, fortio);
        }
      }
    }
    ...
    if (grid->dualp_flag != FILEHEAD_SINGLE_POROSITY) {
      for (k = grid->nz; k < 2 * grid->nz; k++) {
        for (j = 0; j < grid->ny; j++) {
          for (i = 0; i < grid->nx; i++) {
            int global_index =
                ecl_grid_get_global_index__(grid, i, j, k - grid->nz);
            const ecl_cell_type *cell = ecl_grid_get_cell(grid, global_index);

            ecl_cell_fwrite_GRID(grid, cell, true, coords_size, i, j, k,
                                 global_index, coords_kw, corners_kw, fortio);
          }
        }
      }
...
void ecl_grid_fwrite_GRID2(const ecl_grid_type *grid, const char *filename,
                           ert_ecl_unit_enum output_unit) {
  int coords_size = 5;
  bool fmt_file = false;
...

  {
    ...
    for (grid_nr = 0; grid_nr < vector_get_size(grid->LGR_list); grid_nr++) {
      const ecl_grid_type *igrid =
          (const ecl_grid_type *)vector_iget_const(grid->LGR_list, grid_nr);
...
static void ecl_grid_fwrite_main_EGRID_header(const ecl_grid_type *grid,
                                              fortio_type *fortio,
                                              ert_ecl_unit_enum output_unit) {
  int EGRID_VERSION = 3;
...

  {
    ...
    ecl_kw_iset_int(filehead_kw, FILEHEAD_TYPE_INDEX,
                    FILEHEAD_GRIDTYPE_CORNERPOINT);
    ecl_kw_iset_int(filehead_kw, FILEHEAD_DUALP_INDEX, grid->dualp_flag);
    ecl_kw_iset_int(filehead_kw, FILEHEAD_ORGFORMAT_INDEX,
...
static void ecl_grid_fwrite_gridhead_kw(int nx, int ny, int nz, int grid_nr,
                                        fortio_type *fortio) {
...
static int ecl_grid_get_valid_index(const ecl_grid_type *grid, int i, int j,
                                    int k1, int k2) {
  int global_index;
  int k = k1;
  int delta = (k1 < k2) ? 1 : -1;
...
static int ecl_grid_get_bottom_valid_index(const ecl_grid_type *grid, int i,
                                           int j) {
  return ecl_grid_get_valid_index(grid, i, j, grid->nz - 1, -1);
}

static int ecl_grid_get_top_valid_index(const ecl_grid_type *grid, int i,
                                        int j) {
  return ecl_grid_get_valid_index(grid, i, j, 0, grid->nz);
}

static bool ecl_grid_init_coord_section__(const ecl_grid_type *grid, int i,
                                          int j, int i_corner, int j_corner,
  ...
  if (top_index == -1)
    util_exit("% : no cell with a valid geometry description found in (i,j) = "
              "%d,%d - then what? \n",
              __func__, i, j);
    ...
    */
    ...
    int coord_offset = 6 * ((j + j_corner) * (grid->nx + 1) + (i + i_corner));
    {
      ...
      if ((top_point.z == bottom_point.z) && (force_set == false)) {
        ...
        if (grid->use_mapaxes) {
          point_mapaxes_invtransform(&top_point, grid->origo, grid->unit_x,
                                     grid->unit_y);
          point_mapaxes_invtransform(&bottom_point, grid->origo, grid->unit_x,
                                     grid->unit_y);
        }
...
static void ecl_grid_init_coord_section(const ecl_grid_type *grid, int i, int j,
                                        float *coord_float,
                                        double *coord_double) {
  int i_corner = 0;
  int j_corner = 0;
  ...
  ecl_grid_init_coord_section__(grid, i, j, i_corner, j_corner,
...
float *ecl_grid_alloc_coord_data(const ecl_grid_type *grid) {
  float *coord =
      (float *)util_calloc(ecl_grid_get_coord_size(grid), sizeof *coord);
...
void ecl_grid_assert_coord_kw(ecl_grid_type *grid) {
  if (grid->coord_kw == NULL) {
    grid->coord_kw =
        ecl_kw_alloc(COORD_KW, ecl_grid_get_coord_size(grid), ECL_FLOAT);
    ecl_grid_init_coord_data(grid,
                             (float *)ecl_kw_get_void_ptr(grid->coord_kw));
  }
}
    ...
    for (i = 0; i < nx; i++) {
      for (k = 0; k < nz; k++) {
        ...
        for (l = 0; l < 2; l++) {
          ...
          int z3 =
              k * 8 * nx * ny + j * 4 * nx + 2 * nx + 2 * i + l * 4 * nx * ny;
...
ecl_kw_type *ecl_grid_alloc_zcorn_kw(const ecl_grid_type *grid) {
  ecl_kw_type *zcorn_kw =
      ecl_kw_alloc(ZCORN_KW, ecl_grid_get_zcorn_size(grid), ECL_FLOAT);
...
ecl_kw_type *ecl_grid_alloc_coord_kw(const ecl_grid_type *grid) {
  ...
  ecl_kw_type *coord_kw = ecl_kw_alloc(
      COORD_KW, ECL_GRID_COORD_SIZE(grid->nx, grid->ny), ECL_FLOAT);
...
void ecl_grid_init_actnum_data(const ecl_grid_type *grid, int *actnum) {
  ...
  for (i = 0; i < grid->size; i++) {
    ...
    if (cell->coarse_group == COARSE_GROUP_NONE)
      ...
      ecl_coarse_cell_type *coarse_cell =
          ecl_grid_iget_coarse_group(grid, cell->coarse_group);
            ...
            and mark them correctly. */
      {
        ...
        for (j = 0; j < num_active; j++) {
          int global_index =
              ecl_coarse_cell_iget_active_cell_index(coarse_cell, j);
...
void ecl_grid_compressed_kw_copy(const ecl_grid_type *grid,
                                 ecl_kw_type *target_kw,
                                 const ecl_kw_type *src_kw) {
  if ((ecl_kw_get_size(target_kw) == ecl_grid_get_nactive(grid)) &&
      (ecl_kw_get_size(src_kw) == ecl_grid_get_global_size(grid))) {
    int active_index = 0;
    int global_index;
    for (global_index = 0; global_index < ecl_grid_get_global_size(grid);
         global_index++) {
      if (ecl_grid_cell_active1(grid, global_index)) {
        ecl_kw_iset(target_kw, active_index,
                    ecl_kw_iget_ptr(src_kw, global_index));
        active_index++;
      }
    }
  } else
    util_abort("%s: size mismatch target:%d  src:%d  expected %d,%d \n",
               ecl_kw_get_size(target_kw), ecl_kw_get_size(src_kw),
               ecl_grid_get_nactive(grid), ecl_grid_get_global_size(grid));
}

void ecl_grid_global_kw_copy(const ecl_grid_type *grid, ecl_kw_type *target_kw,
                             const ecl_kw_type *src_kw) {
  if ((ecl_kw_get_size(src_kw) == ecl_grid_get_nactive(grid)) &&
      (ecl_kw_get_size(target_kw) == ecl_grid_get_global_size(grid))) {
    int active_index = 0;
    int global_index;
    for (global_index = 0; global_index < ecl_grid_get_global_size(grid);
         global_index++) {
      if (ecl_grid_cell_active1(grid, global_index)) {
        ecl_kw_iset(target_kw, global_index,
                    ecl_kw_iget_ptr(src_kw, active_index));
        active_index++;
      }
    }
  } else
...
ecl_kw_type *ecl_grid_alloc_gridhead_kw(int nx, int ny, int nz, int grid_nr) {
  ...
  ecl_kw_iset_int(gridhead_kw, GRIDHEAD_TYPE_INDEX,
...
static void ecl_grid_fwrite_self_nnc(const ecl_grid_type *grid,
                                     fortio_type *fortio) {
  ...
  }
  {
    ...
    ecl_kw_type *nnc1_kw = ecl_kw_alloc_new_shared(NNC1_KW, num_nnc, ECL_INT,
                                                   int_vector_get_ptr(g1));
    ecl_kw_type *nnc2_kw = ecl_kw_alloc_new_shared(NNC2_KW, num_nnc, ECL_INT,
...
static void ecl_grid_fwrite_EGRID__(ecl_grid_type *grid, fortio_type *fortio,
  ...
  ecl_grid_fwrite_gridhead_kw(grid->nx, grid->ny, grid->nz, grid->lgr_nr,
                              fortio);
...
void ecl_grid_fwrite_EGRID2(ecl_grid_type *grid, const char *filename,
                            ert_ecl_unit_enum output_unit) {
  bool fmt_file = false;
  {
    bool is_fmt;
  ...
  ecl_grid_fwrite_EGRID__(grid, fortio, output_unit);
  {
    ...
    for (grid_nr = 0; grid_nr < vector_get_size(grid->LGR_list); grid_nr++) {
      ecl_grid_type *igrid =
          (ecl_grid_type *)vector_iget(grid->LGR_list, grid_nr);
...
void ecl_grid_fwrite_depth(const ecl_grid_type *grid, fortio_type *init_file,
                           ert_ecl_unit_enum output_unit) {
  ecl_kw_type *depth_kw =
      ecl_kw_alloc("DEPTH", ecl_grid_get_nactive(grid), ECL_FLOAT);
  {
...
void ecl_grid_fwrite_dims(const ecl_grid_type *grid, fortio_type *init_file,
                          ert_ecl_unit_enum output_unit) {
...
void ecl_grid_fprintf_grdecl2(ecl_grid_type *grid, FILE *stream,
                              ert_ecl_unit_enum output_unit) {
  {
    ecl_kw_type *mapunits_kw = ecl_grid_alloc_mapunits_kw(output_unit);
...
void ecl_grid_cell_ri_export(const ecl_grid_type *ecl_grid, int global_index,
                             double *ri_points) {
  const ecl_cell_type *cell = ecl_grid_get_cell(ecl_grid, global_index);
...
static ecl_kw_type *ecl_grid_alloc_volume_kw_active(const ecl_grid_type *grid) {
  ecl_kw_type *volume_kw =
      ecl_kw_alloc("VOLUME", ecl_grid_get_active_size(grid), ECL_DOUBLE);
  {
    double *volume_data = (double *)ecl_kw_get_ptr(volume_kw);
    int active_index;
    for (active_index = 0; active_index < ecl_grid_get_active_size(grid);
         active_index++) {
...
static ecl_kw_type *ecl_grid_alloc_volume_kw_global(const ecl_grid_type *grid) {
  ecl_kw_type *volume_kw =
      ecl_kw_alloc("VOLUME", ecl_grid_get_global_size(grid), ECL_DOUBLE);
  {
    double *volume_data = (double *)ecl_kw_get_ptr(volume_kw);
    int global_index;
    for (global_index = 0; global_index < ecl_grid_get_global_size(grid);
...
ecl_kw_type *ecl_grid_alloc_volume_kw(const ecl_grid_type *grid,
                                      bool active_size) {
  if (active_size)
...
// Note: global_index must be allocated w/ ecl_grid->total_active or
// ecl_grid->global_size int32 data points
//      index_data must be allocated w/ (4 * ecl_grid->total_active or
//      ecl_grid->global_size) int32 data points.
void ecl_grid_export_index(const ecl_grid_type *grid, int *global_index,
...
void ecl_grid_export_data_as_int(int index_size, const int *data_index,
                                 const ecl_kw_type *kw, int *output) {
...
// This function is meant to be used w/ pandas datafram and numpy
// Note: index_size must equal allocated size of output
void ecl_grid_export_data_as_double(int index_size, const int *data_index,
                                    const ecl_kw_type *kw, double *output) {
  for (int i = 0; i < index_size; i++) {
    int di = data_index[i];
    if (di >= 0)
...
// This function is meant to be used w/ pandas datafram and numpy
void ecl_grid_export_volume(const ecl_grid_type *grid, int index_size,
...
// This function is meant to be used w/ pandas datafram and numpy
void ecl_grid_export_position(const ecl_grid_type *grid, int index_size,
                              const int *global_index, double *output) {
  for (int i = 0; i < index_size; i++) {
    int g = global_index[i];
    int j = 3 * i;
...
void export_corners(const ecl_grid_type *grid, int index_size,
                    const int *global_index, double *output) {
  double x[8], y[8], z[8];
  int pos = 0;
  for (int i = 0; i < index_size; i++) {
