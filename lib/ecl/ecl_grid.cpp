...
struct ecl_grid_struct {
  ...
  */
    vector_type *
        LGR_list; /* a vector of ecl_grid instances for LGRs - the index corresponds to the order LGRs are read from file*/
    int_vector_type *
    ...
    std::unordered_map<std::string, ecl_grid_type *>
        LGR_hash; /* a hash of pointers to ecl_grid instances - for name based lookup of lgr. */
    int parent_box
            ...
            for (c = 0; c < 8; c++) {
                ...
                if (ecl_grid->use_mapaxes)
                    point_mapaxes_transform(&cell->corner_list[c],
                                            ecl_grid->origo, ecl_grid->unit_x,
                                            ecl_grid->unit_y);
            }
        }
    }
  ...
  */
    ...
    for (int coarse_group = 0; coarse_group < size; coarse_group++) {
        ...
        if (ecl_coarse_cell_get_num_active(coarse_cell) > 0) {
            int global_index =
                ecl_coarse_cell_iget_active_cell_index(coarse_cell, 0);
            int active_value =
            ...
            if (active_value & CELL_ACTIVE_FRACTURE)
                ecl_grid->inv_fracture_index_map[active_fracture_index] =
                    global_index;
...
static void ecl_grid_set_active_index(ecl_grid_type *ecl_grid) {
    ...
    if (!ecl_grid_have_coarse_cells(ecl_grid)) {
       ...
       groups and single porosity. */
        ...
        if (ecl_grid->dualp_flag != FILEHEAD_SINGLE_POROSITY) {
            for (global_index = 0; global_index < ecl_grid->size;
                 global_index++) {
                ecl_cell_type *cell = ecl_grid_get_cell(ecl_grid, global_index);
                if (cell->active & CELL_ACTIVE_FRACTURE) {
        ...
        for (int coarse_index = 0;
             ...
             coarse_index++) {
            ecl_coarse_cell_type *coarse_cell =
                (ecl_coarse_cell_type *)vector_iget_const(
                    ecl_grid->coarse_cells, coarse_index);
        ...
        for (global_index = 0; global_index < ecl_grid->size; global_index++) {
            ...
            if (cell->active != CELL_NOT_ACTIVE) {
                if (cell->coarse_group == COARSE_GROUP_NONE) {
                    ...
                    ecl_coarse_cell_type *coarse_cell =
                        ecl_grid_iget_coarse_group(ecl_grid,
                                                   cell->coarse_group);
        ...
        for (int coarse_group = 0; coarse_group < size; coarse_group++) {
            ...
            if (ecl_coarse_cell_get_num_active(coarse_cell) > 0) {
                int cell_active_index =
                    ecl_coarse_cell_get_active_index(coarse_cell);
                ...
                for (int i = 0; i < group_size; i++) {
                    ...
                    ecl_cell_type *cell =
                        ecl_grid_get_cell(ecl_grid, global_index);
...
ecl_grid_get_or_create_coarse_cell(ecl_grid_type *ecl_grid, int coarse_nr) {
    ...
    return (ecl_coarse_cell_type *)vector_iget(ecl_grid->coarse_cells,
                                               coarse_nr);
}
...
static void ecl_grid_init_coarse_cells(ecl_grid_type *ecl_grid) {
    if (ecl_grid->coarsening_active) {
        ...
        for (global_index = 0; global_index < ecl_grid->size; global_index++) {
            ...
            if (cell->coarse_group != COARSE_GROUP_NONE) {
                ecl_coarse_cell_type *coarse_cell =
                    ecl_grid_get_or_create_coarse_cell(ecl_grid,
                                                       cell->coarse_group);
                int i, j, k;
...
static void ecl_grid_set_lgr_name_EGRID(ecl_grid_type *lgr_grid,
    ...
    lgr_grid->name =
        (char *)util_alloc_strip_copy((const char *)ecl_kw_iget_ptr(
...

        {
            ...
            for (k = 0; k < nz; k++) {
...

                {
                    int ip;
                    for (ip = 0; ip < 4; ip++)
                        ecl_grid_pillar_cross_planes(&pillars[ip][0], ex[ip],
                                                     ey[ip], ez[ip], z[ip],
                                                     x[ip], y[ip]);
                }
...
#pragma omp parallel for
    for (j = 0; j < ny; j++)
        ecl_grid_init_GRDECL_data_jslice(ecl_grid, zcorn, coord, actnum,
                                         corsnum, j);
}
...
static ecl_grid_type *ecl_grid_alloc_GRDECL_data__(
    ...
    if (ecl_grid) {
        ...
        ecl_grid->coord_kw = ecl_kw_alloc_new("COORD", 6 * (nx + 1) * (ny + 1),
                                              ECL_FLOAT, coord);
        ecl_grid_init_GRDECL_data(ecl_grid, zcorn, coord, actnum, corsnum);
...
static void ecl_grid_copy_content(ecl_grid_type *target_grid,
    ...
    for (global_index = 0; global_index < src_grid->size; global_index++) {
        ecl_cell_type *target_cell =
            ecl_grid_get_cell(target_grid, global_index);
        const ecl_cell_type *src_cell =
            ecl_grid_get_cell(src_grid, global_index);
...

    {
        ...
        for (grid_nr = 0; grid_nr < vector_get_size(src_grid->LGR_list);
            ...
            const ecl_grid_type *src_lgr =
                (const ecl_grid_type *)vector_iget_const(src_grid->LGR_list,
                                                         grid_nr);
...
ecl_grid_type *ecl_grid_alloc_processed_copy(const ecl_grid_type *src_grid,
    ...
    if (zcorn == NULL) {
        ...
        grid =
            ecl_grid_alloc_GRDECL_data(nx, ny, nz, zcorn_float, coord, actnum,
...
static void ecl_grid_init_nnc_cells(ecl_grid_type *grid1, ecl_grid_type *grid2,
  ...
  */
        ...
        {
            ecl_cell_type *grid1_cell =
                ecl_grid_get_cell(grid1, grid1_cell_index);
    ...
    for (i = 0; i < num_nnchead_kw; i++) {
        ...
        if (ecl_file_view_has_kw(lgr_view, NNC1_KW)) {
...

            {
                ecl_grid_type *grid =
                    (lgr_nr > 0)
                        ? ecl_grid_get_lgr_from_lgr_nr(main_grid, lgr_nr)
                        : main_grid;
...
static void ecl_grid_init_nnc_amalgamated(ecl_grid_type *main_grid,
    ...
    for (i = 0; i < num_nncheada_kw; i++) {
        ecl_kw_type *nncheada_kw =
            ecl_file_iget_named_kw(ecl_file, NNCHEADA_KW, i);
        int lgr_nr1 = ecl_kw_iget_int(nncheada_kw, NNCHEADA_ILOC1_INDEX);
        int lgr_nr2 = ecl_kw_iget_int(nncheada_kw, NNCHEADA_ILOC2_INDEX);

        ecl_grid_type *lgr_grid1 =
            ecl_grid_get_lgr_from_lgr_nr(main_grid, lgr_nr1);
        ecl_grid_type *lgr_grid2 =
            ecl_grid_get_lgr_from_lgr_nr(main_grid, lgr_nr2);
...
static ecl_grid_type *ecl_grid_alloc_EGRID_all_grids(const char *grid_file,
            ...
            __func__, grid_file);
    {
        ...
        if (ecl_file) {
            ...
            ecl_grid_type *main_grid = ecl_grid_alloc_EGRID__(
                NULL, ecl_file, 0, apply_mapaxes, ext_actnum);
            int grid_nr;

            for (grid_nr = 1; grid_nr < num_grid; grid_nr++) {
                // The apply_mapaxes argument is ignored for LGR -
                //   it inherits from parent anyway.
                ecl_grid_type *lgr_grid = ecl_grid_alloc_EGRID__(
                    main_grid, ecl_file, grid_nr, false, NULL);
                ecl_grid_add_lgr(main_grid, lgr_grid);
                {
                    ecl_grid_type *host_grid;
                    ecl_kw_type *hostnum_kw = ecl_file_iget_named_kw(
                        ecl_file, HOSTNUM_KW, grid_nr - 1);
                    if (lgr_grid->parent_name == NULL)
                        host_grid = main_grid;
                    else
                        host_grid =
                            ecl_grid_get_lgr(main_grid, lgr_grid->parent_name);
...
static int ecl_grid_dual_porosity_GRID_check(ecl_file_type *ecl_file) {
    ...
    if ((nz % 2) == 1)
        ...
        while (true) {
            matrix_kw =
                ecl_file_iget_named_kw(ecl_file, CORNERS_KW, matrix_index);
            fracture_kw =
  ...
  */

    {
        ...
        //    CORNERS keywords.
        {
            ...
            for (index = 0; index < num_coords; index++) {
                const ecl_kw_type *coords_kw = ecl_file_iget_named_kw(
                    ecl_file, COORDS_KW, index + cell_offset);
                const ecl_kw_type *corners_kw = ecl_file_iget_named_kw(
                    ecl_file, CORNERS_KW, index + cell_offset);
        ...
        for (k = 0; k < nz; k++) {
            for (j = 0; j < ny; j++) {
                for (i = 0; i < nx; i++) {
                    ...
                    ecl_cell_init_regular(cell, offset, i, j, k, global_index,
    ...
    if ((grid_file_type == ECL_GRID_FILE) ||
        ...
        if (grid_fortio) {
            ...
            if (init_restart_filename) {
                ecl_util_get_file_type(init_restart_filename, &data_fmt_file,
                                       NULL);
                data_fortio = fortio_open_reader(
                    init_restart_filename, data_fmt_file, ECL_ENDIAN_FLIP);
            }

            if (grid_file_type == ECL_GRID_FILE)
                input_file_OK =
                    ecl_grid_file_GRID_dims(grid_fortio, data_fortio, dims);
            else
                input_file_OK =
...
char *ecl_grid_alloc_case_filename(const char *case_input) {
    ...
    if (file_type == ECL_GRID_FILE)
        ...
        if ((file_type == ECL_OTHER_FILE) ||
            (file_type ==
             ECL_DATA_FILE)) { /* Case 3 - only basename recognized */
            char *EGRID = ecl_util_alloc_filename(path, basename,
                                                  ECL_EGRID_FILE, false, -1);
            char *GRID = ecl_util_alloc_filename(path, basename, ECL_GRID_FILE,
                                                 false, -1);
            char *FEGRID = ecl_util_alloc_filename(path, basename,
                                                   ECL_EGRID_FILE, true, -1);
            char *FGRID = ecl_util_alloc_filename(path, basename, ECL_GRID_FILE,
                                                  true, -1);
    ...
    if (vector_get_size(g1->coarse_cells) ==
        ...
        for (c = 0; c < vector_get_size(g1->coarse_cells); c++) {
            const ecl_coarse_cell_type *coarse_cell1 =
                (const ecl_coarse_cell_type *)vector_iget_const(
                    g1->coarse_cells, c);
        ...
        if (vector_get_size(g1->LGR_list) == vector_get_size(g2->LGR_list)) {
            ...
            for (grid_nr = 0; grid_nr < vector_get_size(g1->LGR_list);
                ...
                const ecl_grid_type *lgr1 =
                    (const ecl_grid_type *)vector_iget_const(g1->LGR_list,
                                                             grid_nr);
                const ecl_grid_type *lgr2 =
                    (const ecl_grid_type *)vector_iget_const(g2->LGR_list,
                                                             grid_nr);
...
int ecl_grid_get_global_index_from_xy(const ecl_grid_type *ecl_grid, int k,
    ...
    for (j = 0; j < ecl_grid->ny; j++)
        for (i = 0; i < ecl_grid->nx; i++) {
            int global_index = ecl_grid_get_global_index3(ecl_grid, i, j, k);
            if (ecl_cell_layer_contains_xy(
                    ecl_grid_get_cell(ecl_grid, global_index), lower_layer, x,
                    y))
                return global_index;
        }
...
int ecl_grid_get_global_index_from_xy_top(const ecl_grid_type *ecl_grid,
                                          double x, double y) {
    return ecl_grid_get_global_index_from_xy(ecl_grid, ecl_grid->nz - 1, false,
                                             x, y);
}
        ...
        for (j = j1; j < j2; j++)
            for (i = i1; i < i2; i++) {
                ...
                if (!grid->visited[global_index]) {
                    ...
                    if (ecl_grid_cell_contains_xyz1(grid, global_index, p->x,
                                                    p->y, p->z)) {
                        return global_index;
                    }
                }
...
static int ecl_grid_get_global_index_from_xyz_around_box(
    ...
    int global_index =
        ecl_grid_box_contains_xyz(grid, i1, i2, j1, j2, k1, k2, p);
    return global_index;
}
...
bool ecl_grid_get_ij_from_xy(const ecl_grid_type *grid, double x, double y,
    ...
    if (inside) {
        ...
        while (true) {
            if ((i2 - i1) > 1) {
                int ic = (i1 + i2) / 2;
                if (ecl_grid_sublayer_contanins_xy__(grid, x, y, k, i1, ic, j1,
                                                     j2, polygon))
                    i2 = ic;
                else {
                    if (!ecl_grid_sublayer_contanins_xy__(grid, x, y, k, ic, i2,
                                                          j1, j2, polygon))
            ...
            if ((j2 - j1) > 1) {
                int jc = (j1 + j2) / 2;
                if (ecl_grid_sublayer_contanins_xy__(grid, x, y, k, i1, i2, j1,
                                                     jc, polygon))
                    j2 = jc;
                else {
                    if (!ecl_grid_sublayer_contanins_xy__(grid, x, y, k, i1, i2,
                                                          jc, j2, polygon))
                        util_abort("%s: point nowhere to be found ... \n",
                                   __func__);
                    j1 = jc;
...
int ecl_grid_get_active_index3(const ecl_grid_type *ecl_grid, int i, int j,
    ...
    int global_index = ecl_grid_get_global_index3(
        ecl_grid, i, j, k); /* In range: [0,nx*ny*nz) */
...
void ecl_grid_get_corner_xyz(const ecl_grid_type *grid, int i, int j, int k,
...

    {
        ...
        ecl_grid_get_cell_corner_xyz3(grid, i, j, k, corner_nr, xpos, ypos,
                                      zpos);
    }
}
...
static bool ecl_grid_get_property__(const ecl_grid_type *ecl_grid,
    ...
    if (ecl_type_is_numeric(data_type)) {
        int lookup_index =
            ecl_grid_get_property_index__(ecl_grid, ecl_kw, i, j, k);
    ...
    if (ecl_type_is_numeric(data_type)) {
        ...
        double_vector_reset(column);
        {
            ...
            for (k = 0; k < ecl_grid->nz; k++) {
                if (use_global_index) {
                    int global_index =
                        ecl_grid_get_global_index3(ecl_grid, i, j, k);
                    double_vector_iset(
                        column, k, ecl_kw_iget_as_double(ecl_kw, global_index));
                } else {
                    int active_index =
                        ecl_grid_get_active_index3(ecl_grid, i, j, k);
                    if (active_index >= 0)
                        double_vector_iset(
                            column, k,
...

            {
                ...
                for (global_index = 0; global_index < ecl_grid->size;
                    ...
                    if (region_ptr[global_index] == region_value) {
                        if (!active_only ||
                            (ecl_grid->index_map[global_index] >= 0)) {
...
void ecl_grid_grdecl_fprintf_kw(const ecl_grid_type *ecl_grid,
    ...
    if (src_size == ecl_grid->size)
        ...
        ecl_type_enum ecl_type =
            ecl_type_get_type(ecl_kw_get_data_type(ecl_kw));
        ...
        util_abort("%s: size mismatch. ecl_kw must have either nx*ny*ny "
...
void ecl_grid_dump(const ecl_grid_type *grid, FILE *stream) {
    ecl_grid_dump__(grid, stream);
    {
        ...
        for (i = 0; i < vector_get_size(grid->LGR_list); i++)
            ecl_grid_dump__(
                (const ecl_grid_type *)vector_iget_const(grid->LGR_list, i),
                stream);
    }
}
...
void ecl_grid_dump_ascii(ecl_grid_type *grid, bool active_only, FILE *stream) {
    ecl_grid_dump_ascii__(grid, active_only, stream);
    {
        int i;
        for (i = 0; i < vector_get_size(grid->LGR_list); i++)
            ecl_grid_dump_ascii__(
                (ecl_grid_type *)vector_iget(grid->LGR_list, i), active_only,
                stream);
    }
}
...

    {
        ...
        for (k = 0; k < grid->nz; k++) {
            for (j = 0; j < grid->ny; j++) {
                for (i = 0; i < grid->nx; i++) {
                    int global_index =
                        ecl_grid_get_global_index__(grid, i, j, k);
                    const ecl_cell_type *cell =
                        ecl_grid_get_cell(grid, global_index);

                    ecl_cell_fwrite_GRID(grid, cell, false, coords_size, i, j,
                                         k, global_index, coords_kw, corners_kw,
                                         fortio);
                }
            }
        }
        ...
        if (grid->dualp_flag != FILEHEAD_SINGLE_POROSITY) {
            for (k = grid->nz; k < 2 * grid->nz; k++) {
                for (j = 0; j < grid->ny; j++) {
                    for (i = 0; i < grid->nx; i++) {
                        int global_index = ecl_grid_get_global_index__(
                            grid, i, j, k - grid->nz);
                        const ecl_cell_type *cell =
                            ecl_grid_get_cell(grid, global_index);

                        ecl_cell_fwrite_GRID(grid, cell, true, coords_size, i,
                                             j, k, global_index, coords_kw,
                                             corners_kw, fortio);
                    }
                }
            }
...

    {
        ...
        for (grid_nr = 0; grid_nr < vector_get_size(grid->LGR_list);
            ...
            const ecl_grid_type *igrid =
                (const ecl_grid_type *)vector_iget_const(grid->LGR_list,
                                                         grid_nr);
...
static bool ecl_grid_init_coord_section__(const ecl_grid_type *grid, int i,
...

    {
        ...
        const ecl_cell_type *bottom_cell =
            ecl_grid_get_cell(grid, bottom_index);
        ...
        for (i = 0; i < nx; i++) {
            for (k = 0; k < nz; k++) {
                const int cell_index =
                    ecl_grid_get_global_index3(grid, i, j, k);
         ...
         the original, uncoarsened distribution of actnum values. */
            ...
            and mark them correctly. */
            {
                ...
                for (j = 0; j < num_active; j++) {
                    ...
                    int active_value =
                        ecl_coarse_cell_iget_active_value(coarse_cell, j);
...
static void ecl_grid_fwrite_self_nnc(const ecl_grid_type *grid,
    ...
    }
    {
        ...
        ecl_kw_type *nnc1_kw = ecl_kw_alloc_new_shared(
            NNC1_KW, num_nnc, ECL_INT, int_vector_get_ptr(g1));
        ecl_kw_type *nnc2_kw = ecl_kw_alloc_new_shared(
            NNC2_KW, num_nnc, ECL_INT, int_vector_get_ptr(g2));
        ecl_kw_type *nnchead_kw =
            ecl_kw_alloc(NNCHEAD_KW, NNCHEAD_SIZE, ECL_INT);
...
static void ecl_grid_fwrite_EGRID__(ecl_grid_type *grid, fortio_type *fortio,
    ...
    if (!is_lgr) {
        ...
        {
            ecl_kw_type *lgr_parent_kw =
                ecl_kw_alloc(LGR_PARENT_KW, 1, ECL_CHAR);
    ...
    /* Writing main grid data */
    {
        ecl_grid_assert_coord_kw(grid);
        {
            ...
            if (output_unit != grid->unit_system) {
                double scale_factor =
                    ecl_grid_output_scaling(grid, output_unit);
