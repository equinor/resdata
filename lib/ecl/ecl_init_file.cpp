...
static ecl_kw_type *ecl_init_file_alloc_INTEHEAD(const ecl_grid_type *ecl_grid,
                                                 ert_ecl_unit_enum unit_system,
                                                 int phases, time_t start_date,
                                                 int simulator) {
    ecl_kw_type *intehead_kw =
        ecl_kw_alloc(INTEHEAD_KW, INTEHEAD_INIT_SIZE, ECL_INT);
    ...
    ecl_kw_iset_int(intehead_kw, INTEHEAD_NACTIVE_INDEX,
                    ecl_grid_get_active_size(ecl_grid));
...
static ecl_kw_type *ecl_init_file_alloc_LOGIHEAD(int simulator) {
  ...
  */
    ...
    ecl_kw_type *logihead_kw =
        ecl_kw_alloc(LOGIHEAD_KW, LOGIHEAD_INIT_SIZE, ECL_BOOL);
    ...
    ecl_kw_iset_bool(logihead_kw, LOGIHEAD_DIR_RELPERM_INDEX,
                     with_directional_relperm);

    if (simulator == INTEHEAD_ECLIPSE100_VALUE) {
        ecl_kw_iset_bool(logihead_kw, LOGIHEAD_REV_RELPERM100_INDEX,
                         with_reversible_relperm_ECLIPSE100);
        ecl_kw_iset_bool(logihead_kw, LOGIHEAD_RADIAL100_INDEX,
                         radial_grid_ECLIPSE100);
    } else {
        ecl_kw_iset_bool(logihead_kw, LOGIHEAD_REV_RELPERM300_INDEX,
                         with_reversible_relperm_ECLIPSE300);
        ecl_kw_iset_bool(logihead_kw, LOGIHEAD_RADIAL300_INDEX,
    ...
    ecl_kw_iset_bool(logihead_kw, LOGIHEAD_ENDPOINT_SCALING_INDEX,
                     endpoint_scaling);
    ecl_kw_iset_bool(logihead_kw, LOGIHEAD_DIR_ENDPOINT_SCALING_INDEX,
                     directional_endpoint_scaling);
    ecl_kw_iset_bool(logihead_kw, LOGIHEAD_REV_ENDPOINT_SCALING_INDEX,
                     reversible_endpoint_scaling);
    ecl_kw_iset_bool(logihead_kw, LOGIHEAD_ALT_ENDPOINT_SCALING_INDEX,
                     alternative_endpoint_scaling);
    ecl_kw_iset_bool(logihead_kw, LOGIHEAD_MISC_DISPLACEMENT_INDEX,
                     miscible_displacement);
    ecl_kw_iset_bool(logihead_kw, LOGIHEAD_SCALE_WATER_PC_AT_MAX_SAT_INDEX,
                     scale_water_PC_at_max_sat);
    ecl_kw_iset_bool(logihead_kw, LOGIHEAD_SCALE_GAS_PC_AT_MAX_SAT_INDEX,
...
static ecl_kw_type *ecl_init_file_alloc_DOUBHEAD() {
    ecl_kw_type *doubhead_kw =
        ecl_kw_alloc(DOUBHEAD_KW, DOUBHEAD_INIT_SIZE, ECL_DOUBLE);
...
static void ecl_init_file_fwrite_poro(fortio_type *fortio,
                                      const ecl_grid_type *ecl_grid,
                                      const ecl_kw_type *poro) {
    {
        ecl_kw_type *porv = ecl_kw_alloc(
            PORV_KW, ecl_grid_get_global_size(ecl_grid), ECL_FLOAT);
        int global_index;
        bool global_poro =
            (ecl_kw_get_size(poro) == ecl_grid_get_global_size(ecl_grid))
                ? true
                : false;
        for (global_index = 0;
            ...
            if (active_index >= 0) {
                ...
                ecl_kw_iset_float(
                    porv, global_index,
                    ecl_kw_iget_float(poro, poro_index) *
                        ecl_grid_get_cell_volume1(ecl_grid, global_index));
            } else
...
void ecl_init_file_fwrite_header(fortio_type *fortio,
                                 const ecl_grid_type *ecl_grid,
                                 const ecl_kw_type *poro,
    ...
    {
        ecl_kw_type *intehead_kw = ecl_init_file_alloc_INTEHEAD(
            ecl_grid, unit_system, phases, start_date, simulator);
    ...
    if (poro) {
        ...
        if ((poro_size == ecl_grid_get_nactive(ecl_grid)) ||
            (poro_size == ecl_grid_get_global_size(ecl_grid)))
            ecl_init_file_fwrite_poro(fortio, ecl_grid, poro);
        else
            util_abort("%s: keyword PORO has wrong size:%d  Grid: %d/%d \n",
                       __func__, ecl_kw_get_size(poro),
                       ecl_grid_get_nactive(ecl_grid),
                       ecl_grid_get_global_size(ecl_grid));
    }
}
