  ...
  */
    if (ecl_type_is_char(ecl_kw->data_type) ||
        ...
        for (int i = 0; i < ecl_kw->size; i++) {
            size_t buffer_offset = i * sizeof_iotype;
            size_t data_offset = i * sizeof_ctype;
            memcpy(&ecl_kw->data[data_offset], &buffer[buffer_offset],
                   sizeof_iotype);
...
bool ecl_kw_numeric_equal(const ecl_kw_type *ecl_kw1,
    ...
    if (ecl_type_is_float(ecl_kw1->data_type))
        ...
        return ecl_kw_numeric_equal_double(ecl_kw1, ecl_kw2, abs_diff,
                                           rel_diff);
    else
        ...
        util_abort("%s: stride:%d completely broken ...\n", __func__, stride);
    {
        ...
        /* 1: Determine size of the sliced copy. */
        {
            ...
            if (new_size > 0) {
                ...
                ecl_kw_initialize(new_kw, src->header, new_size,
                                  src->data_type);
                ...
                src_index = index1;
                {
                    ...
                    int sizeof_ctype =
                        ecl_type_get_sizeof_ctype(new_kw->data_type);
...
void ecl_kw_resize(ecl_kw_type *ecl_kw, int new_size) {
    if (ecl_kw->shared_data)
        util_abort("%s: trying to allocate data for ecl_kw object which has "
...
bool ecl_kw_fread_data(ecl_kw_type *ecl_kw, fortio_type *fortio) {
    ...
    if (ecl_kw->size > 0) {
        const int blocksize = get_blocksize(ecl_kw->data_type);
        if (fmt_file) {
            const int blocks = ecl_kw->size / blocksize +
                               (ecl_kw->size % blocksize == 0 ? 0 : 1);
            ...
            for (ib = 0; ib < blocks; ib++) {
                int read_elm =
                    util_int_min((ib + 1) * blocksize, ecl_kw->size) -
                    ib * blocksize;
                for (ir = 0; ir < read_elm; ir++) {
                    switch (ecl_kw_get_type(ecl_kw)) {
                    case (ECL_CHAR_TYPE):
                        ecl_kw_fscanf_qstring(&ecl_kw->data[offset], read_fmt,
                                              8, stream);
                        break;
                    case (ECL_STRING_TYPE):
                        ecl_kw_fscanf_qstring(&ecl_kw->data[offset], read_fmt,
            ...
            const int sizeof_iotype =
                ecl_type_get_sizeof_iotype(ecl_kw->data_type);
            bool read_ok = fortio_fread_buffer(fortio, buffer,
                                               ecl_kw->size * sizeof_iotype);
...
void ecl_kw_fwrite_header(const ecl_kw_type *ecl_kw, fortio_type *fortio) {
    ...
    if (fmt_file)
        fprintf(stream, WRITE_HEADER_FMT, ecl_kw->header8, ecl_kw->size,
                type_name);
    else {
...
void ecl_kw_fwrite_param_fortio(fortio_type *fortio, const char *header,
    ...
    ecl_kw_type *ecl_kw =
        ecl_kw_alloc_new_shared(header, size, data_type, data);
...
#define ECL_KW_TYPED_INPLACE_ADD_INDEXED(ctype)                                \
    static void ecl_kw_inplace_add_indexed_##ctype(                            \
        ...
        {                                                                      \
            ...
            const ctype *add_data =                                            \
                (const ctype *)ecl_kw_get_data_ref(add_kw);                    \
            int set_size = int_vector_size(index_set);                         \
            const int *index_data = int_vector_get_const_ptr(index_set);       \
            int i;                                                             \
            for (i = 0; i < set_size; i++) {                                   \
...
#define ECL_KW_TYPED_INPLACE_ADD_SQUARED(ctype)                                \
    static void ecl_kw_inplace_add_squared_##ctype(                            \
        ecl_kw_type *target_kw, const ecl_kw_type *add_kw) {                   \
        if (!ecl_kw_assert_binary_##ctype(target_kw, add_kw))                  \
            util_abort("%s: type/size  mismatch\n", __func__);                 \
        {                                                                      \
            ctype *target_data = (ctype *)ecl_kw_get_data_ref(target_kw);      \
...
#define ECL_KW_TYPED_INPLACE_SUB(ctype)                                        \
    void ecl_kw_inplace_sub_##ctype(ecl_kw_type *target_kw,                    \
        ...
        {                                                                      \
            ctype *target_data = (ctype *)ecl_kw_get_data_ref(target_kw);      \
            const ctype *sub_data =                                            \
                (const ctype *)ecl_kw_get_data_ref(sub_kw);                    \
            int i;                                                             \
            for (i = 0; i < target_kw->size; i++)                              \
...
#define ECL_KW_TYPED_INPLACE_MUL(ctype)                                        \
    void ecl_kw_inplace_mul_##ctype(ecl_kw_type *target_kw,                    \
        ...
        {                                                                      \
            ctype *target_data = (ctype *)ecl_kw_get_data_ref(target_kw);      \
            const ctype *mul_data =                                            \
                (const ctype *)ecl_kw_get_data_ref(mul_kw);                    \
            int i;                                                             \
            for (i = 0; i < target_kw->size; i++)                              \
...
#define ECL_KW_TYPED_INPLACE_DIV(ctype)                                        \
    void ecl_kw_inplace_div_##ctype(ecl_kw_type *target_kw,                    \
        ...
        {                                                                      \
            ctype *target_data = (ctype *)ecl_kw_get_data_ref(target_kw);      \
            const ctype *div_data =                                            \
                (const ctype *)ecl_kw_get_data_ref(div_kw);                    \
            int i;                                                             \
            for (i = 0; i < target_kw->size; i++)                              \
...
void ecl_kw_inplace_inv(ecl_kw_type *my_kw) {
    ...
    ecl_type_enum type = ecl_kw_get_type(my_kw);
    {
        ...
        default:
            util_abort("%s: can only be called on ECL_FLOAT_TYPE and "
                       "ECL_DOUBLE_TYPE - aborting \n",
                       __func__);
        }
    }
}
...

    {
        ...
        if (fortio_fmt_file(fortio))
            ...
            if (fortio_is_fortio_file(fortio))
                kw_file =
                    (ecl_kw_fread_header(ecl_kw, fortio) != ECL_KW_READ_FAIL);
            else
                kw_file = false;
        }
