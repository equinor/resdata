...
struct ecl_kw_struct {
  ...
  char *header8;    /* Header which is right padded with ' ' to become exactly 8
                       characters long. Should only be used internally.*/
...
static char *alloc_read_fmt_string(const ecl_data_type ecl_type) {
  return util_alloc_sprintf("%%%dc", ecl_type_get_sizeof_iotype(ecl_type));
}
...
static char *alloc_write_fmt_string(const ecl_data_type ecl_type) {
  return util_alloc_sprintf(" '%%-%ds'", ecl_type_get_sizeof_iotype(ecl_type));
}
...
static char *ecl_kw_alloc_output_buffer(const ecl_kw_type *ecl_kw) {
  ...
  if (ecl_type_is_char(ecl_kw->data_type) ||
      ecl_type_is_string(ecl_kw->data_type)) {
...
static char *ecl_kw_alloc_input_buffer(const ecl_kw_type *ecl_kw) {
  size_t buffer_size =
      ecl_kw->size * ecl_type_get_sizeof_iotype(ecl_kw->data_type);
  char *buffer = (char *)util_malloc(buffer_size);
...
static void ecl_kw_load_from_input_buffer(ecl_kw_type *ecl_kw, char *buffer) {
  ...
  size_t buffer_size = ecl_kw->size * sizeof_iotype;
  if (ECL_ENDIAN_FLIP) {
    if (ecl_type_is_numeric(ecl_kw->data_type) ||
        ecl_type_is_bool(ecl_kw->data_type))
      util_endian_flip_vector(buffer, sizeof_iotype, ecl_kw->size);
  }

  /*
    Special case bool: Return Eclipse integer representation of bool to native
    bool.
  */
  if (ecl_type_is_bool(ecl_kw->data_type)) {
    int *int_data = (int *)buffer;
  ...
  */
  ...
  /*
    Plain int, double, float data - that can be copied straight over to the
    ->data field.
  */
...
/** Allocates a untyped buffer with exactly the same content as the ecl_kw
 * instances data. */
...
static bool ecl_kw_data_equal__(const ecl_kw_type *ecl_kw, const void *data,
                                int cmp_elements) {
  int cmp = memcmp(ecl_kw->data, data,
                   cmp_elements * ecl_type_get_sizeof_ctype(ecl_kw->data_type));
  if (cmp == 0)
    return true;
  else
    return false;
...
/**
   This function compares two ecl_kw instances, and returns true if they are
   equal.
*/
...
#define ECL_KW_NUMERIC_CMP(ctype)                                              \
  static bool ecl_kw_numeric_equal_##ctype(const ecl_kw_type *ecl_kw1,         \
                                           const ecl_kw_type *ecl_kw2,         \
    ...
    {                                                                          \
      ...
      for (index = 0; index < ecl_kw1->size; index++) {                        \
        equal = util_##ctype##_approx_equal__(data1[index], data2[index],      \
                                              rel_diff, abs_diff);             \
        if (!equal)                                                            \
          break;                                                               \
      }                                                                        \
...
bool ecl_kw_numeric_equal(const ecl_kw_type *ecl_kw1,
                          const ecl_kw_type *ecl_kw2, double abs_diff,
                          double rel_diff) {
  if (!ecl_kw_size_and_type_equal(ecl_kw1, ecl_kw2))
    return false;
...
bool ecl_kw_block_equal(const ecl_kw_type *ecl_kw1, const ecl_kw_type *ecl_kw2,
                        int cmp_elements) {
...
static void ecl_kw_set_shared_ref(ecl_kw_type *ecl_kw, void *data_ptr) {
  if (!ecl_kw->shared_data) {
    if (ecl_kw->data != NULL)
      util_abort("%s: can not change to shared for keyword with allocated "
                 "storage - aborting \n",
                 __func__);
  }
...
static void ecl_kw_initialize(ecl_kw_type *ecl_kw, const char *header, int size,
                              ecl_data_type data_type) {
...
static size_t ecl_kw_fortio_data_size(const ecl_kw_type *ecl_kw) {
  ...
  const int num_blocks =
      ecl_kw->size / blocksize + (ecl_kw->size % blocksize == 0 ? 0 : 1);

  return num_blocks * (4 + 4) + // Fortran fluff for each block
         ecl_kw->size *
             ecl_type_get_sizeof_iotype(ecl_kw->data_type); // Actual data
}
...
*/
ecl_kw_type *ecl_kw_alloc_new(const char *header, int size,
                              ecl_data_type data_type, const void *data) {
  ecl_kw_type *ecl_kw;
...
ecl_kw_type *ecl_kw_alloc(const char *header, int size,
                          ecl_data_type data_type) {
  ecl_kw_type *ecl_kw;
...
ecl_kw_type *ecl_kw_alloc_new_shared(const char *header, int size,
                                     ecl_data_type data_type, void *data) {
  ecl_kw_type *ecl_kw;
  ecl_kw = ecl_kw_alloc_empty();
  ecl_kw_initialize(ecl_kw, header, size, data_type);
...
void ecl_kw_memcpy_data(ecl_kw_type *target, const ecl_kw_type *src) {
  ...
  memcpy(target->data, src->data,
         target->size * ecl_type_get_sizeof_ctype(target->data_type));
}
...
ecl_kw_type *ecl_kw_alloc_slice_copy(const ecl_kw_type *src, int index1,
...
void ecl_kw_resize(ecl_kw_type *ecl_kw, int new_size) {
  if (ecl_kw->shared_data)
    util_abort("%s: trying to allocate data for ecl_kw object which has been "
               "declared with shared storage - aborting \n",
               __func__);
  ...
  if (new_size != ecl_kw->size) {
    ...
    size_t new_byte_size =
        new_size * ecl_type_get_sizeof_ctype(ecl_kw->data_type);
...
ecl_kw_type *ecl_kw_alloc_sub_copy(const ecl_kw_type *src, const char *new_kw,
...
static void ecl_kw_iget_static(const ecl_kw_type *ecl_kw, int i, void *iptr) {
  memcpy(iptr, ecl_kw_iget_ptr_static(ecl_kw, i),
         ecl_type_get_sizeof_ctype(ecl_kw->data_type));
}
...
double ecl_kw_iget_as_double(const ecl_kw_type *ecl_kw, int index) {
  if (ecl_type_is_float(ecl_kw->data_type))
    return ecl_kw_iget_float(ecl_kw,
                             index); /* Here the compiler will silently insert a
...
void ecl_kw_iset_char_ptr(ecl_kw_type *ecl_kw, int index, const char *s) {
    ...
    strings++;
  {
    ...
    for (sub_index = 0; sub_index < strings; sub_index++)
      ecl_kw_iset_string8(ecl_kw, index + sub_index,
                          &s[sub_index * ECL_STRING8_LENGTH]);
  }
}
...
bool ecl_kw_icmp_string(const ecl_kw_type *ecl_kw, int index,
...
#define ECL_KW_SET_INDEXED(ctype, ECL_TYPE)                                    \
  void ecl_kw_set_indexed_##ctype(                                             \
      ecl_kw_type *ecl_kw, const int_vector_type *index_list, ctype value) {   \
    if (ecl_kw_get_type(ecl_kw) != ECL_TYPE)                                   \
      util_abort("%s: Keyword: %s is wrong type - aborting \n", __func__,      \
                 ecl_kw_get_header8(ecl_kw));                                  \
    {                                                                          \
...
#define ECL_KW_SHIFT_INDEXED(ctype, ECL_TYPE)                                  \
  void ecl_kw_shift_indexed_##ctype(                                           \
      ecl_kw_type *ecl_kw, const int_vector_type *index_list, ctype shift) {   \
    if (ecl_kw_get_type(ecl_kw) != ECL_TYPE)                                   \
      util_abort("%s: Keyword: %s is wrong type - aborting \n", __func__,      \
                 ecl_kw_get_header8(ecl_kw));                                  \
    {                                                                          \
...
#define ECL_KW_SCALE_INDEXED(ctype, ECL_TYPE)                                  \
  void ecl_kw_scale_indexed_##ctype(                                           \
      ecl_kw_type *ecl_kw, const int_vector_type *index_list, ctype scale) {   \
    if (ecl_kw_get_type(ecl_kw) != ECL_TYPE)                                   \
      util_abort("%s: Keyword: %s is wrong type - aborting \n", __func__,      \
                 ecl_kw_get_header8(ecl_kw));                                  \
    {                                                                          \
...
static bool ecl_kw_fscanf_qstring(char *s, const char *fmt, int len,
                                  FILE *stream) {
  const char null_char = '\0';
  char last_sep;
  bool OK;
  ...
  if (ecl_kw->size > 0) {
    ...
    if (fmt_file) {
      ...
      for (ib = 0; ib < blocks; ib++) {
        int read_elm =
            util_int_min((ib + 1) * blocksize, ecl_kw->size) - ib * blocksize;
        for (ir = 0; ir < read_elm; ir++) {
          ...
          case (ECL_INT_TYPE): {
            ...
            if (iread != 1)
              util_abort("%s: after reading %d values reading of keyword:%s "
                         "from:%s failed - aborting \n",
                         __func__,
                         offset / ecl_type_get_sizeof_ctype(ecl_kw->data_type),
                         ecl_kw->header8, fortio_filename_ref(fortio));
          } break;
          case (ECL_FLOAT_TYPE): {
            int iread =
                fscanf(stream, read_fmt, (float *)&ecl_kw->data[offset]);
            if (iread != 1) {
              util_abort("%s: after reading %d values reading of keyword:%s "
                         "from:%s failed - aborting \n",
                         __func__,
          ...
          case (ECL_BOOL_TYPE): {
            ...
            if (fscanf(stream, read_fmt, &bool_char) == 1) {
              if (bool_char == BOOL_TRUE_CHAR)
                ...
                util_abort(
                    "%s: Logical value: [%c] not recogniced - aborting \n",
                    __func__, bool_char);
            } else
          ...
          case (ECL_MESS_TYPE):
            ...
            break;
          default:
            util_abort("%s: Internal error: internal eclipse_type: %d not "
                       "recognized - aborting \n",
      ...
      bool read_ok =
          fortio_fread_buffer(fortio, buffer, ecl_kw->size * sizeof_iotype);
...
void ecl_kw_fread_indexed_data(fortio_type *fortio, offset_type data_offset,
                               ecl_data_type data_type, int element_count,
...
bool ecl_kw_fskip_data__(ecl_data_type data_type, const int element_count,
                         fortio_type *fortio) {
  if (element_count <= 0)
    return true;
...
bool ecl_kw_fskip_data(ecl_kw_type *ecl_kw, fortio_type *fortio) {
  return ecl_kw_fskip_data__(ecl_kw_get_data_type(ecl_kw), ecl_kw->size,
                             fortio);
}
...
ecl_read_status_enum ecl_kw_fread_header(ecl_kw_type *ecl_kw,
                                         fortio_type *fortio) {
  const char null_char = '\0';
  FILE *stream = fortio_get_FILE(fortio);
  bool fmt_file = fortio_fmt_file(fortio);
  ...
  if (fmt_file) {
    ...
    memcpy(ecl_type_str, &buffer[ECL_STRING8_LENGTH + sizeof(size)],
           ECL_TYPE_LENGTH);
...
bool ecl_kw_fseek_kw(const char *kw, bool rewind, bool abort_on_error,
                     fortio_type *fortio) {
  ecl_kw_type *tmp_kw = ecl_kw_alloc_empty();
...
bool ecl_kw_fseek_last_kw(const char *kw, bool abort_on_error,
                          fortio_type *fortio) {
  long int init_pos = fortio_ftell(fortio);
  bool kw_found = false;
...
static void ecl_kw_fwrite_data_unformatted(const ecl_kw_type *ecl_kw,
                                           fortio_type *fortio) {
  ...
  int sizeof_iotype = ecl_type_get_sizeof_iotype(ecl_kw->data_type);
  {
    ...
    for (block_nr = 0; block_nr < num_blocks; block_nr++) {
      int this_blocksize =
          util_int_min((block_nr + 1) * blocksize, ecl_kw->size) -
          block_nr * blocksize;
      int record_size = this_blocksize *
                        sizeof_iotype; /* The total size in bytes of the record
                                          written by the fortio layer. */
      fortio_fwrite_record(
          fortio, &iobuffer[block_nr * blocksize * sizeof_iotype], record_size);
    }
  }
  free(iobuffer);
}
...

  {
    ...
    for (block_nr = 0; block_nr < num_blocks; block_nr++) {
      int this_blocksize =
          ...
          block_nr * blocksize;
      int num_lines =
          this_blocksize / columns + (this_blocksize % columns == 0 ? 0 : 1);
      int line_nr;
      for (line_nr = 0; line_nr < num_lines; line_nr++) {
        ...
        for (col_nr = 0; col_nr < num_columns; col_nr++) {
          ...
          case (ECL_MESS_TYPE):
            util_abort("%s: Internal inconsistency : message type keywords "
...
int ecl_kw_get_size(const ecl_kw_type *ecl_kw) { return ecl_kw->size; }
...
ecl_kw_type *ecl_kw_buffer_alloc(buffer_type *buffer) {
  ...
  ecl_kw_initialize(ecl_kw, header, size, data_type);
  ecl_kw_alloc_data(ecl_kw);
  buffer_fread(buffer, ecl_kw->data,
               ecl_type_get_sizeof_ctype(ecl_kw->data_type), ecl_kw->size);
  return ecl_kw;
}
...
void ecl_kw_buffer_store(const ecl_kw_type *ecl_kw, buffer_type *buffer) {
  ...
  buffer_fwrite(buffer, ecl_kw->data,
                ecl_type_get_sizeof_ctype(ecl_kw->data_type), ecl_kw->size);
}

void ecl_kw_fwrite_param_fortio(fortio_type *fortio, const char *header,
                                ecl_data_type data_type, int size, void *data) {
  ecl_kw_type *ecl_kw = ecl_kw_alloc_new_shared(header, size, data_type, data);
  ecl_kw_fwrite(ecl_kw, fortio);
  ecl_kw_free(ecl_kw);
}

void ecl_kw_fwrite_param(const char *filename, bool fmt_file,
                         const char *header, ecl_data_type data_type, int size,
                         void *data) {
...
ecl_kw_type *ecl_kw_alloc_scatter_copy(const ecl_kw_type *src_kw,
                                       int target_size, const int *mapping,
                                       void *def_value) {
  int default_int = 0;
  ...
  ecl_kw_type *new_kw =
      ecl_kw_alloc(src_kw->header, target_size, src_kw->data_type);
...
ecl_kw_type *ecl_kw_alloc_global_copy(const ecl_kw_type *src,
  ...
  ecl_kw_type *global_copy =
      ecl_kw_alloc(ecl_kw_get_header(src), global_size, src->data_type);
...
void ecl_kw_fread_double_param(const char *filename, bool fmt_file,
                               double *double_data) {
...
void ecl_kw_summarize(const ecl_kw_type *ecl_kw) {
  char *type_name = ecl_type_alloc_name(ecl_kw->data_type);
  printf("%8s   %10d:%4s \n", ecl_kw_get_header8(ecl_kw),
         ecl_kw_get_size(ecl_kw), type_name);
  free(type_name);
}
...
bool ecl_kw_size_and_numeric_type_equal(const ecl_kw_type *kw1,
  ...
  return ecl_kw_size_and_type_equal(kw1, kw2) &&
         ecl_type_is_numeric(kw1->data_type);
}

#define ECL_KW_ASSERT_TYPED_BINARY_OP(ctype, ECL_TYPE)                         \
  bool ecl_kw_assert_binary_##ctype(const ecl_kw_type *kw1,                    \
                                    const ecl_kw_type *kw2) {                  \
...
#define ECL_KW_TYPED_INPLACE_ADD_INDEXED(ctype)                                \
  static void ecl_kw_inplace_add_indexed_##ctype(                              \
      ecl_kw_type *target_kw, const int_vector_type *index_set,                \
...
#define ECL_KW_TYPED_INPLACE_ADD(ctype)                                        \
  static void ecl_kw_inplace_add_##ctype(ecl_kw_type *target_kw,               \
                                         const ecl_kw_type *add_kw) {          \
    if (!ecl_kw_assert_binary_##ctype(target_kw, add_kw))                      \
      util_abort("%s: type/size  mismatch\n", __func__);                       \
    {                                                                          \
...
#define ECL_KW_TYPED_INPLACE_ADD_SQUARED(ctype)                                \
  static void ecl_kw_inplace_add_squared_##ctype(ecl_kw_type *target_kw,       \
                                                 const ecl_kw_type *add_kw) {  \
    if (!ecl_kw_assert_binary_##ctype(target_kw, add_kw))                      \
      util_abort("%s: type/size  mismatch\n", __func__);                       \
    {                                                                          \
...
#define ECL_KW_TYPED_INPLACE_SUB(ctype)                                        \
  void ecl_kw_inplace_sub_##ctype(ecl_kw_type *target_kw,                      \
...
#define ECL_KW_TYPED_INPLACE_SUB_INDEXED(ctype)                                \
  static void ecl_kw_inplace_sub_indexed_##ctype(                              \
      ecl_kw_type *target_kw, const int_vector_type *index_set,                \
      const ecl_kw_type *sub_kw) {                                             \
    if (!ecl_kw_assert_binary_##ctype(target_kw, sub_kw))                      \
      util_abort("%s: type/size  mismatch\n", __func__);                       \
...
#define ECL_KW_TYPED_INPLACE_MUL_INDEXED(ctype)                                \
  static void ecl_kw_inplace_mul_indexed_##ctype(                              \
      ecl_kw_type *target_kw, const int_vector_type *index_set,                \
      const ecl_kw_type *mul_kw) {                                             \
    if (!ecl_kw_assert_binary_##ctype(target_kw, mul_kw))                      \
      util_abort("%s: type/size  mismatch\n", __func__);                       \
...
#define ECL_KW_TYPED_INPLACE_DIV_INDEXED(ctype)                                \
  static void ecl_kw_inplace_div_indexed_##ctype(                              \
      ecl_kw_type *target_kw, const int_vector_type *index_set,                \
      const ecl_kw_type *div_kw) {                                             \
    if (!ecl_kw_assert_binary_##ctype(target_kw, div_kw))                      \
      util_abort("%s: type/size  mismatch\n", __func__);                       \
...
void ecl_kw_inplace_inv(ecl_kw_type *my_kw) {
  ...
  ecl_type_enum type = ecl_kw_get_type(my_kw);
  {
    ...
    default:
      util_abort("%s: can only be called on ECL_FLOAT_TYPE and ECL_DOUBLE_TYPE "
                 "- aborting \n",
                 __func__);
    }
  }
...
void ecl_kw_inplace_update_file(const ecl_kw_type *ecl_kw, const char *filename,
                                int index) {
  if (util_file_exists(filename)) {
    ...
    {
      fortio_type *fortio =
          fortio_open_readwrite(filename, fmt_file, ECL_ENDIAN_FLIP);
...
#define ECL_KW_MAX_MIN(ctype)                                                  \
  void ecl_kw_max_min_##ctype(const ecl_kw_type *ecl_kw, ctype *_max,          \
                              ctype *_min) {                                   \
    KW_MAX_MIN(ctype);                                                         \
  }
...
static void ecl_kw_fprintf_data_string(const ecl_kw_type *ecl_kw,
                                       const char *fmt, FILE *stream) {
  int i;
  for (i = 0; i < ecl_kw->size; i++)
    fprintf(stream, fmt,
            &ecl_kw->data[i * ecl_type_get_sizeof_ctype(ecl_kw->data_type)]);
}

void ecl_kw_fprintf_data(const ecl_kw_type *ecl_kw, const char *fmt,
                         FILE *stream) {
  if (ecl_type_is_double(ecl_kw->data_type))
...
static bool ecl_kw_elm_equal_numeric__(const ecl_kw_type *ecl_kw1,
                                       const ecl_kw_type *ecl_kw2, int offset,
...
static bool ecl_kw_elm_equal__(const ecl_kw_type *ecl_kw1,
                               const ecl_kw_type *ecl_kw2, int offset) {
  size_t data_offset = ecl_type_get_sizeof_ctype(ecl_kw1->data_type) * offset;
  int cmp = memcmp(&ecl_kw1->data[data_offset], &ecl_kw2->data[data_offset],
                   ecl_type_get_sizeof_ctype(ecl_kw1->data_type));
  if (cmp == 0)
    return true;
  else
    return false;
...
int ecl_kw_first_different(const ecl_kw_type *ecl_kw1,
                           const ecl_kw_type *ecl_kw2, int offset,
                           double abs_epsilon, double rel_epsilon) {
  if (!ecl_kw_size_and_type_equal(ecl_kw1, ecl_kw2))
...

  {
    ...
    if (((abs_epsilon > 0) || (rel_epsilon > 0)) &&
        ((ecl_kw_get_type(ecl_kw1) == ECL_FLOAT_TYPE) ||
         (ecl_kw_get_type(ecl_kw1) == ECL_DOUBLE_TYPE)))
      numeric_compare = true;
    {
      ...
      while (true) {
        bool equal = (numeric_compare)
                         ? ecl_kw_elm_equal_numeric__(ecl_kw1, ecl_kw2, index,
                                                      abs_epsilon, rel_epsilon)
                         : ecl_kw_elm_equal__(ecl_kw1, ecl_kw2, index);
        if (!equal)
          break;
