...
static ecl_kw_type *
ecl_nnc_data_get_gl_kw(const ecl_file_view_type *init_file_view, const char *kw,
...
static ecl_kw_type *
ecl_nnc_data_get_tranll_kw(const ecl_grid_type *grid,
                           const ecl_file_view_type *init_file_view,
    ...
    while (true) {
        ...
        {
            ecl_kw_type *ecl_kw =
                ecl_file_view_iget_kw(init_file_view, global_kw_index);
            if (strcmp(LGRJOIN_KW, ecl_kw_get_header(ecl_kw)) == 0) {
                ...
                if (ecl_kw_icmp_string(ecl_kw, 0, lgr_name1) &&
                    ...
                    tran_kw = ecl_file_view_iget_kw(init_file_view,
                                                    global_kw_index + 1);
                    break;
                }
            }
...
static ecl_kw_type *
ecl_nnc_data_get_kw(const ecl_grid_type *grid,
                    const ecl_file_view_type *init_file_view, int lgr_nr1,
                    int lgr_nr2, int kw_type) {
    ...
    if (lgr_nr1 == 0 || lgr_nr1 == lgr_nr2)
        ...
        return ecl_nnc_data_get_tranll_kw(grid, init_file_view, lgr_nr1,
                                          lgr_nr2);
    else
        return NULL;
}

static void assert_correct_kw_count(ecl_kw_type *kw, const char *function_name,
                                    int correct_kw_count, int kw_count) {
    if (correct_kw_count != kw_count)
        util_abort("In function %s, reading kw: %s. %d != %d", function_name,
                   ecl_kw_get_header(kw), correct_kw_count, kw_count);
}

static bool ecl_nnc_data_set_values(ecl_nnc_data_type *data,
    ...
    for (int nnc_index = 0; nnc_index < nnc_size; nnc_index++) {
        const ecl_nnc_pair_type *pair =
            ecl_nnc_geometry_iget(nnc_geo, nnc_index);
        ...
        if (grid1 != current_grid1 || grid2 != current_grid2) {
            ...
            assert_correct_kw_count(current_kw, __func__, correct_kw_count,
                                    kw_count);
            current_kw =
                ecl_nnc_data_get_kw(grid, init_file, grid1, grid2, kw_type);
            kw_count = 0;
            if (current_kw) {
        ...
        if (current_kw) {
            data->values[nnc_index] =
                ecl_kw_iget_as_double(current_kw, pair->input_index);
            kw_count++;
        }
    }
...
static ecl_nnc_data_type *
ecl_nnc_data_alloc__(const ecl_grid_type *grid,
                     const ecl_nnc_geometry_type *nnc_geo,
                     const ecl_file_view_type *init_file, int kw_type) {
...
ecl_nnc_data_type *
ecl_nnc_data_alloc_tran(const ecl_grid_type *grid,
                        const ecl_nnc_geometry_type *nnc_geo,
                        const ecl_file_view_type *init_file) {
...
int ecl_nnc_data_get_size(ecl_nnc_data_type *data) { return data->size; }
