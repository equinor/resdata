...
ecl_nnc_data_get_gl_kw(const ecl_file_view_type *init_file_view, const char *kw,
  ...
  for (int kw_index = 0, head_index = 0; kw_index < file_num_kw; ++kw_index) {
    ...
    if (correct_lgrheadi && strcmp(kw, current_kw) == 0) {
      /* This is to calculate who fare from lgrheadi we found the TRANGL/TRANNNC
       * key word */
      int steps = kw_index - head_index;
      /* We only support a file format where TRANNNC is 3 steps and TRANGL is 4
       * or 6 steps from LGRHEADI */
