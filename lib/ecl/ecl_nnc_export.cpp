...
/**
 * Return true if the NNC information is stored in the Intersect format, false
 * otherwise. In the Intersect format, the NNC information stored in the grid is
 * unrealiable. The correct NNC data is stored in the init file instead
 */
bool ecl_nnc_intersect_format(const ecl_grid_type *grid,
  ...
  if (!ecl_file_has_kw(init_file, NNC1_KW) ||
    ...
    return false;
  // In the specific case we are treating, there should be just 1 occurrence of
  // the kw
  const auto nnc1_num =
      ecl_kw_get_size(ecl_file_iget_named_kw(init_file, NNC1_KW, 0));
  const auto nnc2_num =
      ecl_kw_get_size(ecl_file_iget_named_kw(init_file, NNC2_KW, 0));
  const auto tran_num =
      ecl_kw_get_size(ecl_file_iget_named_kw(init_file, TRANNNC_KW, 0));
...
static int ecl_nnc_export_intersect__(const ecl_file_type *init_file,
                                      ecl_nnc_type *nnc_data, int *nnc_offset) {
...
static int ecl_nnc_export__(const ecl_grid_type *grid, int lgr_index1,
                            const ecl_file_type *init_file,
                            ecl_nnc_type *nnc_data, int *nnc_offset) {
  ...
  for (global_index1 = 0; global_index1 < ecl_grid_get_global_size(grid);
       global_index1++) {
    const nnc_info_type *nnc_info =
        ecl_grid_get_cell_nnc_info1(grid, global_index1);
    if (nnc_info) {
      int lgr_index2;
      for (lgr_index2 = 0; lgr_index2 < nnc_info_get_size(nnc_info);
        ...
        const ecl_kw_type *tran_kw = ecl_nnc_export_get_tranx_kw(
...
int ecl_nnc_export(const ecl_grid_type *grid, const ecl_file_type *init_file,
  ...
  if (ecl_nnc_intersect_format(grid, init_file)) {
    // Intersect format
    total_valid_trans =
        ecl_nnc_export_intersect__(init_file, nnc_data, &nnc_index);
  } else {
    // Eclipse format
    total_valid_trans =
        ecl_nnc_export__(grid, 0, init_file, nnc_data, &nnc_index);
    {
      for (int lgr_index = 0; lgr_index < ecl_grid_get_num_lgr(grid);
           lgr_index++) {
...
static int ecl_nnc_sort_cmp__(const void *nnc1, const void *nnc2) {
  return ecl_nnc_sort_cmp((const ecl_nnc_type *)nnc1,
  ...
  while (true) {
      ...
      break;
    {
      ...
      if (strcmp(LGRJOIN_KW, ecl_kw_get_header(ecl_kw)) == 0) {
        ...
        if (ecl_kw_icmp_string(ecl_kw, 0, lgr_name1) &&
            ecl_kw_icmp_string(ecl_kw, 1, lgr_name2)) {
      ...
      while (!finished) {
        ...
        if (correct_lgrheadi) {
          if (strcmp(kw, current_kw) == 0) {
            steps = global_kw_index -
                    head_index; /* This is to calculate who fare from lgrheadi
                                   we found the TRANGL/TRANNNC key word */
            if (steps == 3 || steps == 4 ||
                steps ==
                    6) { /* We only support a file format where TRANNNC is 3
                            steps and TRANGL is 4 or 6 steps from LGRHEADI */
              tran_kw = ecl_kw;
              finished = true;
              break;
            }
