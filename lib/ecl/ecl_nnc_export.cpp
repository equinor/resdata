...
int ecl_nnc_export_get_size(const ecl_grid_type *grid,
    ...
    return ecl_nnc_intersect_format(grid, init_file)
               ? ecl_kw_get_size(
                     ecl_file_iget_named_kw(init_file, TRANNNC_KW, 0))
               :                           // Intersect format
        ...
        if (nnc_info) {
            ...
            for (lgr_index2 = 0; lgr_index2 < nnc_info_get_size(nnc_info);
                ...
                for (index2 = 0; index2 < nnc_vector_get_size(nnc_vector);
                    ...
                    if (tran_kw) {
                        nnc.trans =
                            ecl_kw_iget_as_double(tran_kw, nnc.input_index);
                        valid_trans++;
                    } else {
    ...
    if (ecl_nnc_intersect_format(grid, init_file)) {
            ...
            ecl_nnc_export__(grid, 0, init_file, nnc_data, &nnc_index);
        {
            for (int lgr_index = 0; lgr_index < ecl_grid_get_num_lgr(grid);
                ...
                total_valid_trans += ecl_nnc_export__(
...
ecl_kw_type *ecl_nnc_export_get_tran_kw(const ecl_file_type *init_file,
    ...
    if (lgr_nr == 0) {
        ...
        if ((strcmp(kw, TRANNNC_KW) == 0) || (strcmp(kw, TRANGL_KW) == 0)) {
            ...
            while (!finished) {
                ecl_kw_type *ecl_kw =
                    ecl_file_iget_kw(init_file, global_kw_index);
                const char *current_kw = ecl_kw_get_header(ecl_kw);
                if (strcmp(LGRHEADI_KW, current_kw) == 0) {
                    if (ecl_kw_iget_int(ecl_kw, LGRHEADI_LGR_NR_INDEX) ==
                        lgr_nr) {
...
ecl_kw_type *ecl_nnc_export_get_tranx_kw(const ecl_grid_type *grid,
    ...
    if (lgr_nr1 == lgr_nr2)
        ...
        if (lgr_nr1 == 0)
            ...
            return ecl_nnc_export_get_tranll_kw(grid, init_file, lgr_nr1,
                                                lgr_nr2);
    }
}
