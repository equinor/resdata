...
/**
   ...
   which is NOT modified). Then we can select/deselect cells based on:
     ...
     4. Subsections of i/j/k - ecl_region_select_i1i2() /
   ecl_region_seleect_j1j2() / ...
   ...
   ecl_region = ecl_region_alloc( ecl_grid , false );                       //
   Start with nothing selected ecl_region_select_in_interval( ecl_region , soil
   , 0.50, 1.00);          // Select all cells with soil > 0.50
   ecl_region_select_equal( ecl_region , regions , 3 );                     //
   Only consider ECLIPSE region 3. ecl_region_select_k1k2( ecl_region , 5 , 8);
   // Select layers 5,6,7,8
   {
      int num_cells         = ecl_region_get_global_size( ecl_region );     //
   How many cells are active const int * cell_list = ecl_region_get_global_list(
   ecl_region );     // Get a list of indices int i; printf("%d cells satisfy
   your selection. The cells are: \n");
...
struct ecl_region_struct {
  ...
  bool *active_mask; /* This marks active|inactive in the region, which is
                        unrelated to active in the grid. */
  int_vector_type *global_index_list; /* This is a list of the cells in the
                                         region - irrespective of whether they
                                         are active in the grid or not. */
  int_vector_type *active_index_list; /* This means cells in the region which
                                         are also active in the grid */
  int_vector_type *
      global_active_list; /* This is a list of (maximum) nactive elements, where
                             the values are in the [0,..nx*ny*nz) range. */
...
ecl_region_type *ecl_region_alloc(const ecl_grid_type *ecl_grid,
  ...
  ecl_grid_get_dims(ecl_grid, &region->grid_nx, &region->grid_ny,
  ...
  region->name = NULL;
  ecl_region_reset(region); /* This MUST be called to ensure that xxx_valid is
                               correctly initialized. */
  return region;
}

ecl_region_type *ecl_region_alloc_copy(const ecl_region_type *ecl_region) {
  ecl_region_type *new_region =
      ecl_region_alloc(ecl_region->parent_grid, ecl_region->preselect);
  memcpy(new_region->active_mask, ecl_region->active_mask,
         ecl_region->grid_vol * sizeof *ecl_region->active_mask);
...
static void ecl_region_assert_active_index_list(ecl_region_type *region) {
  if (!region->active_index_list_valid) {
    ...
    for (global_index = 0; global_index < region->grid_vol; global_index++) {
      if (region->active_mask[global_index]) {
        int active_index =
            ecl_grid_get_active_index1(region->parent_grid, global_index);
        if (active_index >= 0) {
...
ecl_region_get_global_active_list(ecl_region_type *region) {
  ecl_region_assert_active_index_list(region);
...
static void ecl_region_select_equal__(ecl_region_type *region,
  ...
  if (!ecl_type_is_int(ecl_kw_get_data_type(ecl_kw)))
    util_abort("%s: sorry - select by equality is only supported for integer "
               "keywords \n",
               __func__);
  {
    ...
    if (global_kw) {
      ...
      for (active_index = 0; active_index < region->grid_active;
        ...
        if (kw_data[active_index] == value) {
          int global_index =
              ecl_grid_get_global_index1A(region->parent_grid, active_index);
          region->active_mask[global_index] = select;
        }
      }
    }
...
void ecl_region_select_equal(ecl_region_type *region, const ecl_kw_type *ecl_kw,
                             int value) {
...
static void ecl_region_select_bool_equal__(ecl_region_type *region,
                                           ...
                                           bool value, bool select) {
  bool global_kw;
  ecl_region_assert_kw(region, ecl_kw, &global_kw);
  if (!ecl_type_is_bool(ecl_kw_get_data_type(ecl_kw)))
    util_abort("%s: sorry - select by equality is only supported for boolean "
               "keywords \n",
               __func__);
  {
    if (global_kw) {
...
static void ecl_region_select_in_interval__(ecl_region_type *region,
  ...
  if (!ecl_type_is_float(ecl_kw_get_data_type(ecl_kw)))
    util_abort("%s: sorry - select by in_interval is only supported for float "
               "keywords \n",
               __func__);
  {
...
void ecl_region_select_in_interval(ecl_region_type *region,
                                   const ecl_kw_type *ecl_kw, float min_value,
                                   float max_value) {
...
/**
   ...
   Float / Int / double *  Active / Global  *  More / Less ==> In total 12 code
   blocks written out ...
*/
...
*/
static void ecl_region_select_with_limit__(ecl_region_type *region,
                                           const ecl_kw_type *ecl_kw,
                                           float limit, bool select_less,
                                           bool select) {
  bool global_kw;
...
void ecl_region_select_smaller(ecl_region_type *ecl_region,
                               const ecl_kw_type *ecl_kw, float limit) {
...
static void ecl_region_cmp_select__(ecl_region_type *region,
                                    const ecl_kw_type *kw1,
                                    const ecl_kw_type *kw2, bool select_less,
                                    bool select) {
  bool global_kw;
  ecl_region_assert_kw(region, kw1, &global_kw);
  if (!ecl_type_is_float(ecl_kw_get_data_type(kw1)))
    util_abort(
        "%s: sorry - select by cmp() is only supported for float keywords \n",
        __func__);
  {
...
static void ecl_region_select_from_box__(ecl_region_type *region,
                                         const ecl::ecl_box &ecl_box,
                                         bool select) {
...
static void ecl_region_select_from_ijkbox__(ecl_region_type *region, int i1,
...
void ecl_region_select_from_ijkbox(ecl_region_type *region, int i1, int i2,
                                   int j1, int j2, int k1, int k2) {
  ecl_region_select_from_ijkbox__(region, i1, i2, j1, j2, k1, k2, true);
}

void ecl_region_deselect_from_ijkbox(ecl_region_type *region, int i1, int i2,
                                     int j1, int j2, int k1, int k2) {
...
static void ecl_region_select_i1i2__(ecl_region_type *region, int i1, int i2,
                                     bool select) {
  if (i1 > i2)
  ...
  i2 = util_int_min(region->grid_nx - 1, i2);
  {
    ...
    for (k = 0; k < region->grid_nz; k++)
      for (j = 0; j < region->grid_ny; j++)
        for (i = i1; i <= i2; i++) {
          int global_index =
              ecl_grid_get_global_index3(region->parent_grid, i, j, k);
          region->active_mask[global_index] = select;
        }
  }
  ecl_region_invalidate_index_list(region);
...
static void ecl_region_select_j1j2__(ecl_region_type *region, int j1, int j2,
                                     bool select) {
  if (j1 > j2)
    util_abort("%s: i1 > i2 - this is illogical ... \n", __func__);
...
static void ecl_region_select_k1k2__(ecl_region_type *region, int k1, int k2,
                                     bool select) {
  if (k1 > k2)
    util_abort("%s: i1 > i2 - this is illogical ... \n", __func__);
...
static void ecl_region_select_from_depth__(ecl_region_type *region,
...
static void ecl_region_select_from_volume__(ecl_region_type *region,
  ...
  for (global_index = 0; global_index < region->grid_vol; global_index++) {
    double cell_size =
        ecl_grid_get_cell_volume1(region->parent_grid, global_index);
    if (select_small) {
...
static void ecl_region_select_from_dz__(ecl_region_type *region,
  ...
  for (global_index = 0; global_index < region->grid_vol; global_index++) {
    double cell_dz =
        ecl_grid_get_cell_thickness1(region->parent_grid, global_index);
    if (select_thin) {
...
static void ecl_region_select_active_cells__(ecl_region_type *ecl_region,
  ...
  for (global_index = 0; global_index < ecl_region->grid_vol; global_index++) {
    if (select_active) {
      if (ecl_grid_get_active_index1(ecl_region->parent_grid, global_index) >=
          0)
...
static void ecl_region_select_global_index__(ecl_region_type *region,
...
void ecl_region_deselect_global_index(ecl_region_type *region,
                                      int global_index) {
...
static void ecl_region_cylinder_select__(ecl_region_type *region, double x0,
...
void ecl_region_select_in_cylinder(ecl_region_type *region, double x0,
                                   double y0, double R) {
  ecl_region_cylinder_select__(region, x0, y0, R, 1, 0, true, true);
}

void ecl_region_deselect_in_cylinder(ecl_region_type *region, double x0,
                                     double y0, double R) {
  ecl_region_cylinder_select__(region, x0, y0, R, 1, 0, true, false);
}

void ecl_region_select_in_zcylinder(ecl_region_type *region, double x0,
                                    double y0, double R, double z1, double z2) {
  ecl_region_cylinder_select__(region, x0, y0, R, util_double_min(z1, z2),
                               util_double_max(z1, z2), true, true);
}

void ecl_region_deselect_in_zcylinder(ecl_region_type *region, double x0,
                                      double y0, double R, double z1,
                                      double z2) {
  ecl_region_cylinder_select__(region, x0, y0, R, util_double_min(z1, z2),
...
static void ecl_region_plane_select__(ecl_region_type *region,
                                      const double n[3], const double p[3],
                                      bool select_above, bool select) {
  const double a = n[0];
  const double b = n[1];
  const double c = -n[2];
...
void ecl_region_select_above_plane(ecl_region_type *region, const double n[3],
                                   const double p[3]) {
  ecl_region_plane_select__(region, n, p, true, true);
}

void ecl_region_select_below_plane(ecl_region_type *region, const double n[3],
                                   const double p[3]) {
  ecl_region_plane_select__(region, n, p, false, true);
}

void ecl_region_deselect_above_plane(ecl_region_type *region, const double n[3],
                                     const double p[3]) {
  ecl_region_plane_select__(region, n, p, true, false);
}

void ecl_region_deselect_below_plane(ecl_region_type *region, const double n[3],
                                     const double p[3]) {
...

  {
    ...
    for (i = 0; i < region->grid_nx; i++) {
      for (j = 0; j < region->grid_ny; j++) {
        ...
        bool inside;
        int global_index =
            ecl_grid_get_global_index3(region->parent_grid, i, j, define_k);
...
static void ecl_region_select_active_index__(ecl_region_type *region,
...
void ecl_region_deselect_active_index(ecl_region_type *region,
                                      int active_index) {
...
static void ecl_region_select_from_layer__(ecl_region_type *region,
  ...
  layer_cells_equal(layer, layer_value, i_list, j_list);
  {
    ...
    for (index = 0; index < int_vector_size(i_list); index++) {
      int global_index = ecl_grid_get_global_index3(region->parent_grid,
                                                    i[index], j[index], k);
      region->active_mask[global_index] = select;
    }
  }
...
bool ecl_region_contains_ijk(const ecl_region_type *ecl_region, int i, int j,
                             int k) {
  int global_index =
      ecl_grid_get_global_index3(ecl_region->parent_grid, i, j, k);
...
void ecl_region_set_kw_double(ecl_region_type *ecl_region, ecl_kw_type *ecl_kw,
...
void ecl_region_shift_kw_float(ecl_region_type *ecl_region, ecl_kw_type *ecl_kw,
                               float value, bool force_active) {
  const int_vector_type *index_set =
      ecl_region_get_kw_index_list(ecl_region, ecl_kw, force_active);
...
void ecl_region_shift_kw_double(ecl_region_type *ecl_region,
                                ecl_kw_type *ecl_kw, double value,
                                bool force_active) {
  const int_vector_type *index_set =
      ecl_region_get_kw_index_list(ecl_region, ecl_kw, force_active);
...
void ecl_region_scale_kw_float(ecl_region_type *ecl_region, ecl_kw_type *ecl_kw,
                               float value, bool force_active) {
  const int_vector_type *index_set =
      ecl_region_get_kw_index_list(ecl_region, ecl_kw, force_active);
...
void ecl_region_kw_idiv(ecl_region_type *ecl_region, ecl_kw_type *ecl_kw,
                        const ecl_kw_type *div_kw, bool force_active) {
  const int_vector_type *index_set =
      ecl_region_get_kw_index_list(ecl_region, ecl_kw, force_active);
  ecl_kw_inplace_div_indexed(ecl_kw, index_set, div_kw);
}

void ecl_region_kw_imul(ecl_region_type *ecl_region, ecl_kw_type *ecl_kw,
                        const ecl_kw_type *mul_kw, bool force_active) {
  const int_vector_type *index_set =
      ecl_region_get_kw_index_list(ecl_region, ecl_kw, force_active);
  ecl_kw_inplace_mul_indexed(ecl_kw, index_set, mul_kw);
}

void ecl_region_kw_isub(ecl_region_type *ecl_region, ecl_kw_type *ecl_kw,
                        const ecl_kw_type *delta_kw, bool force_active) {
  const int_vector_type *index_set =
      ecl_region_get_kw_index_list(ecl_region, ecl_kw, force_active);
...
void ecl_region_kw_copy(ecl_region_type *ecl_region, ecl_kw_type *ecl_kw,
                        const ecl_kw_type *src_kw, bool force_active) {
...
bool ecl_region_equal(const ecl_region_type *region1,
  ...
  if (region1->parent_grid ==
      region2->parent_grid) { // Must be exactly the same grid instance to
                              // compare as equal.
