...
struct ecl_rft_file_struct {
  ...
  std::string filename;
  std::vector<ecl_rft_node_type *> data; /* This vector just contains all the
                                            rft nodes in one long vector. */
...
char *ecl_rft_file_alloc_case_filename(const char *case_input) {
  ...
  if (file_type == ECL_RFT_FILE)
    ...
    if ((file_type == ECL_OTHER_FILE) ||
        (file_type ==
         ECL_DATA_FILE)) { /* Impossible to infer formatted/unformatted from the
                              case_input */
      char *RFT_file =
          ecl_util_alloc_filename(path, basename, ECL_RFT_FILE, false, -1);
      char *FRFT_file =
      ...
      char *RFT_file =
          ecl_util_alloc_filename(path, basename, ECL_RFT_FILE, fmt_file, -1);
...
int ecl_rft_file_get_size__(const ecl_rft_file_type *rft_file,
  ...
  if ((well_pattern == NULL) && (recording_time < 0))
    ...
    for (size_t i = 0; i < rft_file->data.size(); i++) {
      ...
      /*OK - we either do not care about the well, or alternatively the well
       * matches. */
      if (recording_time >= 0) {
