...
*/
static ecl_rft_enum
translate_from_sting_to_ecl_rft_enum(const char *data_type_string) {
...
ecl_rft_node_type *ecl_rft_node_alloc_new(const char *well_name,
  ...
  ecl_rft_enum data_type =
      translate_from_sting_to_ecl_rft_enum(data_type_string);
...
static ecl_rft_node_type *
ecl_rft_node_alloc_empty(const char *data_type_string) {
  ecl_rft_enum data_type =
      translate_from_sting_to_ecl_rft_enum(data_type_string);
...
void ecl_rft_node_append_cell(ecl_rft_node_type *rft_node,
  ...
  if (rft_node->MSW) {
    auto pos_iter = std::upper_bound(
        rft_node->cells.begin(), rft_node->cells.end(), cell, ecl_rft_cell_lt);
...
static ecl_kw_type *
ecl_rft_node_get_pressure_kw(ecl_rft_node_type *rft_node,
                             const ecl_file_view_type *rft) {
  if (rft_node->data_type == RFT)
    ...
    if (ecl_kw_element_sum_float(conpres_kw) > 0.0)
      return conpres_kw;
    else if (ecl_file_view_has_kw(rft, PRESSURE_KW))
      return ecl_file_view_iget_named_kw(rft, PRESSURE_KW, 0);
    else {
      fprintf(stderr,
              "WARNING: %s returned a CONPRES_KW with all values at zero. "
              "PRESSURE_KW not found.\n",
              __func__);
      return conpres_kw;
    }
  }
...
static void ecl_rft_node_init_RFT_cells(ecl_rft_node_type *rft_node,
                                        const ecl_file_view_type *rft_view) {
  const ecl_kw_type *conipos =
      ecl_file_view_iget_named_kw(rft_view, CONIPOS_KW, 0);
  const ecl_kw_type *conjpos =
      ecl_file_view_iget_named_kw(rft_view, CONJPOS_KW, 0);
  const ecl_kw_type *conkpos =
      ecl_file_view_iget_named_kw(rft_view, CONKPOS_KW, 0);
  const ecl_kw_type *depth_kw =
      ecl_file_view_iget_named_kw(rft_view, DEPTH_KW, 0);
  const ecl_kw_type *swat_kw =
      ecl_file_view_iget_named_kw(rft_view, SWAT_KW, 0);
  const ecl_kw_type *sgas_kw =
      ecl_file_view_iget_named_kw(rft_view, SGAS_KW, 0);
  const ecl_kw_type *pressure_kw =
      ecl_rft_node_get_pressure_kw(rft_node, rft_view);
...

  {
    ...
    for (c = 0; c < ecl_kw_get_size(conipos); c++) {
      /* The connection coordinates are shifted -= 1; i.e. all internal usage is
       * offset 0. */
      ecl_rft_cell_type *cell = ecl_rft_cell_alloc_RFT(
...
static void ecl_rft_node_init_PLT_cells(ecl_rft_node_type *rft_node,
                                        const ecl_file_view_type *rft_view) {
  /* For PLT there is quite a lot of extra information which is not yet
   * internalized. */
  const ecl_kw_type *conipos =
      ecl_file_view_iget_named_kw(rft_view, CONIPOS_KW, 0);
  const ecl_kw_type *conjpos =
  ...
  const float *WR = ecl_kw_get_float_ptr(
      ecl_file_view_iget_named_kw(rft_view, CONWRAT_KW, 0));
  const float *GR = ecl_kw_get_float_ptr(
      ecl_file_view_iget_named_kw(rft_view, CONGRAT_KW, 0));
  const float *OR = ecl_kw_get_float_ptr(
      ecl_file_view_iget_named_kw(rft_view, CONORAT_KW, 0));
  const float *P =
      ecl_kw_get_float_ptr(ecl_rft_node_get_pressure_kw(rft_node, rft_view));
  const float *depth = ecl_kw_get_float_ptr(
      ecl_file_view_iget_named_kw(rft_view, CONDEPTH_KW, 0));
  const float *flowrate = ecl_kw_get_float_ptr(
      ecl_file_view_iget_named_kw(rft_view, CONVTUB_KW, 0));
  const float *oil_flowrate = ecl_kw_get_float_ptr(
      ecl_file_view_iget_named_kw(rft_view, CONOTUB_KW, 0));
  const float *gas_flowrate = ecl_kw_get_float_ptr(
      ecl_file_view_iget_named_kw(rft_view, CONGTUB_KW, 0));
  const float *water_flowrate = ecl_kw_get_float_ptr(
      ecl_file_view_iget_named_kw(rft_view, CONWTUB_KW, 0));
  const float *connection_start = NULL;
  const float *connection_end = NULL;

  /* The keywords CONLENST_KW and CONLENEN_KW are ONLY present if we are dealing
   * with a MSW well. */
  if (ecl_file_view_has_kw(rft_view, CONLENST_KW))
    connection_start = ecl_kw_get_float_ptr(
        ecl_file_view_iget_named_kw(rft_view, CONLENST_KW, 0));

  if (ecl_file_view_has_kw(rft_view, CONLENEN_KW))
    connection_end = ecl_kw_get_float_ptr(
        ecl_file_view_iget_named_kw(rft_view, CONLENEN_KW, 0));

  {
    ...
    for (c = 0; c < ecl_kw_get_size(conipos); c++) {
      ...
      /* The connection coordinates are shifted -= 1; i.e. all internal usage is
       * offset 0. */
      cell = ecl_rft_cell_alloc_PLT(
          i[c] - 1, j[c] - 1, k[c] - 1, depth[c], P[c], OR[c], GR[c], WR[c], cs,
          ce, flowrate[c], oil_flowrate[c], gas_flowrate[c], water_flowrate[c]);
      ecl_rft_node_append_cell(rft_node, cell);
    }
  }
}

static void ecl_rft_node_init_cells(ecl_rft_node_type *rft_node,
                                    const ecl_file_view_type *rft_view) {
...
ecl_rft_node_type *ecl_rft_node_alloc(const ecl_file_view_type *rft_view) {
  ...
  ecl_rft_node_type *rft_node = ecl_rft_node_alloc_empty(
      (const char *)ecl_kw_iget_ptr(welletc, WELLETC_TYPE_INDEX));

  if (rft_node != NULL) {
    ecl_kw_type *date_kw = ecl_file_view_iget_named_kw(rft_view, DATE_KW, 0);
    {
      char *tmp = util_alloc_strip_copy(
          (const char *)ecl_kw_iget_ptr(welletc, WELLETC_NAME_INDEX));
    ...
    /* Time information. */
    {
      ...
      rft_node->recording_date = ecl_util_make_date(
          time[DATE_DAY_INDEX], time[DATE_MONTH_INDEX], time[DATE_YEAR_INDEX]);
    }
    rft_node->days =
        ecl_kw_iget_float(ecl_file_view_iget_named_kw(rft_view, TIME_KW, 0), 0);
    if (ecl_file_view_has_kw(rft_view, CONLENST_KW))
      rft_node->MSW = true;
    else
      rft_node->MSW = false;
...
int ecl_rft_node_get_size(const ecl_rft_node_type *rft_node) {
  return rft_node->cells.size();
}
time_t ecl_rft_node_get_date(const ecl_rft_node_type *rft_node) {
...
const ecl_rft_cell_type *
ecl_rft_node_iget_cell_sorted(ecl_rft_node_type *rft_node, int index) {
  return rft_node->cells[index];
}
...
void ecl_rft_node_iget_ijk(const ecl_rft_node_type *rft_node, int index, int *i,
                           int *j, int *k) {
  const ecl_rft_cell_type *cell = ecl_rft_node_iget_cell(rft_node, index);
...
const ecl_rft_cell_type *
ecl_rft_node_lookup_ijk(const ecl_rft_node_type *rft_node, int i, int j,
                        int k) {
  int index = 0;
...
static void assert_type_and_index(const ecl_rft_node_type *rft_node,
                                  ecl_rft_enum target_type, int index) {
...
static void ecl_rft_node_fill_welletc(ecl_kw_type *welletc,
...
void ecl_rft_node_fwrite(const ecl_rft_node_type *rft_node, fortio_type *fortio,
                         ert_ecl_unit_enum unit_set) {
