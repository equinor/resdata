...
ecl_rft_node_get_pressure_kw(ecl_rft_node_type *rft_node,
    ...
    if (rft_node->data_type == RFT)
        ...
        ecl_kw_type *conpres_kw =
            ecl_file_view_iget_named_kw(rft, CONPRES_KW, 0);
...

    {
        ...
        for (c = 0; c < ecl_kw_get_size(conipos); c++) {
            ...
            cell = ecl_rft_cell_alloc_PLT(i[c] - 1, j[c] - 1, k[c] - 1,
                                          depth[c], P[c], OR[c], GR[c], WR[c],
                                          cs, ce, flowrate[c], oil_flowrate[c],
...
ecl_rft_node_type *ecl_rft_node_alloc(const ecl_file_view_type *rft_view) {
    ...
    if (rft_node != NULL) {
        ecl_kw_type *date_kw =
            ecl_file_view_iget_named_kw(rft_view, DATE_KW, 0);
        {
        ...
        /* Time information. */
        {
            ...
            rft_node->recording_date =
                ecl_util_make_date(time[DATE_DAY_INDEX], time[DATE_MONTH_INDEX],
                                   time[DATE_YEAR_INDEX]);
        }
        rft_node->days = ecl_kw_iget_float(
            ecl_file_view_iget_named_kw(rft_view, TIME_KW, 0), 0);
        if (ecl_file_view_has_kw(rft_view, CONLENST_KW))
            rft_node->MSW = true;
        else
            rft_node->MSW = false;
...
void ecl_rft_node_fwrite(const ecl_rft_node_type *rft_node, fortio_type *fortio,
...

    {
        ...
        ecl_kw_type *pressure =
            ecl_kw_alloc(PRESSURE_KW, size_cells, ECL_FLOAT);
