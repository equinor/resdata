...
static ecl_rst_file_type *ecl_rst_file_alloc(const char *filename) {
    ...
    ecl_rst_file_type *rst_file =
        (ecl_rst_file_type *)util_malloc(sizeof *rst_file);
...
ecl_rst_file_type *ecl_rst_file_open_read(const char *filename) {
    ...
    rst_file->fortio =
        fortio_open_reader(filename, rst_file->fmt_file, ECL_ENDIAN_FLIP);
    return rst_file;
}
...
ecl_rst_file_type *ecl_rst_file_open_write_seek(const char *filename,
    ...
    rst_file->fortio =
        fortio_open_readwrite(filename, rst_file->fmt_file, ECL_ENDIAN_FLIP);
    /*
  ...
  */
    if (!rst_file->fortio) {
        rst_file->fortio =
            fortio_open_writer(filename, rst_file->fmt_file, ECL_ENDIAN_FLIP);
        return rst_file;
    }
...
ecl_rst_file_type *ecl_rst_file_open_append(const char *filename) {
    ecl_rst_file_type *rst_file = ecl_rst_file_alloc(filename);
    rst_file->fortio =
        fortio_open_append(filename, rst_file->fmt_file, ECL_ENDIAN_FLIP);
    return rst_file;
}
...
static ecl_kw_type *ecl_rst_file_alloc_INTEHEAD(ecl_rst_file_type *rst_file,
    ...
    ecl_kw_type *intehead_kw =
        ecl_kw_alloc(INTEHEAD_KW, INTEHEAD_RESTART_SIZE, ECL_INT);
    ...
    /* All well properties are hardcoded to zero. */
    {
        ...
        {
            ecl_util_set_date_values(rsthead->sim_time, &rsthead->day,
...
static ecl_kw_type *ecl_rst_file_alloc_LOGIHEAD(int simulator) {
    ...
    ecl_kw_type *logihead_kw =
        ecl_kw_alloc(LOGIHEAD_KW, LOGIHEAD_RESTART_SIZE, ECL_BOOL);
...
static ecl_kw_type *ecl_rst_file_alloc_DOUBHEAD(ecl_rst_file_type *rst_file,
                                                double days) {
    ecl_kw_type *doubhead_kw =
        ecl_kw_alloc(DOUBHEAD_KW, DOUBHEAD_RESTART_SIZE, ECL_DOUBLE);
...
void ecl_rst_file_fwrite_header(ecl_rst_file_type *rst_file, int seqnum,
    ...
    {
        ecl_kw_type *logihead_kw =
            ecl_rst_file_alloc_LOGIHEAD(INTEHEAD_ECLIPSE100_VALUE);
    ...
    {
        ecl_kw_type *doubhead_kw =
            ecl_rst_file_alloc_DOUBHEAD(rst_file, rsthead_data->sim_days);
