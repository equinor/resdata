...
ecl_rsthead_type *ecl_rsthead_alloc_from_kw(int report_step,
    ...
    ecl_rsthead_type *rsthead =
        (ecl_rsthead_type *)util_malloc(sizeof *rsthead);
...
ecl_rsthead_type *ecl_rsthead_alloc(const ecl_file_view_type *rst_view,
                                    int report_step) {
    const ecl_kw_type *intehead_kw =
        ecl_file_view_iget_named_kw(rst_view, INTEHEAD_KW, 0);
    const ecl_kw_type *doubhead_kw =
        ecl_file_view_iget_named_kw(rst_view, DOUBHEAD_KW, 0);
    ...
    if (ecl_file_view_has_kw(rst_view, SEQNUM_KW)) {
        const ecl_kw_type *seqnum_kw =
            ecl_file_view_iget_named_kw(rst_view, SEQNUM_KW, 0);
        report_step = ecl_kw_iget_int(seqnum_kw, 0);
    }

    return ecl_rsthead_alloc_from_kw(report_step, intehead_kw, doubhead_kw,
                                     logihead_kw);
}
...
void ecl_rsthead_free(ecl_rsthead_type *rsthead) { free(rsthead); }
