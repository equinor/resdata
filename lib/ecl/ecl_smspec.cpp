...
struct ecl_smspec_struct {
  ...
  */
  ...
  node_map
      gen_var_index /* This is "everything" - things can either be found as
                       gen_var("WWCT:OP_X") or as well_var("WWCT" , "OP_X") */
      ;

  std::map<std::string, node_map>
      well_var_index; /* Indexes for all well variables:
                         {well1: {var1: index1 , var2: index2} , well2: {var1:
                         index1 , var2: index2}} */
  std::map<std::string, node_map>
  ...
  std::string
      key_join_string;     /* The string used to join keys when building gen_key
                              keys - typically ":" -     but arbitrary - NOT necessary
                              to be able to invert the joining. */
  std::string header_file; /* FULL path to the currenbtly loaded header_file. */

  bool formatted; /* Has this summary instance been loaded from a formatted
                     (i.e. FSMSPEC file) or unformatted (i.e. SMSPEC) file. */
  time_t sim_start_time; /* When did the simulation start - worldtime. */

  int time_index; /* The fields time_index, day_index, month_index and
                     year_index */
...
Region var         VAR_TYPE:index  (where index is NOT from the nums vector, it
it is just an offset). Completion var:    VAR_TYPE:WELL_NAME:NUM
....
...
ecl_smspec_type *ecl_smspec_alloc_empty(bool write_mode,
  ...
  /*
    ...
    to the possibility of extracting unit system information from the SMSPEC
    file.
  */
...
/*
  The ecl_smspec_iget_node() function is only retained for compatibility; should
  be replaced with calls to the more explicit:
  ecl_smspec_iget_node_w_node_index().
*/
...
static void ecl_smspec_fwrite_STARTDAT(const ecl_smspec_type *smspec,
  ...
  ecl_util_set_datetime_values(smspec->sim_start_time, &second, &minute, &hour,
                               &mday, &month, &year);
...
ecl_smspec_type *ecl_smspec_alloc_restart_writer(
  ...
  return ecl_smspec_alloc_writer__(key_join_string, restart_case, restart_step,
...
static void ecl_smspec_install_gen_keys(ecl_smspec_type *smspec,
  ...
  /* Insert the (optional) extra mapping for block related variables and
   * region_2_region variables: */
  {
...
bool ecl_smspec_needs_wgname(ecl_smspec_var_type var_type) {
  ...
  default:
    util_exit(
        "Sorry: support for variables of type:%s is not implemented in %s.\n",
...
static void ecl_smspec_load_restart(ecl_smspec_type *ecl_smspec,
  ...
  if (ecl_file_has_kw(header, RESTART_KW)) {
    ...
    char tmp_base[73]; /* To accomodate a maximum of 9 items which consist of 8
                          characters each. */
    char *restart_base;
    int i;
    tmp_base[0] = '\0';
    ...
    if (strlen(restart_base)) { /* We ignore the empty ones. */
      ...
      */
      ...
      smspec_header = ecl_util_alloc_exfilename(path.c_str(), restart_base,
      ...
      if (smspec_header) {
        if (!util_same_file(
          ...
          if (util_is_abs_path(restart_base))
            ...
            char *tmp_path =
                util_alloc_filename(path.c_str(), restart_base, NULL);
...
const ecl::smspec_node *ecl_smspec_add_node(ecl_smspec_type *ecl_smspec,
  ...
  int params_index = ecl_smspec->smspec_nodes.size();
  return ecl_smspec_insert_node(ecl_smspec,
...
static bool ecl_smspec_check_header(ecl_file_type *header) {
  ...
  for (size_t i = 0; i < num_req_keywords && OK; i++) {
    OK &= ecl_file_has_kw(
        header, get_active_keyword_alias(header, smspec_required_keywords[i]));
  }
      ...
      for (params_index = 0; params_index < ecl_kw_get_size(wells);
        ...
        if (ecl_smspec_lgr_var_type(var_type)) {
          ...
          ecl_smspec_insert_node(
              ecl_smspec,
              std::unique_ptr<ecl::smspec_node>(new ecl::smspec_node(
                  params_index, kw, well, unit, lgr_name, lgr_i, lgr_j, lgr_k,
                  default_value, ecl_smspec->key_join_string.c_str())));
          free(lgr_name);
        } else
          ecl_smspec_insert_node(
              ecl_smspec,
              std::unique_ptr<ecl::smspec_node>(new ecl::smspec_node(
                  params_index, kw, well, num, unit, ecl_smspec->grid_dims,
                  default_value, ecl_smspec->key_join_string.c_str())));
...
ecl_smspec_type *ecl_smspec_fread_alloc(const char *header_file,
  ...
  if (ecl_smspec_fread_header(ecl_smspec, header_file, include_restart)) {
    ...
    if (day_node != NULL) {
      ...
      ecl_smspec->month_index =
          smspec_node_get_params_index(&ecl_smspec->misc_var_index["MONTH"]);
      ecl_smspec->year_index =
          smspec_node_get_params_index(&ecl_smspec->misc_var_index["YEAR"]);
    }
    ...
    if ((ecl_smspec->time_index == -1) && (ecl_smspec->day_index == -1)) {
      ...
      */
      ...
      util_abort(
          "%s: Sorry the SMSPEC file seems to lack all time information, need "
          "either TIME, or DAY/MONTH/YEAR information. Can not proceed.",
          __func__);
      return NULL;
    }
    return ecl_smspec;
...
ecl_smspec_get_well_var_node(const ecl_smspec_type *smspec, const char *well,
  ...
  if (!node_ptr)
    throw std::out_of_range("The well: " + std::string(well) + " variable: " +
                            std::string(var) + " combination does not exist.");
...
ecl_smspec_get_group_var_node(const ecl_smspec_type *smspec, const char *group,
  ...
  if (!node_ptr)
    throw std::out_of_range("The group: " + std::string(group) + " variable: " +
                            std::string(var) + " combination does not exist.");
...
int ecl_smspec_get_group_var_params_index(const ecl_smspec_type *ecl_smspec,
  ...
  const auto node_ptr =
      ecl_smspec_get_str_key_var_node(ecl_smspec->group_var_index, group, var);
  return node_valid_index(node_ptr);
}
...
ecl_smspec_get_well_completion_var_node(const ecl_smspec_type *ecl_smspec,
                                        ...
                                        int cell_nr) {
  const auto node_ptr =
      ecl_smspec_get_well_completion_var_node__(ecl_smspec, well, var, cell_nr);
  if (!node_ptr)
...
// const char * ecl_smspec_iget_unit( const ecl_smspec_type * smspec , int
// node_index ) {
//  const ecl::smspec_node * smspec_node = ecl_smspec_iget_node( smspec ,
//  node_index ); return smspec_node_get_unit( smspec_node );
//}
//
// int ecl_smspec_iget_num( const ecl_smspec_type * smspec , int node_index ) {
//  const ecl::smspec_node * smspec_node = ecl_smspec_iget_node( smspec ,
//  node_index ); return smspec_node_get_num( smspec_node );
//}
//
// const char * ecl_smspec_iget_wgname( const ecl_smspec_type * smspec , int
// node_index ) {
//  const ecl::smspec_node * smspec_node = ecl_smspec_iget_node( smspec ,
//  node_index ); return smspec_node_get_wgname( smspec_node );
//}
//
// const char * ecl_smspec_iget_keyword( const ecl_smspec_type * smspec , int
// index ) {
...
/*void ecl_smspec_sort( ecl_smspec_type * smspec ) {
  std::sort(smspec->smspec_nodes.begin(), smspec->smspec_nodes.end(),
smspec_node_lt);
