...
struct ecl_subsidence_struct {
    ...
    bool *aquifer_cell;
    hash_type *
        surveys; /* A hash table containg ecl_subsidence_survey_type instances; one instance
...
static UTIL_SAFE_CAST_FUNCTION(ecl_subsidence_survey, ECL_SUBSIDENCE_SURVEY_ID)
    ...
    if (ecl_file_view_has_kw(restart_view, RPORV_KW)) {
        survey->dynamic_porevolume =
            (double *)util_calloc(ecl_subsidence->grid_cache->size(),
...
ecl_subsidence_get_survey(const ecl_subsidence_type *subsidence,
    ...
    if (name == NULL)
        ...
        return (ecl_subsidence_survey_type *)hash_get(subsidence->surveys,
                                                      name);
}
...
double ecl_subsidence_eval(const ecl_subsidence_type *subsidence,
    ...
    return ecl_subsidence_survey_eval(base_survey, monitor_survey, region,
                                      utm_x, utm_y, depth, compressibility,
                                      poisson_ratio);
}
