...
struct ecl_subsidence_struct {
  ...
  bool *aquifer_cell;
  hash_type *surveys;      /* A hash table containg ecl_subsidence_survey_type
                              instances; one instance      for each interesting time. */
...
struct ecl_subsidence_survey_struct {
  ...
  const bool *aquifer_cell;   /* Is this cell a numerical aquifer cell - must be
                                 disregarded. */
...
ecl_subsidence_survey_alloc_empty(const ecl_subsidence_type *sub,
                                  const char *name) {
  ecl_subsidence_survey_type *survey =
      (ecl_subsidence_survey_type *)util_malloc(sizeof *survey);
  ...
  survey->pressure =
      (double *)util_calloc(sub->grid_cache->size(), sizeof *survey->pressure);
...
static UTIL_SAFE_CAST_FUNCTION(ecl_subsidence_survey, ECL_SUBSIDENCE_SURVEY_ID)
    ...
    static ecl_subsidence_survey_type *ecl_subsidence_survey_alloc_PRESSURE(
        ecl_subsidence_type *ecl_subsidence,
        const ecl_file_view_type *restart_view, const char *name) {

  ecl_subsidence_survey_type *survey =
      ecl_subsidence_survey_alloc_empty(ecl_subsidence, name);
  ...
  ecl_kw_type *init_porv_kw = ecl_file_iget_named_kw(
      ecl_subsidence->init_file, PORV_KW, 0); /*Global indexing*/
  ecl_kw_type *pressure_kw = ecl_file_view_iget_named_kw(
      restart_view, PRESSURE_KW, 0); /*Active indexing*/

  ecl_kw_type *rporv_kw = NULL;
  if (ecl_file_view_has_kw(restart_view, RPORV_KW)) {
    survey->dynamic_porevolume = (double *)util_calloc(
        ecl_subsidence->grid_cache->size(), sizeof *survey->dynamic_porevolume);
  ...
  for (active_index = 0; active_index < size; active_index++) {
    ...
    survey->pressure[active_index] =
        ecl_kw_iget_float(pressure_kw, active_index);

    if (rporv_kw)
      survey->dynamic_porevolume[active_index] =
          ecl_kw_iget_float(rporv_kw, active_index);
  }
  return survey;
}

static void
ecl_subsidence_survey_free(ecl_subsidence_survey_type *subsidence_survey) {
...
static void ecl_subsidence_survey_free__(void *__subsidence_survey) {
  ecl_subsidence_survey_type *subsidence_survey =
...
ecl_subsidence_survey_eval(const ecl_subsidence_survey_type *base_survey,
  ...
  if (monitor_survey != NULL) {
    for (index = 0; index < size; index++)
      weight[index] =
          base_survey->porv[index] *
          (base_survey->pressure[index] - monitor_survey->pressure[index]);
  } else {
    for (index = 0; index < size; index++)
  ...
  deltaz = compressibility * 31.83099 * (1 - poisson_ratio) *
           ecl_grav_common_eval_biot_savart(grid_cache, region,
...
static double ecl_subsidence_survey_eval_geertsma(
    const ecl_subsidence_survey_type *base_survey,
    const ecl_subsidence_survey_type *monitor_survey, ecl_region_type *region,
  ...
  for (int index = 0; index < size; index++) {
    if (monitor_survey) {
      weight[index] =
          scale_factor * cell_volume[index] *
          (base_survey->pressure[index] - monitor_survey->pressure[index]);
    } else {
      weight[index] =
          scale_factor * cell_volume[index] * (base_survey->pressure[index]);
    }
  }

  deltaz = ecl_grav_common_eval_geertsma(
...
static double ecl_subsidence_survey_eval_geertsma_rporv(
    const ecl_subsidence_survey_type *base_survey,
    const ecl_subsidence_survey_type *monitor_survey, ecl_region_type *region,
    double utm_x, double utm_y, double depth, double youngs_modulus,
    double poisson_ratio, double seabed) {
  ...
  if (!base_survey->dynamic_porevolume) {
    util_abort("%s: Keyword RPORV not defined in .UNRST file for %s. Please "
               "add RPORV keyword to output in RPTRST clause in .DATA file.\n",
...
ecl_subsidence_type *ecl_subsidence_alloc(const ecl_grid_type *ecl_grid,
  ...
  ecl_subsidence_type *ecl_subsidence =
      (ecl_subsidence_type *)util_malloc(sizeof *ecl_subsidence);
  ecl_subsidence->init_file = init_file;
  ecl_subsidence->grid_cache = new ecl::ecl_grid_cache(ecl_grid);
  ecl_subsidence->aquifer_cell = ecl_grav_common_alloc_aquifer_cell(
      *(ecl_subsidence->grid_cache), init_file);

  ecl_subsidence->surveys = hash_alloc();
  return ecl_subsidence;
}

static void ecl_subsidence_add_survey__(ecl_subsidence_type *subsidence,
                                        const char *name,
                                        ecl_subsidence_survey_type *survey) {
  hash_insert_hash_owned_ref(subsidence->surveys, name, survey,
                             ecl_subsidence_survey_free__);
}
...
ecl_subsidence_add_survey_PRESSURE(ecl_subsidence_type *subsidence,
  ...
  ecl_subsidence_survey_type *survey =
      ecl_subsidence_survey_alloc_PRESSURE(subsidence, restart_view, name);
...
static ecl_subsidence_survey_type *
ecl_subsidence_get_survey(const ecl_subsidence_type *subsidence,
                          const char *name) {
  if (name == NULL)
...
double ecl_subsidence_eval(const ecl_subsidence_type *subsidence,
  ...
  ecl_subsidence_survey_type *base_survey =
      ecl_subsidence_get_survey(subsidence, base);
  ecl_subsidence_survey_type *monitor_survey =
      ecl_subsidence_get_survey(subsidence, monitor);
  return ecl_subsidence_survey_eval(base_survey, monitor_survey, region, utm_x,
...
double ecl_subsidence_eval_geertsma(const ecl_subsidence_type *subsidence,
  ...
  ecl_subsidence_survey_type *monitor_survey =
      ecl_subsidence_get_survey(subsidence, monitor);
  return ecl_subsidence_survey_eval_geertsma(
      base_survey, monitor_survey, region, utm_x, utm_y, depth, youngs_modulus,
...
double ecl_subsidence_eval_geertsma_rporv(const ecl_subsidence_type *subsidence,
  ...
  ecl_subsidence_survey_type *monitor_survey =
      ecl_subsidence_get_survey(subsidence, monitor);
  return ecl_subsidence_survey_eval_geertsma_rporv(
      base_survey, monitor_survey, region, utm_x, utm_y, depth, youngs_modulus,
      poisson_ratio, seabed);
}
