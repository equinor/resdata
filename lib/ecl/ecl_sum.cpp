...
void ecl_sum_set_case(ecl_sum_type *ecl_sum, const char *input_arg) {
    ...
    free(ecl_sum->ext);
    {
        ...
        ecl_sum->ecl_case =
            util_alloc_filename(path.c_str(), base.c_str(), NULL);
        if (path.size())
...
ecl_sum_alloc_writer__(const char *ecl_case, const char *restart_case,
    ...
    if (ecl_sum) {
        ...
        if (restart_case)
            ...
            ecl_sum->smspec = ecl_smspec_alloc_writer(
...
ecl_sum_type *ecl_sum_fread_alloc_case__(const char *input_file,
    ...
    bool lazy_load = true;
    int file_options = 0;
    return ecl_sum_fread_alloc_case2__(
        input_file, key_join_string, include_restart, lazy_load, file_options);
}
...
double ecl_sum_get_block_var(const ecl_sum_type *ecl_sum, int time_index,
    ...
    int params_index = ecl_smspec_get_block_var_params_index(
        ecl_sum->smspec, block_var, block_nr);
    return ecl_sum_data_iget(ecl_sum->data, time_index, params_index);
}
...
int ecl_sum_get_well_completion_var_index(const ecl_sum_type *ecl_sum,
                                          const char *well, const char *var,
                                          int cell_nr) {
    return ecl_smspec_get_well_completion_var_params_index(ecl_sum->smspec,
                                                           well, var, cell_nr);
}
  ...
  */
    ...
    for (int report_step = 0; report_step < time_t_vector_size(times);
        ...
        if (input_t < start_time) {
            ...
            for (int i = 1; i < ecl_smspec_num_nodes(ecl_sum->smspec); i++) {
                ...
                const ecl::smspec_node &node =
                    ecl_smspec_iget_node_w_node_index(ecl_sum->smspec, i);
                if (!node.is_rate())
                    value = ecl_sum_iget_first_value(ecl_sum,
            ...
            for (int i = 1; i < ecl_smspec_num_nodes(ecl_sum->smspec); i++) {
                ...
                const ecl::smspec_node &node =
                    ecl_smspec_iget_node_w_node_index(ecl_sum->smspec, i);
                if (!node.is_rate())
                    value = ecl_sum_iget_last_value(ecl_sum,
                                                    node.get_params_index());
                double_vector_iset(data, i - 1, value);
            }
        } else {
        ...
        for (int data_index = 0;
             data_index < ecl_sum_vector_get_size(ecl_sum_vector);
             data_index++) {
...
double_vector_type *ecl_sum_alloc_data_vector(const ecl_sum_type *ecl_sum,
                                              int data_index,
                                              bool report_only) {
    return ecl_sum_data_alloc_data_vector(ecl_sum->data, data_index,
                                          report_only);
}
...
void ecl_sum_fprintf(const ecl_sum_type *ecl_sum, FILE *stream,
...

    {
        ...
        for (ivar = 0; ivar < stringlist_get_size(var_list); ivar++) {
            if (ecl_sum_has_general_var(ecl_sum,
    ...
    if (report_only) {
        ...
        for (report = first_report; report <= last_report; report++) {
            if (ecl_sum_data_has_report_step(ecl_sum->data, report)) {
                int time_index;
                time_index =
                    ecl_sum_data_iget_report_end(ecl_sum->data, report);
                __ecl_sum_fprintf_line(ecl_sum, stream, time_index, has_var,
        ...
        util_alloc_file_components(input_file, &path, &base, NULL);
        {
            ...
            if (header_file != NULL) {
                same_case = util_same_file(
                    header_file, ecl_smspec_get_header_file(ecl_sum->smspec));
                free(header_file);
            }
        }
...
double ecl_sum_days_from_time(const ecl_sum_type *ecl_sum, time_t sim_time) {
    double seconds_diff =
        util_difftime(ecl_smspec_get_start_time(ecl_sum->smspec), sim_time,
                      NULL, NULL, NULL, NULL);
...
bool ecl_sum_report_step_compatible(const ecl_sum_type *ecl_sum1,
    ...
    if (ecl_sum1 == ecl_sum2)
        ...
        return ecl_sum_data_report_step_compatible(ecl_sum1->data,
                                                   ecl_sum2->data);
}
...
double ecl_sum_get_first_value_gen_key(const ecl_sum_type *ecl_sum,
    ...
    const ecl::smspec_node *node =
        ecl_sum_get_general_var_node(ecl_sum, gen_key);
    return ecl_sum_iget_first_value(ecl_sum,
