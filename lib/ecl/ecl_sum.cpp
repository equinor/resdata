...
/**
   ...
   Header in ECLIPSE.SMSPEC file                  Actual data; many 'PARAMS'
   blocks in .Snnnn or .UNSMRY file

   ------------------------------.........       -----------   -----------
   -----------   -----------   ----------- | WGNAMES    KEYWORDS   NUMS | INDEX
   :       | PARAMS  |   | PARAMS  |   | PARAMS  |   | PARAMS  |   | PARAMS  |
   |----------------------------|........:       |---------|   |---------|
   |---------|   |---------|   |---------| | OP-1       WOPR       X    |   0 :
   |  652    |   |   752   |   |  862    |   |   852   |   |    962  | | OP-1
   WWPR       X    |   1    :       |   45    |   |    47   |   |   55    |   |
   59   |   |     62  | | GI-1       WGIR       X    |   2    :       |  500 |
   |   500   |   |  786    |   |   786   |   |    486  | | :+:+:+:+   FOPT X |
   3    :       | 7666    |   |  7666   |   | 8811    |   |  7688   |   |   8649
   | | :+:+:+:+   RPR        5    |   4    :       |  255    |   |   255   |   |
   266    |   |   257   |   |    277  | | :+:+:+:+   BPR        3457 |   5    :
   |  167    |   |   167   |   |  189    |   |   201   |   |    166  |
   ------------------------------.........       -----------   -----------
   -----------   -----------   -----------
...
struct ecl_sum_struct {
  ...
  char *ecl_case; /* This is the current case, with optional path component. ==
                     path + base*/
  char *ext; /* Only to support selective loading of formatted|unformatted and
                unified|multiple. (can be NULL) */
};
...
static ecl_sum_type *ecl_sum_alloc__(const char *input_arg,
...
static bool ecl_sum_fread_data(ecl_sum_type *ecl_sum,
...
static void ecl_sum_fread_history(ecl_sum_type *ecl_sum, bool lazy_load,
                                  int file_options) {
  char *restart_header = ecl_util_alloc_filename(
      NULL, ecl_smspec_get_restart_case(ecl_sum->smspec),
      ECL_SUMMARY_HEADER_FILE, ecl_smspec_get_formatted(ecl_sum->smspec), -1);
  ecl_sum_type *restart_case = ecl_sum_fread_alloc_case2__(
...
static bool ecl_sum_fread(ecl_sum_type *ecl_sum, const char *header_file,
                          ...
                          bool include_restart, bool lazy_load,
                          int file_options) {
  ecl_sum->smspec = ecl_smspec_fread_alloc(
      header_file, ecl_sum->key_join_string, include_restart);
  if (ecl_sum->smspec) {
    bool fmt_file;
  ...
  if (ecl_sum_fread_data(ecl_sum, data_files, include_restart, lazy_load,
    ...
    ecl_file_enum file_type =
        ecl_util_get_file_type(stringlist_iget(data_files, 0), NULL, NULL);
    ...
    if (file_type == ECL_SUMMARY_FILE)
      ...
      util_abort("%s: FileTypeError, should be ECL_SUMMARY_FILE OR "
...
static bool ecl_sum_fread_case(ecl_sum_type *ecl_sum, bool include_restart,
  ...
  ecl_util_alloc_summary_files(ecl_sum->path, ecl_sum->base, ecl_sum->ext,
                               &header_file, summary_file_list);
  if ((header_file != NULL) && (stringlist_get_size(summary_file_list) > 0)) {
    caseOK = ecl_sum_fread(ecl_sum, header_file, summary_file_list,
...
ecl_sum_type *ecl_sum_fread_alloc(const char *header_file,
                                  const stringlist_type *data_files,
                                  const char *key_join_string,
                                  bool include_restart, bool lazy_load,
                                  int file_options) {
...
ecl_sum_tstep_type *ecl_sum_add_tstep(ecl_sum_type *ecl_sum, int report_step,
                                      double sim_seconds) {
  ecl_sum_tstep_type *new_tstep =
      ecl_sum_data_add_new_tstep(ecl_sum->data, report_step, sim_seconds);
  return new_tstep;
}

static ecl_sum_type *
ecl_sum_alloc_writer__(const char *ecl_case, const char *restart_case,
  ...
  if (ecl_sum) {
    ...
    if (restart_case)
      ecl_sum->smspec = ecl_smspec_alloc_restart_writer(
          key_join_string, restart_case, restart_step, sim_start, time_in_days,
          nx, ny, nz);
    else
      ecl_sum->smspec = ecl_smspec_alloc_writer(key_join_string, sim_start,
...
ecl_sum_type *
ecl_sum_alloc_restart_writer2(const char *ecl_case, const char *restart_case,
                              int restart_step, bool fmt_output, bool unified,
                              const char *key_join_string, time_t sim_start,
                              bool time_in_days, int nx, int ny, int nz) {
  return ecl_sum_alloc_writer__(ecl_case, restart_case, restart_step,
                                fmt_output, unified, key_join_string, sim_start,
                                time_in_days, nx, ny, nz);
}

/*
  This does not take in the restart_step argument is depcrecated. You should use
  the ecl_sum_alloc_restart_writer2() function.
*/
...
ecl_sum_type *ecl_sum_alloc_writer(const char *ecl_case, bool fmt_output,
                                   bool unified, const char *key_join_string,
                                   time_t sim_start, bool time_in_days, int nx,
                                   int ny, int nz) {
  return ecl_sum_alloc_writer__(ecl_case, NULL, 0, fmt_output, unified,
...
void ecl_sum_fwrite(const ecl_sum_type *ecl_sum) {
  ...
  ecl_sum_data_fwrite(ecl_sum->data, ecl_sum->ecl_case, ecl_sum->fmt_case,
                      ecl_sum->unified);
}
...
ecl_sum_type *ecl_sum_fread_alloc_case2__(const char *input_file,
                                          const char *key_join_string,
                                          bool include_restart, bool lazy_load,
                                          int file_options) {
...
ecl_sum_type *ecl_sum_fread_alloc_case__(const char *input_file,
  ...
  bool lazy_load = true;
  int file_options = 0;
  return ecl_sum_fread_alloc_case2__(input_file, key_join_string,
                                     include_restart, lazy_load, file_options);
}
...
bool ecl_sum_case_exists(const char *input_file) {
  ...
  case_exists = ecl_util_alloc_summary_files(path, basename, extension,
...
double ecl_sum_get_from_sim_days(const ecl_sum_type *ecl_sum, double sim_days,
                                 const ecl::smspec_node *node) {
...
double ecl_sum_get_well_var(const ecl_sum_type *ecl_sum, int time_index,
                            const char *well, const char *var) {
  int params_index =
      ecl_smspec_get_well_var_params_index(ecl_sum->smspec, well, var);
...
double ecl_sum_get_well_var_from_sim_time(const ecl_sum_type *ecl_sum,
                                          ...
                                          const char *var) {
  const ecl::smspec_node &node =
      ecl_smspec_get_well_var_node(ecl_sum->smspec, well, var);
...
double ecl_sum_get_group_var(const ecl_sum_type *ecl_sum, int time_index,
                             const char *group, const char *var) {
  int params_index =
      ecl_smspec_get_group_var_params_index(ecl_sum->smspec, group, var);
  return ecl_sum_data_iget(ecl_sum->data, time_index, params_index);
}

double ecl_sum_get_group_var_from_sim_time(const ecl_sum_type *ecl_sum,
                                           time_t sim_time, const char *group,
                                           const char *var) {
  const ecl::smspec_node &node =
      ecl_smspec_get_group_var_node(ecl_sum->smspec, group, var);
  return ecl_sum_get_from_sim_time(ecl_sum, sim_time, &node);
}

double ecl_sum_get_group_var_from_sim_days(const ecl_sum_type *ecl_sum,
...
double ecl_sum_get_field_var(const ecl_sum_type *ecl_sum, int time_index,
                             const char *var) {
  int params_index =
      ecl_smspec_get_field_var_params_index(ecl_sum->smspec, var);
  return ecl_sum_data_iget(ecl_sum->data, time_index, params_index);
}
...
double ecl_sum_get_block_var(const ecl_sum_type *ecl_sum, int time_index,
                             const char *block_var, int block_nr) {
  int params_index = ecl_smspec_get_block_var_params_index(ecl_sum->smspec,
                                                           block_var, block_nr);
  return ecl_sum_data_iget(ecl_sum->data, time_index, params_index);
}

int ecl_sum_get_block_var_index_ijk(const ecl_sum_type *ecl_sum,
                                    const char *block_var, int i, int j,
                                    int k) {
  return ecl_smspec_get_block_var_params_index_ijk(ecl_sum->smspec, block_var,
                                                   i, j, k);
}
...
double ecl_sum_get_block_var_ijk(const ecl_sum_type *ecl_sum, int time_index,
                                 const char *block_var, int i, int j, int k) {
  int index = ecl_sum_get_block_var_index_ijk(ecl_sum, block_var, i, j, k);
  return ecl_sum_data_iget(ecl_sum->data, time_index, index);
}

double ecl_sum_get_block_var_ijk_from_sim_time(const ecl_sum_type *ecl_sum,
                                               time_t sim_time,
                                               const char *block_var, int i,
                                               int j, int k) {
  const ecl::smspec_node &node =
      ecl_smspec_get_block_var_node_ijk(ecl_sum->smspec, block_var, i, j, k);
  return ecl_sum_get_from_sim_time(ecl_sum, sim_time, &node);
}

double ecl_sum_get_block_var_ijk_from_sim_days(const ecl_sum_type *ecl_sum,
                                               double sim_days,
                                               const char *block_var, int i,
                                               int j, int k) {
  const ecl::smspec_node &node =
...
bool ecl_sum_has_region_var(const ecl_sum_type *ecl_sum, const char *var,
                            int region_nr) {
  return ecl_smspec_has_region_var(ecl_sum->smspec, var, region_nr);
}

double ecl_sum_get_region_var(const ecl_sum_type *ecl_sum, int time_index,
...
double ecl_sum_get_region_var_from_sim_time(const ecl_sum_type *ecl_sum,
                                            time_t sim_time, const char *var,
                                            int region_nr) {
  const ecl::smspec_node &node =
      ecl_smspec_get_region_var_node(ecl_sum->smspec, var, region_nr);
  return ecl_sum_get_from_sim_time(ecl_sum, sim_time, &node);
}

double ecl_sum_get_region_var_from_sim_days(const ecl_sum_type *ecl_sum,
                                            double sim_days, const char *var,
                                            int region_nr) {
  const ecl::smspec_node &node =
      ecl_smspec_get_region_var_node(ecl_sum->smspec, var, region_nr);
...
double ecl_sum_get_misc_var(const ecl_sum_type *ecl_sum, int time_index,
                            const char *var) {
...
int ecl_sum_get_well_completion_var_index(const ecl_sum_type *ecl_sum,
                                          const char *well, const char *var,
                                          int cell_nr) {
  return ecl_smspec_get_well_completion_var_params_index(ecl_sum->smspec, well,
                                                         var, cell_nr);
}
...
bool ecl_sum_has_well_completion_var(const ecl_sum_type *ecl_sum,
                                     const char *well, const char *var,
                                     int cell_nr) {
  return ecl_smspec_has_well_completion_var(ecl_sum->smspec, well, var,
                                            cell_nr);
}

double ecl_sum_get_well_completion_var(const ecl_sum_type *ecl_sum,
                                       int time_index, const char *well,
...
ecl_sum_get_general_var_node(const ecl_sum_type *ecl_sum,
  ...
  const ecl::smspec_node &node =
      ecl_smspec_get_general_var_node(ecl_sum->smspec, lookup_kw);
  return &node;
}
...
double ecl_sum_get_general_var(const ecl_sum_type *ecl_sum, int time_index,
                               const char *lookup_kw) {
...
void ecl_sum_get_interp_vector(const ecl_sum_type *ecl_sum, time_t sim_time,
                               const ecl_sum_vector_type *key_words,
                               double_vector_type *data) {
  ecl_sum_data_get_interp_vector(ecl_sum->data, sim_time, key_words, data);
}

void ecl_sum_fwrite_interp_csv_line(const ecl_sum_type *ecl_sum,
                                    time_t sim_time,
                                    const ecl_sum_vector_type *key_words,
                                    FILE *fp) {
...
ecl_sum_type *ecl_sum_alloc_resample(const ecl_sum_type *ecl_sum,
  ...
  /*
    If lower and  / or upper extrapolation is set to true it makes sure that
    resampling returns the first / last value of the simulation or in the case
    of derivate / rate then it gets zero. if these are set to false, we jus
    throw exception
  */
  ...
  const ecl::smspec_node &node =
      ecl_smspec_iget_node_w_node_index(ecl_sum->smspec, 0);
  if (util_string_equal(smspec_node_get_unit(&node), "DAYS"))
    time_in_days = true;

  // create elc_sum_resampled with TIME node only
  ecl_sum_type *ecl_sum_resampled = ecl_sum_alloc_writer(
      ecl_case, ecl_sum->fmt_case, ecl_sum->unified, ecl_sum->key_join_string,
      input_start, time_in_days, grid_dims[0], grid_dims[1], grid_dims[2]);

  // add remaining nodes
  for (int i = 0; i < ecl_smspec_num_nodes(ecl_sum->smspec); i++) {
    const ecl::smspec_node &node =
        ecl_smspec_iget_node_w_node_index(ecl_sum->smspec, i);
  ...
  */
  ...
  double_vector_type *data =
      double_vector_alloc(ecl_sum_vector_get_size(ecl_sum_vector), 0);
  ...
  for (int report_step = 0; report_step < time_t_vector_size(times);
    ...
    if (input_t < start_time) {
      // clamping to the first value for t < start_time or if it is a rate than
      // derivative is 0
      ...
      // clamping to the last value for t > end_time or if it is a rate than
      // derivative is 0
      for (int i = 1; i < ecl_smspec_num_nodes(ecl_sum->smspec); i++) {
        double value = 0;
        const ecl::smspec_node &node =
    ...
    /* Add timestep corresponding to the interpolated data in the resampled
     * case. */
    ecl_sum_tstep_type *tstep = ecl_sum_add_tstep(
        ecl_sum_resampled, report_step, input_t - input_start);
    for (int data_index = 0;
         data_index < ecl_sum_vector_get_size(ecl_sum_vector); data_index++) {
      double value = double_vector_iget(data, data_index);
      int params_index = data_index + 1; // The +1 shift is because the first
                                         // element in the tstep is time value.
...
const char *ecl_sum_get_wgname(const ecl_sum_type *ecl_sum,
                               const char *gen_key) {
  const ecl::smspec_node *node = ecl_sum_get_general_var_node(ecl_sum, gen_key);
  return smspec_node_get_wgname(node);
}

const char *ecl_sum_get_keyword(const ecl_sum_type *ecl_sum,
                                const char *gen_key) {
  const ecl::smspec_node *node = ecl_sum_get_general_var_node(ecl_sum, gen_key);
...
void ecl_sum_init_double_vector_interp(const ecl_sum_type *ecl_sum,
                                       ...
                                       double *data) {
  const ecl::smspec_node &node =
      ecl_smspec_get_general_var_node(ecl_sum->smspec, gen_key);
  ecl_sum_data_init_double_vector_interp(ecl_sum->data, node, time_points,
                                         data);
}

void ecl_sum_init_datetime64_vector(const ecl_sum_type *ecl_sum, int64_t *data,
                                    int multiplier) {
...
void ecl_sum_init_double_frame_interp(const ecl_sum_type *ecl_sum,
                                      ...
                                      const time_t_vector_type *time_points,
                                      double *data) {
  ecl_sum_data_init_double_frame_interp(ecl_sum->data, keywords, time_points,
                                        data);
}
...
static int ecl_sum_get_limiting(const ecl_sum_type *ecl_sum, int smspec_index,
                                double limit, bool gt) {
  const int length = ecl_sum_data_get_length(ecl_sum->data);
  int internal_index = 0;
  do {
    double value =
        ecl_sum_data_iget(ecl_sum->data, internal_index, smspec_index);
    if (gt) {
      if (value > limit)
        break;
    } else {
...
int ecl_sum_get_first_gt(const ecl_sum_type *ecl_sum, int param_index,
                         double limit) {
  return ecl_sum_get_limiting(ecl_sum, param_index, limit, true);
}

int ecl_sum_get_first_lt(const ecl_sum_type *ecl_sum, int param_index,
                         double limit) {
...
static void __ecl_sum_fprintf_line(const ecl_sum_type *ecl_sum, FILE *stream,
                                   int internal_index,
  ...
  fprintf(stream, fmt->days_fmt,
          ecl_sum_iget_sim_days(ecl_sum, internal_index));
    ...
    for (ivar = 0; ivar < int_vector_size(var_index); ivar++) {
      if (bool_vector_iget(has_var, ivar)) {
        ...
        fprintf(stream, fmt->value_fmt,
                ecl_sum_iget(ecl_sum, internal_index,
...
static void ecl_sum_fprintf_header(const ecl_sum_type *ecl_sum,
                                   const stringlist_type *key_list,
                                   const bool_vector_type *has_var,
                                   FILE *stream, const ecl_sum_fmt_type *fmt) {
...
void ecl_sum_fprintf(const ecl_sum_type *ecl_sum, FILE *stream,
      ...
      if (ecl_sum_has_general_var(ecl_sum, stringlist_iget(var_list, ivar))) {
        ...
        int_vector_iset(var_index, ivar,
                        ecl_sum_get_general_var_params_index(
                            ecl_sum, stringlist_iget(var_list, ivar)));
      } else {
        fprintf(
            stderr,
            "** Warning: could not find variable: \'%s\' in summary file \n",
  ...
  if (report_only) {
    ...
    int time_index;
    for (time_index = 0; time_index < ecl_sum_get_data_length(ecl_sum);
         time_index++)
      __ecl_sum_fprintf_line(ecl_sum, stream, time_index, has_var, var_index,
                             date_string, fmt);
  }
...
static void ecl_sum_fmt_init_csv(ecl_sum_fmt_type *fmt, const char *date_format,
...
void ecl_sum_export_csv(const ecl_sum_type *ecl_sum, const char *filename,
  ...
  bool same_case = false;
  {
    ...
    util_alloc_file_components(input_file, &path, &base, NULL);
    {
      ...
      char *header_file = ecl_util_alloc_exfilename(
          path, base, ECL_SUMMARY_HEADER_FILE, fmt_file, -1);
      if (header_file != NULL) {
        same_case = util_same_file(header_file,
                                   ecl_smspec_get_header_file(ecl_sum->smspec));
        free(header_file);
      }
    }
    ...
    for (i = 0; i < time_t_vector_size(sim_time); i++)
      double_vector_iset(
          ...
          ecl_sum_data_get_from_sim_time(
              ecl_sum->data, time_t_vector_iget(sim_time, i), node));
  }
}
...
double ecl_sum_days_from_time(const ecl_sum_type *ecl_sum, time_t sim_time) {
  double seconds_diff =
      util_difftime(ecl_smspec_get_start_time(ecl_sum->smspec), sim_time, NULL,
                    NULL, NULL, NULL);
...
char *ecl_sum_alloc_well_key(const ecl_sum_type *ecl_sum, const char *keyword,
                             const char *wgname) {
...
double_vector_type *ecl_sum_alloc_seconds_solution(const ecl_sum_type *ecl_sum,
  ...
  const ecl::smspec_node *node = ecl_sum_get_general_var_node(ecl_sum, gen_key);
  return ecl_sum_data_alloc_seconds_solution(ecl_sum->data, *node, cmp_value,
                                             rates_clamp_lower);
}
...
double_vector_type *ecl_sum_alloc_days_solution(const ecl_sum_type *ecl_sum,
                                                ...
                                                double cmp_value,
                                                bool rates_clamp_lower) {
  double_vector_type *solution = ecl_sum_alloc_seconds_solution(
      ecl_sum, gen_key, cmp_value, rates_clamp_lower);
...
time_t_vector_type *ecl_sum_alloc_time_solution(const ecl_sum_type *ecl_sum,
  ...
  {
    double_vector_type *seconds = ecl_sum_alloc_seconds_solution(
        ecl_sum, gen_key, cmp_value, rates_clamp_lower);
