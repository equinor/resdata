...
Header direction: ecl_smspec   DAYS     WWCT:OP_3     FOPT     BPR:15,10,25
                               --------------------------------------------
  /|\                           0.00    0.00          0.00           256.00 <--
One timestep ecl_sum_tstep |                           10.00 0.56         10.00
255.00 Time direction: ecl_sum_data  20.00    0.61         18.70 253.00 | 30.00
0.63         21.20           251.00 |                           ...
...
struct ecl_sum_tstep_struct {
  ...
  std::vector<float> data; /* A memcpy copy of the PARAMS vector in ecl_kw
                              instance - the raw data. */
  time_t sim_time;         /* The true time (i.e. 20.th of october 2010) of
                              corresponding to this timestep. */
  int ministep;       /* The ECLIPSE internal time-step number; one ministep per
                         numerical timestep. */
  int report_step;    /* The report step this time-step is part of - in general
                         there can be many timestep for each report step. */
  double sim_seconds; /* Accumulated simulation time up to this ministep. */
  int internal_index; /* Used for lookups of the next / previous ministep based
                         on an existing ministep. */
  const ecl_smspec_type *smspec; /* The smespec header information for this
                                    tstep - must be compatible. */
};

ecl_sum_tstep_type *
ecl_sum_tstep_alloc_remap_copy(const ecl_sum_tstep_type *src,
...
static ecl_sum_tstep_type *ecl_sum_tstep_alloc(int report_step, int ministep_nr,
...
void ecl_sum_tstep_free(ecl_sum_tstep_type *ministep) { delete ministep; }
...
static void ecl_sum_tstep_set_time_info_from_seconds(ecl_sum_tstep_type *tstep,
                                                     time_t sim_start,
                                                     double sim_seconds) {
...
static void ecl_sum_tstep_set_time_info_from_date(ecl_sum_tstep_type *tstep,
                                                  time_t sim_start,
                                                  time_t sim_time) {
...
static void ecl_sum_tstep_set_time_info(ecl_sum_tstep_type *tstep,
                                        const ecl_smspec_type *smspec) {
  ...
  if (sim_time_index >= 0) {
    ...
    util_abort("%s: Hmmm - could not extract date/time information from SMSPEC "
               "header file? \n",
               __func__);
}
...
ecl_sum_tstep_type *ecl_sum_tstep_alloc_from_file(
    int report_step, int ministep_nr, const ecl_kw_type *params_kw,
    const char *src_file, const ecl_smspec_type *smspec) {

  int data_size = ecl_kw_get_size(params_kw);

  if (data_size == ecl_smspec_get_params_size(smspec)) {
    ecl_sum_tstep_type *ministep =
        ecl_sum_tstep_alloc(report_step, ministep_nr, smspec);
    ...
    */
    fprintf(stderr,
            "** Warning size mismatch between timestep loaded from:%s(%d) and "
            "header:%s(%d) - timestep discarded.\n",
            src_file, data_size, ecl_smspec_get_header_file(smspec),
...
ecl_sum_tstep_type *ecl_sum_tstep_alloc_new(int report_step, int ministep,
                                            float sim_seconds,
                                            const ecl_smspec_type *smspec) {
  ecl_sum_tstep_type *tstep =
      ecl_sum_tstep_alloc(report_step, ministep, smspec);
  tstep->data = ecl_smspec_get_params_default(smspec);

  ecl_sum_tstep_set_time_info_from_seconds(
      tstep, ecl_smspec_get_start_time(smspec), sim_seconds);
  ecl_sum_tstep_iset(tstep, ecl_smspec_get_time_index(smspec),
                     sim_seconds / ecl_smspec_get_time_seconds(smspec));
  return tstep;
}
...
void ecl_sum_tstep_set_from_key(ecl_sum_tstep_type *tstep, const char *gen_key,
                                float value) {
  const ecl::smspec_node &smspec_node =
      ecl_smspec_get_general_var_node(tstep->smspec, gen_key);
