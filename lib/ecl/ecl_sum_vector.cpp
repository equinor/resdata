...
static void ecl_sum_vector_add_node(ecl_sum_vector_type * vector, const ecl::smspec_node_type * node, const char * key ) {
...
bool ecl_sum_vector_add_key( ecl_sum_vector_type * ecl_sum_vector, const char * key){
  if (ecl_sum_has_general_var( ecl_sum_vector->ecl_sum , key)) {
    const ecl::smspec_node_type * node = ecl_sum_get_general_var_node( ecl_sum_vector->ecl_sum , key );
    ecl_sum_vector_add_node(ecl_sum_vector, node, key);
    return true;
  } else
    return false;
