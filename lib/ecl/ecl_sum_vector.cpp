...
static void ecl_sum_vector_add_node(ecl_sum_vector_type *vector,
...
static void ecl_sum_vector_add_invalid_key(ecl_sum_vector_type *vector,
                                           const char *key) {
...
bool ecl_sum_vector_add_key(ecl_sum_vector_type *ecl_sum_vector,
                            const char *key) {
    if (ecl_sum_has_general_var(ecl_sum_vector->ecl_sum, key)) {
        const ecl::smspec_node *node =
            ecl_sum_get_general_var_node(ecl_sum_vector->ecl_sum, key);
        ecl_sum_vector_add_node(ecl_sum_vector, node, key);
        return true;
    } else
        return false;
...
void ecl_sum_vector_add_keys(ecl_sum_vector_type *ecl_sum_vector,
                             const char *pattern) {
    stringlist_type *keylist = ecl_sum_alloc_matching_general_var_list(
        ecl_sum_vector->ecl_sum, pattern);
