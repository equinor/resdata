...
static char *alloc_string_name(const ecl_data_type ecl_type) {
  return util_alloc_sprintf("C%03d", ecl_type_get_sizeof_iotype(ecl_type));
}
...
ecl_data_type ecl_type_create(const ecl_type_enum type,
                              const size_t element_size) {
  ecl_data_type ecl_type =
      (type == ECL_STRING_TYPE ? ECL_STRING(element_size)
                               : ecl_type_create_from_type(type));

  if ((size_t)ecl_type_get_sizeof_iotype(ecl_type) != element_size)
    util_abort("%s: element_size mismatch for type %d, was: %d, expected: %d\n",
...
bool ecl_type_is_numeric(const ecl_data_type ecl_type) {
  return (ecl_type_is_int(ecl_type) || ecl_type_is_float(ecl_type) ||
          ecl_type_is_double(ecl_type));
}

bool ecl_type_is_alpha(const ecl_data_type ecl_type) {
  return (ecl_type_is_char(ecl_type) || ecl_type_is_mess(ecl_type) ||
