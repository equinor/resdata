...
ecl_data_type *ecl_type_alloc_python(const ecl_type_enum type,
...
void ecl_type_free_python(ecl_data_type *data_type) { free(data_type); }
 ...
 *
 */
ecl_kw_type *ecl_kw_fscanf_alloc_grdecl_dynamic_python(
    FILE *stream, const char *kw, bool strict, const ecl_data_type *data_type) {
  return ecl_kw_fscanf_alloc_grdecl_dynamic__(stream, kw, strict, *data_type);
}

ecl_kw_type *ecl_kw_alloc_python(const char *header, int size,
...
void ecl_kw_fread_indexed_data_python(fortio_type *fortio,
                                      offset_type data_offset,
                                      const ecl_data_type *data_type,
                                      int element_count,
                                      const int_vector_type *index_map,
                                      char *buffer) {
  return ecl_kw_fread_indexed_data(fortio, data_offset, *data_type,
