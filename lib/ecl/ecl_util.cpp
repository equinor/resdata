...
static char *ecl_util_alloc_filename_static(const char *path, const char *base,
    ...
    default:
        util_abort("%s: Invalid input file_type to ecl_util_alloc_filename - "
                   "aborting \n",
                   __func__);
...
char *ecl_util_alloc_exfilename_anyfmt(const char *path, const char *base,
    ...
    char *filename = ecl_util_alloc_filename(path, base, file_type,
                                             fmt_file_first, report_nr);
    if (!util_file_exists(filename)) {
        free(filename);
        filename = ecl_util_alloc_filename(path, base, file_type,
                                           !fmt_file_first, report_nr);
    }
...
int ecl_util_select_filelist(const char *path, const char *base,
    ...
    if (file_type == ECL_SUMMARY_FILE || file_type == ECL_RESTART_FILE)
        return ecl_util_select_predicate_filelist(
            path, base, file_type, fmt_file, upper_case, filelist);
...
void ecl_util_memcpy_typed_data(void *_target_data, const void *_src_data,
    ...
    if (ecl_type_is_equal(target_type, src_type))
        memcpy(_target_data, _src_data,
               size * ecl_type_get_sizeof_ctype(src_type));
    else {
