...
void fault_block_add_cell(fault_block_type *fault_block, int i, int j) {
  ...
  int_vector_append(
      ...
      ecl_grid_get_global_index3(fault_block->grid, i, j, fault_block->k));
  fault_block->valid_center = false;
  layer_iset_cell_value(fault_block_layer_get_layer(fault_block->parent_layer),
                        i, j, fault_block->block_id);
}
...
bool fault_block_trace_edge(const fault_block_type *block,
  ...
  if (fault_block_get_size(block) > 0) {
    std::vector<int_point2d_type> corner_list;
    {
      ...
      layer_trace_block_edge(fault_block_layer_get_layer(block->parent_layer),
    ...
    while (true) {
...

      {
        const geo_polygon_type *polyline =
            geo_polygon_collection_iget_polygon(polylines, polyline_index);
...
void fault_block_list_neighbours(const fault_block_type *block,
  ...
  int_vector_reset(neighbour_list);
  {
    ...
    for (c = 0; c < int_vector_size(block->i_list); c++) {
      ...
      if (fault_block_connected_neighbour(block, i, j, i - 1, j, connected_only,
                                          polylines))
      ...
      if (fault_block_connected_neighbour(block, i, j, i, j - 1, connected_only,
                                          polylines))
        int_vector_append(neighbour_list,
...
void fault_block_copy_content(fault_block_type *target_block,
  ...
  for (b = 0; b < int_vector_size(src_block->i_list); b++)
    fault_block_add_cell(target_block, int_vector_iget(src_block->i_list, b),
