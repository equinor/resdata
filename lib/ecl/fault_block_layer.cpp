...
bool fault_block_layer_scan_kw(fault_block_layer_type *layer,
    ...
    if (ecl_kw_get_size(fault_block_kw) !=
        ...
        int i, j;
        int max_block_id = 0;
        layer_type *work_layer = layer_alloc(ecl_grid_get_nx(layer->grid),
                                             ecl_grid_get_ny(layer->grid));
...
fault_block_layer_type *fault_block_layer_alloc(const ecl_grid_type *grid,
    ...
    if ((k < 0) || (k >= ecl_grid_get_nz(grid)))
        ...
        layer->layer =
            layer_alloc(ecl_grid_get_nx(grid), ecl_grid_get_ny(grid));
