/*
   ...
   The file 'fault_block_layer.c' is part of ERT - Ensemble based Reservoir
   Tool.
      ...
      if (cell_value != 0) {
                                  ...
                                  j_list);
        {
          ...
          for (c = 0; c < int_vector_size(i_list); c++)
            fault_block_add_cell(fault_block, int_vector_iget(i_list, c),
...
bool fault_block_layer_scan_kw(fault_block_layer_type *layer,
  ...
  if (ecl_kw_get_size(fault_block_kw) != ecl_grid_get_global_size(layer->grid))
    ...
    int i, j;
    int max_block_id = 0;
    layer_type *work_layer =
        layer_alloc(ecl_grid_get_nx(layer->grid), ecl_grid_get_ny(layer->grid));
...
bool fault_block_layer_export(const fault_block_layer_type *layer,
  ...
  if (ecl_type_is_int(ecl_kw_get_data_type(faultblock_kw)) &&
      (ecl_kw_get_size(faultblock_kw) ==
       ecl_grid_get_global_size(layer->grid))) {
    int i, j;
