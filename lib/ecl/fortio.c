...
struct fortio_struct {
  ...
  bool fmt_file; /* This is not really used by the fortio instance - but it is
                    very convenient to store it here. */
...
static bool fortio_is_fortran_stream__(FILE *stream, bool endian_flip) {
  const bool strict_checking = true; /* True: requires that *ALL* records in the
                                        file are fortran formatted */
...
void fortio_fskip_buffer(fortio_type *fortio, int buffer_size) {
  ...
  if (bytes_skipped > buffer_size)
    util_abort("%s: hmmmm - something is broken. The individual records in %s "
               "did not sum up to the expected buffer size \n",
...
// bool          fortio_endian_flip(const fortio_type *fortio)   { return
// fortio->endian_flip_header; }
...
const char *fortio_filename_ref(const fortio_type *fortio) {
  return (const char *)fortio->filename;
}
