...
void test_coarse_cell(const ecl_grid_type *grid, ecl_coarse_cell_type *cell) {
    const int_vector_type *global_index_list =
        ecl_coarse_cell_get_index_vector(cell);
...
int main(int argc, char **argv) {
...

    {
        ...
        test_assert_int_equal(ecl_kw_get_size(swat0),
                              ecl_grid_get_active_size(GRID));
        test_assert_int_equal(ecl_kw_get_size(porv),
                              ecl_grid_get_global_size(GRID));
    }

    {
        ...
        for (ic = 0; ic < ecl_grid_get_num_coarse_groups(GRID); ic++) {
            ecl_coarse_cell_type *coarse_cell =
                ecl_grid_iget_coarse_group(GRID, ic);
