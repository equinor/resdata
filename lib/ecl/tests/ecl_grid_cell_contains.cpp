...
bool get_test_point1(const ecl_grid_type *grid, int global_index, double *_xpos,
  ...
  *_zpos = zpos * 0.25;
  ...
  return ecl_grid_cell_contains_xyz1(grid, global_index, *_xpos, *_ypos,
                                     *_zpos);
}

bool get_test_point3(const ecl_grid_type *grid, int i, int j, int k,
  ...
  for (int k = 0; k < nz - 1; k++) {
    for (int j = 0; j < ny; j++) {
      for (int i = 0; i < nx; i++) {
        ...
        for (int l = 0; l < 4; l++) {
          ecl_grid_get_cell_corner_xyz1(grid, g1, l + 4, &p1[0], &p1[1],
                                        &p1[2]);
        ...
        for (int l = 0; l < 4; l++) {
          ...
          ecl_grid_get_cell_corner_xyz1(grid, g1, l + 4, &p2[0], &p2[1],
                                        &p2[2]);
...
void assert_contains(const ecl_grid_type *grid, int i, int j, int k, double x,
                     double y, double z) {
                  ...
                  for (i2 = imin; i2 < imax; i2++) {
                    if ((i != i2) && (j != j2) && (k != k2)) {
                      if (ecl_grid_get_cell_twist3(grid, i2, j2, k2) == 0)
                        test_assert_false(ecl_grid_cell_contains_xyz3(
                            grid, i2, j2, k2, x, y, z));
                    }
                  }
                }
...
void test_find(ecl_grid_type *grid) {
  ...
  for (init_index = 0; init_index < ecl_grid_get_global_size(grid);
       init_index += delta) {
    if (ecl_grid_get_cell_twist1(grid, init_index) == 0) {
      ...
      if (get_test_point1(grid, init_index, &x, &y, &z)) {
        find_index =
            ecl_grid_get_global_index_from_xyz(grid, x, y, z, start_index);
        if (init_index != find_index) {
          ...
          printf("       Regular: %d / %d \n",
                 ecl_grid_cell_regular3(grid, i1, j1, k1),
                 ecl_grid_cell_regular3(grid, i2, j2, k2));
          printf("       init: contains(%7d) : %d   (%d,%d,%d) V:%g \n",
                 init_index,
                 ecl_grid_cell_contains_xyz1(grid, init_index, x, y, z), i1 + 1,
                 j1 + 1, k1 + 1, ecl_grid_get_cell_volume1(grid, init_index));
          printf("ERROR: find: contains(%7d) : %d   (%d,%d,%d) V:%g  \n\n",
                 find_index,
                 ecl_grid_cell_contains_xyz1(grid, find_index, x, y, z), i2 + 1,
                 j2 + 1, k2 + 1, ecl_grid_get_cell_volume1(grid, find_index));
...
void test_corners() {
  ...
  test_assert_int_equal(
      -1, ecl_grid_get_global_index_from_xyz(grid, -1, -1, -1, 0));
  test_assert_int_equal(-1,
                        ecl_grid_get_global_index_from_xyz(grid, -1, 1, 1, 0));
  test_assert_int_equal(-1,
                        ecl_grid_get_global_index_from_xyz(grid, 1, -1, 1, 0));
  test_assert_int_equal(-1,
                        ecl_grid_get_global_index_from_xyz(grid, 1, 1, -1, 0));
  ...
  test_assert_int_equal(
      -1, ecl_grid_get_global_index_from_xyz(grid, 3.5, 3.5, 3.5, 0));
  test_assert_int_equal(-1,
                        ecl_grid_get_global_index_from_xyz(grid, 3.5, 1, 1, 0));
  test_assert_int_equal(-1,
                        ecl_grid_get_global_index_from_xyz(grid, 1, 3.5, 1, 0));
  test_assert_int_equal(-1,
                        ecl_grid_get_global_index_from_xyz(grid, 1, 1, 3.5, 0));

  {
    ...
    test_assert_int_equal(0,
                          ecl_grid_get_global_index_from_xyz(grid, x, y, z, 0));

    for (i = 1; i < 8; i++) {
      ecl_grid_get_cell_corner_xyz3(grid, 0, 0, 0, i, &x, &y, &z);
      test_assert_int_not_equal(
          0, ecl_grid_get_global_index_from_xyz(grid, x, y, z, 0));
    }

    // Corner 1
    ecl_grid_get_cell_corner_xyz3(grid, 2, 0, 0, 1, &x, &y, &z);
    test_assert_int_equal(ecl_grid_get_global_index3(grid, 2, 0, 0),
                          ecl_grid_get_global_index_from_xyz(grid, x, y, z, 0));

    // Corner 2
    ecl_grid_get_cell_corner_xyz3(grid, 0, 2, 0, 2, &x, &y, &z);
    test_assert_int_equal(ecl_grid_get_global_index3(grid, 0, 2, 0),
                          ecl_grid_get_global_index_from_xyz(grid, x, y, z, 0));

    // Corner 3
    ecl_grid_get_cell_corner_xyz3(grid, 2, 2, 0, 3, &x, &y, &z);
    test_assert_int_equal(ecl_grid_get_global_index3(grid, 2, 2, 0),
                          ecl_grid_get_global_index_from_xyz(grid, x, y, z, 0));

    // Corner 4
    ecl_grid_get_cell_corner_xyz3(grid, 0, 0, 2, 4, &x, &y, &z);
    test_assert_int_equal(ecl_grid_get_global_index3(grid, 0, 0, 2),
                          ecl_grid_get_global_index_from_xyz(grid, x, y, z, 0));

    // Corner 5
    ecl_grid_get_cell_corner_xyz3(grid, 2, 0, 2, 5, &x, &y, &z);
    test_assert_int_equal(ecl_grid_get_global_index3(grid, 2, 0, 2),
                          ecl_grid_get_global_index_from_xyz(grid, x, y, z, 0));

    // Corner 6
    ecl_grid_get_cell_corner_xyz3(grid, 0, 2, 2, 6, &x, &y, &z);
    test_assert_int_equal(ecl_grid_get_global_index3(grid, 0, 2, 2),
                          ecl_grid_get_global_index_from_xyz(grid, x, y, z, 0));

    // Corner 7
    ecl_grid_get_cell_corner_xyz3(grid, 2, 2, 2, 7, &x, &y, &z);
    test_assert_int_equal(ecl_grid_get_global_index3(grid, 2, 2, 2),
                          ecl_grid_get_global_index_from_xyz(grid, x, y, z, 0));
  }
