    ...
    for (int k = 0; k < nz - 1; k++) {
        for (int j = 0; j < ny; j++) {
            for (int i = 0; i < nx; i++) {
                ...
                for (int l = 0; l < 4; l++) {
                    ecl_grid_get_cell_corner_xyz1(grid, g1, l + 4, &p1[0],
                                                  &p1[1], &p1[2]);
                    ecl_grid_get_cell_corner_xyz1(grid, g2, l, &p2[0], &p2[1],
                                                  &p2[2]);
                ...
                for (int l = 0; l < 4; l++) {
                    ecl_grid_get_cell_corner_xyz1(grid, g1, l, &p1[0], &p1[1],
                                                  &p1[2]);
                    ecl_grid_get_cell_corner_xyz1(grid, g1, l + 4, &p2[0],
                                                  &p2[1], &p2[2]);
                            ...
                            for (k2 = kmin; k2 < kmax; k2++) {
                                for (j2 = jmin; j2 < jmax; j2++) {
                                    for (i2 = imin; i2 < imax; i2++) {
                                        if ((i != i2) && (j != j2) &&
                                            (k != k2)) {
                                            if (ecl_grid_get_cell_twist3(
                                                    grid, i2, j2, k2) == 0)
                                                test_assert_false(
                                                    ecl_grid_cell_contains_xyz3(
                                                        grid, i2, j2, k2, x, y,
                                                        z));
                                        }
                                    }
    ...
    for (init_index = 0; init_index < ecl_grid_get_global_size(grid);
        ...
        if (ecl_grid_get_cell_twist1(grid, init_index) == 0) {
            ...
            if (get_test_point1(grid, init_index, &x, &y, &z)) {
                find_index = ecl_grid_get_global_index_from_xyz(grid, x, y, z,
                                                                start_index);
                if (init_index != find_index) {
                    ...
                    printf(
                        ...
                        init_index,
                        ecl_grid_cell_contains_xyz1(grid, init_index, x, y, z),
                        i1 + 1, j1 + 1, k1 + 1,
                    ...
                    printf(
                        ...
                        "\n\n",
                        find_index,
                        ecl_grid_cell_contains_xyz1(grid, find_index, x, y, z),
                        i2 + 1, j2 + 1, k2 + 1,
...
void test_corners() {
    ...
    test_assert_int_equal(
        -1, ecl_grid_get_global_index_from_xyz(grid, 3.5, 1, 1, 0));
    test_assert_int_equal(
        -1, ecl_grid_get_global_index_from_xyz(grid, 1, 3.5, 1, 0));
    test_assert_int_equal(
        -1, ecl_grid_get_global_index_from_xyz(grid, 1, 1, 3.5, 0));
