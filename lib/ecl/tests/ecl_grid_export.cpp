...
void export_actnum(const ecl_grid_type *ecl_grid, ecl_file_type *ecl_file) {
    ...
    int *actnum =
        (int *)util_malloc(ecl_kw_get_size(actnum_kw) * sizeof *actnum);
...
void export_coord(const ecl_grid_type *grid, ecl_file_type *ecl_file) {
    ...
    test_assert_int_equal(ecl_kw_get_size(coord_kw),
                          ecl_grid_get_coord_size(grid));
    {
        float *coord_float = (float *)util_malloc(
            ecl_grid_get_coord_size(grid) * sizeof *coord_float);
        double *coord_double = (double *)util_malloc(
            ecl_grid_get_coord_size(grid) * sizeof *coord_double);
...
void export_zcorn(const ecl_grid_type *grid, ecl_file_type *ecl_file) {
    ...
    test_assert_int_equal(ecl_kw_get_size(zcorn_kw),
                          ecl_grid_get_zcorn_size(grid));
    {
        float *zcorn_float = (float *)util_malloc(
            ecl_grid_get_zcorn_size(grid) * sizeof *zcorn_float);
        double *zcorn_double = (double *)util_malloc(
            ecl_grid_get_zcorn_size(grid) * sizeof *zcorn_double);
...
void copy_processed(const ecl_grid_type *src) {
    ...
    {
        int *actnum =
            (int *)util_malloc(ecl_grid_get_global_size(src) * sizeof *actnum);
        int index = 0;
        ...
        {
            ecl_grid_type *copy =
                ecl_grid_alloc_processed_copy(src, NULL, actnum);
            test_assert_int_equal(1, ecl_grid_get_active_size(src) -
                                         ecl_grid_get_active_size(copy));
            ecl_grid_free(copy);
        }
        free(actnum);
    }
    ...
    {
        double *zcorn_double = (double *)util_malloc(
            ecl_grid_get_zcorn_size(src) * sizeof *zcorn_double);
        int i = 0;
        int j = 0;
        int k = 0;
        ...
        {
            ecl_grid_type *copy =
                ecl_grid_alloc_processed_copy(src, zcorn_double, NULL);
            test_assert_double_equal(ecl_grid_get_cell_volume3(src, i, j, k),
                                     ecl_grid_get_cell_volume3(copy, i, j, k));
            ecl_grid_free(copy);
        }

        for (int c = 0; c < 4; c++) {
            double dz =
                zcorn_double[ecl_grid_zcorn_index(src, i, j, k, c + 4)] -
                zcorn_double[ecl_grid_zcorn_index(src, i, j, k, c)];
...
void export_mapaxes(const ecl_grid_type *grid, ecl_file_type *ecl_file) {
    if (ecl_file_has_kw(ecl_file, "MAPAXES")) {
        ecl_kw_type *mapaxes_kw =
            ecl_file_iget_named_kw(ecl_file, "MAPAXES", 0);
        double mapaxes[6];
        int i;
        ...
        for (i = 0; i < 6; i++)
            test_assert_double_equal(ecl_kw_iget_float(mapaxes_kw, i),
                                     mapaxes[i]);
    }
}
...
int main(int argc, char **argv) {
    ecl::util::TestArea ta("grid_export");
    {
        ...
        if (argc == 1) {
            ecl_grid_type *grid =
                ecl_grid_alloc_rectangular(4, 4, 2, 1, 1, 1, NULL);
            grid_file = test_grid;
...

        {
            ...
            if (argc == 1)
                test_assert_true(ecl_grid_get_unit_system(ecl_grid) ==
                                 ECL_METRIC_UNITS);
