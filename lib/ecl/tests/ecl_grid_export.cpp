...
void export_coord(const ecl_grid_type *grid, ecl_file_type *ecl_file) {
  ...
  {
    float *coord_float = (float *)util_malloc(ecl_grid_get_coord_size(grid) *
                                              sizeof *coord_float);
    double *coord_double = (double *)util_malloc(ecl_grid_get_coord_size(grid) *
                                                 sizeof *coord_double);
...
void export_zcorn(const ecl_grid_type *grid, ecl_file_type *ecl_file) {
  ...
  {
    float *zcorn_float = (float *)util_malloc(ecl_grid_get_zcorn_size(grid) *
                                              sizeof *zcorn_float);
    double *zcorn_double = (double *)util_malloc(ecl_grid_get_zcorn_size(grid) *
                                                 sizeof *zcorn_double);
...
void copy_processed(const ecl_grid_type *src) {
  ...
  {
    double *zcorn_double = (double *)util_malloc(ecl_grid_get_zcorn_size(src) *
                                                 sizeof *zcorn_double);
    int i = 0;
    int j = 0;
    int k = 0;
    ...
    for (int c = 0; c < 4; c++) {
      double dz = zcorn_double[ecl_grid_zcorn_index(src, i, j, k, c + 4)] -
    ...
    }
    {
      ecl_grid_type *copy =
          ecl_grid_alloc_processed_copy(src, zcorn_double, NULL);
      test_assert_double_equal(ecl_grid_get_cell_volume3(src, i, j, k) * 2,
                               ecl_grid_get_cell_volume3(copy, i, j, k));
      ecl_grid_free(copy);
    }
