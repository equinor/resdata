...
void test_write_depth(const ecl_grid_type *grid) {
    ecl::util::TestArea ta("write_depth");
    {
        fortio_type *init_file =
            fortio_open_writer("INIT", false, ECL_ENDIAN_FLIP);
    ...
    }
    {
        ...
        test_assert_int_equal(ecl_kw_get_size(depth),
                              ecl_grid_get_nactive(grid));
        for (int i = 0; i < ecl_grid_get_nactive(grid); i++)
            test_assert_double_equal(ecl_kw_iget_as_double(depth, i),
                                     ecl_grid_get_cdepth1A(grid, i));
...
void test_write_dims(const ecl_grid_type *grid) {
    ...
    }
    {
        ...
        for (int i = 0; i < ecl_grid_get_nactive(grid); i++) {
            test_assert_double_equal(ecl_kw_iget_as_double(DX, i),
                                     ecl_grid_get_cell_dx1A(grid, i));
            test_assert_double_equal(ecl_kw_iget_as_double(DY, i),
                                     ecl_grid_get_cell_dy1A(grid, i));
            test_assert_double_equal(ecl_kw_iget_as_double(DZ, i),
                                     ecl_grid_get_cell_dz1A(grid, i));
        }
        ecl_file_close(init_file);
    }
}
...
ecl_grid_type *create_grid() {
    ...
    ecl_grid_type *ecl_grid = ecl_grid_alloc_rectangular(
