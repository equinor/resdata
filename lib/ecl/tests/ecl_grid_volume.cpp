...
int main(int argc, char **argv) {
  ...
  char *grid_file =
      ecl_util_alloc_filename(NULL, path_case, ECL_EGRID_FILE, false, 0);
  char *init_file =
      ecl_util_alloc_filename(NULL, path_case, ECL_INIT_FILE, false, 0);
  ...
  for (iactive = 0; iactive < ecl_grid_get_nactive(grid); ++iactive) {
    ...
    double eclipse_volume = ecl_kw_iget_float(porv_kw, iglobal) /
                            ecl_kw_iget_float(poro_kw, iactive);
    ...
    if (!util_double_approx_equal__(grid_volume, eclipse_volume, 2.5e-3,
      ...
      printf("Error in cell: %d V1: %g    V2: %g   diff:%g %% \n", iglobal,
             grid_volume, eclipse_volume, diff);
      error_count++;
      error_found = true;
    }
  }
