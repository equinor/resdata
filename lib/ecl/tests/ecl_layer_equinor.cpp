/*
   ...
   The file 'ecl_layer_equinor.c' is part of ERT - Ensemble based Reservoir
   Tool.
...
ecl_kw_type *alloc_faultblock_kw(const char *filename, int grid_size) {
  ...
  ecl_kw_type *kw =
      ecl_kw_fscanf_alloc_grdecl(stream, "FAULTBLK", grid_size, ECL_INT);
  fclose(stream);

  return kw;
}

void test_layer(const ecl_grid_type *ecl_grid, const ecl_kw_type *faultblock_kw,
                int k) {
...
int main(int argc, char **argv) {
  ...
  ecl_kw_type *faultblock_kw =
      alloc_faultblock_kw(argv[2], ecl_grid_get_global_size(ecl_grid));
  int k;
