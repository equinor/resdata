/*
   ...
   The file 'test_ecl_nnc_data_equinor.c' is part of ERT - Ensemble based
   Reservoir Tool.
...
int find_index(ecl_nnc_geometry_type *nnc_geo, int grid1, int grid2, int indx1,
               int indx2) {
  int index = -1;
...
void test_alloc_file_tran(char *filename) {
  char *grid_file_name =
      ecl_util_alloc_filename(NULL, filename, ECL_EGRID_FILE, false, -1);
  char *init_file_name =
      ecl_util_alloc_filename(NULL, filename, ECL_INIT_FILE, false, -1);
  ...
  test_assert_true(ecl_nnc_geometry_size(nnc_geo) ==
                   ecl_nnc_data_get_size(nnc_geo_data));
  ...
  test_assert_double_equal(13.784438,
                           ecl_nnc_data_iget_value(nnc_geo_data, index));
...
void test_alloc_file_flux(char *filename, int file_num) {
  char *grid_file_name =
      ecl_util_alloc_filename(NULL, filename, ECL_EGRID_FILE, false, -1);
  char *restart_file_name = ecl_util_alloc_filename(
      NULL, filename, ECL_RESTART_FILE, false, file_num);
  ...
  ecl_nnc_geometry_type *nnc_geo = ecl_nnc_geometry_alloc(grid);
  {
    ...
    ecl_nnc_data_type *nnc_flux_data =
        ecl_nnc_data_alloc_wat_flux(grid, nnc_geo, view_file);
