...
int count_kw_data(const ecl_file_type *file, ecl_grid_type *grid,
...
void test_count(const char *name) {
    char *grid_file_name =
        ecl_util_alloc_filename(NULL, name, ECL_EGRID_FILE, false, -1);
    char *init_file_name =
        ecl_util_alloc_filename(NULL, name, ECL_INIT_FILE, false, -1);
...
void test_nnc_export_missing_TRANX(const char *name) {
    ...
    if (util_entry_exists(init_file_name)) {
        ecl_grid_type *grid = ecl_grid_alloc(grid_file_name);
        ecl_file_type *init_file = ecl_file_open(init_file_name, 0);
        ecl_nnc_type *nnc_data1 = (ecl_nnc_type *)util_calloc(
            ecl_nnc_export_get_size(grid, init_file), sizeof *nnc_data1);
...
void test_export(const char *name, bool have_tran_data) {
    ...
    if (util_entry_exists(init_file_name)) {
        ...
        ecl_nnc_type *nnc_data1 = (ecl_nnc_type *)util_calloc(
            ecl_nnc_export_get_size(grid, init_file), sizeof *nnc_data1);
        ecl_nnc_type *nnc_data2 = (ecl_nnc_type *)util_calloc(
            ecl_nnc_export_get_size(grid, init_file), sizeof *nnc_data2);

        {
            int nnc_offset = 0;
            int block_nr = 0;
            for (block_nr = 0;
                 block_nr < ecl_file_get_num_named_kw(grid_file, NNCHEAD_KW);
                 block_nr++) {
                ecl_file_view_type *nnc_view = ecl_file_alloc_global_blockview(
                    grid_file, NNCHEAD_KW, block_nr);

                if (ecl_file_view_has_kw(nnc_view, NNC1_KW)) {
                    ecl_kw_type *nnc1_kw =
                        ecl_file_view_iget_named_kw(nnc_view, NNC1_KW, 0);
                    ecl_kw_type *nnc2_kw =
                        ecl_file_view_iget_named_kw(nnc_view, NNC2_KW, 0);
                    ecl_kw_type *nnchead =
                        ecl_file_view_iget_named_kw(nnc_view, NNCHEAD_KW, 0);
                    int lgr_nr1 = ecl_kw_iget_int(nnchead, NNCHEAD_LGR_INDEX);
                    int lgr_nr2 = ecl_kw_iget_int(nnchead, NNCHEAD_LGR_INDEX);
                    ecl_kw_type *nnc_tran = ecl_nnc_export_get_tranx_kw(
                    ...
                    test_assert_int_equal(ecl_kw_get_size(nnc1_kw),
                                          ecl_kw_get_size(nnc_tran));
                    {
                        ...
                        while (i < ecl_kw_get_size(nnc1_kw)) {
                            ...
                            if (g1 < ecl_grid_get_global_size(lgr) &&
                                ...
                                nnc_data1[i + nnc_offset].trans =
                                    ecl_kw_iget_as_double(nnc_tran, i);
                ...
                if (ecl_file_view_has_kw(nnc_view, NNCL_KW)) {
                    ecl_kw_type *nncl_kw =
                        ecl_file_view_iget_named_kw(nnc_view, NNCL_KW, 0);
                    ecl_kw_type *nncg_kw =
                        ecl_file_view_iget_named_kw(nnc_view, NNCG_KW, 0);
                    ecl_kw_type *nnchead =
                        ecl_file_view_iget_named_kw(nnc_view, NNCHEAD_KW, 0);
                    int lgr_nr1 = 0;
                    int lgr_nr2 = ecl_kw_iget_int(nnchead, NNCHEAD_LGR_INDEX);
                    ecl_kw_type *nnc_tran = ecl_nnc_export_get_tranx_kw(
                        grid, init_file, 0, lgr_nr2);
                    int i;

                    test_assert_int_equal(ecl_kw_get_size(nncl_kw),
                                          ecl_kw_get_size(nnc_tran));
                    for (i = 0; i < ecl_kw_get_size(nncl_kw); i++) {
                        nnc_data1[i + nnc_offset].grid_nr1 = lgr_nr1;
                        nnc_data1[i + nnc_offset].grid_nr2 = lgr_nr2;
                        nnc_data1[i + nnc_offset].global_index1 =
                            ecl_kw_iget_int(nncg_kw, i) - 1;
                        nnc_data1[i + nnc_offset].global_index2 =
                            ecl_kw_iget_int(nncl_kw, i) - 1;
                        nnc_data1[i + nnc_offset].trans =
                            ecl_kw_iget_as_double(nnc_tran, i);
                        nnc_data1[i + nnc_offset].input_index = i;
                    }
            ...
            {
                for (block_nr = 0; block_nr < ecl_file_get_num_named_kw(
                                                  grid_file, NNCHEADA_KW);
                     block_nr++) {
                    ecl_kw_type *nnc1_kw =
                        ecl_file_iget_named_kw(grid_file, NNA1_KW, block_nr);
                    ecl_kw_type *nnc2_kw =
                        ecl_file_iget_named_kw(grid_file, NNA2_KW, block_nr);
                    ecl_kw_type *nnchead = ecl_file_iget_named_kw(
                        grid_file, NNCHEADA_KW, block_nr);
                    int lgr_nr1 =
                        ecl_kw_iget_int(nnchead, NNCHEADA_ILOC1_INDEX);
                    int lgr_nr2 =
                        ecl_kw_iget_int(nnchead, NNCHEADA_ILOC2_INDEX);
                    ecl_kw_type *nnc_tran;
                    int i;

                    nnc_tran = ecl_nnc_export_get_tranx_kw(grid, init_file,
                                                           lgr_nr1, lgr_nr2);
                    test_assert_not_NULL(nnc_tran);
                    test_assert_int_equal(ecl_kw_get_size(nnc1_kw),
                                          ecl_kw_get_size(nnc_tran));
                    for (i = 0; i < ecl_kw_get_size(nnc1_kw); i++) {
                        ...
                        nnc_data1[i + nnc_offset].grid_nr2 = lgr_nr2;
                        nnc_data1[i + nnc_offset].global_index1 =
                            ecl_kw_iget_int(nnc1_kw, i) - 1;
                        nnc_data1[i + nnc_offset].global_index2 =
                            ecl_kw_iget_int(nnc2_kw, i) - 1;
                        nnc_data1[i + nnc_offset].trans =
                            ecl_kw_iget_as_double(nnc_tran, i);
                        nnc_data1[i + nnc_offset].input_index = i;
                    }
            ...
            test_assert_int_equal(nnc_offset,
                                  ecl_nnc_export_get_size(grid, init_file));
...

        {
            ...
            for (i = 0; i < size; i++)
                test_assert_int_equal(
                    0, ecl_nnc_sort_cmp(&nnc_data1[i], &nnc_data2[i]));
        }
...

        {
            ...
            ecl_nnc_data_type *nnc_geo_data =
                ecl_nnc_data_alloc_tran(grid, nnc_geo, view_file);

            test_assert_int_equal(ecl_nnc_export_get_size(grid, init_file),
                                  ecl_nnc_geometry_size(nnc_geo));
            for (int i = 0; i < ecl_nnc_geometry_size(nnc_geo); i++) {
                const ecl_nnc_pair_type *nnc_pair =
                    ecl_nnc_geometry_iget(nnc_geo, i);
                ...
                test_assert_double_equal(
                    ecl_nnc_data_iget_value(nnc_geo_data, i), nnc1->trans);
            }
...
void test_sort() {
    ...
    for (i = 0; i < (N - 1); i++)
        test_assert_int_equal(-1,
                              ecl_nnc_sort_cmp(&nnc_list[i], &nnc_list[i + 1]));
