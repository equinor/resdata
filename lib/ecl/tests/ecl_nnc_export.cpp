  ...
  if (util_entry_exists(init_file_name)) {
...

    {
      ...
      for (block_nr = 0;
           ...
           block_nr++) {
        ecl_file_view_type *nnc_view =
            ecl_file_alloc_global_blockview(grid_file, NNCHEAD_KW, block_nr);
        ...
        if (ecl_file_view_has_kw(nnc_view, NNC1_KW)) {
          ...
          ecl_kw_type *nnc_tran =
              ecl_nnc_export_get_tranx_kw(grid, init_file, lgr_nr1, lgr_nr2);
          if (!have_tran_data) {
        ...
        if (ecl_file_view_has_kw(nnc_view, NNCL_KW)) {
          ...
          int lgr_nr2 = ecl_kw_iget_int(nnchead, NNCHEAD_LGR_INDEX);
          ecl_kw_type *nnc_tran =
              ecl_nnc_export_get_tranx_kw(grid, init_file, 0, lgr_nr2);
          int i;
      ...
      {
        for (block_nr = 0;
             block_nr < ecl_file_get_num_named_kw(grid_file, NNCHEADA_KW);
             block_nr++) {
          ecl_kw_type *nnc1_kw =
          ...
          ecl_kw_type *nnchead =
              ecl_file_iget_named_kw(grid_file, NNCHEADA_KW, block_nr);
...

    {
      ...
      for (i = 0; i < size; i++)
        test_assert_int_equal(0,
                              ecl_nnc_sort_cmp(&nnc_data1[i], &nnc_data2[i]));
    }
...

    {
      ...
      for (int i = 0; i < ecl_nnc_geometry_size(nnc_geo); i++) {
        ...
        test_assert_double_equal(ecl_nnc_data_iget_value(nnc_geo_data, i),
                                 nnc1->trans);
      }
...
void install_SIGNALS(void) {
  ...
  signal(SIGTERM,
         util_abort_signal); /* If killing the program with SIGTERM (the default
                                kill signal) you will get a backtrace. Killing
                                with SIGKILL (-9) will not give a backtrace.*/
}
