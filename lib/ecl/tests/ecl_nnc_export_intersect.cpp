...
ERT::ert_unique_ptr<ecl_file_type, ecl_file_close> make_intersect_init_file() {
  ...
  auto init_filename =
      util_alloc_tmp_file("/tmp", "ecl_nnc_export_intersect_init_file", false);
...
int main(int argc, char **argv) {
  ...
  for (auto i = 0; i < int(nnc_data.size()); ++i) {
    ...
    test_assert_int_equal(nnc.global_index2,
                          2 * i + 1); // as set in make_intersect_init_file()
    test_assert_double_equal(nnc.trans,
                             2.5 * i); // as set in make_intersect_init_file()
