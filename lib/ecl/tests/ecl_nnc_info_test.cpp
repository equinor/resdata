/*
   ...
   The file 'ecl_nnc_info_test.c' is part of ERT - Ensemble based Reservoir
   Tool.
...
void basic_test() {
  ...
  test_assert_ptr_equal(nnc_cells.data(),
                        nnc_vector_get_grid_index_list(nnc_vector).data());
  ...
  test_assert_ptr_equal(nnc_info_get_vector(nnc_info, 1),
                        nnc_info_iget_vector(nnc_info, 1));
