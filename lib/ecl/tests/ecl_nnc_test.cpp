                    ...
                    for (i = 0; i < ecl_kw_get_size(nnc1_kw); i++) {
                        ...
                        if (g2 <
                            ...
                            const nnc_info_type *nnc_info =
                                ecl_grid_get_cell_nnc_info1(lgr, g1);
        ...
        /* Global -> lgr */
        {
            if (ecl_file_view_has_kw(nnc_view, NNCG_KW)) {
                ...
                for (i = 0; i < ecl_kw_get_size(nncg_kw); i++) {
                    ...
                    const nnc_info_type *nnc_info =
                        ecl_grid_get_cell_nnc_info1(ecl_grid, g);
                    test_assert_not_NULL(nnc_info);
                    {
                        const std::vector<int> &index_list =
                            nnc_info_get_grid_index_list(nnc_info, lgr_nr);
        ...
        /* Amalgamated: LGR -> LGR */
        {
            if (ecl_file_view_has_kw(nnc_view, NNCHEADA_KW)) {
                ecl_kw_type *nncheada_kw =
                    ecl_file_view_iget_named_kw(nnc_view, NNCHEADA_KW, 0);
                ecl_kw_type *nnc1_kw =
                    ecl_file_view_iget_named_kw(nnc_view, NNA1_KW, 0);
                ecl_kw_type *nnc2_kw =
                    ecl_file_view_iget_named_kw(nnc_view, NNA2_KW, 0);
                int lgr_nr1 =
                    ecl_kw_iget_int(nncheada_kw, NNCHEADA_ILOC1_INDEX);
                int lgr_nr2 =
                    ecl_kw_iget_int(nncheada_kw, NNCHEADA_ILOC2_INDEX);

                ecl_grid_type *lgr1 =
                    ecl_grid_get_lgr_from_lgr_nr(ecl_grid, lgr_nr1);
                for (int i = 0; i < ecl_kw_get_size(nnc1_kw); i++) {
                    const int g1 = ecl_kw_iget_int(nnc1_kw, i) - 1;
                    const int g2 = ecl_kw_iget_int(nnc2_kw, i) - 1;

                    const nnc_info_type *nnc_info =
                        ecl_grid_get_cell_nnc_info1(lgr1, g1);
                    const std::vector<int> &index_list =
