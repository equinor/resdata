...
void test_empty() {
    ecl_rst_file_type *rst_file =
        ecl_rst_file_open_write_seek("EMPTY.UNRST", 0);
...
void test_file(const char *src_file, const char *target_file, int report_step,
    ...
    {
        ecl_rst_file_type *rst_file =
            ecl_rst_file_open_write_seek(target_file, report_step);
...
void test_Xfile() {
    ...
    {
        fortio_type *f =
            fortio_open_writer("TEST.X0010", false, ECL_ENDIAN_FLIP);
...
void test_UNRST0() {
    ...
    {
        fortio_type *f =
            fortio_open_writer("TEST.UNRST", false, ECL_ENDIAN_FLIP);
        write_seqnum(f, 0);
        write_keyword(f, "INTEHEAD", ECL_INT);
        write_keyword(f, "PRESSURE", ECL_FLOAT);
        write_keyword(f, "SWAT", ECL_FLOAT);
