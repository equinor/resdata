...
ecl_sum_type *test_alloc_ecl_sum() {
  ...
  ecl_sum_type *ecl_sum = ecl_sum_alloc_writer("/tmp/CASE", false, true, ":",
...
void test_correct_time_vector() {
  ...
  ecl_sum_type *ecl_sum_resampled =
      ecl_sum_alloc_resample(ecl_sum, "kk", t, false, false);
  test_assert_int_equal(ecl_sum_get_report_time(ecl_sum_resampled, 2),
                        util_make_date_utc(6, 1, 2010));

  const ecl_smspec_type *smspec_resampled =
      ecl_sum_get_smspec(ecl_sum_resampled);
  const ecl::smspec_node &node1 =
      ecl_smspec_iget_node_w_params_index(smspec_resampled, 1);
  const ecl::smspec_node &node2 =
      ecl_smspec_iget_node_w_params_index(smspec_resampled, 2);
  const ecl::smspec_node &node3 =
      ecl_smspec_iget_node_w_params_index(smspec_resampled, 3);
  ...
  test_assert_double_equal(
      3.33333, ecl_sum_get_from_sim_time(
                   ecl_sum_resampled, util_make_date_utc(6, 1, 2010), &node1));
  test_assert_double_equal(
      3.33333, ecl_sum_get_from_sim_time(
                   ecl_sum_resampled, util_make_date_utc(2, 1, 2010), &node2));
  test_assert_double_equal(
      10.0000, ecl_sum_get_from_sim_time(
                   ecl_sum_resampled, util_make_date_utc(4, 1, 2010), &node3));
...
void test_resample_extrapolate_rate() {
  ...
  ecl_sum_type *ecl_sum_resampled =
      ecl_sum_alloc_resample(ecl_sum, "kk", t, true, true);
  ...
  test_assert_double_equal(
      0, ecl_sum_get_from_sim_time(ecl_sum_resampled,
                                   util_make_date_utc(1, 1, 2009), &node3));
  test_assert_double_equal(
      10.000, ecl_sum_get_from_sim_time(
                  ecl_sum_resampled, util_make_date_utc(4, 1, 2010), &node3));
  test_assert_double_equal(
      0, ecl_sum_get_from_sim_time(ecl_sum_resampled,
  ...
  test_assert_double_equal(
      2.000, ecl_sum_get_from_sim_time(ecl_sum_resampled,
                                       util_make_date_utc(4, 1, 2010), &node1));
  test_assert_double_equal(
      6.000, ecl_sum_get_from_sim_time(
                 ecl_sum_resampled, util_make_date_utc(12, 1, 2010), &node1));
