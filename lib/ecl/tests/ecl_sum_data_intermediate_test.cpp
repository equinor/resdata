...
void write_CASE1(bool unified) {
    ...
    ecl_sum_type *ecl_sum = ecl_sum_alloc_writer("CASE1", false, unified, ":",
    ...
    const ecl::smspec_node *node1 =
        ecl_sum_add_var(ecl_sum, "BPR", NULL, 1, "BARS", 0.0);
    const ecl::smspec_node *node2 =
        ecl_sum_add_var(ecl_sum, "BPR", NULL, 2, "BARS", 0.0);
    const ecl::smspec_node *node3 =
        ecl_sum_add_var(ecl_sum, "BPR", NULL, 3, "BARS", 0.0);
    ...
    for (int report_step = 0; report_step < num_dates; report_step++) {
        {
            ...
            ecl_sum_tstep_set_from_node(tstep, *node2,
                                        (1 + report_step) * 100 + 10.0);
            ecl_sum_tstep_set_from_node(tstep, *node3,
...
void verify_CASE2(int length) {
    ecl_sum_type *sum =
        ecl_sum_fread_alloc_case2__("CASE2", ":", false, true, 0);
...
void write_CASE2(bool unified) {
    write_CASE1(unified);
    {
        ...
        for (int report_step = 1; report_step <= num_dates; report_step++) {
            {
                ecl_sum_tstep_type *tstep =
                    ecl_sum_add_tstep(ecl_sum, report_step + 3, sim_seconds);
...
void verify_CASE3(int length) {
    ...
    ecl_sum_type *sum =
        ecl_sum_fread_alloc_case2__("CASE3", ":", false, true, 0);
    ...
    for (int i = 0; i < params_size; i++) {
        ...
        if (i == 0) {
            const ecl::smspec_node &node =
                ecl_smspec_iget_node_w_params_index(ecl_sum_get_smspec(sum), i);
...
void write_CASE3(bool unified) {
    write_CASE2(unified);
    {
        ...
        for (int report_step = 1; report_step <= num_dates; report_step++) {
            {
                ecl_sum_tstep_type *tstep =
                    ecl_sum_add_tstep(ecl_sum, report_step + 5, sim_seconds);
...
void verify_CASE4() {
    ...
    d = ecl_sum_alloc_data_vector(sum, 0, false);
    double_vector_free(d);
    d = ecl_sum_alloc_data_vector(sum, 1, false);
    double_vector_free(d);
    d = ecl_sum_alloc_data_vector(sum, 2, false);
    double_vector_free(d);
...
void write_CASE4(bool unified) {
    ...
    write_CASE3(unified);
    {
        ...
        ecl_kw_type *keywords =
            ecl_file_iget_named_kw(smspec_file, "KEYWORDS", 0);
        ...
        ecl_kw_type *wgnames =
            ecl_file_iget_named_kw(smspec_file, "WGNAMES", 0);
        ...
        for (int i = 0; i < num_params; i++) {
            ecl_kw_type *params_kw =
                ecl_file_iget_named_kw(sum_file, "PARAMS", i);
