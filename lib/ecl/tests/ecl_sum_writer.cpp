        ...
        for (int step = 0; step < num_ministep; step++) {
...

            {
                ...
                test_assert_double_equal(
                    ecl_sum_tstep_get_from_node(tstep, *node1), sim_seconds);
                test_assert_double_equal(
                    ecl_sum_tstep_get_from_node(tstep, *node2),
                    sim_seconds * 10);
                test_assert_double_equal(
                    ecl_sum_tstep_get_from_node(tstep, *node3),
                    sim_seconds * 100);
            }
...
void test_long_restart_names() {
    ...
    ecl::util::TestArea ta("suM_write_restart_long_name");
    {
        int restart_step = 77;
        time_t start_time = util_make_date_utc(1, 1, 2010);
        ecl_sum_type *ecl_sum = ecl_sum_alloc_restart_writer2(
