...
double write_summary(const char *name, time_t start_time, int nx, int ny,
  ...
  ecl_sum_type *ecl_sum = ecl_sum_alloc_writer(name, false, true, ":",
  ...
  const ecl::smspec_node *node1 =
      ecl_smspec_add_node(smspec, "FOPT", "Barrels", 99.0);
  const ecl::smspec_node *node2 =
      ecl_smspec_add_node(smspec, "BPR", 567, "BARS", 0.0);
  const ecl::smspec_node *node3 =
      ecl_smspec_add_node(smspec, "WWCT", "OP-1", "(1)", 0.0);
    ...
    for (int step = 0; step < num_ministep; step++) {
...

      {
        ecl_sum_tstep_type *tstep =
            ecl_sum_add_tstep(ecl_sum, report_step + 1, sim_seconds);
        ...
        test_assert_double_equal(ecl_sum_tstep_get_from_node(tstep, *node1),
                                 sim_seconds);
        test_assert_double_equal(ecl_sum_tstep_get_from_node(tstep, *node2),
                                 sim_seconds * 10);
        test_assert_double_equal(ecl_sum_tstep_get_from_node(tstep, *node3),
                                 sim_seconds * 100);
      }
...
int write_restart_summary(const char *name, const char *restart_name,
                          ...
                          double ministep_length) {
  ecl_sum_type *ecl_sum = ecl_sum_alloc_restart_writer(
...
void test_write_read() {
  ...
  int num_ministep = 10;
  double ministep_length =
      86400; // Seconds - numerical value chosen to avoid rounding problems when
             // converting between seconds and days.
  {
    ...
    util_inplace_forward_seconds_utc(&end_time, (num_dates * num_ministep - 1) *
                                                    ministep_length);
    ...
    test_assert_throw(ecl_sum_get_general_var(ecl_sum, 1, "NO_SUCH_KEY"),
                      std::out_of_range);
    ecl_sum_free(ecl_sum);
  }
}
...
void test_ecl_sum_alloc_restart_writer() {
  ecl::util::TestArea ta("sum_write_restart");
  {
    ...
    int sim_seconds = write_summary(name1, start_time, nx, ny, nz, num_dates,
                                    num_ministep, ministep_length);
    sim_seconds = write_restart_summary(name2, name1, num_dates, sim_seconds,
                                        start_time, nx, ny, nz, num_dates,
                                        num_ministep, ministep_length);
    ...
    ecl_kw_type *restart_kw =
        ecl_file_view_iget_named_kw(view_file, "RESTART", 0);
    test_assert_int_equal(8, ecl_kw_get_size(restart_kw));
    test_assert_string_equal("CASE1   ",
                             (const char *)ecl_kw_iget_ptr(restart_kw, 0));
    test_assert_string_equal("        ",
                             (const char *)ecl_kw_iget_ptr(restart_kw, 1));

    for (int time_index = 0; time_index < ecl_sum_get_data_length(case1);
         time_index++)
      test_assert_double_equal(
          ecl_sum_get_general_var(case1, time_index, "FOPT"),
          ecl_sum_get_general_var(case2, time_index, "FOPT"));
...
void test_long_restart_names() {
  ...
  ecl::util::TestArea ta("suM_write_restart_long_name");
  {
    ...
    time_t start_time = util_make_date_utc(1, 1, 2010);
    ecl_sum_type *ecl_sum =
        ecl_sum_alloc_restart_writer2(name, restart_case, restart_step, false,
                                      true, ":", start_time, true, 3, 3, 3);
    ecl_sum_fwrite(ecl_sum);
    ecl_sum_free(ecl_sum);
    ...
    {
      ecl_smspec_type *smspec = ecl_smspec_alloc_restart_writer(
          ":",
