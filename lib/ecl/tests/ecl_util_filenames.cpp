/*
   ...
   The file 'ecl_util_filenames.c' is part of ERT - Ensemble based Reservoir
   Tool.
...
void test_file_list() {
  ...
  for (int i = 0; i < 5; i++) {
    char *fname =
        ecl_util_alloc_filename(NULL, "case", ECL_RESTART_FILE, true, 2 * i);
  ...
  for (int i = 0; i < 10; i++) {
    char *summary_file1 =
        ecl_util_alloc_filename("path", "CASE1", ECL_SUMMARY_FILE, false, i);
    char *summary_file2 =
        ecl_util_alloc_filename("path", "CASE2", ECL_SUMMARY_FILE, true, i);
    char *restart_file1 =
        ecl_util_alloc_filename("path", "CASE1", ECL_RESTART_FILE, false, i);
