        ...
        printf("<ECLIPSE RFT FILE>\n");
        {
            ...
            for (iw = 0; iw < stringlist_get_size(wells); iw++) {
                ...
                printf("    <WELL>\n");
                {
                    int it;
                    for (it = 0;
                         it < ecl_rft_file_get_well_occurences(rft_file, well);
                         it++) {
                        const ecl_rft_node_type *node =
                            ecl_rft_file_iget_well_rft(rft_file, well, it);
                        time_t date = ecl_rft_node_get_date(node);
                        {
                                   ...
                                   mday, month, year);
                            {
                                ...
                                for (icell = 0; icell < num_cells; icell++) {
                                    int i, j, k;
                                    ecl_rft_node_iget_ijk(node, icell, &i, &j,
                                                          &k);
                                    printf("            <cell>\n");
                                    printf("                <PRESSURE> %g "
                                           "</PRESSURE> \n",
                                           ecl_rft_node_iget_pressure(node,
                                                                      icell));
                                    printf(
                                        "                <DPETH>    %g "
                                        "</DEPTH>     \n",
