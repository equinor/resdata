/*
   ...
   The file 'test_ecl_nnc_data.c' is part of ERT - Ensemble based Reservoir
   Tool.
...
void test_alloc_global_only(bool data_in_file) {
  ecl::util::TestArea ta("nnc-INIT");
  {
    ...
    ecl_grid_add_self_nnc(grid0, 2, nx * ny + 2, 2);
    {
      ...
      /*
      Create a dummy INIT file which *ony* contains a TRANNC keyword with the
      correct size.
      */
      {
        ecl_kw_type *trann_nnc =
            ecl_kw_alloc(TRANNNC_KW, ecl_nnc_geometry_size(nnc_geo), ECL_FLOAT);
        fortio_type *f =
      ...
      ecl_nnc_data_type *nnc_geo_data =
          ecl_nnc_data_alloc_tran(grid0, nnc_geo, view_file);
