/*
   ...
   The file 'well_segment_conn_load.c' is part of ERT - Ensemble based Reservoir
   Tool.
...
int main(int argc, char **argv) {
  ...
  test_assert_not_NULL(rst_head);
  {
    ...
    for (well_nr = 0; well_nr < rst_head->nwells; well_nr++) {
      ...
      well_conn_collection_load_from_kw(connections, iwel_kw, icon_kw, scon_kw,
                                        xcon_kw, well_nr, rst_head);
      {
        ...
        if (well_segment_collection_load_from_kw(
                segments, well_nr, iwel_kw, iseg_kw, rseg_loader, rst_head,
                load_segment_information, &is_MSW_well)) {
          ...
          {
            int is;
            for (is = 0; is < well_segment_collection_get_size(segments);
                 is++) {
              ...
              test_assert_int_not_equal(well_segment_get_id(segment),
                                        well_segment_get_outlet_id(segment));
              test_assert_ptr_not_equal(segment,
          ...
          well_segment_collection_add_branches(segments, branches);
          {
            ...
            for (ib = 0; ib < well_branch_collection_get_size(branches); ib++) {
              const well_segment_type *start_segment =
                  well_branch_collection_iget_start_segment(branches, ib);
