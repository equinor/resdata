/*
   ...
   The file 'well_segment_conn.c' is part of ERT - Ensemble based Reservoir
   Tool.
...
int main(int argc, char **argv) {
  ...
  double *rseg_data = (double *)util_calloc(100, sizeof *rseg_data);
  {
    ...
    well_segment_type *ws =
        well_segment_alloc(segment_id, outlet_segment_id, branch_nr, rseg_data);
    well_conn_type *conn1 =
        well_conn_alloc_MSW(1, 1, 1, CF, well_conn_dirX, true, segment_id);
    well_conn_type *conn2 =
        well_conn_alloc_MSW(1, 1, 1, CF, well_conn_dirX, true, segment_id + 1);

    test_assert_false(well_segment_has_global_grid_connections(ws));

    test_assert_true(
        well_segment_add_connection(ws, ECL_GRID_GLOBAL_GRID, conn1));
    test_assert_false(
        well_segment_add_connection(ws, ECL_GRID_GLOBAL_GRID, conn2));

    test_assert_true(
        well_segment_has_grid_connections(ws, ECL_GRID_GLOBAL_GRID));
    ...
    test_assert_true(well_conn_collection_is_instance(
        well_segment_get_connections(ws, ECL_GRID_GLOBAL_GRID)));
    test_assert_true(well_conn_collection_is_instance(
