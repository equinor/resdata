...
int main(int argc, char **argv) {
  ...
  test_assert_not_NULL(rst_head);
  {
    ...
    for (well_nr = 0; well_nr < rst_head->nwells; well_nr++) {
      ...
      well_conn_collection_load_from_kw(connections, iwel_kw, icon_kw, well_nr,
                                        rst_head);
      {
        well_segment_collection_type *segments =
            well_segment_collection_alloc();

        if (well_segment_collection_load_from_kw(segments, well_nr, iwel_kw,
                                                 iseg_kw, rseg_kw, rst_head)) {
