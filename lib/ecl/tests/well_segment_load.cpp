/*
   ...
   The file 'well_segment_load.c' is part of ERT - Ensemble based Reservoir
   Tool.
...
int main(int argc, char **argv) {
...

  {
    ...
    for (well_nr = 0; well_nr < rst_head->nwells; well_nr++) {
      ...
      int seg_well_nr =
          ecl_kw_iget_int(iwel_kw, iwel_offset + IWEL_SEGMENTED_WELL_NR_INDEX) -
          1; // -1: Ordinary well.
      if (seg_well_nr >= 0) {
        int segment_index;
        int segment_count = 0;
        ...
        for (segment_index = 0; segment_index < rst_head->nsegmx;
          ...
          if (well_segment_active(segment)) {
            ...
            test_assert_int_equal(well_segment_collection_get_size(segments),
                                  segment_count + 1);
            test_assert_ptr_equal(
                well_segment_collection_iget(segments, segment_count), segment);
            segment_count++;
          } else
...

      {
        ...
        test_assert_int_equal(
            well_segment_collection_load_from_kw(segments2, well_nr, iwel_kw,
                                                 iseg_kw, rseg_loader, rst_head,
                                                 load_segments, &is_MSW_well),
