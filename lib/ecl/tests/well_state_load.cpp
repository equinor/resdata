...
int main(int argc, char **argv) {
  test_install_SIGNALS();
  {
    ...
    for (global_well_nr = 0; global_well_nr < header->nwells;
      ...
      test_assert_true(
          well_state_has_grid_connections(well_state, ECL_GRID_GLOBAL_GRID));
                          ...
                          load_segment_information);
      {
        ...
        if (well_state_is_MSW(well_state)) {
          ...
          test_assert_int_not_equal(well_segment_collection_get_size(segments),
                                    0);
          test_assert_int_not_equal(well_branch_collection_get_size(branches),
                                    0);
        } else {
