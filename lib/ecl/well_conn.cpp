...
well_conn_type *well_conn_alloc_from_kw(const ecl_kw_type *icon_kw,
  ...
  */
  int int_direction =
      ecl_kw_iget_int(icon_kw, icon_offset + ICON_DIRECTION_INDEX);
  ...
  bool is_open =
      (ecl_kw_iget_int(icon_kw, icon_offset + ICON_STATUS_INDEX) > 0);
  ...
  if (scon_kw) {
    ...
    connection_factor =
        ecl_kw_iget_as_double(scon_kw, scon_offset + SCON_CF_INDEX);
  }

  {
    int segment_id =
        ecl_kw_iget_int(icon_kw, icon_offset + ICON_SEGMENT_INDEX) -
        ECLIPSE_WELL_SEGMENT_OFFSET + WELL_SEGMENT_OFFSET;
    well_conn_type *conn =
        well_conn_alloc__(i, j, k, connection_factor, dir, is_open, segment_id,
                          matrix_connection);
    ...
    if (xcon_kw) {
      ...
      conn->water_rate =
          ecl_kw_iget_as_double(xcon_kw, xcon_offset + XCON_WRAT_INDEX);
      conn->gas_rate =
          ecl_kw_iget_as_double(xcon_kw, xcon_offset + XCON_GRAT_INDEX);
      conn->oil_rate =
          ecl_kw_iget_as_double(xcon_kw, xcon_offset + XCON_ORAT_INDEX);
      conn->volume_rate =
          ecl_kw_iget_double(xcon_kw, xcon_offset + XCON_QR_INDEX);
    }
...
well_conn_type *well_conn_alloc_wellhead(const ecl_kw_type *iwel_kw,
  ...
  if (conn_i >= 0) {
    ...
    if (matrix_connection)
      ...
      return well_conn_alloc_fracture(conn_i, conn_j, conn_k, connection_factor,
                                      (well_conn_dir_enum)open, well_conn_dirZ);
  } else
    // The well is completed in this LGR - however the wellhead is in another
    // LGR.
    return NULL;
}
