/*
   ...
   The file 'well_conn_collection.c' is part of ERT - Ensemble based Reservoir
   Tool.
...
/*
  The collection only stores a refernce to the object, which will be destroyed
  by 'someone else'.
*/
...
int well_conn_collection_load_from_kw(well_conn_collection_type *wellcc,
                                      ...
                                      const ecl_kw_type *xcon_kw, int iwell,
                                      const ecl_rsthead_type *rst_head) {

  const int iwel_offset = rst_head->niwelz * iwell;
  int num_connections =
      ecl_kw_iget_int(iwel_kw, iwel_offset + IWEL_CONNECTIONS_INDEX);
  int iconn;
