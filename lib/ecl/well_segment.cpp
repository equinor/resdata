...
struct well_segment_struct {
  ...
  int segment_id;
  int branch_id;
  int outlet_segment_id; // This is in the global index space given by the ISEG
                         // keyword.
...
well_segment_type *
well_segment_alloc_from_kw(const ecl_kw_type *iseg_kw,
                           const well_rseg_loader_type *rseg_loader,
                           const ecl_rsthead_type *header, int well_nr,
  ...
  if (rseg_loader == NULL) {
    ...
    int outlet_segment_id =
        ecl_kw_iget_int(iseg_kw, iseg_offset + ISEG_OUTLET_INDEX) -
        ECLIPSE_WELL_SEGMENT_OFFSET + WELL_SEGMENT_OFFSET; // -1
    int branch_id = ecl_kw_iget_int(iseg_kw, iseg_offset + ISEG_BRANCH_INDEX) -
...
bool well_segment_link(well_segment_type *segment,
  ...
  if (segment->outlet_segment_id == outlet_segment->segment_id) {
    ...
    /*
       This is a quite fatal topological error - and aborting is probaly the
       wisest thing to do. I.e.  the function well_segment_link_strict() is
       recommended.
    */
    return false;
}
...
bool well_segment_well_is_MSW(int well_nr, const ecl_kw_type *iwel_kw,
                              const ecl_rsthead_type *rst_head) {
  int iwel_offset = rst_head->niwelz * well_nr;
  int segment_well_nr =
      ecl_kw_iget_int(iwel_kw, iwel_offset + IWEL_SEGMENTED_WELL_NR_INDEX) - 1;
