/*
   ...
   The file 'well_segment_collection.c' is part of ERT - Ensemble based
   Reservoir Tool.
...
void well_segment_collection_free(
    well_segment_collection_type *segment_collection) {
  for (int i = 0;
       i < static_cast<int>(segment_collection->__segment_storage.size()); i++)
...
int well_segment_collection_load_from_kw(
    ...
    const ecl_kw_type *iwel_kw, const ecl_kw_type *iseg_kw,
    const well_rseg_loader_type *rseg_loader, const ecl_rsthead_type *rst_head,
    ...
    if (load_segments) {
      for (segment_index = 0; segment_index < rst_head->nsegmx;
        ...
        well_segment_type *segment = well_segment_alloc_from_kw(
            iseg_kw, rseg_loader, rst_head, segment_well_nr, segment_index,
            segment_id);
...
void well_segment_collection_link(
  ...
  for (index = 0; index < segment_collection->__segment_storage.size();
    ...
    if (!well_segment_nearest_wellhead(segment)) {
      well_segment_type *target_segment =
          well_segment_collection_get(segment_collection, outlet_segment_id);
