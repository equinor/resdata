...
reservoir. As ind             icated by the use of isolated boxes the ECLIPSE
model contains no geomtric concept linking the different connections into a
...
well_state_get_grid_connections( well_state , LGR_NAME); if (connections) {
 ...
 2. If - AND ONLY IF - the well is a multisegment well, you can query
    the well_state object for information about segments and branches:
       ...
       if (well_state_is_MSW( well_state )) {
          well_segment_collection_type * segments = well_state_get_segments(
well_state ); well_branch_collection_type * branches = well_state_get_branches(
well_state ); int branch_nr;

          for (branch_nr = 0; branch_nr < well_branch_collection_get_size(
branches ); branch_nr++) { well_segment_type * segment =
well_branch_collection_iget_start_segment( branches , branhc_nr ); while
(segment) {
...
struct well_state_struct {
  ...
  std::vector<well_conn_type *>
      index_wellhead; // An well_conn_type instance representing the wellhead -
                      // indexed by grid_nr.
  std::map<std::string, well_conn_type *>
      name_wellhead; // An well_conn_type instance representing the wellhead -
                     // indexed by lgr_name.
};
...
void well_state_add_wellhead(well_state_type *well_state,
                             ...
                             const ecl_kw_type *iwel_kw, int well_nr,
...
static bool well_state_add_rates(well_state_type *well_state,
                                 ecl_file_view_type *rst_view, int well_nr) {

  bool has_xwel_kw = ecl_file_view_has_kw(rst_view, XWEL_KW);
  if (has_xwel_kw) {
    const ecl_kw_type *xwel_kw =
        ecl_file_view_iget_named_kw(rst_view, XWEL_KW, 0);
    ...
    well_state->oil_rate =
        ecl_kw_iget_double(xwel_kw, offset + XWEL_RES_ORAT_ITEM);
    well_state->gas_rate =
        ecl_kw_iget_double(xwel_kw, offset + XWEL_RES_GRAT_ITEM);
    well_state->water_rate =
        ecl_kw_iget_double(xwel_kw, offset + XWEL_RES_WRAT_ITEM);
    well_state->volume_rate =
        ecl_kw_iget_double(xwel_kw, offset + XWEL_RESV_ITEM);
...
static int well_state_get_lgr_well_nr(const well_state_type *well_state,
  ...
  if (ecl_file_view_has_kw(file_view, ZWEL_KW)) {
    ...
    const ecl_kw_type *zwel_kw =
        ecl_file_view_iget_named_kw(file_view, ZWEL_KW, 0);
    int num_wells = header->nwells;
    well_nr = 0;
    while (true) {
      bool found = false;
      {
        char *lgr_well_name = (char *)util_alloc_strip_copy(
            (const char *)ecl_kw_iget_ptr(zwel_kw, well_nr * header->nzwelz));
...
static void well_state_add_connections__(well_state_type *well_state,
  ...
  const ecl_kw_type *iwel_kw =
      ecl_file_view_iget_named_kw(rst_view, IWEL_KW, 0);
  ...
  if (ecl_file_view_has_kw(rst_view, ICON_KW)) {
    const ecl_kw_type *icon_kw =
        ecl_file_view_iget_named_kw(rst_view, ICON_KW, 0);
...
static void
well_state_add_global_connections(well_state_type *well_state,
                                  const ecl_file_view_type *rst_view,
                                  int well_nr) {
  well_state_add_connections__(well_state, rst_view, ECL_GRID_GLOBAL_GRID, 0,
                               well_nr);
}
...
static void well_state_add_LGR_connections(well_state_type *well_state,
  ...
  for (int lgr_index = 0; lgr_index < num_lgr; lgr_index++) {
    ecl_file_view_type *lgr_view =
        ecl_file_view_add_blockview(file_view, LGR_KW, lgr_index);
    /*
...
void well_state_add_connections(
    well_state_type *well_state, const ecl_grid_type *grid,
    ecl_file_type *rst_file, // Either an open .Xnnnn file or UNRST file
                             // restricted to one report step
    int well_nr) {

  well_state_add_connections2(well_state, grid,
                              ecl_file_get_active_view(rst_file), well_nr);
}
...
bool well_state_add_MSW(well_state_type *well_state, ecl_file_type *rst_file,
                        int well_nr, bool load_segment_information) {

  return well_state_add_MSW2(well_state, ecl_file_get_active_view(rst_file),
...
bool well_state_add_MSW2(well_state_type *well_state,
  ...
  if (ecl_file_view_has_kw(rst_view, ISEG_KW)) {
    ...
    const ecl_kw_type *iseg_kw =
        ecl_file_view_iget_named_kw(rst_view, ISEG_KW, 0);
...
well_state_type *well_state_alloc_from_file(ecl_file_type *ecl_file,
                                            const ecl_grid_type *grid,
                                            int report_nr, int global_well_nr,
                                            bool load_segment_information) {
  return well_state_alloc_from_file2(ecl_file_get_active_view(ecl_file), grid,
                                     report_nr, global_well_nr,
                                     load_segment_information);
}

well_state_type *well_state_alloc_from_file2(ecl_file_view_type *file_view,
                                             const ecl_grid_type *grid,
                                             int report_nr, int global_well_nr,
                                             bool load_segment_information) {
  if (ecl_file_view_has_kw(file_view, IWEL_KW)) {
    ...
    const ecl_kw_type *global_iwel_kw =
        ecl_file_view_iget_named_kw(file_view, IWEL_KW, 0);
    const ecl_kw_type *global_zwel_kw =
        ecl_file_view_iget_named_kw(file_view, ZWEL_KW, 0);
    ...
    const int iwel_offset = global_header->niwelz * global_well_nr;
    {
      ...
      {
        int int_state =
            ecl_kw_iget_int(global_iwel_kw, iwel_offset + IWEL_STATUS_INDEX);
        if (int_state > 0)
          open = true;
        else
          open = false;
      ...
      {
        int int_type =
            ecl_kw_iget_int(global_iwel_kw, iwel_offset + IWEL_TYPE_INDEX);
        type = well_state_translate_ecl_type_int(int_type);
      }

      {
        const int zwel_offset = global_header->nzwelz * global_well_nr;
        name = (char *)util_alloc_strip_copy((const char *)ecl_kw_iget_ptr(
            global_zwel_kw,
            zwel_offset)); // Hardwired max 8 characters in Well Name
      }

      well_state = well_state_alloc(name, global_well_nr, open, type, report_nr,
                                    global_header->sim_time);
      free(name);
    ...
    /* This seems a bit weird - have come over E300 restart files without the
     * IWEL keyword. */
    return NULL;
}
