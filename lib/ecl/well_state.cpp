...
void well_state_add_connections(
    ...
    ecl_file_type *
        rst_file, // Either an open .Xnnnn file or UNRST file restricted to one report step
    int well_nr) {
...
well_state_type *well_state_alloc_from_file2(ecl_file_view_type *file_view,
    ...
    if (ecl_file_view_has_kw(file_view, IWEL_KW)) {
        ...
        const int iwel_offset = global_header->niwelz * global_well_nr;
        {
            ...
            well_type_enum type = ECL_WELL_ZERO;
            {
                int int_state = ecl_kw_iget_int(
                    global_iwel_kw, iwel_offset + IWEL_STATUS_INDEX);
                if (int_state > 0)
                    open = true;
                else
                    open = false;
            ...
            {
                int int_type = ecl_kw_iget_int(global_iwel_kw,
