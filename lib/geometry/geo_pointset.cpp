...
static void geo_pointset_resize(geo_pointset_type *pointset,
                                int new_alloc_size) {
    pointset->xcoord = (double *)util_realloc(
        pointset->xcoord, new_alloc_size * sizeof *pointset->xcoord);
    pointset->ycoord = (double *)util_realloc(
        pointset->ycoord, new_alloc_size * sizeof *pointset->ycoord);
    if (pointset->internal_z)
        pointset->zcoord = (double *)util_realloc(
            pointset->zcoord, new_alloc_size * sizeof *pointset->zcoord);
...
geo_pointset_type *geo_pointset_alloc(bool internal_z) {
    geo_pointset_type *pointset =
        (geo_pointset_type *)util_malloc(sizeof *pointset);
...
void geo_pointset_memcpy(const geo_pointset_type *src,
    ...
    if (src->internal_z != target->internal_z)
        util_abort("%s: when copying geo_poitset they must have equal value "
                   "for internal_z\n",
                   __func__);
    ...
    if (copy_zdata) {
        if (src->internal_z)
            memcpy(target->zcoord, src->zcoord,
                   src->size * sizeof *src->zcoord);
        else
            util_abort("%s: can not pass copy_zdata = true for pointset with "
                       "shared z\n",
                       __func__);
    ...
    if (pointset1->size == pointset2->size) {
        ...
        if ((memcmp(pointset1->xcoord, pointset2->xcoord, byte_size) == 0) &&
            ...
            if (pointset1->internal_z == pointset2->internal_z) {
                if (pointset1->zcoord) {
                    if (memcmp(pointset1->zcoord, pointset2->zcoord,
                               byte_size) == 0)
                        equal = true;
                } else
                    equal = true;
