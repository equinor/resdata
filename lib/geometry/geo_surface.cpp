...
static UTIL_SAFE_CAST_FUNCTION(geo_surface, GEO_SURFACE_TYPE_ID)
  ...
  for (iy = 0; iy < surface->ny; iy++) {
    for (ix = 0; ix < surface->nx; ix++) {
      double x =
          surface->origo[0] + ix * surface->vec1[0] + iy * surface->vec2[0];
      double y =
          surface->origo[1] + ix * surface->vec1[1] + iy * surface->vec2[1];
      if (zcoord != NULL) {
...
static void geo_surface_fload_irap_header(geo_surface_type *surface,
  ...
  if (fscanf(stream, "%d  %d  %lg  %lg  %lg  %lg  %lg  %lg  %d  %lg", &const996,
             &ny, &xinc, &yinc, &xstart, &xend, &ystart, &yend, &nx,
             &angle) == 10) {
    {
      // Some information is rewritten/not used.
      double d;
      int i;
      if (fscanf(stream, "%lg %lg %d %d %d %d %d %d %d ", &d, &d, &i, &i, &i,
                 &i, &i, &i, &i) != 9)
...
static bool geo_surface_fload_irap(geo_surface_type *surface,
  ...
  bool read_ok = true;
  {
    ...
    geo_surface_fload_irap_header(surface, stream);
    {
      ...
      if (loadz) {
        zcoord =
            (double *)util_calloc(surface->nx * surface->ny, sizeof *zcoord);
...
bool geo_surface_equal_header(const geo_surface_type *surface1,
  ...
  equal = equal &&
          (util_double_approx_equal(surface1->rot_angle, surface2->rot_angle));

  {
    ...
    for (i = 0; i < 2; i++) {
      ...
      equal = equal &&
              (util_double_approx_equal(surface1->vec1[i], surface2->vec1[i]));
      equal = equal &&
              (util_double_approx_equal(surface1->vec2[i], surface2->vec2[i]));
    }
  }
