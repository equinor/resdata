...
typedef enum {
  ECL_FILE_CLOSE_STREAM =
      1, /*
            This flag will close the underlying FILE object between each access;
            this is mainly to save filedescriptors in cases where many ecl_file
            instances are open at the same time. */
  //
  ECL_FILE_WRITABLE =
      2 /*
           This flag opens the file in a mode where it can be updated and
           modified, but it must still exist and be readable. I.e. this should
           not compared with the normal: fopen(filename , "w") where an existing
           file is truncated to zero upon successfull open.
        */
} ecl_file_flag_type;
...
void ecl_file_view_fprintf_kw_list(const ecl_file_view_type *ecl_file_view,
                                   FILE *stream);
ecl_file_view_type *
ecl_file_view_add_blockview(ecl_file_view_type *ecl_file_view,
...
ecl_file_view_add_blockview2(ecl_file_view_type *ecl_file_view,
                             ...
                             int occurence);
ecl_file_view_type *
ecl_file_view_add_restart_view(ecl_file_view_type *file_view, int seqnum_index,
...
void ecl_file_view_add_child(ecl_file_view_type *parent,
