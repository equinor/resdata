...
typedef enum { ECL_KW_READ_OK = 0, ECL_KW_READ_FAIL = 1 } ecl_read_status_enum;
...
int ecl_kw_first_different(const ecl_kw_type *kw1, const ecl_kw_type *kw2,
...
void ecl_kw_fread_indexed_data(fortio_type *fortio, offset_type data_offset,
                               ecl_data_type, int element_count,
...
void ecl_kw_fwrite_param(const char *, bool, const char *, ecl_data_type, int,
                         void *);
void ecl_kw_fwrite_param_fortio(fortio_type *, const char *, ecl_data_type, int,
                                void *);
...
#define ECL_KW_SHIFT_TYPED_HEADER(ctype)                                       \
    void ecl_kw_shift_##ctype(ecl_kw_type *ecl_kw, ctype shift_factor)
...
#define ECL_KW_IGET_TYPED_HEADER(type)                                         \
    type ecl_kw_iget_##type(const ecl_kw_type *, int)
...
#define ECL_KW_ISET_TYPED_HEADER(type)                                         \
    void ecl_kw_iset_##type(ecl_kw_type *, int, type)
...
#define ECL_KW_GET_TYPED_PTR_HEADER(type)                                      \
    type *ecl_kw_get_##type##_ptr(const ecl_kw_type *)
