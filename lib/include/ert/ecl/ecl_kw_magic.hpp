...
TABDIMS_NRPVTO_ITEM, TABDIMS_NPPVTO_ITEM and TABDIMS_NTPVTO_ITEM. Observe the
following:
...
Three columns -
-----------                                     |                           |
(not in TABDIMS) 25.00000                                       | | 50.00000 | |
 75.00000                                       |                           |
100.00000                                       |                           |
   *                                            |                           |
-----------                                     |                           |
...
/*-----------------------------------------------------------------*/
#define LOGIHEAD_REV_RELPERM100_INDEX                                          \
  3 /* The indices for reversible relperm and */
#define LOGIHEAD_RADIAL100_INDEX                                               \
  4 /* use of radial grids is interchanged between */
...
#define LOGIHEAD_CONSTANT_OILCOMPR_INDEX                                       \
  (39 - 1) /* Constant oil compressibility (PVCDO)? */
...
#define SEQNUM_KW                                                              \
  "SEQNUM" /* Contains the report step as the only data; not                   \
...
#define FLRWATLL_KW                                                            \
  "FLRWATA+" // nnc wtr flux, grid1 != 0, grid2 != 0, grid1 != grid2
#define FLROILLL_KW                                                            \
  "FLROILA+" // nnc oil flux, grid1 != 0, grid2 != 0, grid1 != grid2
#define FLRGASLL_KW                                                            \
  "FLRGASA+" // nnc gas flux, grid1 != 0, grid2 != 0, grid1 != grid2

#define INTEHEAD_NWELLS_INDEX 16 // Number of wells
#define INTEHEAD_NIWELZ_INDEX                                                  \
  24 // Number of elements pr. well in the IWEL array.
#define INTEHEAD_NXWELZ_INDEX                                                  \
  26 // Number of elements pr. well in the XWEL array.
...
#define INTEHEAD_NICONZ_INDEX                                                  \
  32 // Number of elements pr completion in the ICON array.
#define INTEHEAD_NSCONZ_INDEX                                                  \
  33 // Number of elements pr completion in the SCON array
#define INTEHEAD_NXCONZ_INDEX                                                  \
  34 // Number of elements pr completion in the XCON array
#define INTEHEAD_NIGRPZ_INDEX                                                  \
  36 // Number of elements pr group in the IGRP array.
...
#define STARTDAT_KW "STARTDAT" /* Intgere keyword containing day,month,year.   \
                                */
#define WGNAMES_KW                                                             \
  "WGNAMES" /* The names of wells/groups for the summary vectors. */
#define NAMES_KW "NAMES" /* Alias for WGNAMES_KW. */
#define KEYWORDS_KW                                                            \
  "KEYWORDS" /* The variable type for the various summary vectors. */
#define UNITS_KW "UNITS" /* The units, i.e SM^3/DAY the summary vectors. */
#define DIMENS_KW                                                              \
  "DIMENS" /* The dimensions of the grid - also used in the GRID files. */
#define NUMS_KW                                                                \
  "NUMS" /* Extra numeric qualifiers for the summary variables, like cell      \
            number. */
#define LGRS_KW "LGRS" /* The lgr name for a vector originating from an lgr.   \
                        */
#define NUMLX_KW                                                               \
  "NUMLX" /* For block variables defined in a an lgr this is i coordinate in   \
             the lgr. */
...
/* Summary data files: */
#define SEQHDR_KW                                                              \
  "SEQHDR" // Contains a single 'magic' integer - not used in libecl.
...
/* Common keywords */
#define TIME_KW                                                                \
  "TIME"               /* The days since simulation start when                 \
...
#define WELLETC_KW                                                             \
  "WELLETC" /* The type of date RFT|PLT|SEGMENT and well name are              \
...
#define CONOTUB_KW                                                             \
  "CONOTUB" /* Volumetric oil flow at tubing head conditions. */
#define CONGTUB_KW                                                             \
  "CONGTUB" /* Volumetric gas flow at tubing head conditions. */
#define CONWTUB_KW                                                             \
  "CONWTUB" /* Volumetric water flow at tubing head conditions. */

#define WELLETC_TYPE_INDEX                                                     \
  5 /* At this keyword the WELLETC keyword contains a string                   \
       containing 'R', 'P' , or 'S' for RFT, PLT or SEGMENT data               \
       respectively.*/
#define WELLETC_NAME_INDEX                                                     \
  1 /* The name of well being investigated is on this index of                 \
       the WELLETC keyword. */
...
#define MAPAXES_KW                                                             \
  "MAPAXES" /* Keyword used to transform from grid coordinates to              \
               world coordinates. */
#define LGR_KW                                                                 \
  "LGR" /* Name of LGR; for GRID files it can contain two elements,            \
...
#define NNC2_KW "NNC2" /*Downstream cell numbers for non-neighbour             \
                          connections*/
#define NNCL_KW                                                                \
  "NNCL" /*Cell numbers for LGR cells that are connected to global grid        \
            cells*/
#define NNCG_KW "NNCG" /*Cell numbers for global cells connected to LGR        \
                          cells*/

#define NNCHEAD_KW "NNCHEAD" /*Non-neighbour connection header*/
#define NNCHEAD_SIZE 10
#define NNCHEAD_NUMNNC_INDEX                                                   \
  0 /*Item 1 in non-neighbour connection header: number of NNCs. Only present  \
       for main grid*/
#define NNCHEAD_LGR_INDEX                                                      \
  1 /*Item 2 in non-neighbour connection header: LGR number (0 for global      \
       grid)*/

#define NNCHEADA_KW "NNCHEADA" /*Header for NNC's between two amalgamated      \
                                  LGRs*/
...
#define COORDS_KW                                                              \
  "COORDS" /* The (x,y) coordinates of the top and bottom of the pillars       \
              constituting the grid. */
#define ZCORN_KW "ZCORN"   /* Z coordinate where pillars cross planes. */
#define ACTNUM_KW "ACTNUM" /* Integer flag of with active=0,1. */
#define HOSTNUM_KW                                                             \
  "HOSTNUM" /* For cells in LGR - pointing back to cell nr in                  \
               parent grid. */
...
#define CORNERS_KW                                                             \
  "CORNERS" /* Vector containing (x,y,z) x 8 elements - all corners in a cell. \
             */
