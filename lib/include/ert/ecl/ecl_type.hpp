...
#define ECL_INT                                                                \
    ecl_data_type { ECL_INT_TYPE, sizeof(int) }
#define ECL_FLOAT                                                              \
    ecl_data_type { ECL_FLOAT_TYPE, sizeof(float) }
#define ECL_DOUBLE                                                             \
    ecl_data_type { ECL_DOUBLE_TYPE, sizeof(double) }
#define ECL_BOOL                                                               \
    ecl_data_type { ECL_BOOL_TYPE, sizeof(bool) }
#define ECL_CHAR                                                               \
    ecl_data_type { ECL_CHAR_TYPE, ECL_STRING8_LENGTH + 1 }
#define ECL_MESS                                                               \
    ecl_data_type { ECL_MESS_TYPE, 0 }
#define ECL_STRING(size)                                                       \
    ecl_data_type { ECL_STRING_TYPE, (size) + 1 }
}
...
#define ECL_CHAR                                                               \
    (ecl_data_type) {                                                          \
        .type = ECL_CHAR_TYPE, .element_size = ECL_STRING8_LENGTH + 1          \
    }
#define ECL_INT                                                                \
    (ecl_data_type) { .type = ECL_INT_TYPE, .element_size = sizeof(int) }
#define ECL_FLOAT                                                              \
    (ecl_data_type) { .type = ECL_FLOAT_TYPE, .element_size = sizeof(float) }
#define ECL_DOUBLE                                                             \
    (ecl_data_type) { .type = ECL_DOUBLE_TYPE, .element_size = sizeof(double) }
#define ECL_BOOL                                                               \
    (ecl_data_type) { .type = ECL_BOOL_TYPE, .element_size = sizeof(bool) }
#define ECL_MESS                                                               \
    (ecl_data_type) { .type = ECL_MESS_TYPE, .element_size = 0 }
#define ECL_STRING(size)                                                       \
    (ecl_data_type) { .type = ECL_STRING_TYPE, .element_size = (size) + 1 }
