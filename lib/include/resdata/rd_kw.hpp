...
public:
    ...
    /* RD_BOOL_TYPE is stored as std::vector<char> (0/1 values), so
       mutating an element in place through a reference is not possible.
       Use set_bool() to assign a RD_BOOL_TYPE element instead. */
    void set_bool(size_t index, bool value);
    ...
    void set_string_array(size_t index, const std::string &s) {
        ...
        size_t chunks = std::max<size_t>(1, (s.size() + len - 1) / len);
    ...
    /* Reads a selection of elements (given by @index_map) from the data
       section of a single keyword. The @kw_offset argument is the byte offset of
...
/* RD_BOOL_TYPE is stored as std::vector<char> (0/1 values) in. There is
   no mutating `bool &KW::at<bool>()`; Use KW::set_bool() to
   mutate a RD_BOOL_TYPE element instead. */
template <> bool KW::at<bool>(size_t index) const;

template <> inline bool &KW::at<bool>(size_t index) = delete;
...
inline std::string format_kw_element_fmt(size_t width) {
    return " '%-" + std::to_string(width) + "." + std::to_string(width) + "s'";
}
