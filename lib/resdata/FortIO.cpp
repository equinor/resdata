...
int FortIO::fclean() {
    ...
    m_stream.clear();
    return fseek_(current_pos, SEEK_SET) ? 0 : -1;
}
...
/** To preserve FILE* semantics we operate on rdbuf*/
std::streampos FortIO::seek_buffer(offset_type offset,
                                   std::ios_base::seekdir dir) const {
    return m_stream.rdbuf()->pubseekoff(
        offset, dir, m_open_mode & (std::ios_base::in | std::ios_base::out));
}

offset_type FortIO::ftell() const { return seek_buffer(0, std::ios_base::cur); }
...
bool FortIO::fseek_(offset_type offset, int whence) {
    ...
    m_stream.clear();
    if (seek_buffer(offset, dir) == std::streampos(-1)) {
...
void FortIO::rewind() const {
    m_stream.clear();
    seek_buffer(0, std::ios_base::beg);
}
