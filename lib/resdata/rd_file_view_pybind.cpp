...
PYBIND11_MODULE(_rd_file_view, m) {
    ...
    m.def("_create_block_view",
          [](py::handle self, std::string kw, int index) {
              return create_view_reference(
                  rd_file_view_add_blockview(
                      from_cwrap<rd_file_view_type>(self), kw.c_str(), index),
                  self);
          });
    ...
    m.def("_restart_view", [](py::handle self, int seqnum_index,
                              int report_step, std::int64_t sim_time,
                              double sim_days) {
        return create_view_reference(
            rd_file_view_add_restart_view(from_cwrap<rd_file_view_type>(self),
                                          seqnum_index, report_step,
