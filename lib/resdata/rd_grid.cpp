...
static rd_grid_type *rd_grid_alloc_empty(rd_grid_type *global_grid,
    ...
    grid->ny = ny;
    grid->nz = nz;
    grid->size = static_cast<size_t>(nx) * ny * nz;
    grid->lgr_nr = lgr_nr;
...
static void rd_grid_init_nnc_cells(rd_grid_type *grid1, rd_grid_type *grid2,
    ...
    const auto &grid1_nnc_cells = keyword1->get_vector<int>();
    const auto &grid2_nnc_cells = keyword2->get_vector<int>();
    size_t nnc_count = keyword2->size();
    ...
    for (size_t nnc_index = 0; nnc_index < nnc_count; nnc_index++) {
        ...
        */
        if ((FILEHEAD_SINGLE_POROSITY != grid1->dualp_flag) &&
            ((grid1_cell_index >= static_cast<int>(grid1->size)) ||
             (grid2_cell_index >= static_cast<int>(grid2->size))))
            break;
        ...
        nnc_info_add_nnc(grid1_cell.nnc_info.get(), grid2->lgr_nr,
                         grid2_cell_index, static_cast<int>(nnc_index));
    }
}
    ...
    for (size_t g = 0; g < g1->size; g++) {
        ...
        if (!this_equal) {
            if (verbose) {
                ...
                printf("Difference in cell: %zu : %d,%d,%d  nnc_equal:%d "
                       "Volume:%g \n",
                       g, i, j, k,
...
*/
static bool rd_grid_cell_contains_xyz3(const rd_grid_type *rd_grid, int i,
...
int rd_grid_get_global_index_from_xyz(rd_grid_type *grid, double x, double y,
    ...
    for (int index = 0; index < rd_grid_get_global_size(grid); index++) {
...
static void rd_grid_get_cell_corner_xyz3(const rd_grid_type *grid, int i, int j,
    ...
    if (rd_type_is_numeric(data_type)) {
        ...
        if (kw_size == rd_grid->nx * rd_grid->ny * rd_grid->nz)
            ...
            throw std::invalid_argument(fmt::format(
                ...
                "kw_size = {}",
                rd_grid->size, rd_grid->total_active, rd_kw->size()));
...
static bool rd_grid_test_lgr_consistency2(const rd_grid_type *parent,
    ...
    for (const auto &child_cell : child->cells) {
        int parent_cell = child_cell.host_cell;
        if (parent_cell >= 0) {
...
void rd_grid_compressed_kw_copy(const rd_grid_type *grid, rd::KW *target_kw,
    ...
    if ((rd::kw_get_size(target_kw) == rd_grid_get_nactive(grid)) &&
        ...
        throw std::invalid_argument(fmt::format(
            ...
            "expected {},{}",
            target_kw->size(), src_kw->size(), rd_grid_get_nactive(grid),
...
void rd_grid_global_kw_copy(const rd_grid_type *grid, rd::KW *target_kw,
    ...
    if ((rd::kw_get_size(src_kw) == rd_grid_get_nactive(grid)) &&
        ...
        throw std::invalid_argument(fmt::format(
            ...
            "expected {},{}",
            target_kw->size(), src_kw->size(), rd_grid_get_global_size(grid),
...
rd_grid_alloc_hostnum_kw(const rd_grid_type *grid) {
    ...
    std::vector<int> hostnum(grid->size);
    for (size_t i = 0; i < grid->size; i++)
        hostnum[i] = grid->cells.at(i).host_cell + 1;
    return std::make_unique<rd::KW>(HOSTNUM_KW, hostnum);
}
...
rd_grid_alloc_corsnum_kw(const rd_grid_type *grid) {
    ...
    std::vector<int> corsnum(grid->size);
    for (size_t i = 0; i < grid->size; i++)
        corsnum[i] = grid->cells.at(i).coarse_group + 1;
    return std::make_unique<rd::KW>(CORSNUM_KW, corsnum);
}
...
rd_grid_alloc_volume_kw_active(const rd_grid_type *grid) {
    int size = rd_grid_get_active_size(grid);
    std::vector<double> volume(size);
    for (int active_index = 0; active_index < size; active_index++)
        volume[active_index] = rd_grid_get_cell_volume1A(grid, active_index);
    return std::make_unique<rd::KW>("VOLUME", volume);
}

static std::unique_ptr<rd::KW>
rd_grid_alloc_volume_kw_global(const rd_grid_type *grid) {
    int size = rd_grid_get_global_size(grid);
    std::vector<double> volume(size);
    for (int global_index = 0; global_index < size; global_index++)
        volume[global_index] = rd_grid_get_cell_volume1(grid, global_index);
    return std::make_unique<rd::KW>("VOLUME", volume);
}
