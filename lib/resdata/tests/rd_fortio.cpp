...
void test_fortio_is_instance(const char *filename) {
    {
        fortio_type *fortio =
            fortio_open_reader(filename, false, RD_ENDIAN_FLIP);
        test_assert_not_NULL(fortio);
...
void test_fortio_safe_cast(const char *filename) {
    void *i_am_a_fortio = fortio_open_reader(filename, false, RD_ENDIAN_FLIP);
...
int main(int argc, char **argv) {
    util_install_signals();
    {
        ...
        test_assert_util_abort("fortio_safe_cast", test_fortio_unsafe_cast,
                               NULL);
