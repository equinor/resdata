...
void test_create2() {
    ...
    {
        rd_grid_type *grid =
            rd_grid_alloc_dx_dy_dz_tops(nx, ny, nz, dx, dy, dz, tops, NULL);

        test_assert_int_equal(nx * ny * nz, rd_grid_get_global_size(grid));
        test_assert_int_equal(nx * ny * nz, rd_grid_get_active_size(grid));
        ...
        for (int k = 0; k < nz; k++) {
            for (int j = 0; j < ny; j++) {
                for (int i = 0; i < nx; i++) {
                    ...
                    test_assert_double_equal(rd_grid_get_cell_volume1(grid, g),
                                             dx[g] * dy[g] * dz[g]);
                }
            }
        }
