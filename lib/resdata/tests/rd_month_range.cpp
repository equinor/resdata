...
void test_body(time_t_vector_type *date_list, int offset) {
    ...
    for (i = offset; i < (time_t_vector_size(date_list) - 1); i++) {
        ...
        test_assert_time_t_equal(current_date,
                                 util_make_pure_date_utc(current_date));
    }
}

void test_append(const char *name, time_t_vector_type *date_list, time_t date1,
                 time_t date2, bool force_append_end) {
    int offset = time_t_vector_size(date_list);

    printf("%s ...", name);
    fflush(stdout);

    rd_append_month_range(date_list, date1, date2, force_append_end);

    // The start:
    test_assert_true(util_make_pure_date_utc(date1) <=
                     time_t_vector_iget(date_list, offset));
    ...
    // The tail:
    if (force_append_end)
        test_assert_time_t_equal(time_t_vector_get_last(date_list),
                                 util_make_pure_date_utc(date2));
    else
        test_assert_true(
            util_is_first_day_in_month_utc(time_t_vector_get_last(date_list)));

    printf(" OK \n");
}

void test_init(const char *name, time_t_vector_type *date_list,
               time_t start_date, time_t end_date) {
    printf("%s ...", name);
    fflush(stdout);
    {
        rd_init_month_range(date_list, start_date, end_date);
        test_assert_time_t_equal(time_t_vector_get_first(date_list),
                                 util_make_pure_date_utc(start_date));
        test_body(date_list, 1);
        test_assert_time_t_equal(time_t_vector_get_last(date_list),
                                 util_make_pure_date_utc(end_date));
    }
    printf(" OK \n");
}
