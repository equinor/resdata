...
TEST_CASE_METHOD(Tmpdir, "fread_alloc rejects a truncated quoted string",
                 "[rd_kw]") {
    auto good = (dirname / "GOOD.txt").string();
    {
        rd::KW kw{"KW", {"FOPRTEST", "BPR"}};
        ERT::FortIO fortio(good, std::ios_base::out, /*fmt_file=*/true);
        kw.fwrite(fortio);
    }
    ...
    auto bad = (dirname / "BAD.txt").string();
    {
        // Drop the closing quote and trailing newline
    ...
    REQUIRE_THROWS_WITH(
        rd::KW::fread(fortio),
        ContainsSubstring("reading 'xxxxxxxx' formatted string failed"));
}
