...
TEST_CASE("rd_type invalid input throws", "[unittest]") {
    ...
    SECTION("mismatched element_size throws invalid_argument") {
        REQUIRE_THROWS_AS(rd_type_create(RD_INT_TYPE, sizeof(int) + 1),
                          std::invalid_argument);
        REQUIRE_THROWS_WITH(
            rd_type_create(RD_INT_TYPE, sizeof(int) + 1),
            Catch::Matchers::ContainsSubstring("element_size mismatch"));
    }
}
