...
struct buffer_struct {
    ...
    size_t
        content_size; /* The extent of initialized data in the buffer - i.e. the meaningful content in the buffer. */
...
void buffer_fwrite_char_ptr(buffer_type *buffer, const char *string_ptr) {
    buffer_fwrite(buffer, string_ptr, sizeof *string_ptr,
                  strlen(string_ptr) + 1);
}
...

    {
        ...
        if (shift < 0)
            if (labs(shift) > offset)
                offset = labs(
                    shift); /* We are 'trying' to left shift beyond the start of the buffer. */

        move_size = buffer->content_size - offset;
        memmove(&buffer->data[offset + shift], &buffer->data[offset],
                move_size);
...
void buffer_replace_string(buffer_type *buffer, size_t offset, size_t old_size,
    ...
    buffer_replace_data(buffer, offset, old_size, new_string,
                        strlen(new_string));
}
...
bool buffer_strstr(buffer_type *buffer, const char *expr) {
    ...
    if (strlen(expr) > 0) {
        const char *match_ptr =
            memcmp_strstr(&buffer->data[buffer->pos],
                          buffer->content_size - buffer->pos, expr);
        if (match_ptr) {
...
size_t buffer_stream_fwrite_n(const buffer_type *buffer, size_t offset,
                   ...
                   offset, offset);
    {
        ...
        if (len < 0)
            util_abort(
                "%s: invalid length spesifier - tried to write %ld bytes \n",
                __func__, len);
...
size_t buffer_fwrite_compressed(buffer_type *buffer, const void *ptr,
    ...
    buffer->content_size =
        buffer
            ->pos; /* Invalidating possible buffer content coming after the compressed content; that is uninterpretable anyway. */

    if (byte_size > 0) {
        size_t remaining_size = buffer->alloc_size - buffer->pos;
        size_t compress_bound = __compress_bound(byte_size);
        if (compress_bound > remaining_size)
            buffer_resize__(buffer, remaining_size + compress_bound,
                            abort_on_error);
