...
__hash_alloc_ordered_keylist(const hash_type *hash,
    ...
    qsort(sort_list, hash_get_size(hash), sizeof *sort_list,
          &hash_sortlist_cmp);
    sorted_keylist =
        (char **)calloc(hash_get_size(hash), sizeof *sorted_keylist);
    for (i = 0; i < hash_get_size(hash); i++) {
...
hash_type *hash_alloc_from_options(const stringlist_type *options) {
    ...
    for (iopt = 0; iopt < num_options; iopt++) {
        ...
        util_binary_split_string(stringlist_iget(options, iopt), ":", true,
                                 &option, &value);
...
struct hash_iter_struct {
    ...
    char **
        keylist; /* The keys in the hash table - at the moment of hash_iter_alloc(). */
    int num_keys;
