...
struct hash_struct {
  ...
  uint32_t size;     /* This is the size of the internal table **NOT**NOT** the
...
static void *__hash_get_node(const hash_type *__hash, const char *key,
  ...
  hash_node_type *node = NULL;
  {
    ...
    if (node == NULL && abort_on_error)
      util_abort(
          "%s: tried to get from key:%s which does not exist - aborting \n",
          __func__, key);
  }
  return node;
}
...
static void __hash_insert_node(hash_type *hash, hash_node_type *node) {
  uint32_t table_index = hash_node_get_table_index(node);
  {
    /*
      ...
      it is removed.
    */
    hash_node_type *existing_node =
        (hash_node_type *)__hash_get_node(hash, hash_node_get_key(node), false);
...
static void hash_del__(hash_type *hash, const char *key) {
  ...
  const uint32_t table_index = (global_index % hash->size);
  hash_node_type *node =
      hash_sll_get(hash->table[table_index], global_index, key);
...
void *hash_safe_get(const hash_type *hash, const char *key) {
  hash_node_type *hash_node =
      (hash_node_type *)__hash_get_node(hash, key, false);
  if (hash_node != NULL) {
...
void hash_free__(void *void_hash) { hash_free(hash_safe_cast(void_hash)); }
...
void hash_insert_copy(hash_type *hash, const char *key, const void *value,
                      copyc_ftype *copyc, free_ftype *del) {
  hash_node_type *hash_node;
  if (copyc == NULL || del == NULL)
    util_abort("%s: must provide copy constructer and delete operator for "
               "insert copy - aborting \n",
               __func__);
  {
...
static hash_sort_type *hash_alloc_sort_list(const hash_type *hash,
  ...
  int i;
  hash_sort_type *sort_list =
      (hash_sort_type *)calloc(hash_get_size(hash), sizeof *sort_list);
  for (i = 0; i < hash_get_size(hash); i++)
...
static char **
__hash_alloc_ordered_keylist(const hash_type *hash,
                             int(hash_get_cmp_value)(const void *)) {
  int i;
  char **sorted_keylist;
  char **tmp_keylist = hash_alloc_keylist(hash);
  hash_sort_type *sort_list =
      hash_alloc_sort_list(hash, (const char **)tmp_keylist);

  for (i = 0; i < hash_get_size(hash); i++)
    sort_list[i].cmp_value =
        hash_get_cmp_value(hash_get(hash, sort_list[i].key));
...
static char **__hash_alloc_key_sorted_list(const hash_type *hash,
                                           int (*cmp)(const void *,
                                                      const void *)) {
...
hash_type *hash_alloc_from_options(const stringlist_type *options) {
  ...
  for (iopt = 0; iopt < num_options; iopt++) {
    ...
    util_binary_split_string(stringlist_iget(options, iopt), ":", true, &option,
                             &value);
    if ((option != NULL) && (value != NULL))
      hash_insert_hash_owned_ref(opt_hash, option,
...
struct hash_iter_struct {
  ...
  char **keylist;        /* The keys in the hash table - at the moment of
                            hash_iter_alloc(). */
  int num_keys;
  int current_key_num; /* This integer retains the state. */
};

void hash_iter_restart(hash_iter_type *iter) { iter->current_key_num = 0; }
...
const char *hash_iter_get_next_key(hash_iter_type *iter) {
  ...
  if (!hash_has_key(iter->hash, key))
    util_abort(
        "%s: Programming error. Using hash_iter with multi-threading??\n",
        __func__);
...
/**
   ...
   function func on each of the elements in the table inplace:
       ...
       func( value );                      <-- The call is inplace - with no
   arguments,
       ....                                    and no return value. The content
   of 'value' can obviously change, but 'value' itself must still be a valid
   reference!
*/
