...
static node_data_type *node_data_copyc(const node_data_type *src,
  ...
  if (src->buffer_size > 0) {
    /*
       The source node has internal storage - it has been allocated with
       _alloc_buffer()
    */
    if (deep_copy)
      ...
      next = node_data_alloc__(src->data, src->ctype, src->buffer_size, NULL,
                               NULL); /* The copy does not have destructor. */
  } else {
    if (deep_copy) {
      if (src->copyc == NULL)
        util_abort("%s: Tried allocate deep_copy of mnode with no constructor "
                   "- aborting. \n",
                   __func__);
