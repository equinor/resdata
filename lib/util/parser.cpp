...
struct basic_parser_struct {
  char *splitters; /* The string is split into tokens on the occurence of one of
                      these characters - and they are removed. */
  char *specials;  /* This exactly like the splitters - but these characters are
                      retained as tokens. */
  char *delete_set; /* The chracters are just plain removed - but without any
                       splitting on them. */
  char *quoters;
  char *comment_start;
  char *comment_end;
...
static void __verify_string_length(const char *s) {
  if ((s != NULL) && (strlen(s) == 0))
    util_abort(
        "%s: invalid input to parser_set_xxx function - zero length string.\n",
        __func__);
}
...
static int length_of_quotation(const char *buffer) {
  assert(buffer != NULL);
  {
    ...
    if (current == '\0') /* We ran through the whole string without finding the
                            end of the quotation - abort HARD. */
...
static int length_of_comment(const char *buffer_position,
  ...
  if (in_comment) {
    ...
    while (buffer_position[length] != '\0' && in_comment) {
      if (strncmp(&buffer_position[length], comment_end, len_comment_end) ==
          0) {
        in_comment = false;
  ...
  while (position < buffer_size) {
    ...
    */
    if (is_in_quoters(buffer[position], parser)) {
      ...
      char *token =
          alloc_quoted_token(&buffer[position], length, strip_quote_marks);
...
bool basic_parser_fseek_string(const basic_parser_type *parser, FILE *stream,
    ...
    util_strupr(string);
  {
    ...
    if (strstr(string, parser->comment_start) != NULL)
      util_abort("%s: sorry the string contains a comment start - will never "
                 "find it ... \n",
    ...
    do {
      ...
      /* Special treatment of quoters - does not properly handle escaping of the
       * quoters. */
      if (is_in_quoters(c, parser)) {
        ...
        if (!fseek_quote_end(c, stream)) {
          ...
          fprintf(stderr,
                  "Warning: unterminated quotation starting at line: %d \n",
        ...
        if (comment_start) {
          ...
          if (!util_fseek_string(stream, parser->comment_end, true, true)) {
            ...
            */
            ...
            fprintf(stderr,
                    "Warning: unterminated comment starting at line: %d \n",
                    util_get_current_linenr(stream));
            util_fseek(stream, 0, SEEK_END);
          }
          continue;
...
/**
   This function takes an input buffer, and updates the buffer (in place)
   according to:
...
char *basic_parser_fread_alloc_file_content(const char *filename,
  ...
  basic_parser_type *parser = basic_parser_alloc(
      NULL, quote_set, NULL, delete_set, comment_start, comment_end);
  char *buffer = util_fread_alloc_file_content(filename, NULL);
