...
struct rng_struct {
  ...
  rng_forward_ftype
      *forward; /* Brings the rng forward - returning a random unsigned int
                   value.  This is the fundamental source of random numbers, and
                   all other random numbers are derived from this through
                   scaling/shifting/type conversion/... */
  rng_set_state_ftype *set_state; /* Takes a char * buffer as input and sets the
                                     state of the rng; should set the rng into a
                                     default state if arg == NULL. */
  rng_get_state_ftype *get_state;
  rng_alloc_ftype *alloc_state; /* Creates a new instance of this rng. */
  rng_free_ftype *free_state;
  rng_fscanf_ftype *fscanf_state; /* Loads the state from a formatted file with
                                     (integer representation of) bytes. */
  rng_fprintf_ftype
      *fprintf_state; /* Writes the state as a formatted series of bytes. */
  /******************************************************************/
  rng_alg_type type;
  void *state;    /* The current state - the return value from alloc_state() -
                     passed as parameter to all the function pointers. */
...
rng_type *rng_alloc(rng_alg_type type, rng_init_mode init_mode) {
  ...
  switch (type) {
  case (MZRAN):
    rng = rng_alloc__(mzran_alloc, mzran_free, mzran_forward, mzran_set_state,
                      mzran_get_state, mzran_fscanf_state, mzran_fprintf_state,
                      type, MZRAN_STATE_SIZE, MZRAN_MAX_VALUE);
    break;
  default:
