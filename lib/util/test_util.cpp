...
void test_assert_bool_not_equal__(bool b1, bool b2, const char *file,
                                  int line) {
  if (b1 == b2)
...
void test_assert_mem_equal__(const void *p1, const void *p2, size_t byte_size,
  ...
  if (memcmp(p1, p2, byte_size) != 0) {
    ...
    test_error_exit("%s:%d => Memory regions have different content. First "
                    "difference at offset:%ld\n",
                    file, line, offset);
  }
}
...
void test_assert_double_equal__(double d1, double d2, const char *file,
  ...
  if (!test_check_double_equal(d1, d2))
    test_error_exit(
        "%s:%d => double values:%g %g are not sufficiently similar\n", file,
        line, d1, d2);
}
...
void test_assert_float_equal__(float d1, float d2, const char *file, int line) {
  if (!test_check_float_equal(d1, d2))
    test_error_exit(
        "%s:%d => float values:%g %g are not sufficiently similar\n", file,
        line, d1, d2);
}
...
void test_util_addr2line() {
  ...
  test_assert_true(
      util_addr2line_lookup(bt_addr[0], &func_name, &file_name, &line_nr));
...
bool util_addr2line_lookup(const void *bt_addr, char **func_name,
                           char **file_name, int *line_nr) {
  return false;
}

jmp_buf *util_abort_test_jump_buffer() { return NULL; }
