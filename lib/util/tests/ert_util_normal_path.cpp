/*
   ...
   The file 'ert_util_normal_path.c' is part of ERT - Ensemble based Reservoir
   Tool.
...
void test_beyond_root() {
  ...
  test_path(backref_cwd2, cwd); // The input is a semi-absolute path, and we
                                // compare with the absolute path cwd().
  free(backref_cwd1);
  free(backref_cwd2);
  free(cwd);
}
