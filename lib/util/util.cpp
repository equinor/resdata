...
void util_endian_flip_vector(void *data, int element_size, int elements) {
  ...
  case (4): {
    ...
    */
    ...
    if (elements & 1) {
      // Odd number of elements - flip the last element as an ordinary 32 bit
      // swap.
...
bool util_fseek_string(FILE *stream, const char *__string, bool skip_string,
    ...
    util_strupr(string);
  {
    ...
    do {
      ...
      if (c == string[0]) { /* OK - we got the first character right - lets try
                               in more detail: */
    ...
    if (string_found) {
      ...
      util_fseek(stream, initial_pos,
                 SEEK_SET); /* Could not find the string reposition at initial
                               position. */
  }
  free(string);
  return string_found;
...
bool util_is_cwd(const char *path) {
  ...
  if (util_stat(path, &path_stat) == 0) {
    if (S_ISDIR(path_stat.st_mode)) {
      ...
      */
      util_abort(
          "%s: Internal error - function not properly implmented on Windows \n",
          __func__);
#else
      stat_type cwd_stat;
...
static char *util_alloc_cwd_abs_path(const char *path) {
  if (util_is_abs_path(path))
    ...
    char *cwd = util_alloc_cwd();
    char *abs_path =
        (char *)util_realloc(cwd, strlen(cwd) + 1 + strlen(path) + 1);
...
char *util_alloc_realpath(const char *input_path) {
  ...
  ERROR - Have symlinks and not realpath()
      ?
#endif
      if (!util_entry_exists(input_path)) util_abort(
          "%s: input_path:%s does not exist - failed.\n", __func__, input_path);
...
/**
   ...
   In the manual realpath() neither "/../" nor symlinks are resolved. If path ==
   NULL the function will return cwd().
*/
...
char *util_alloc_rel_path(const char *__root_path, const char *path) {
  ...
  if (util_is_abs_path(root_path) && util_is_abs_path(path)) {
    ...
    char *rel_path =
        util_alloc_string_copy(""); // In case strcmp(root_path , path) == 0 the
...

    {
      ...
      int common_length = 0;
      while (true) {
        if (strcmp(root_path_list[common_length], path_list[common_length]) ==
            0)
          common_length++;
        else
          break;
...
static int util_strcmp_numeric__(const char *s1, const char *s2,
                                 bool float_mode) {
  /*
  ...
  */
  {
    ...
    if ((*end2 == '\0') && (*end1 == '\0')) {
      /* The whole string has been read and converted to a number - for both
       * strings. */
      if (num1 < num2)
        return -1;
      else if (num1 > num2)
    ...
    while (!complete) {
      ...
      if (!complete) {
        ...
        */
        ...
        if (isnumeric(s1[offset1], float_mode) &&
            isnumeric(s2[offset2], float_mode)) {
          char *end1;
          char *end2;
          double num1, num2;
          /*
...
bool util_copy_file__(const char *src_file, const char *target_file,
  ...
  if (util_same_file(src_file, target_file)) {
    fprintf(stderr,
            "%s Warning: trying to copy %s onto itself - nothing done\n",
            __func__, src_file);
    return false;
  } else {
    {
      ...
      bool result = util_copy_stream(src_stream, target_stream, buffer_size,
                                     buffer, abort_on_error);
...
bool util_copy_file(const char *src_file, const char *target_file) {
  ...
  size_t buffer_size = util_size_t_max(
      32, util_file_size(src_file)); /* The copy stream function will hang if
                                        buffer size == 0 */
  do {
  ...
  {
    bool result = util_copy_file__(src_file, target_file, buffer_size, buffer,
                                   abort_on_error);
    free(buffer);
    return result;
  }
...
bool util_entry_readable(const char *entry) {
  ...
  if (util_stat(entry, &buffer) == 0)
    ...
    return false; /* If stat failed - typically not existing entry - we return
                     false. */
}
...
/**
    ...
    * The function is **NOT** based purely on string parsing, but also
      ...
      ------------------------------------
      util_alloc_file_components("/some/random/not_existing/path" , &path ,
   &base , &ext)
...
void util_alloc_file_components(const char *file, char **_path,
...
bool util_same_file(const char *file1, const char *file2) {
  ...
  stat1 =
      util_stat(file1, &buffer1); // In the case of symlinks the stat call will
                                  // stat the target file and not the link.
...
bool util_fmt_bit8_stream(FILE *stream) {
  ...
  bool fmt_file;
  {
    ...
    if (elm_read < min_read)
      util_abort("%s: file is too small to automatically determine "
...
static bool util_make_datetime_utc__(int sec, int min, int hour, int mday,
                                     int month, int year, bool force_set,
                                     time_t *t);
...
/*
  ...
  the fields.
*/
void util_set_datetime_values_utc(time_t t, int *sec, int *min, int *hour,
                                  int *mday, int *month, int *year) {
...
bool util_sscanf_date_utc(const char *date_token, time_t *t) {
  ...
  if (date_token && sscanf(date_token, "%d%c%d%c%d", &day, &sep1, &month, &sep2,
                           &year) == 5) {
    if (t)
...
static bool util_make_datetime_utc__(int sec, int min, int hour, int mday,
  ...
  ts.tm_isdst =
      -1; /* Negative value means mktime tries to determine automagically
             whether Daylight Saving Time is in effect. */
  {
    ...
    if ((ts.tm_sec == sec) && (ts.tm_min == min) && (ts.tm_hour == hour) &&
        (ts.tm_mday == mday) && (ts.tm_mon == (month - 1)) &&
        (ts.tm_year == (year - 1900)))
      valid = true;
...
char *util_realloc_string_copy(char *old_string, const char *src) {
  if (src != NULL) {
    ...
    char *copy =
        (char *)util_realloc(old_string, (strlen(src) + 1) * sizeof *copy);
    strcpy(copy, src);
    return copy;
  } else {
    if (old_string != NULL)
...
char *util_alloc_multiline_string(const char **item_list, int len) {
  ...
  multiline_string =
      (char *)util_realloc(multiline_string, (strlen(multiline_string) +
                                              strlen(UTIL_NEWLINE_STRING) + 1) *
...
/**
   ...
   o Characters in the split_set at the front and back are discarded
     _BEFORE_ the actual splitting process.
       ...
       util_binary_split_string(":A:B:C:D:" , ":" , true , )   => "A"     &
   "B:C:D" util_binary_split_string(":A:B:C:D:" , ":" , false , )  => "A:B:C" &
   "D"
...
void util_binary_split_string(const char *__src, const char *sep_set,
                              bool split_on_first, char **__first_part,
                              char **__second_part) {
...
int static util_string_replace_inplace__(char **_buffer, const char *expr,
  ...
  char *buffer = *_buffer;
  int buffer_size =
      strlen(buffer) + 1; /* The variable buffer_size is the TOTAL size -
...
int util_string_replace_inplace(char **_buffer, const char *expr,
                                const char *subs) {
  return util_string_replace_inplace__(_buffer, expr, subs);
}

/**
  This allocates a copy of buff_org where occurences of the string expr are
  replaced with subs.
*/
...
/**
   This allocates a copy of buff_org where occurences of expr[i] are replaced
   with subs[i] for i=1,..,num_expr.
*/
...
/**
  This will alloc a copy of buff_org were char's in the last strings are
  removed.
*/
...
void util_fread(void *ptr, size_t element_size, size_t items, FILE *stream,
                const char *caller) {
...
void *util_realloc(void *old_ptr, size_t new_size) {
  /* The realloc documentation as ambigous regarding realloc() with size 0 - WE
   * return NULL. */
  if (new_size == 0) {
    if (old_ptr != NULL)
      free(old_ptr);
...
void *util_malloc(size_t size) { return util_malloc__(size); }
...
void util_install_signals(void) {
  ...
  signal(SIGSEGV, util_abort_signal); /* Segmentation violation, i.e.
                                         overwriting memory ... */
  signal(SIGTERM,
         util_abort_signal); /* If killing the enkf program with SIGTERM (the
                                default kill signal) you will get a backtrace.
                                Killing with SIGKILL (-9) will not give a
                                backtrace.*/

  sighandler_t previous_abort =
      signal(SIGABRT, util_abort_signal); /* Signal abort. */
  if (previous_abort != util_abort_signal) {
    /* Let previously installed abort handler (i.e. python faulthandler) be
     * called in the end */
...
char *util_alloc_tmp_file(const char *path, const char *prefix,
  ...
  do {
    ...
    if (include_pid)
      sprintf(file, "%s%c%s-%d-%ld", path, UTIL_PATH_SEP_CHAR, tmp_prefix, pid,
              rand_int);
    else
...
char *util_alloc_parent_path(const char *path) {
  ...
  if (path) {
    ...
    if (path_ncomp > 0) {
      ...
      parent_path = (char *)util_realloc(parent_path,
                                         current_length * sizeof *parent_path);
      parent_path[0] = '\0';
      ...
      for (ip = 0; ip < path_ncomp - 1; ip++) {
        ...
        if (min_length >= current_length) {
          ...
          parent_path = (char *)util_realloc(
              parent_path, current_length * sizeof *parent_path);
        }
...
int util_chdir(const char *path) { return chdir(path); }
#endif
