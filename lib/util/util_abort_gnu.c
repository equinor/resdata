...
#define __USE_GNU // Must be defined to get access to the dladdr() function; Man
                  // page says the symbol should be: _GNU_SOURCE but that does
                  // not seem to work?
#define _GNU_SOURCE // Must be defined _before_ #include <errno.h> to get the
...
static bool util_addr2line_lookup__(const void *bt_addr, char **func_name,
  ...
  *func_name =
      NULL; // If dladdr() succeeds, but addr2line fails the func_name pointer
            // will be set, but the function will return false anyway.
  *file_name = NULL;
  *line_nr = 0;
  {
      ...
      *func_name = util_alloc_string_copy(dl_info.dli_sname);
      if (util_file_exists(executable)) {
        ...
        /* 1: Run addr2line application */
        {
          ...
          util_spawn_blocking("addr2line", 3, (const char **)argv, stdout_file,
                              NULL);
        ...
        if (util_file_exists(stdout_file)) {
          ...
          if (strcmp(tmp_fname, UNDEFINED_FUNCTION) != 0) {
            ...
            util_binary_split_string(stdout_file_name, ":", false, file_name,
                                     &line_string);
...
static pthread_mutex_t __abort_mutex =
    PTHREAD_MUTEX_INITIALIZER; /* Used purely to serialize the util_abort()
                                  routine. */
...
static void util_fprintf_backtrace(FILE *stream) {
  ...
  for (i = 0; i < size; i++) {
    ...
    if (util_addr2line_lookup(bt_addr[i], &func_name, &file_name, &line_nr)) {
      int pad_length;
      const char *function;
      // Seems it can return true - but with func_name == NULL?! Static/inlinded
      // functions?
      if (func_name)
        function = func_name;
      else
    ...
    */
    ...
    if (abort_dump != stderr) {
      ...
      fprintf(stderr,
              ...
              "environment variable \"ERT_SHOW_BACKTRACE\" will show the "
              "backtrace on stderr.\n",
              filename);
    }
    ...
    fprintf(stderr,
            "Error while handling error. Exiting. File: %s Function: %s, Line: "
            "%d\n",
            file, function, line);
