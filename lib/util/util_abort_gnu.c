...
static bool util_addr2line_lookup__(const void *bt_addr, char **func_name,
                                    char **file_name, int *line_nr,
                                    bool subtract_base_adress) {
    *func_name =
        NULL; // If dladdr() succeeds, but addr2line fails the func_name pointer will be set, but the function will return false anyway.
    *file_name = NULL;
    *line_nr = 0;
    {
        ...
        if (dladdr(bt_addr, &dl_info)) {
            ...
            *func_name = util_alloc_string_copy(dl_info.dli_sname);
            if (util_file_exists(executable)) {
                char *stdout_file =
                    util_alloc_tmp_file("/tmp", "addr2line", true);
                /* 1: Run addr2line application */
                {
                    ...
                    util_spawn_blocking("addr2line", 3, (const char **)argv,
                                        stdout_file, NULL);
                ...
                if (util_file_exists(stdout_file)) {
                    ...
                    if (strcmp(tmp_fname, UNDEFINED_FUNCTION) != 0) {
                        char *stdout_file_name =
                            util_fscanf_alloc_line(stream, &at_eof);
                        char *line_string = NULL;
                        util_binary_split_string(stdout_file_name, ":", false,
                                                 file_name, &line_string);
                        if (line_string &&
                            util_sscanf_int(line_string, line_nr))
                            address_found = true;
...
bool util_addr2line_lookup(const void *bt_addr, char **func_name,
    ...
    if (util_addr2line_lookup__(bt_addr, func_name, file_name, line_nr, false))
        ...
        return util_addr2line_lookup__(bt_addr, func_name, file_name, line_nr,
                                       true);
}
...
static pthread_mutex_t __abort_mutex =
    PTHREAD_MUTEX_INITIALIZER; /* Used purely to serialize the util_abort() routine. */
...
static void util_fprintf_backtrace(FILE *stream) {
    ...
    for (i = 0; i < size; i++) {
        ...
        if (util_addr2line_lookup(bt_addr[i], &func_name, &file_name,
                                  &line_nr)) {
            int pad_length;
            const char *function;
            ...
            pad_length =
                util_int_max(2, 2 + max_func_length - strlen(function));
            padding = realloc_padding(padding, pad_length);
            fprintf(stream, with_linenr_format, i, function, padding, file_name,
                    line_nr);
        } else {
            if (func_name != NULL) {
                int pad_length =
                    util_int_max(2, 2 + max_func_length - strlen(func_name));
                padding = realloc_padding(padding, pad_length);
...
char *util_alloc_dump_filename() {
    ...
    strftime(day, 32, "%Y%m%d-%H%M%S", localtime(&timestamp));
    {
        ...
        if (pwd)
            filename = util_alloc_sprintf("/tmp/ert_abort_dump.%s.%s.log",
                                          pwd->pw_name, day);
        else
            filename =
                util_alloc_sprintf("/tmp/ert_abort_dump.%d.%s.log", uid, day);
...
jmp_buf *util_abort_test_jump_buffer() { return &jump_buffer; }
    ...
    */
        ...
        if (abort_dump != stderr) {
            ...
            fprintf(stderr,
                    ...
                    "the environment variable \"ERT_SHOW_BACKTRACE\" will show "
                    "the backtrace on stderr.\n",
                    filename);
        }
        ...
        fprintf(stderr,
                "Error while handling error. Exiting. File: %s Function: %s, "
                "Line: %d\n",
                file, function, line);
