...
static DIR *util_walk_opendir__(const char *root_path) {
  ...
  if (dirH == NULL) {
    if (errno == EACCES)
      fprintf(stderr,
              "** Warning could not open directory:%s - permission denied - "
              "IGNORED.\n",
              root_path);
    else
        ...
        if (dp != NULL) {
          if (dp->d_name[0] != '.') {
            ...
            if (util_is_file(full_src_path)) {
              util_copy_file__(full_src_path, full_target_path, buffer_size,
                               buffer, true);
            } else {
              if (util_is_directory(full_src_path) &&
                  !util_is_link(full_src_path))
                util_copy_directory__(full_src_path, full_target_path,
                                      buffer_size, buffer);
            }
...
/**
   ...
   The call:
   util_walk_directory("Root" , file_callback , file_arg , dir_callback ,
   dir_arg);
        ...
        if (dp->d_name[0] != '.') {
          ...
          if ((util_is_directory(full_path) && (!util_is_link(full_path)))) {
            ...
            if (dir_callback != NULL)
              descend = dir_callback(root_path, dp->d_name, current_depth,
                                     dir_callback_arg);

            if (descend &&
                util_file_exists(
                    full_path)) /* The callback might have removed it. */
              util_walk_directory__(full_path, depth_first, current_depth + 1,
                                    file_callback, file_callback_arg,
                                    dir_callback, dir_callback_arg);
          }
          free(full_path);
        }
      }
...
static void util_walk_directory__(const char *root_path, bool depth_first,
  ...
  if (depth_first) {
    util_walk_descend__(root_path, depth_first, current_depth, file_callback,
