...
static void spawn_init_redirection__(posix_spawn_file_actions_t *file_actions,
  ...
  */
  if (stdout_file)
    status += posix_spawn_file_actions_addopen(
        file_actions, STDOUT_FILENO, stdout_file, O_WRONLY | O_TRUNC | O_CREAT,
        S_IRUSR | S_IWUSR);

  if (stderr_file)
    status += posix_spawn_file_actions_addopen(
        file_actions, STDERR_FILENO, stderr_file, O_WRONLY | O_TRUNC | O_CREAT,
        S_IRUSR | S_IWUSR);

  if (status != 0)
    util_abort("%s: something failed while setting up redirect stdout:%s  "
               "stderr:%s \n",
...
pid_t util_spawn(const char *executable, int argc, const char **argv,
...

  {
    ...
    {
      int spawn_status;
      if (util_is_executable(executable)) { // the executable is in current
                                            // directory or an absolute path
        spawn_status = posix_spawn(&pid, executable, &file_actions, &spawn_attr,
                                   argv__, environ);
      } else { // Try to find executable in path
        spawn_status = posix_spawnp(&pid, executable, &file_actions,
