...
pid_t util_spawn(const char *executable, int argc, const char **argv,
...

    {
        ...
        {
            int spawn_status;
            if (util_is_executable(
                    executable)) { // the executable is in current directory or an absolute path
                spawn_status = posix_spawn(&pid, executable, &file_actions,
            ...
            if (spawn_status != 0)
                util_abort(
                    "%s: failed to spawn external command: \'%s\': %s \n",
