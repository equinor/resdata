...
void util_compress_buffer(const void *data, int data_size, void *zbuffer,
                          unsigned long *compressed_size) {
    int compress_result;
    if (data_size > 0) {
        compress_result = compress((Bytef *)zbuffer, compressed_size,
    ...
    */
        ...
        if (compress_result == Z_STREAM_ERROR) {
            ...
            fprintf(stderr, "** "
                            ...
                            "--------- **\n");
            fprintf(stderr,
                    "** Unrecognized return value:%d from compress(). "
                    "Proceeding as **\n",
                    compress_result);
...
/**
   ...
   output.
*/
void *util_alloc_compressed_buffer(const void *data, int data_size,
                                   unsigned long *compressed_size) {
    ...
    }
    {
...

        {
            ...
            do {
                ...
                util_compress_buffer(&data[offset], this_block_size, zbuffer,
                                     &compressed_size);
                fwrite(&compressed_size, sizeof compressed_size, 1, stream);
                {
                    unsigned long bytes_written =
                        fwrite(zbuffer, 1, compressed_size, stream);
                    if (bytes_written < compressed_size)
                        util_abort("%s: wrote only %d/%ld bytes to compressed "
                                   "file  - aborting \n",
    ...
    do {
        ...
        fread(&compressed_size, sizeof compressed_size, 1, stream);
        {
            unsigned long bytes_read =
                fread(zbuffer, 1, compressed_size, stream);
            if (bytes_read < compressed_size)
                util_abort("%s: read only %d/%d bytes from compressed file - "
                           "aborting \n",
                           __func__, bytes_read, compressed_size);
        }
        uncompress_result = uncompress(&data[offset], &block_size,
                                       (const Bytef *)zbuffer, compressed_size);
      ...
      */
        ...
        offset += block_size;
        {
            ...
            if (file_offset != offset)
                util_abort("%s: something wrong when reding compressed stream "
                           "- aborting \n",
                           __func__);
        }
    } while (offset < size);
