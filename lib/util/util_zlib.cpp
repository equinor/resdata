  ...
  if (data_size > 0) {
    ...
    */
    ...
    if (compress_result == Z_STREAM_ERROR) {
      ...
      fprintf(
          stderr,
          "** Unrecognized return value:%d from compress(). Proceeding as **\n",
          compress_result);
      ...
      do {
        ...
        fwrite(&compressed_size, sizeof compressed_size, 1, stream);
        {
          ...
          if (bytes_written < compressed_size)
            util_abort(
                "%s: wrote only %d/%ld bytes to compressed file  - aborting \n",
  ...
  do {
    ...
    fread(&compressed_size, sizeof compressed_size, 1, stream);
    {
      ...
      if (bytes_read < compressed_size)
        util_abort(
            "%s: read only %d/%d bytes from compressed file - aborting \n",
    ...
    offset += block_size;
    {
      ...
      if (file_offset != offset)
        util_abort(
            "%s: something wrong when reding compressed stream - aborting \n",
            __func__);
    }
  } while (offset < size);
  free(zbuffer);
