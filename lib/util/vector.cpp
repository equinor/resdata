...
struct vector_struct {
  ...
  int alloc_size; /* The number of elements allocated in the data vector - in
                     general > size. */
...
static void vector_insert__(vector_type *vector, int index,
    ...
    vector_resize__(vector, 2 * (vector->alloc_size + 1));
  {
    ...
    vector->data[index] = NULL; /* Otherwise the destructor might try to pick up
                                   on it in the vector_iset__() call below */
  }
  vector->size++;
...
static void vector_push_node(vector_type *vector, node_data_type *node) {
    ...
    vector_resize__(vector, 2 * (vector->alloc_size + 1));
  {
    ...
    if (bytes > 0) {
      ...
      vector->data[0] = NULL; /* Otherwise the destructor might try to pick up
                                 on it in the vector_iset__() call below */
    }
  }
  vector->size++;
...
void *vector_pop_front(vector_type *vector) {
               ...
               __func__);
  {
    ...
    node_data_free_container(node); /* Free the container holding data. */
    {
      int bytes = (vector->size - 1) *
                  sizeof *vector
                      ->data; /* Move the storage one element to  the left
                                 (could als be implemented with an offset??). */
...
void vector_clear(vector_type *vector) {
  ...
  for (i = 0; i < vector->size; i++) {
    ...
    vector->data[i] = NULL; /* This is essential to protect against unwaranted
                               calls to destructors when data is reused. */
  }
  vector->size = 0;
}
