...
class EclDataType(BaseCClass):
    ...
    _alloc = EclPrototype(
        "void* ecl_type_alloc_python(ecl_type_enum, size_t)", bind=False
    )
    ...
    _get_element_size = EclPrototype(
        "size_t ecl_type_get_sizeof_iotype_python(ecl_data_type)"
    )
    ...
    def _assert_valid_arguments(self, type_enum, element_size, type_name):
        if type_name is not None:
            ...
            if element_size is None:
                raise ValueError(
                    "When creating an ECL_STRING one must " + "provide an element size!"
                )
