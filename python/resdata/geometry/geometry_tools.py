...
class GeometryTools(object):
    ...
    def lineIntersection(p1, p2, p3, p4):
        ...
        denominator = (p4[1] - p3[1]) * (p2[0] - p1[0]) - (p4[0] - p3[0]) * (
            p2[1] - p1[1]
        )
        numerator_a = (p4[0] - p3[0]) * (p1[1] - p3[1]) - (p4[1] - p3[1]) * (
            p1[0] - p3[0]
        )
        numerator_b = (p2[0] - p1[0]) * (p1[1] - p3[1]) - (p2[1] - p1[1]) * (
            p1[0] - p3[0]
        )

        # coincident?
        if (
            abs(numerator_a) < GeometryTools.EPSILON
            and abs(numerator_b) < GeometryTools.EPSILON
            and abs(denominator) < GeometryTools.EPSILON
        ):
            return None
    ...
    def slicePolygon(bounding_polygon, poly_line):
        ...
        internal_intersection = GeometryTools.lineIntersection(
            p1, intersection1[1], p2, intersection2[1]
        )
    ...
    def rayLineIntersection(point, ray, p1, p2, flip_ray=False):
        ...
        if (
            abs(r_x_s) < GeometryTools.EPSILON
            and abs(q_m_p_x_r) < GeometryTools.EPSILON
        ):
        ...
        if flip_ray:
            return GeometryTools.rayLineIntersection(
                point, (-ray[0], -ray[1]), p1, p2, False
            )
        else:
            return None
    ...
    def joinPolylines(polyline1, polyline2):
        ...
        d_list = [
            (GeometryTools.distance(p0, pa), [p0, pa]),
    ...
    def nearestPointOnPolyline(p, polyline):
        if len(polyline) > 1:
            ...
            if not index1 is None:
                ...
                dy1 = p1[1] - p0[1]
                dx1 = p1[0] - p0[0]
                intercept1 = GeometryTools.rayLineIntersection(
                    p, (dy1, -dx1), p0, p1, True
                )
                if intercept1:
            ...
            if not index2 is None:
                ...
                dy2 = p2[1] - p0[1]
                dx2 = p2[0] - p0[0]
                intercept2 = GeometryTools.rayLineIntersection(
                    p, (dy2, -dx2), p0, p2, True
                )
                if intercept2:
