...
class GridTest(EclTest):
    ...
    def create(self, filename, load_actnum=True):
        ...
        specgrid = EclKW.read_grdecl(
            fileH, "SPECGRID", ecl_type=EclDataType.ECL_INT, strict=False
        )
    ...
    @skipIf(
        EclTest.slowTestShouldNotRun(), "Slow test of numActive large memory skipped!"
    )
    ...
    def test_no_mapaxes_check_for_nan(self):
        grid_paths = [
            "Equinor/ECLIPSE/NoMapaxes/ECLIPSE.EGRID",
            "Equinor/ECLIPSE/NoMapaxes/ECLIPSE.GRID",
        ]
