...
class PolygonSlicingTest(EclTest):
    ...
    def test_ray_line_intersection(self):
        ...
        self.assertEqual(
            (0.5, 0.5), GeometryTools.rayLineIntersection(p0, ray1, p1, p3)
        )
        self.assertIsNone(
            GeometryTools.rayLineIntersection(p0, ray2, p1, p3)
        )  # ray2 is ray1 reversed (no backwards intersections)

        self.assertEqual(
            (1.0, 0.5), GeometryTools.rayLineIntersection(p0, ray3, p2, p3)
        )
