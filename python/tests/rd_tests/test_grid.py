...
def createWrapperGrid(grid):
    ...
    corners = [grid.getCellCorner(i, ijk=pos) for i, pos in enumerate(corner_pos)]
...
class GridTest(ResdataTest):
    ...
    def test_posXYEdge(self):
        ...
        self.assertEqual(
            grid.findCellCornerXY(nx - 0.25, ny, 0), (nx + 1) * (ny + 1) - 1
        )
        self.assertEqual(
    ...
    def test_unique_containment(self):
        ...
        for steps_per_unit, grid in test_base:
            ...
            for x, y, z in itertools.product(x_space, y_space, z_space):
                hits = [
                    grid.cell_contains(x, y, z, i) for i in range(grid.getGlobalSize())
                ].count(True)
