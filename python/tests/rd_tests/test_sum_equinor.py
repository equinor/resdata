...
class SumTest(ResdataTest):
    def setUp(self):
        ...
        self.rd_sum = Summary(self.case)

        self.assertIsInstance(self.rd_sum, Summary)

    def test_load(self):
        self.assertIsNotNone(self.rd_sum, "Load failed")
    ...
    def test_last(self):
        ...
        self.assertFloatEqual(self.rd_sum.last_value("FGPT"), 6605249024.0)
    ...
    def test_restart(self):
        hist = Summary(
            self.createTestPath(
                "Equinor/ECLIPSE/sum-restart/history/T07-4A-W2011-18-P1"
            )
        )
        base = Summary(
            self.createTestPath("Equinor/ECLIPSE/sum-restart/prediction/BASECASE")
        )
        pred = Summary(
            self.createTestPath("Equinor/ECLIPSE/sum-restart/prediction/BASECASE"),
    ...
    def test_case1(self):
        self.assertTrue(self.rd_sum.path == self.createTestPath(path))
        self.assertTrue(self.rd_sum.base == base)
        self.assertTrue(self.rd_sum.case == self.createTestPath(case))
        self.assertTrue(self.rd_sum.abs_path == self.createTestPath(path))
    ...
    def test_aquifer(self):
        case = Summary(
            self.createTestPath("Equinor/ECLIPSE/Aquifer/06_PRESSURE_R009-0")
        )
    ...
    def test_write(self):
        with TestAreaContext("my_space") as area:
            ...
            intersect_summary.fwrite(rd_case=write_location)

            reloaded_summary = Summary(write_location)
    ...
    def test_ix_write(self):
        for data_set in [
            ...
            with TestAreaContext("my_space" + data_set.split("/")[-1]) as area:
                ...
                reloaded_summary = Summary(write_location)
                self.assertEqual(
                    list(intersect_summary.keys()), list(reloaded_summary.keys())
                )

    def test_ix_caseII(self):
        troll_summary = Summary(
            self.createTestPath(
                "Equinor/ECLIPSE/ix/troll/IX_NOPH3_R04_75X75X1_GRID2.SMSPEC"
            )
        )
    ...
    def test_resample(self):
        ...
        delta = self.rd_sum.get_end_time() - resampled.get_end_time()
        ...
        for key in keys:
            for time_index, t in enumerate(time_points):
                self.assertFloatEqual(
                    ...
                    self.rd_sum.get_interp_direct(key, t),
                )

    def test_summary_units(self):
        self.assertEqual(self.rd_sum.unit_system, UnitSystem.METRIC)
