...
def test_that_select_below_plane_selects_cells_strictly_on_negative_side(empty_region):
    # The plane x=5.5 splits the 10x10x1 grid so that the
    # 5x10 slab of cells with i in [0,4] lies strictly below (x<5.5).
    empty_region.select_below_plane((1.0, 0.0, 0.0), (5.5, 0.0, 0.0))
    assert empty_region.global_size() == 50


@pytest.fixture
def lower_quadrant():
    # A polygon covering only the [0,5]x[0,5] quadrant
    return [(0.0, 0.0), (0.0, 5.0), (5.0, 5.0), (5.0, 0.0)]


def test_that_select_outside_polygon_leaves_only_cells_outside_polygon(
    empty_region, lower_quadrant
):
    empty_region.select_outside_polygon(lower_quadrant)
    assert empty_region.global_size() == 75


def test_that_deselect_outside_polygon_only_removes_cells_outside_polygon(
    full_region, lower_quadrant
):
    full_region.deselect_outside_polygon(lower_quadrant)
...
# In the following tests, "supports active sized" and "supports global sized"
# means that for kw with size nx*ny*nz we select among all indices. For
# kw with size grid.num_active(), we select among active indices
...
def test_that_select_false_supports_active_sized_bool_kw(empty_region, active_bool_kw):
    false_region = empty_region.copy()
    # no values in active_bool_kw is false so we select no indices
